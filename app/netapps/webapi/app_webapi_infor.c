...
extern void app_webapi_make_infor_data(cJSON *json);

static void app_webapi_server_infor_read(struct mg_connection* nc, struct http_message* hm)
{
	cJSON *root = NULL;
	cJSON *json1 = NULL;
	char *out = NULL;
	...
	cJSON_AddItemToObject(root, WEBAPI_INFOR_STR, json1);

	app_webapi_make_infor_data(json1);
...
void app_webapi_server_infor(struct mg_connection* nc, struct http_message* hm)
{
	...
	{
		app_webapi_server_infor_read(nc, hm);
	}
	else
	{
