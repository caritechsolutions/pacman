...
#define DBG_ERR(...)              do {\
                                            app_log_debug("\033[1;34m""[%s:%d]: ", __FUNCTION__, __LINE__);\
...
public:
	...
	void debugprintf(const char *fmt, ...) __attribute__((format(printf,2,3)));
	void logprintf(const char *fmt, ...) __attribute__((format(printf,2,3)));
