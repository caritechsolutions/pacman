...
static int dvbs_tp_lock(void* pTpInfo)
{
	...
	struct dvbs_tsid_parameters tsid_params = {0};
	...
	app_pin_func_switch(p->TpInfo.TunerSelect);
#endif
	//set mode
	ioctl(p->TpInfo.FrontendDev, FE_SET_FRONTEND_TUNE_MODE, TEST_DVBS2_NORMAL);
	...
	params.u.dvbs_param.symbol_rate = p->Sym/1000;
	...
	ret = 0;
#endif
	if((0 <= p->tsid) && (p->tsid <= 255)){
		tsid_params.ts_id = p->tsid;
		if(ioctl(p->TpInfo.FrontendDev, FE_SET_MULTI_TSID, &tsid_params) < 0){
			app_log_error("\nTEST, set tsid error, %s, %d\n",__FUNCTION__,__LINE__);
			return -1;
		}
	}else if(p->tsid != -1){//default -1 means don't need config
		return -1;
	}
	return ret;
}
...
static int _parse_tp_para(char *Command, void **ppInOutFrontendInfo, void **ppInOutTpInfo)
{
	...
	if(p == NULL)
	{// DEFAULT SOURCE
		sg_DvbsTpInfo.TpInfo.DemuxPara.TsSource = DEMUX_TS1;// source 0
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
	...
	// TSID
	p = strstr(Command, KEYWORD_TSID);
	if(p == NULL)
	{// DEFAULT TSID
		sg_DvbsTpInfo.tsid = -1;//don't need config tsid, tsid range from 0 to 255
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
		sscanf(p,"%*[^=]=%[^ ]", Buffer);
		sscanf(Buffer, "%d", (int*)&sg_DvbsTpInfo.tsid);
	}
