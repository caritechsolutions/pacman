...
static int dvbt2_tp_lock(void* pTpInfo)
{
	...
	// set tp
	params.frequency = p->Fre*1000;
	params.inversion = INVERSION_OFF;
	params.u.dvbt_param.bandwidth = p->BandWidth;

	params.u.dvbt_param.code_rate_HP = p->DataPLPId;
	params.u.dvbt_param.code_rate_LP = p->CommonPLPExist;
	params.u.dvbt_param.constellation = p->CommonPLPID;
...
static int _parse_tp_para(char *Command, void **ppInOutFrontendInfo, void **ppInOutTpInfo)
{
	...
	if(p == NULL)
	{// DEFAULT TSMODE
		sg_DvbT2TpInfo.TpInfo.DemuxPara.TsMode = 1;// paralle
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
	...
	if(p == NULL)
	{// DEFAULT TS SELECT
		sg_DvbT2TpInfo.TpInfo.TunerSelect = 1; // tuner 1
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
