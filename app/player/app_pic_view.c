...
//resourse
#define IMG_BUTTON1_PLAY_FOCUS		"s_mpicon_play_focus"
#define IMG_BUTTON1_PLAY_UNFOCUS	"s_mpicon_play_unfocus"
#define IMG_BUTTON1_PAUSE_FOCUS		"s_mpicon_pause_focus"
#define IMG_BUTTON1_PAUSE_UNFOCUS	"s_mpicon_pause_unfocus"
...
#define IMG_WARNING_WAIT			"MP_WAIT"
#define IMG_WARNING_ERROR              "MP_ERROR"
...
#define IMAGE_BOX_BUTTOM		        "pic_view_image_box"
...
{
	if(pic_box_button1_ctrol == PLAY_PIC_CTROL_PAUSE)
	{
...
int pic_status_report(PicPlayStatus player_status)
{
	switch(player_status)
	{
		...
		case PIC_PLAY_ERROR:
			...
			if(pic_box_button1_ctrol == PLAY_PIC_CTROL_PLAY)
			{
//				play_list* list = NULL;
...
static status_t popmsg_topright_ctrl(play_pic_ctrol_state button, uint32_t value)
{
	...
	{
		APP_Printf("[PIC] pic_popmsg_topright_timer remove it\n");
	...
	if(timeout)
	{
		if(pic_popmsg_topright_timer)
		{
			APP_Printf("[PIC] pic_popmsg_topright_timer is already created\n");
			return GXCORE_ERROR;
		}
...
static int pic_box_hide(void* usrdata)
{
    ...
    GUI_SetProperty(IMAGE_BOX_BUTTOM, "state", "hide");

	GUI_SetProperty(PIC_LINE0, "state", "hide");
	GUI_SetProperty(PIC_LINE1, "state", "hide");
    GUI_SetProperty(TEXT_PIC_NAME1, "rolling_stop", NULL);
...
static int pic_box_show(void* usrdata)
{
    ...
    GUI_SetProperty(IMAGE_BOX_BUTTOM, "state", "show");
...
*/
		...
		case APPK_ZOOM:
			...
			return EVENT_TRANSFER_STOP;	*/
		case APPK_SET:
			box_sel = PIC_BOX_BUTTON_SET;
			GUI_SetProperty(BOX_PIC_CTROL, "select", &box_sel);
			pic_box_ctrol(PIC_BOX_BUTTON_SET);
			return EVENT_TRANSFER_STOP;
		case APPK_YELLOW:
			box_sel = PIC_BOX_BUTTON_INFO;
			GUI_SetProperty(BOX_PIC_CTROL, "select", &box_sel);
			pic_box_ctrol(PIC_BOX_BUTTON_INFO);
			return EVENT_TRANSFER_STOP;
		default:
			break;
	}
...
int app_pic_play_status_get(void)
{
    if(pic_box_button1_ctrol == PLAY_PIC_CTROL_PLAY)
        return 2;// playing
    else if(pic_box_button1_ctrol == PLAY_PIC_CTROL_PAUSE)
        return 1;// pause
    else if(pic_box_button1_ctrol == PLAY_PIC_CTROL_STOP)
        return 0;// stop
    return -1;
}
#endif
