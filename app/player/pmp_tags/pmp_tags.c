#include "app_misc.h"
#include "pmp_tags.h"
#include "gxbus.h"
#include "app.h"

#define SHA_PMOTAG_LENGTH 20

const char* _get_path_without_mount(const char* path)
{
    // from "/mnt/usbxx/XXXX" get "/XXXX"
    if(path == NULL)
        return path;

    int len = strlen(path);
    int i = 0;
    int separator = 0;
    for(i=0; i<len; ++i)
    {
        if(*(path+i) == '/' )
        {
            ++separator;
        }
        if(separator == 3)
        {
            return (path+i);
        }
    }

    return path;
}

int32_t pmp_get_cur_tag_num(void)
{
	int32_t value = 0;
	GxBus_ConfigGetInt(CUR_TAG_NO, &value, 0);
	return value;
}

status_t pmp_set_cur_tag_num(void)
{
	int32_t  cur_tag_no;
	cur_tag_no = pmp_get_cur_tag_num();
	if(cur_tag_no<(MAX_TAGS_NUM-1))
	{
		cur_tag_no+=1;
	}
	else
	{
		cur_tag_no = 0;
	}
	GxBus_ConfigSetInt(CUR_TAG_NO, cur_tag_no);
	return GXCORE_SUCCESS;
}

status_t pmp_init_tags(void)
{
	int32_t ret = 0;
	int32_t i;
	char tag_url[32];
	char tag_time[32];
	handle_t group_tag = 0;

	group_tag = GxBus_ConfigGroupOpen();
	ret = GxBus_ConfigGroupSetInt(group_tag, CUR_TAG_NO, 0);
	if(GXCONFIG_FAILURE == ret) return GXCORE_ERROR;

	for(i=0;i<MAX_TAGS_NUM;i++)
	{
		memset(tag_url,0,sizeof(tag_url));
		memset(tag_time,0,sizeof(tag_time));
		sprintf(tag_url,"TAG%d>URL", i);
		sprintf(tag_time,"TAG%d>TIME", i);

		GxBus_ConfigGroupSet(group_tag, tag_url, "null");
		GxBus_ConfigGroupSetInt(group_tag, tag_time,0);
	}

	GxBus_ConfigGroupSave(group_tag);
	GxBus_ConfigGroupClose(group_tag);

	return GXCORE_SUCCESS;
}

status_t pmp_get_tags(int32_t tag_no,unsigned char* hash,int32_t* ptime)
{
	char tag_url[32];
	char tag_time[32];

	if(tag_no>MAX_TAGS_NUM||tag_no<0||hash == NULL||ptime == NULL)
		return GXCORE_ERROR;

	sprintf(tag_url,"TAG%d>URL", tag_no);
	sprintf(tag_time,"TAG%d>TIME", tag_no);

	GxBus_ConfigGet(tag_url, (char*)hash, SHA_PMOTAG_LENGTH*2 , "");
	GxBus_ConfigGetInt(tag_time, ptime, 0);

	return GXCORE_SUCCESS;
}

status_t pmp_set_tags(int32_t tag_no,unsigned char* hash,int32_t time)
{
	char tag_url[32];
	char tag_time[32];

	if(tag_no>MAX_TAGS_NUM||tag_no<0||hash == NULL)
		return GXCORE_ERROR;

	sprintf(tag_url,"TAG%d>URL", tag_no);
	sprintf(tag_time,"TAG%d>TIME", tag_no);

	GxBus_ConfigSet(tag_url, (char*)hash);
	GxBus_ConfigSetInt(tag_time,time);

	return GXCORE_SUCCESS;
}

int32_t pmp_load_tag(const char* url)
{
    uint8_t i;
    unsigned char hash[SHA_PMOTAG_LENGTH] = {0};
    unsigned char str1[SHA_PMOTAG_LENGTH*2] = {0};
    unsigned char str2[SHA_PMOTAG_LENGTH*2] = {0};
    int32_t time;
    int32_t value;
    int n;

    value = pmpset_get_int(PMPSET_SAVE_TAGS);
    if(PMPSET_TONE_OFF == value)
        return 0;

    if(url == NULL)
        return 0;

    const char* path_url = _get_path_without_mount(url);
    n = strlen(path_url);
    if(SHA1((const unsigned char*)path_url, n, hash) < 0)
    {
        return GXCORE_ERROR;
    }
    hex2str(hash,str1,sizeof(hash),sizeof(str1));
    for(i=0;i<MAX_TAGS_NUM;i++)
    {
        pmp_get_tags(i,str2,&time);
        if(NULL != str2)
        {
            if(strncmp((char*)str1, (char*)str2,sizeof(str1)) == 0)
            {
                return time;
            }
        }
    }
    return 0;
}

status_t pmp_save_tag(const char* url,int32_t time)
{
    uint8_t i=0,tag_no=0xff;
    unsigned char str1[SHA_PMOTAG_LENGTH*2] = {0};
    unsigned char hash[SHA_PMOTAG_LENGTH] = {0};
    unsigned char str2[SHA_PMOTAG_LENGTH*2] = {0};
    int32_t tag_time;
    int32_t value;
    int n;

    value = pmpset_get_int(PMPSET_SAVE_TAGS);
    if(PMPSET_TONE_OFF == value)
        return GXCORE_ERROR;

    if(url == NULL)
        return GXCORE_ERROR;

    const char* path_url = _get_path_without_mount(url);

    n = strlen(path_url);
    if(SHA1((const unsigned char*)path_url, n, hash) < 0)
    {
        return GXCORE_ERROR;
    }
    hex2str(hash,str1,sizeof(hash),sizeof(str1));
    for(i=0;i<MAX_TAGS_NUM;i++)
    {
        pmp_get_tags(i,str2,&tag_time);
        if(NULL != str2)
        {
            if(strncmp((char*)str1, (char*)str2,sizeof(str1)) == 0)
            {
                tag_no = i;
                break;
            }
        }
    }

    if(tag_no == 0xff)
    {
        tag_no = pmp_get_cur_tag_num();
        pmp_set_cur_tag_num();
    }
    pmp_set_tags(tag_no,str1,time);
    return GXCORE_SUCCESS;
}




