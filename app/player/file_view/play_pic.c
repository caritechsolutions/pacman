...
static int  _auto_switch_pic(void *userdata)
{
	...
	switch(sequence)
	{
		...
		default:
			app_log_error("[PIC] auto_switch_pic sequence:%d not support\n", sequence);
			break;
	}
...
status_t _auto_switch_start(void)
{
	...
	/*if(pic_switch_timer)
	{
		app_log_info("[PIC] play_pic_auto_switch_start pic_switch_timer is already created\n");
		return GXCORE_ERROR;
	}*/
...
status_t play_pic(int file_no)
{
	...
	char* path = NULL;
	int ret = 0;
	int32_t pic_special_effects = 0;
	PicPlayStatus play_status = PIC_PLAY_START;

	pic_special_effects = pmpset_get_int(PMPSET_PIC_SPECIAL_EFFECTS);
	list = play_list_get(PLAY_LIST_TYPE_PIC);
	if(NULL == list)
	{
		ret = GXCORE_ERROR;
    ...
    GDI_StopImageNoClear("xxx.gif");//255848
	app_log_info("[PLAY] picture %s\n", path);
	pic_status_report(play_status);
	IMG_Set_PlayMode(pic_special_effects);
