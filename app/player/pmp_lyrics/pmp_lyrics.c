...
#ifdef  NOTEPAD_PATCH
static int shownotepad=0;
static void set_Notepad(void)
{
	shownotepad=1;
}

static void clear_Notepad(void)
{
	shownotepad=0;
}
static  int lrc_string_set(void *userdata)
{
	//app_log_debug("consule thread %d\n",GxCore_ThreadGetId());
 	//app_log_debug("show lrcset %d %d\n",shownotepad,GxCore_ThreadGetId());
    if(shownotepad)
	{
        if((lrc_para != NULL)&&(lrc_para->lrc_buff!=NULL))
        {
...
static  int lrc_scroll(void *userdata)
{
	...
	if(cur_time_ms < lrc_list_node->time)
	{
		lrc_no = 0;
	}
	/*find next*/
	else
	{
		for (lrc_list_node = lrc_list_record;lrc_list_node != NULL;)
		{
			...
			if(lrc_list_node->next)
			{
				...
				if(cur_time_ms < lrc_list_node->time)
				{
					app_log_info("[LRC] seek backward, find from first\n");
					lrc_list_node = lrc_para->list;
					continue;
				}
				/*find node*/
				else if((cur_time_ms >= (lrc_list_node->time)) && (cur_time_ms < (lrc_list_node->next->time)))
				{
			...
			else
			{
				...
				{
					app_log_info("[LRC] seek backward from last node\n");
					lrc_list_node = lrc_para->list;
					continue;
				}
			}
	...
	if(lrc_no != lrc_para->record_line)
	{
        ...
        int i = 0;
        char line_array[LYRICS_BUFF_LEN] = {0};
        char line_buf[LYRICS_BUFF_LEN] ={0};
        memset(line_array, 0, 32);
        strcat(line_array, "[");

        for(i=0;i<lrc_list_node->skip;i++)
        {

            memset(line_buf, 0, LYRICS_BUFF_LEN);
            if(i == (lrc_list_node->skip-1))
            {
                sprintf(line_buf,"%d",lrc_no+3+i);
            }
            else
            {
                sprintf(line_buf,"%d,",lrc_no+3+i);
            }
            strcat(line_array, line_buf);
        }
        strcat(line_array,"]");
        GUI_SetProperty(lrc_para->widget_lrc, "active_array", line_array);
...
static void lrc_load(const char* file)
{
	Lrc* thiz = NULL;
    ...
    GXGDI_Rect lrc_rect = {0};
    GDI_Interval interval= {0};
    int x,y,w,h = 0;
    sscanf((widget_get_property(gui_get_widget(NULL,"music_view_notepad_lrc"),"rect")),"[%d,%d,%d,%d]",&x,&y,&w,&h);
    lrc_rect.x = x;
    lrc_rect.y = y;
    lrc_rect.w = w;
    lrc_rect.h = h;
	...
	if(thiz != NULL)
	{
		...
		while(!lrc_time_tag_iter_is_null(&time_iter))
		{
            ...
            skip_line = GXGDI_GetStringLines(str,&lrc_rect,&interval);
            TempList->skip = skip_line;
            skip_line = TempList->number+skip_line;
			len = strlen(lrc_para->lrc_buff) + strlen(str) + 20;
			if(len > lrc_para->lrc_buff_size)
			{
				app_log_error("[LRC] lrc_buff_size overflow\n");
				break;
			}
			...
			lrc_get_time_tag_by_time(thiz, start_time + 1);
			time_iter = lrc_time_tag_iter_next(&time_iter);
		}
		...
		#ifdef  NOTEPAD_PATCH
		app_log_debug("msg thread %d\n",GxCore_ThreadGetId());
		set_Notepad();
		#else
		app_log_debug("before show lrc\n");
		GUI_SetProperty(lrc_para->widget_lrc, "string", lrc_para->lrc_buff);
		app_log_debug("atfer  show lrc\n");
		#endif
		i = 0;
	}
}

static void lrc_unload(void)
{
...
static void lrc_timer_start(void)
{
	APP_TIMER_ADD(lrc_para->timer,  lrc_scroll, 300, TIMER_REPEAT);
}

static void lrc_timer_stop(void)
{
	if(lrc_para->timer)
	{
...
{
	app_log_info("[LRC] lyrics_create\n");
	...
	memset(lrc_para->lrc_buff, 0, LYRICS_BUFF_SIZE);
...
{
	app_log_info("[LRC] lyrics_destroy\n");
...
int lyrics_start(char* file)
{
	...
	app_log_info("[LRC] lyrics_start, %s\n", file_load);
	...
	if(lrc_para->file)
	{
		...
		{
			app_log_info("[LRC] already load %s\n", file_load);
			return 0;
		}
		else
		{
	...
	{
		app_log_info("[LRC] lrc_buff %dB relloc to %dB\n", lrc_para->lrc_buff_size, file_size);
...
{
	app_log_info("[LRC] lyrics_resume\n");
