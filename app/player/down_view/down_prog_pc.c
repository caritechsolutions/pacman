...
static status_t item_release(down_item_program* item)
{
	...
	GXxmlFree(item->title);
	GXxmlFree(item->link);
	GXxmlFree(item->descriptor);
	GXxmlFree(item->pubdate);
	DOWN_FREE(item->size);
	DOWN_FREE(item);
...
static status_t list_create(down_list_program* list, GXxmlNodePtr GXxml_node)
{
	...
	app_log_debug("[DOWN] create program list, from class pc, item num:%d\n", list->item_num);
...
static status_t list_release(down_list_program* list)
{
	...
	app_log_debug("[DOWN] list release, item num:%d\n", list->item_num);
