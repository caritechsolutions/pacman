/* $Id: miniwget.c,v 1.80 2019/02/10 13:22:05 nanard Exp $ */
/* Project : miniupnp
...
getHTTPResponse(SOCKET s, int * size, int * status_code)
{
	...
	content_buf = (char *)download_memhole_malloc(content_buf_len);
	if(content_buf == NULL)
	{
		free(header_buf);
#ifdef DEBUG
	...
	while((n = receivedata(s, buf, sizeof(buf), 5000, NULL)) > 0)
	{
		if(endofheaders == 0)
		{
			...
			if(header_buf_used + n > header_buf_len) {
				...
				if(tmp == NULL) {
					...
					free(header_buf);
					download_memhole_free((unsigned char *)content_buf);
					*size = -1;
					return NULL;
				}
				header_buf = tmp;
		...
		if(chunked)
		{
			...
			while(i < n)
			{
				...
				{
					char * tmp;
					unsigned int old_content_buf_len = content_buf_len;
					if((content_length >= 0) && ((unsigned int)content_length >= (content_buf_used + bytestocopy))) {
						content_buf_len = content_length;
					} else {
						content_buf_len = content_buf_used + bytestocopy;
					}
					tmp = (char *)download_memhole_realloc((unsigned char *)content_buf, old_content_buf_len, content_buf_len);
					if(tmp == NULL) {
						/* memory allocation error */
						download_memhole_free((unsigned char *)content_buf);
						free(header_buf);
		...
		if(content_length > 0) {
			int percent = 0;
			printf("\b\b\b\b%d%%", content_buf_used * 100 / content_length);
			fflush(stdout);

			percent = content_buf_used * 100 / content_length / 2;
			percent = percent > 50 ? 50:percent;
			update_progress_percent(percent);
			if (http_download_is_stop() == true) {
				content_buf_used = 0;
				printf("http get stop\n");
				goto end_of_stream;
			}
		}
	...
	if (content_buf_used < (unsigned int)content_length) {
		printf("receivedata timeout\n");
		content_buf_used = 0;
	}
end_of_stream:
	free(header_buf);
