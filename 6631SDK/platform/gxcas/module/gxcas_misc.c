...
void* GxCas_AlignMalloc(size_t size, size_t alignment)
{
	...
	if ((p1 = GxCore_Malloc(size + offset)) == NULL)
		return NULL;
...
static uint32_t dvb_crc_finger[256] =
{
	...
	0x54BF6A4 , 0x808D07D , 0xCC9CDCA , 0x7897AB07, 0x7C56B6B0, 0x71159069, 0x75D48DDE,
	...
	0x285E1D47, 0x36194D42, 0x32D850F5, 0x3F9B762C, 0x3B5A6B9B,  0x315D626, 0x7D4CB91 ,
	0xA97ED48 , 0xE56F0FF , 0x1011A0FA, 0x14D0BD4D, 0x19939B94, 0x1D528623, 0xF12F560E,
	...
	0x18197087, 0x1CD86D30, 0x29F3D35 , 0x65E2082 , 0xB1D065B , 0xFDC1BEC , 0x3793A651,
...
uint8_t GxCas_Os_Sleep(uint32_t ms)
{
	...
	if (ms >= 100) {
		...
		while(1) {
			...
			temp_value = endMs - startMs;
			if (temp_value >= ms)
				return 0;

			temp_value = ms - temp_value;
			if(temp_value <= 9){
				return 0;
			}else if(temp_value < 20){
				GxCore_ThreadDelay(temp_value);
			}else{
...
int32_t GxCas_ChipId()
{
	...
	chip_id = GxChipDetect(handle);
	GxAVDestroyDevice(handle);
	return chip_id;
}
