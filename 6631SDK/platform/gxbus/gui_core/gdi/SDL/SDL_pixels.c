...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_pixels.c,v 1.12 2004/11/30 14:28:20 slouken Exp $";
#endif
 ...
 */
SDL_PixelFormat *SDL_AllocFormat(int bpp, Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
	SDL_PixelFormat *format;
	Uint32 mask;

	/* Allocate an empty pixel format structure */
	format = (SDL_PixelFormat *)GUI_MALLOCZ(sizeof(*format));
	if (format == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	case 1:
		...
		format->palette = (SDL_Palette *) GUI_MALLOC(sizeof(SDL_Palette));
		...
		(format->palette)->colors = (SDL_Color *) GUI_MALLOC((format->palette)->ncolors * sizeof(SDL_Color));
	...
	case 8:
		...
		format->palette = (SDL_Palette *) GUI_MALLOCZ(sizeof(SDL_Palette));
		if (format->palette == NULL) {
			SDL_FreeFormat(format);
			SDL_OutOfMemory();
			return (NULL);
		...
		(format->palette)->colors = (SDL_Color *) GUI_MALLOCZ((format->palette)->ncolors * sizeof(SDL_Color));
		if ((format->palette)->colors == NULL) {
			SDL_FreeFormat(format);
			SDL_OutOfMemory();
			return (NULL);
...
/*
 * GxCore_Free a previously allocated format structure
 */
	...
	if (format) {
		if (format->palette) {
			if (format->palette->colors) {
				GUI_FREE(format->palette->colors);
			}
			GUI_FREE(format->palette);
		}
		GUI_FREE(format);
	}
}
...
Uint32 SDL_MapRGB(SDL_PixelFormat * format, Uint8 r, Uint8 g, Uint8 b)
{
	if (format->palette == NULL) {
		return (r >> format->Rloss) << format->Rshift
		    | (g >> format->Gloss) << format->Gshift | (b >> format->Bloss) << format->Bshift | format->Amask;
	} else {
...
Uint32 SDL_MapRGBA(SDL_PixelFormat * format, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	if (format->palette == NULL) {
		return (r >> format->Rloss) << format->Rshift
		    ...
		    | (b >> format->Bloss) << format->Bshift | ((a >> format->Aloss) << format->Ashift & format->Amask);
	} else {
		return SDL_FindColor(format->palette, r, g, b);
	}
}
...
void SDL_ApplyGamma(Uint16 * gamma, SDL_Color * colors, SDL_Color * output, int ncolors)
{
	int i;
...
static Uint8 *Map1to1(SDL_Palette * src, SDL_Palette * dst, int *identical)
{
	...
	if (identical) {
		if (src->ncolors <= dst->ncolors) {
			...
			if (memcmp(src->colors, dst->colors, src->ncolors * sizeof(SDL_Color)) == 0) {
				*identical = 1;
				return (NULL);
			}
		}
	...
	map = (Uint8 *) GxCore_Malloc(src->ncolors);
	if (map == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	for (i = 0; i < src->ncolors; ++i) {
		map[i] = SDL_FindColor(dst, src->colors[i].r, src->colors[i].g, src->colors[i].b);
	}
	return (map);
}
...
static Uint8 *Map1toN(SDL_Palette * src, SDL_PixelFormat * dst)
{
	...
	map = (Uint8 *) GxCore_Malloc(src->ncolors * bpp);
	if (map == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	for (i = 0; i < src->ncolors; ++i) {
		ASSEMBLE_RGBA(&map[i * bpp], dst->BytesPerPixel, dst,
			      src->colors[i].r, src->colors[i].g, src->colors[i].b, alpha);
	}
	return (map);
}
...
SDL_BlitMap *SDL_AllocBlitMap(void)
{
	...
	map = (SDL_BlitMap *) GUI_MALLOCZ(sizeof(*map));
	if (map == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	map->sw_data = (struct private_swaccel *)GUI_MALLOCZ(sizeof(*map->sw_data));
	if (map->sw_data == NULL) {
		SDL_FreeBlitMap(map);
		SDL_OutOfMemory();
		return (NULL);
...
int SDL_MapSurface(SDL_Surface * src, SDL_Surface * dst)
{
	...
	default:
		...
		case 1:
			/* BitField --> Palette */
			map->table = MapNto1(srcfmt, dstfmt->palette, &map->identity);
			if (!map->identity) {
				if (map->table == NULL) {
					return (-1);
				}
