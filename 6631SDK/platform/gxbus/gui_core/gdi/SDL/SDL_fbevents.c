...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_fbevents.c,v 1.13 2004/01/04 16:49:25 slouken Exp $";
#endif
		...
		for (i = 0; i < NR_KEYS; ++i) {
			...
			if (ioctl(fd, KDGKBENT, &entry) == 0) {
				...
				if ((KTYP(entry.kb_value) == KT_LATIN) ||
				    (KTYP(entry.kb_value) == KT_ASCII) || (KTYP(entry.kb_value) == KT_LETTER)) {
					vga_keymap[map][i] = entry.kb_value;
				}
			}
		}
...
int FB_OpenKeyboard(_THIS)
{
	...
	if (keyboard_fd < 0) {
		...
		/* Try to query for a GxCore_Free virtual terminal */
		tty0_fd = -1;
...
#ifdef DEBUG_MOUSE
					gxlogf( "Using IMPS2 mouse\n");
#endif
					mouse_drv = MOUSE_IMPS2;
				} else {
#ifdef DEBUG_MOUSE
					gxlogf( "Using PS2 mouse\n");
#endif
					mouse_drv = MOUSE_PS2;
				}
			}
...
#ifdef DEBUG_MOUSE
			gxlogf( "Using Microsoft mouse on %s\n", mousedev);
#endif
			mouse_drv = MOUSE_MS;
		}
	}
...
static void handle_mouse(_THIS)
{
	...
	nread += start;
#ifdef DEBUG_MOUSE
	gxlogf( "Read %d bytes from mouse, start = %d\n", nread, start);
#endif
...
static void switch_vt(_THIS, unsigned short which)
{
	...
	screen_contents = (Uint8 *) GxCore_Malloc(screen_arealen);
	if (screen_contents) {
