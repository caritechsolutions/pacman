...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_timer.c,v 1.3 2002/03/06 11:23:02 slouken Exp $";
#endif
...
#ifdef DEBUG_TIMERS
			gxlogd("Executing timer %p (thread = %d)\n", t, SDL_ThreadID());
#endif
...
SDL_TimerID SDL_AddTimer(Uint32 interval, SDL_NewTimerCallback callback, void *param)
{
	...
	SDL_mutexP(SDL_timer_mutex);
	t = (SDL_TimerID) GxCore_Malloc(sizeof(struct _SDL_TimerID));
	if (t) {
...
#ifdef DEBUG_TIMERS
	gxlogd("SDL_AddTimer(%d) = %08x num_timers = %d\n", interval, (Uint32) t, num_timers);
#endif
	SDL_mutexV(SDL_timer_mutex);
	return t;
}
...
#ifdef DEBUG_TIMERS
	gxlogd("SDL_RemoveTimer(%08x) = %d num_timers = %d thread = %d\n", (Uint32) id, removed, num_timers,
	       SDL_ThreadID());
#endif
	SDL_mutexV(SDL_timer_mutex);
	return removed;
...
int SDL_SetTimer(Uint32 ms, SDL_TimerCallback callback)
{
	...
	if (ms) {
		if (SDL_timer_threaded) {
			retval = (SDL_AddTimer(ms, callback_wrapper, (void *)callback) != NULL);
		} else {
			SDL_timer_running = 1;
