...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_video.c,v 1.49 2004/11/25 15:47:49 pmandin Exp $";
#endif
...
SDL_VideoDevice *gx_current_video = NULL;
...
int SDL_VideoInit(const char *driver_name, Uint32 flags)
{
	...
	/* Check to make sure we don't overwrite 'gx_current_video' */
	if (gx_current_video != NULL) {
		SDL_VideoQuit();
	}
	...
	gx_current_video = video;
	gx_current_video->name = bootstrap[i]->name;
	...
	SDL_VideoSurface = SDL_CreateRGBSurface(video_flags, 0, 0,
						vformat.BitsPerPixel, vformat.Rmask, vformat.Gmask, vformat.Bmask, 0);
	...
	if (SDL_VideoSurface->format->palette) {
		...
		video->SetColors(video, 0, vf->palette->ncolors, vf->palette->colors);
	}
#endif
...
{
	if (gx_current_video != NULL) {
		strncpy(namebuf, gx_current_video->name, maxlen - 1);
		namebuf[maxlen - 1] = '\0';
		return (namebuf);
	}
	return (NULL);
...
SDL_Surface *SDL_GetVideoSurface(void)
{
	...
	visible = NULL;
	if (gx_current_video) {
		visible = gx_current_video->visible;
	}
	return (visible);
}
...
{
	SDL_VideoDevice *video = gx_current_video;
	SDL_VideoDevice *this = gx_current_video;
	SDL_Rect **modes;
...
int SDL_VideoModeOK(int width, int height, int bpp, Uint32 flags)
{
	...
	for (b = 0; !supported && SDL_closest_depths[table][b]; ++b) {
		...
		if (sizes == (SDL_Rect **) 0) {
			...
			supported = 1;
			continue;
		} else if (gx_current_video->handles_any_size) {
	...
	if (SDL_ShadowSurface->format->palette) {
		...
		if (depth == (SDL_VideoSurface->format)->BitsPerPixel) {
			memcpy(SDL_ShadowSurface->format->palette->colors,
			       ...
			       SDL_VideoSurface->format->palette->ncolors * sizeof(SDL_Color));
		} else {
...
SDL_Surface *SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags)
{
	...
	if (video->physpal) {
		GxCore_Free(video->physpal->colors);
		GxCore_Free(video->physpal);
		video->physpal = NULL;
	}
	if (video->gammacols) {
		GxCore_Free(video->gammacols);
		video->gammacols = NULL;
	}
...
#undef SDL_PROC
	...
	if ((flags & SDL_OPENGLBLIT) == SDL_OPENGLBLIT) {
		...
		if ((bpp == 16) &&
			...
			video->is_32bit = 0;
			SDL_VideoSurface = SDL_CreateRGBSurface(flags, width, height, 16, 31 << 11, 63 << 5, 31, 0);
		} else
#endif				/* OpenGL 1.2 */
		{
			video->is_32bit = 1;
			SDL_VideoSurface = SDL_CreateRGBSurface(flags, width, height, 32,
		...
		/* GxCore_Free the original video mode surface (is this safe?) */
		...
		video->glTexImage2D(GL_TEXTURE_2D,
				    0,
				    video->is_32bit ? GL_RGBA : GL_RGB, 256, 256, 0, video->is_32bit ? GL_RGBA : GL_RGB,
#ifdef GL_VERSION_1_2
	...
	if (!(SDL_VideoSurface->flags & SDL_OPENGL) &&
	    ((!(flags & SDL_ANYFORMAT) &&
	     ...
	     ((flags & SDL_HWPALETTE) && !(SDL_VideoSurface->flags & SDL_HWPALETTE)) ||
	     ...
	     ((flags & SDL_DOUBLEBUF) &&
	      (SDL_VideoSurface->flags & SDL_HWSURFACE) && !(SDL_VideoSurface->flags & SDL_DOUBLEBUF))
	    )) {
...
SDL_Surface *SDL_DisplayFormat(SDL_Surface * surface)
{
	...
	if (((SDL_PublicSurface->flags & SDL_HWSURFACE) == SDL_HWSURFACE) && gx_current_video->info.blit_hw)
		flags = SDL_HWSURFACE;
	else
		flags = SDL_SWSURFACE;
...
static void SetPalette_logical(SDL_Surface * screen, SDL_Color * colors, int firstcolor, int ncolors)
{
	...
	if (colors != (pal->colors + firstcolor)) {
		memcpy(pal->colors + firstcolor, colors, ncolors * sizeof(*colors));
	}
	...
	if ((screen == SDL_ShadowSurface) && vidpal) {
		 ...
		 */
		memcpy(vidpal->colors + firstcolor, colors, ncolors * sizeof(*colors));
	}
...
static int SetPalette_physical(SDL_Surface * screen, SDL_Color * colors, int firstcolor, int ncolors)
{
	...
	if (video->physpal) {
		 ...
		 */
		memcpy(video->physpal->colors + firstcolor, colors, ncolors * sizeof(*colors));
	}
	if (screen == SDL_ShadowSurface) {
		if (SDL_VideoSurface->flags & SDL_HWPALETTE) {
			...
			if (video->gamma) {
				if (!video->gammacols) {
					...
					video->gammacols = GxCore_Malloc(pp->ncolors * sizeof(SDL_Color));
...
int SDL_SetPalette(SDL_Surface * screen, int which, SDL_Color * colors, int firstcolor, int ncolors)
{
	...
	if (which & SDL_PHYSPAL) {
		...
		if (!video->physpal && !(which & SDL_LOGPAL)) {
			...
			int size;
			SDL_Palette *pp = GxCore_Malloc(sizeof(*pp));
			if (!pp) {
				return 0;
			}
			gx_current_video->physpal = pp;
			pp->ncolors = pal->ncolors;
			size = pp->ncolors * sizeof(SDL_Color);
			pp->colors = GxCore_Malloc(size);
			if (!pp->colors) {
				return 0;
			}
...
void SDL_VideoQuit(void)
{
	...
	if (gx_current_video) {
		...
		/* GxCore_Free any lingering surfaces */
		ready_to_go = SDL_ShadowSurface;
		SDL_ShadowSurface = NULL;
		SDL_FreeSurface(ready_to_go);
		if (SDL_VideoSurface != NULL) {
		...
		if (video->gamma) {
			GxCore_Free(video->gamma);
			video->gamma = NULL;
		}
		if (video->wm_title != NULL) {
			GxCore_Free(video->wm_title);
			video->wm_title = NULL;
		}
		if (video->wm_icon != NULL) {
			GxCore_Free(video->wm_icon);
			video->wm_icon = NULL;
		}

		/* Finish cleaning up video subsystem */
		video->Free(this);
		gx_current_video = NULL;
	}
	return;
}
	...
	for (i = 0; i < numrects; i++) {
		...
		for (y = 0; y <= rects[i].h / 256; y++) {
			...
			for (x = 0; x <= rects[i].w / 256; x++) {
				...
				this->glFlush();
				this->glTexSubImage2D(GL_TEXTURE_2D,
						      0, 0, 0, update.w, update.h, this->is_32bit ? GL_RGBA : GL_RGB,
#ifdef GL_VERSION_1_2
...
void SDL_WM_SetCaption(const char *title, const char *icon)
{
	...
	if (video) {
		if (title) {
			...
			video->wm_title = (char *)GxCore_Malloc(strlen(title) + 1);
			if (video->wm_title != NULL) {
		...
		if (icon) {
			...
			video->wm_icon = (char *)GxCore_Malloc(strlen(icon) + 1);
			if (video->wm_icon != NULL) {
...
void SDL_WM_SetIcon(SDL_Surface * icon, Uint8 * mask)
{
	...
	if (icon && video->SetIcon) {
		...
		if (mask == NULL) {
			...
			int flags = 0;
			mask = (Uint8 *) GxCore_Malloc(mask_len);
			if (mask == NULL) {
				return;
			}
...
#ifdef DEBUG_GRAB
	gxlogd("SDL_WM_GrabInputRaw(%d) ... ", mode);
#endif
...
#ifdef DEBUG_GRAB
	gxlogd("SDL_WM_GrabInput(%d) ... ", mode);
#endif
