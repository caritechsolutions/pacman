...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_fbelo.c,v 1.7 2004/01/04 16:49:25 slouken Exp $";
#endif
...
int eloGetPacket(unsigned char *buffer, int *buffer_p, int *checksum, int fd)
{
	...
	if (fd == 0) {
		...
		num_bytes = read(fd, (char *)(buffer + *buffer_p), ELO_PACKET_SIZE - *buffer_p);
	}

	if (num_bytes < 0) {
#ifdef DEBUG_MOUSE
		gxlogd ( "System error while reading from Elographics touchscreen.\n");
#endif
		return 0;
	}
...
#ifdef DEBUG_MOUSE
		gxlogd("System error while sending to Elographics touchscreen.\n");
#endif
		return 0;
	} else {
		return 1;
...
int eloWaitAck(int fd)
{
	...
	if (eloWaitReply(ELO_ACK, packet, fd)) {
		...
		if (nb_errors != 0) {
#ifdef DEBUG_MOUSE
			gxlogd ( "Elographics acknowledge packet reports %d errors\n", nb_errors);
#endif
		}
		return 1;
	} else {
...
int eloInitController(int fd)
{
	...
	if (result < 0) {
#ifdef DEBUG_MOUSE
		gxlogd ( "Unable to configure Elographics touchscreen port\n");
#endif
		return 0;
	}
...
#ifdef DEBUG_MOUSE
		gxlogd ( "Not at the specified rate or model 2310, will continue\n");
#endif
	}
	...
	if (eloSendQuery(req, reply, fd)) {
#ifdef DEBUG_MOUSE
		gxlogd ( "Ok, controller configured!\n");
#endif
	} else {
#ifdef DEBUG_MOUSE
		gxlogd ( "Unable to ask Elographics touchscreen identification\n");
#endif
		return 0;
	}
...
#ifdef DEBUG_MOUSE
		gxlogd ( "Unable to change Elographics touchscreen operating mode\n");
#endif
		return 0;
	}
	...
	if (!eloSendControl(req, fd)) {
#ifdef DEBUG_MOUSE
		gxlogd ( "Unable to change Elographics touchscreen reports timings\n");
#endif
		return 0;
	}
