...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_error.c,v 1.5 2004/01/04 16:49:10 slouken Exp $";
#endif
...
void SDL_SetError(const char *fmt, ...)
{
	...
	while (*fmt) {
		if (*fmt++ == '%') {
			...
			case 'c':
				error->args[error->argc++].value_c = va_arg(ap, unsigned char);
				break;
#endif
			case 'd':
			...
			case 's':
				{
					...
					strncpy((char *)error->args[index].buf, va_arg(ap, char *), ERR_MAX_STRLEN);
...
#endif
				case 'd':
					len = PrintInt(msg, maxlen, error->args[argi++].value_i);
					msg += len;
					maxlen -= len;
					break;
				case 'f':
					len = PrintDouble(msg, maxlen, error->args[argi++].value_f);
					msg += len;
					maxlen -= len;
					break;
				case 'p':
					len = PrintPointer(msg, maxlen, error->args[argi++].value_ptr);
					msg += len;
					maxlen -= len;
					break;
...
Uint8 *SDL_GetErrorMsg(Uint8 * errstr, unsigned int maxlen)
{
	...
	errstr16 = (Uint16 *) GxCore_Malloc(maxlen * (sizeof *errstr16));
	if (!errstr16) {
	...
	/* GxCore_Free UNICODE buffer (if necessary) */
	GxCore_Free(errstr16);
