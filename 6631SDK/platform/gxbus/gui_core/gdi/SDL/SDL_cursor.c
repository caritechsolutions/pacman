...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_cursor.c,v 1.9 2004/01/04 16:49:21 slouken Exp $";
#endif
...
SDL_Cursor *SDL_CreateCursor(Uint8 * data, Uint8 * mask, int w, int h, int hot_x, int hot_y)
{
	...
	cursor = (SDL_Cursor *) GxCore_Malloc(sizeof *cursor);
	if (cursor == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	cursor->hot_x = hot_x;
	cursor->hot_y = hot_y;
	cursor->data = (Uint8 *) GxCore_Malloc((w / 8) * h * 2);
	cursor->mask = cursor->data + ((w / 8) * h);
	cursor->save[0] = (Uint8 *) GxCore_Malloc(savelen * 2);
...
void SDL_SetCursor(SDL_Cursor * cursor)
{
	...
	if (SDL_cursor && (SDL_cursorstate & CURSOR_VISIBLE)) {
		...
		if (SDL_cursor->wm_cursor && video->ShowWMCursor(this, SDL_cursor->wm_cursor))
...
void SDL_FreeCursor(SDL_Cursor * cursor)
{
	if (cursor) {
		...
		if (cursor != SDL_defcursor) {
			...
			if (cursor->data) {
				GxCore_Free(cursor->data);
			}
			if (cursor->save[0]) {
				GxCore_Free(cursor->save[0]);
			}
...
void SDL_WarpMouse(Uint16 x, Uint16 y)
{
	...
	x += (this->screen->offset % this->screen->pitch) / this->screen->format->BytesPerPixel;
...
static void SDL_DrawCursorFast(SDL_Surface * screen, SDL_Rect * area)
{
	...
	case 1:{
			...
			dst = (Uint8 *) screen->pixels +
			    (SDL_cursor->area.y + area->y) * screen->pitch + SDL_cursor->area.x;
	...
	case 2:{
			...
			dst = (Uint16 *) screen->pixels +
			    (SDL_cursor->area.y + area->y) * screen->pitch / 2 + SDL_cursor->area.x;
	...
	case 3:{
			...
			dst = (Uint8 *) screen->pixels +
			    (SDL_cursor->area.y + area->y) * screen->pitch + SDL_cursor->area.x * 3;
	...
	case 4:{
			...
			dst = (Uint32 *) screen->pixels +
			    (SDL_cursor->area.y + area->y) * screen->pitch / 4 + SDL_cursor->area.x;
...
static void SDL_DrawCursorSlow(SDL_Surface * screen, SDL_Rect * area)
{
	...
	dst = (Uint8 *) screen->pixels + (SDL_cursor->area.y + area->y) * screen->pitch + SDL_cursor->area.x * dstbpp;
...
void SDL_DrawCursorNoLock(SDL_Surface * screen)
{
	...
	/* Copy mouse background */
	{
		...
		if ((screen == SDL_VideoSurface) || FORMAT_EQUAL(screen->format, SDL_VideoSurface->format)) {
...
void SDL_DrawCursor(SDL_Surface * screen)
{
	...
	if ((screen == SDL_VideoSurface) && ((screen->flags & SDL_HWSURFACE) != SDL_HWSURFACE)) {
		SDL_VideoDevice *video = gx_current_video;
		SDL_VideoDevice *this = gx_current_video;
		SDL_Rect area;
