...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_RLEaccel.c,v 1.11 2004/08/21 13:10:58 slouken Exp $";
#endif
...
int SDL_RLEBlit(SDL_Surface * src, SDL_Rect * srcrect, SDL_Surface * dst, SDL_Rect * dstrect)
{
	...
	x = dstrect->x;
	y = dstrect->y;
	dstbuf = (Uint8 *) dst->pixels + y * dst->pitch + x * src->format->BytesPerPixel;
...
static int RLEAlphaSurface(SDL_Surface * surface)
{
	...
	unsigned masksum;
	Uint8 *rlebuf, *dst;
	int (*copy_opaque) (void *, Uint32 *, int, SDL_PixelFormat *, SDL_PixelFormat *);
	int (*copy_transl) (void *, Uint32 *, int, SDL_PixelFormat *, SDL_PixelFormat *);
	...
	rlebuf = (Uint8 *) GxCore_Malloc(maxsize);
	if (!rlebuf) {
		SDL_OutOfMemory();
		return -1;
	}
...
#undef ADD_TRANSL_COUNTS
	...
	if ((surface->flags & SDL_PREALLOC) != SDL_PREALLOC && (surface->flags & SDL_HWSURFACE) != SDL_HWSURFACE) {
		GxCore_Free(surface->pixels);
		surface->pixels = NULL;
	}

	/* GxCore_Realloc the buffer to release unused memory */
	{
		Uint8 *p = GxCore_Realloc(rlebuf, dst - rlebuf);
		if (!p)
			p = rlebuf;
...
#undef ADD_COUNTS
	...
	/* GxCore_Realloc the buffer to release unused memory */
	{
		/* If GxCore_Realloc returns NULL, the original block is left intact */
		Uint8 *p = GxCore_Realloc(rlebuf, dst - rlebuf);
		if (!p)
			p = rlebuf;
		surface->map->sw_data->aux_data = p;
...
int SDL_RLESurface(SDL_Surface * surface)
{
	...
	if ((surface->flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY) {
		...
		if ((surface->flags & SDL_SRCALPHA) == SDL_SRCALPHA && surface->format->Amask != 0)
...
static SDL_bool UnRLEAlpha(SDL_Surface * surface)
{
	...
	int (*uncopy_opaque) (Uint32 *, void *, int, RLEDestFormat *, SDL_PixelFormat *);
	int (*uncopy_transl) (Uint32 *, void *, int, RLEDestFormat *, SDL_PixelFormat *);
	int w = surface->w;
	...
	surface->pixels = GxCore_Malloc(surface->h * surface->pitch);
	if (!surface->pixels) {
		return (SDL_FALSE);
	}
...
void SDL_UnRLESurface(SDL_Surface * surface, int recode)
{
	if ((surface->flags & SDL_RLEACCEL) == SDL_RLEACCEL) {
		...
		if (surface->map && surface->map->sw_data->aux_data) {
			GxCore_Free(surface->map->sw_data->aux_data);
