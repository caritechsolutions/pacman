...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_fbvideo.c,v 1.12 2004/08/21 11:34:24 slouken Exp $";
#endif
...
static void FB_SavePalette(_THIS, struct fb_fix_screeninfo *finfo, struct fb_var_screeninfo *vinfo);
...
static SDL_VideoDevice *FB_CreateDevice(int devindex)
{
	...
	this = (SDL_VideoDevice *) GxCore_Malloc(sizeof(SDL_VideoDevice));
	if (this) {
		memset(this, 0, (sizeof *this));
		this->hidden = (struct SDL_PrivateVideoData *)
		    GxCore_Malloc((sizeof *this->hidden));
	}
...
static int FB_CheckMode(_THIS, struct fb_var_screeninfo *vinfo, int index, unsigned int *w, unsigned int *h)
{
	int mode_okay;
...
#ifdef FBCON_DEBUG
		gxlogf ( "Checked mode %dx%d at %d bpp, got mode %dx%d at %d bpp\n", *w, *h, (index + 1) * 8,
...
static int FB_AddMode(_THIS, int index, unsigned int w, unsigned int h)
{
...
#ifdef FBCON_DEBUG
			gxlogf ( "We already have mode %dx%d at %d bytes per pixel\n", w, h, index + 1);
#endif
			return (0);
		}
	}
	...
	for (i = 0; i < (sizeof(vesa_timings) / sizeof(vesa_timings[0])); ++i) {
		if ((w == vesa_timings[i].xres) && (h == vesa_timings[i].yres) && vesa_timings[i].pixclock) {
			next_mode = i;
			break;
		}
	}
	if (next_mode == -1) {
#ifdef FBCON_DEBUG
		gxlogf ( "No valid timing line for mode %dx%d\n", w, h);
#endif
		return (0);
	}

	/* Set up the new video mode rectangle */
	mode = (SDL_Rect *) GxCore_Malloc(sizeof *mode);
	if (mode == NULL) {
		SDL_OutOfMemory();
		return (-1);
	}
	...
	mode->h = h;
#ifdef FBCON_DEBUG
	gxlogf( "Adding mode %dx%d at %d bytes per pixel\n", w, h, index + 1);
#endif

	/* Allocate the new list of modes, and fill in the new mode */
	next_mode = SDL_nummodes[index];
	SDL_modelist[index] = (SDL_Rect **)
	    GxCore_Realloc(SDL_modelist[index], (1 + next_mode + 1) * sizeof(SDL_Rect *));
...
static int FB_VideoInit(_THIS, SDL_PixelFormat * vformat)
{
	...
	#ifdef i386
	mapped_offset = (((long)finfo.smem_start) - (((long)finfo.smem_start) & ~(PAGE_SIZE - 1)));
	#endif
	...
	if (finfo.accel && finfo.mmio_len) {
		...
		mapped_io = mmap(NULL, mapped_iolen, PROT_READ | PROT_WRITE, MAP_SHARED, console_fd, mapped_memlen);
		if (mapped_io == (char *)-1) {
	...
	if (mapped_io) {
...
#ifdef FBACCEL_DEBUG
			gxlogf ("Matrox hardware accelerator!\n");
#endif
			FB_MatroxAccel(this, finfo.accel);
			break;
		case FB_ACCEL_3DFX_BANSHEE:
#ifdef FBACCEL_DEBUG
			gxlogf("3DFX hardware accelerator!\n");
#endif
		...
		case FB_ACCEL_NV4:
#ifdef FBACCEL_DEBUG
			gxlogf("NVidia hardware accelerator!\n");
#endif
			FB_RivaAccel(this, finfo.accel);
			break;
		default:
#ifdef FBACCEL_DEBUG
			gxlogf("Unknown hardware accelerator.\n");
#endif
			break;
		}
	}
...
{
	gxlogf( "Printing vinfo:\n");
	gxlogf( "\txres: %d\n", vinfo->xres);
	gxlogf( "\tyres: %d\n", vinfo->yres);
	gxlogf( "\txres_virtual: %d\n", vinfo->xres_virtual);
	gxlogf( "\tyres_virtual: %d\n", vinfo->yres_virtual);
	gxlogf( "\txoffset: %d\n", vinfo->xoffset);
	gxlogf( "\tyoffset: %d\n", vinfo->yoffset);
	gxlogf( "\tbits_per_pixel: %d\n", vinfo->bits_per_pixel);
	gxlogf( "\tgrayscale: %d\n", vinfo->grayscale);
	gxlogf( "\tnonstd: %d\n", vinfo->nonstd);
	gxlogf( "\tactivate: %d\n", vinfo->activate);
	gxlogf( "\theight: %d\n", vinfo->height);
	gxlogf( "\twidth: %d\n", vinfo->width);
	gxlogf( "\taccel_flags: %d\n", vinfo->accel_flags);
	gxlogf( "\tpixclock: %d\n", vinfo->pixclock);
	gxlogf( "\tleft_margin: %d\n", vinfo->left_margin);
	gxlogf( "\tright_margin: %d\n", vinfo->right_margin);
	gxlogf( "\tupper_margin: %d\n", vinfo->upper_margin);
	gxlogf( "\tlower_margin: %d\n", vinfo->lower_margin);
	gxlogf( "\thsync_len: %d\n", vinfo->hsync_len);
	gxlogf( "\tvsync_len: %d\n", vinfo->vsync_len);
	gxlogf( "\tsync: %d\n", vinfo->sync);
	gxlogf( "\tvmode: %d\n", vinfo->vmode);
...
static int choose_vesa_mode(struct fb_var_screeninfo *vinfo)
{
	...
	matched = 0;
	for (i = 0; i < (sizeof(vesa_timings) / sizeof(vesa_timings[0])); ++i) {
		if ((vinfo->xres == vesa_timings[i].xres) && (vinfo->yres == vesa_timings[i].yres)) {
#ifdef FBCON_DEBUG
			gxlogf( "Using VESA timings for %dx%d\n", vinfo->xres, vinfo->yres);
#endif
...
static SDL_Surface *FB_SetVGA16Mode(_THIS, SDL_Surface * current, int width, int height, int bpp, Uint32 flags)
{
	...
	current->pixels = GxCore_Malloc(current->h * current->pitch);
...
static SDL_Surface *FB_SetVideoMode(_THIS, SDL_Surface * current, int width, int height, int bpp, Uint32 flags)
{
...
#ifdef FBCON_DEBUG
	gxlogf( "Printing original vinfo:\n");
	print_vinfo(&vinfo);
#endif
	if ((vinfo.xres != width) || (vinfo.yres != height) || (vinfo.bits_per_pixel != bpp) || (flags & SDL_DOUBLEBUF)) {
	...
	if (!SDL_ReallocFormat(current, vinfo.bits_per_pixel, Rmask, Gmask, Bmask, 0)) {
		return (NULL);
	}
	...
	surfaces_mem = (char *)current->pixels + vinfo.yres_virtual * current->pitch;
...
void FB_DumpHWSurfaces(_THIS)
{
	...
	gxlogd("Memory left: %d (%d total)\n", surfaces_memleft, surfaces_memtotal);
	gxlogd("\n");
	gxlogd("         Base  Size\n");
	for (bucket = &surfaces; bucket; bucket = bucket->next) {
		gxlogd("Bucket:  %p, %d (%s)\n", bucket->base, bucket->size, bucket->used ? "used" : "GxCore_Free");
		if (bucket->prev) {
			if (bucket->base != bucket->prev->base + bucket->prev->size) {
				gxlogd("Warning, corrupt bucket list! (prev)\n");
			}
		} else {
			if (bucket != &surfaces) {
				gxlogd("Warning, corrupt bucket list! (!prev)\n");
		...
		if (bucket->next) {
			if (bucket->next->base != bucket->base + bucket->size) {
				gxlogd("Warning, corrupt bucket list! (next)\n");
			}
		}
	}
	gxlogd("\n");
...
static int FB_InitHWSurfaces(_THIS, SDL_Surface * screen, char *base, int size)
{
	...
	if (surfaces_memleft > 0) {
		bucket = (vidmem_bucket *) GxCore_Malloc(sizeof(*bucket));
		if (bucket == NULL) {
			SDL_OutOfMemory();
			return (-1);
		}
...
#ifdef FBCON_DEBUG
	gxlogf( "Allocating bucket of %d bytes\n", size);
#endif
...
#ifdef FBCON_DEBUG
		gxlogf( "Adding new GxCore_Free bucket of %d bytes\n", extra);
#endif
		newbucket = (vidmem_bucket *) GxCore_Malloc(sizeof(*newbucket));
		if (newbucket == NULL) {
			SDL_OutOfMemory();
			return (-1);
		}
...
#ifdef DGA_DEBUG
		gxlogd("Freeing bucket of %d bytes\n", bucket->size);
#endif
...
#ifdef DGA_DEBUG
			gxlogd("Merging with next bucket, for %d total bytes\n", bucket->size + bucket->next->size);
#endif
...
#ifdef DGA_DEBUG
			gxlogd("Merging with previous bucket, for %d total bytes\n", bucket->prev->size + bucket->size);
#endif
			freeable = bucket;
			bucket->prev->size += bucket->size;
...
static void FB_SavePalette(_THIS, struct fb_fix_screeninfo *finfo, struct fb_var_screeninfo *vinfo)
{
	...
	if (finfo->visual == FB_VISUAL_PSEUDOCOLOR) {
		...
		saved_cmap = (__u16 *) GxCore_Malloc(3 * saved_cmaplen * sizeof(*saved_cmap));
		if (saved_cmap != NULL) {
	...
	if (finfo->visual == FB_VISUAL_DIRECTCOLOR) {
		...
		for (i = 0; i < 256; ++i) {
			new_entries[(0 * 256) + i] =
			    new_entries[(1 * 256) + i] = new_entries[(2 * 256) + i] = (i << 8) | i;
		}
...
static int FB_SetColors(_THIS, int firstcolor, int ncolors, SDL_Color * colors)
{
	...
	if ((ioctl(console_fd, FBIOPUTCMAP, &cmap) < 0) || !(this->screen->flags & SDL_HWPALETTE)) {
...
static void FB_VideoQuit(_THIS)
{
	...
	if (this->screen) {
		/* Clear screen and tell SDL not to GxCore_Free the pixels */
	...
	for (i = 0; i < NUM_MODELISTS; ++i) {
		if (SDL_modelist[i] != NULL) {
			for (j = 0; SDL_modelist[i][j]; ++j) {
				GxCore_Free(SDL_modelist[i][j]);
			}
			GxCore_Free(SDL_modelist[i]);
			SDL_modelist[i] = NULL;
		}
	}
