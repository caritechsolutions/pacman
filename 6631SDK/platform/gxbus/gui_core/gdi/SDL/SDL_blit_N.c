...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_blit_N.c,v 1.9 2004/01/04 16:49:21 slouken Exp $";
#endif
...
static void Blit_RGB888_index8(SDL_BlitInfo * info)
{
	...
	if (map == NULL) {
		...
		while (height--) {
#ifdef USE_DUFFS_LOOP
			DUFFS_LOOP(RGB888_RGB332(pixel, *src); *dst++ = map[pixel]; ++src;, width);
#else
			for (c = width / 4; c; --c) {
				/* Pack RGB into 8bit pixel */
...
#ifdef USE_DUFFS_LOOP
	while (height--) {
		DUFFS_LOOP(RGB888_RGB555(dst, src); ++src; ++dst;, width);
		src += srcskip;
		dst += dstskip;
	}
#else
...
#ifdef USE_DUFFS_LOOP
	while (height--) {
		DUFFS_LOOP(RGB888_RGB565(dst, src); ++src; ++dst;, width);
		src += srcskip;
		dst += dstskip;
	}
#else
...
static void BlitNto1(SDL_BlitInfo * info)
{
	...
	if (map == NULL) {
		while (height--) {
#ifdef USE_DUFFS_LOOP
			DUFFS_LOOP(DISEMBLE_RGB(src, srcbpp, srcfmt, pixel, sR, sG, sB); if (1) {
				   /* Pack RGB into 8bit pixel */
				   *dst = ((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0));}
				   dst++; src += srcbpp;, width) ;
#else
			for (c = width; c; --c) {
...
static void BlitNto1Key(SDL_BlitInfo * info)
{
	...
	if (palmap == NULL) {
		while (height--) {
			DUFFS_LOOP( {
				   DISEMBLE_RGB(src, srcbpp, srcfmt, pixel, sR, sG, sB); if ((pixel & rgbmask) != ckey) {
				   /* Pack RGB into 8bit pixel */
				   *dst = ((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0));}
				   dst++; src += srcbpp;}
				   , width) ;
		...
		while (height--) {
			DUFFS_LOOP( {
				   DISEMBLE_RGB(src, srcbpp, srcfmt, pixel, sR, sG, sB); if ((pixel & rgbmask) != ckey) {
				   /* Pack RGB into 8bit pixel */
				   *dst = palmap[((sR >> 5) << (3 + 2)) | ((sG >> 5) << (2)) | ((sB >> 6) << (0))];}
				   dst++; src += srcbpp;}
				   , width) ;
			src += srcskip;
			dst += dstskip;
...
static void BlitNtoNKey(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP( {
			   Uint32 pixel;
			   unsigned sR;
			   unsigned sG; unsigned sB; RETRIEVE_RGB_PIXEL(src, srcbpp, pixel); if (pixel != ckey) {
			   RGB_FROM_PIXEL(pixel, srcfmt, sR, sG, sB);
			   ASSEMBLE_RGBA(dst, dstbpp, dstfmt, sR, sG, sB, alpha);}
			   dst += dstbpp; src += srcbpp;}
			   , width) ;
		src += srcskip;
		dst += dstskip;
	}
...
static void BlitNtoNKeyCopyAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP( {
			   DISEMBLE_RGBA(src, srcbpp, srcfmt, pixel, sR, sG, sB, sA); if ((pixel & rgbmask) != ckey) {
			   ASSEMBLE_RGBA(dst, dstbpp, dstfmt, sR, sG, sB, sA);}
			   dst += dstbpp; src += srcbpp;}
			   , width) ;
		src += srcskip;
...
SDL_loblit SDL_CalculateBlitN(SDL_Surface * surface, int blit_index)
{
	...
	if (dstfmt->BitsPerPixel == 8) {
		...
		if ((srcfmt->BytesPerPixel == 4) &&
		    (srcfmt->Rmask == 0x00FF0000) && (srcfmt->Gmask == 0x0000FF00) && (srcfmt->Bmask == 0x000000FF)) {
		...
		for (which = 0; table[which].srcR; ++which) {
			if (srcfmt->Rmask == table[which].srcR &&
			    ...
			    (a_need & table[which].alpha) == a_need && (table[which].cpu_mmx == SDL_HasMMX()))
				break;
		}
	...
	if (blitfun == ConvertMMX)
		gxlogf ( "Using mmx blit\n");
	else if (blitfun == ConvertX86)
		gxlogf( "Using asm blit\n");
	else
#endif
	if ((blitfun == SDL_BlitNtoN) || (blitfun == SDL_BlitNto1))
		gxlogf( "Using C blit\n");
	else
		gxlogf( "Using optimized C blit\n");
#endif				/* DEBUG_ASM */
