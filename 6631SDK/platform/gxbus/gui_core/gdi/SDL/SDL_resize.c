...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_resize.c,v 1.5 2004/01/04 16:49:17 slouken Exp $";
#endif
...
int SDL_PrivateResize(int w, int h)
{
	...
	if (!w || !h || ((last_resize.w == w) && (last_resize.h == h))) {
		return (0);
	}
	last_resize.w = w;
	last_resize.h = h;
	if (!SDL_VideoSurface || ((w == SDL_VideoSurface->w) && (h == SDL_VideoSurface->h))) {
		return (0);
	}
