...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_blit_1.c,v 1.6 2004/01/04 16:49:21 slouken Exp $";
#endif
...
#ifdef USE_DUFFS_LOOP
		DUFFS_LOOP( {
			   o = *src * 4; dst[0] = map[o++]; dst[1] = map[o++]; dst[2] = map[o++];}
			   src++; dst += 3;, width);
#else
		for (c = width; c; --c) {
			o = *src * 4;
...
static void Blit1to3Key(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP( {
			   if (*src != ckey) {
			   o = *src * 4; dst[0] = palmap[o++]; dst[1] = palmap[o++]; dst[2] = palmap[o++];}
			   src++; dst += 3;}
			   , width) ;
		src += srcskip;
		dst += dstskip;
...
static void Blit1toNAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		...
		DUFFS_LOOP4( {
			    ...
			    ASSEMBLE_RGB(dst, dstbpp, dstfmt, dR, dG, dB); src++; dst += dstbpp;}, width);
		src += srcskip;
		dst += dstskip;
	}
}
...
static void Blit1toNAlphaKey(SDL_BlitInfo * info)
{
	...
	while (height--) {
		...
		DUFFS_LOOP( {
			   ...
			   DISEMBLE_RGB(dst, dstbpp, dstfmt,
					pixel, dR, dG, dB);
			   ALPHA_BLEND(sR, sG, sB, A, dR, dG, dB); ASSEMBLE_RGB(dst, dstbpp, dstfmt, dR, dG, dB);}
			   src++; dst += dstbpp;}
			   , width) ;
		src += srcskip;
		dst += dstskip;
