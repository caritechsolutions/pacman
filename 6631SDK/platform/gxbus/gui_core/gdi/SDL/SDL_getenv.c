...
int SDL_putenv(const char *variable)
{
	...
	if (bufferlen > SDL_envmemlen) {
		char *newmem = (char *)GxCore_Realloc(SDL_envmem, bufferlen);
		if (newmem == NULL) {
			return -1;
		}
		SDL_envmem = newmem;
...
int SDL_putenv(const char *variable)
{
	...
	new_variable = GxCore_Strdup(variable);
	if (!new_variable) {
		return (-1);
	}
	...
	if (!added) {
		new_env = GxCore_Realloc(SDL_env, (i + 2) * sizeof(char *));
		if (new_env) {
			SDL_env = new_env;
			SDL_env[i++] = new_variable;
			SDL_env[i++] = (char *)0;
...
char *SDL_getenv(const char *name)
{
	...
	if (SDL_env) {
		...
		for (i = 0; SDL_env[i] && !value; ++i) {
			if ((strncmp(SDL_env[i], name, len) == 0) && (SDL_env[i][len] == '=')) {
...
int main(int argc, char *argv[])
{
	...
	gxlogd("Checking for non-existent variable... ");
	fflush(stdout);
	...
	gxlogd("Setting FIRST=VALUE1 in the environment... ");
	fflush(stdout);
	...
	gxlogd("Getting FIRST from the environment... ");
	fflush(stdout);
	...
	gxlogd("Setting SECOND=VALUE2 in the environment... ");
	fflush(stdout);
	...
	gxlogd("Getting SECOND from the environment... ");
	fflush(stdout);
	...
	gxlogd("Setting FIRST=NOVALUE in the environment... ");
	fflush(stdout);
