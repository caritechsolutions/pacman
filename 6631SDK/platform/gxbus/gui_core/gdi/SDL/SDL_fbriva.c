...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_fbriva.c,v 1.5 2004/01/04 16:49:25 slouken Exp $";
#endif
...
static void NV3WaitIdle(_THIS)
{
	...
	while ((Rop->FifoFree < FifoEmptyCount) || (*(mapped_io + PGRAPH_OFFSET + 0x000006B0) & 0x01)) ;
}

static void NV4WaitIdle(_THIS)
{
	RivaRop *Rop = (RivaRop *) (mapped_io + ROP_OFFSET);
	while ((Rop->FifoFree < FifoEmptyCount) || (*(mapped_io + PGRAPH_OFFSET + 0x00000700) & 0x01)) ;
}
