...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_systhread.c,v 1.5 2004/01/04 16:49:19 slouken Exp $";
#endif
...
int SDL_SYS_CreateThread(SDL_Thread * thread, void *args)
{
	...
	/* Create the thread and go! */
	#ifdef LINUX_OS
	thread->handle = clone(RunThread, stack, (CLONE_VM | CLONE_FS | CLONE_FILES | CLONE_SIGHAND), args);
	#endif
	if (thread->handle < 0) {
		GxCore_Free(thread->data);
		SDL_SetError("Not enough resources to create thread");
		return (-1);
	}
	return (0);
...
void SDL_SYS_WaitThread(SDL_Thread * thread)
{
	...
	sprintf(command, "ps ax|fgrep -v fgrep|fgrep -v '<zombie>'|fgrep %d >/dev/null", thread->handle);
