...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_surface.c,v 1.19 2004/08/21 05:29:45 slouken Exp $";
#endif
				  ...
				  Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
	...
	if (screen && ((screen->flags & SDL_HWSURFACE) == SDL_HWSURFACE)) {
		...
		if ((flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY) {
			if (!gx_current_video->info.blit_hw_CC) {
				flags &= ~SDL_HWSURFACE;
			}
		}
		if ((flags & SDL_SRCALPHA) == SDL_SRCALPHA) {
			if (!gx_current_video->info.blit_hw_A) {
				flags &= ~SDL_HWSURFACE;
			}
		}
	} else {
	...
	surface = (SDL_Surface *) GUI_MALLOCZ(sizeof(*surface));
	if (surface == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	/* Get the pixels */
	if (((flags & SDL_HWSURFACE) == SDL_SWSURFACE) || (video->AllocHWSurface(this, surface) < 0)) {
		if (surface->w && surface->h) {
			surface->pixels = (void *)GUI_MALLOCZ(surface->h * surface->pitch);
...
int SDL_SetColorKey(SDL_Surface * surface, Uint32 flag, Uint32 key)
{
	...
	if ((flag == (surface->flags & (SDL_SRCCOLORKEY | SDL_RLEACCELOK))) && (key == surface->format->colorkey)) {
		return (0);
	}
	...
	if (flag) {
		...
		if ((surface->flags & SDL_HWACCEL) == SDL_HWACCEL) {
			if ((video->SetHWColorKey == NULL) || (video->SetHWColorKey(this, surface, key) < 0)) {
...
int SDL_SetAlpha(SDL_Surface * surface, Uint32 flag, Uint8 value)
{
	...
	/* Optimize away operations that don't change anything */
	if ((flag == (surface->flags & (SDL_SRCALPHA | SDL_RLEACCELOK))) && (!flag || value == oldalpha)) {
		return (0);
	}
	...
	if (flag) {
		...
		if ((surface->flags & SDL_HWACCEL) == SDL_HWACCEL) {
			if ((video->SetHWAlpha == NULL) || (video->SetHWAlpha(this, surface, value) < 0)) {
				surface->flags &= ~SDL_HWACCEL;
			}
		}
		if (flag & SDL_RLEACCELOK) {
	 ...
	 */
	if ((surface->flags & SDL_HWACCEL) == SDL_HWACCEL
	    || oldflags != surface->flags || (((oldalpha + 1) ^ (value + 1)) & 0x100))
		SDL_InvalidateMap(surface->map);
	return (0);
}
...
int SDL_SetAlphaChannel(SDL_Surface * surface, Uint8 value)
{
	...
	if ((surface->format->Amask != 0xFF000000) && (surface->format->Amask != 0x000000FF)) {
...
int SDL_LowerBlit(SDL_Surface * src, SDL_Rect * srcrect, SDL_Surface * dst, SDL_Rect * dstrect)
{
	...
	if ((src->flags & SDL_HWACCEL) == SDL_HWACCEL) {
		if (src == SDL_VideoSurface) {
			hw_srcrect = *srcrect;
			hw_srcrect.x += gx_current_video->offset_x;
			hw_srcrect.y += gx_current_video->offset_y;
			srcrect = &hw_srcrect;
		}
		if (dst == SDL_VideoSurface) {
			hw_dstrect = *dstrect;
			hw_dstrect.x += gx_current_video->offset_x;
			hw_dstrect.y += gx_current_video->offset_y;
			dstrect = &hw_dstrect;
		}
...
int SDL_FillRect(SDL_Surface * dst, SDL_Rect * dstrect, Uint32 color)
{
	...
	if (((dst->flags & SDL_HWSURFACE) == SDL_HWSURFACE) && video->info.blit_fill) {
		SDL_Rect hw_rect;
		if (dst == SDL_VideoSurface) {
			hw_rect = *dstrect;
			hw_rect.x += gx_current_video->offset_x;
			hw_rect.y += gx_current_video->offset_y;
			dstrect = &hw_rect;
		}
	...
	row = (Uint8 *) dst->pixels + dstrect->y * dst->pitch + dstrect->x * dst->format->BytesPerPixel;
 ...
 */
SDL_Surface *SDL_ConvertSurface(SDL_Surface * surface, SDL_PixelFormat * format, Uint32 flags)
{
	SDL_Surface *convert;
	Uint32 colorkey = 0;
	Uint8 alpha = 0;
	...
	if (format->palette != NULL) {
		...
		for (i = 0; i < format->palette->ncolors; ++i) {
			if ((format->palette->colors[i].r != 0) ||
			    (format->palette->colors[i].g != 0) || (format->palette->colors[i].b != 0))
				break;
		}
	...
	if (format->palette && convert->format->palette) {
		memcpy(convert->format->palette->colors,
		       format->palette->colors, format->palette->ncolors * sizeof(SDL_Color));
...
/*
 * GxCore_Free a surface created by the above function.
 */
void SDL_FreeSurface(SDL_Surface * surface)
{
	/* GxCore_Free anything that's not NULL, and not the screen surface */
	if ((surface == NULL) || (gx_current_video && ((surface == SDL_ShadowSurface) || (surface == SDL_VideoSurface)))) {
		return;
	}
