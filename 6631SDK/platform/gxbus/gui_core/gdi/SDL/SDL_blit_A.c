...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_blit_A.c,v 1.9 2004/04/11 19:47:28 slouken Exp $";
#endif
...
static void BlitNto1SurfaceAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    ...
			    ALPHA_BLEND(sR, sG, sB, A, dR, dG, dB); dR &= 0xff; dG &= 0xff; dB &= 0xff;
...
static void BlitNto1PixelAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    ...
			    dG = dstfmt->palette->colors[*dst].g;
			    dB = dstfmt->palette->colors[*dst].b;
			    ALPHA_BLEND(sR, sG, sB, sA, dR, dG, dB); dR &= 0xff; dG &= 0xff; dB &= 0xff;
			    /* Pack RGB into 8bit pixel */
			    if (palmap == NULL) {
			    *dst = ((dR >> 5) << (3 + 2)) | ((dG >> 5) << (2)) | ((dB >> 6) << (0));}
			    else {
...
static void BlitNto1SurfaceAlphaKey(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP( {
			   ...
			   unsigned dR;
			   unsigned dG;
			   unsigned dB; DISEMBLE_RGB(src, srcbpp, srcfmt, pixel, sR, sG, sB); if (pixel != ckey) {
			   dR = dstfmt->palette->colors[*dst].r;
			   dG = dstfmt->palette->colors[*dst].g;
			   dB = dstfmt->palette->colors[*dst].b;
			   ALPHA_BLEND(sR, sG, sB, A, dR, dG, dB); dR &= 0xff; dG &= 0xff; dB &= 0xff;
...
static void BlitRGBtoRGBSurfaceAlpha128MMX(SDL_BlitInfo * info)
{
	...
	*(Uint64 *) load = 0xFF000000FF000000ULL;	/* dst alpha mask */
	...
	while (height--) {
		DUFFS_LOOP_DOUBLE2( {
				   Uint32 s = *srcp++;
				   Uint32 d = *dstp; *dstp++ = ((((s & 0x00fefefe) + (d & 0x00fefefe)) >> 1)
	...
	*(Uint64 *) load = 0xFF000000FF000000ULL;
	...
	while (height--) {
		DUFFS_LOOP4( {
			    ...
			    movd_m2r((*srcp), mm1);	/* src(ARGB) -> mm1 (0000ARGB) */
			    movd_m2r((*dstp), mm2);	/* dst(ARGB) -> mm2 (0000ARGB) */
			    pand_r2r(mm0, mm1); pand_r2r(mm6, mm2); por_r2r(mm1, mm2); movd_r2m(mm2, (*dstp));}
			    else {
			    movd_m2r((*srcp), mm1);	/* src(ARGB) -> mm1 (0000ARGB) */
			    punpcklbw_r2r(mm1, mm1);	/* AARRGGBB -> mm1 */
			    pand_r2r(mm3, mm1);	/* 0A0R0G0B -> mm1 */
	...
	if (alpha == 128) {
		...
		while (height--) {
			DUFFS_LOOP_DOUBLE2( {
					   ...
					   s = *srcp;
					   d = *dstp;
					   s1 = s & 0xff00ff; d1 = d & 0xff00ff; d1 = (d1 + ((s1 - d1) * alpha >> 8))
					   & 0xff00ff;
					   s &= 0xff00;
					   d &= 0xff00;
					   d = (d + ((s - d) * alpha >> 8)) & 0xff00;
					   *dstp = d1 | d | 0xff000000; ++srcp; ++dstp;}
					   , {
					   /* Two Pixels Blend */
					   s = *srcp;
					   d = *dstp;
					   ...
					   *dstp++ = d1 | ((d << 8) & 0xff00) | 0xff000000;
					   ...
					   d1 += (s1 - d1) * alpha >> 8;
					   d1 &= 0xff00ff;
					   *dstp = d1 | ((d >> 8) & 0xff00) | 0xff000000; ++srcp; ++dstp;}
					   , width);
			srcp += srcskip;
			dstp += dstskip;
		}
...
static void BlitRGBtoRGBPixelAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    Uint32 dalpha; Uint32 d; Uint32 s1; Uint32 d1; Uint32 s = *srcp; Uint32 alpha = s >> 24;
			    /* FIXME: Here we special-case opaque alpha since the
			       compositioning used (>>8 instead of /255) doesn't handle
			       it correctly. Also special-case alpha=0 for speed?
			       Benchmark this! */
			    ...
			    *dstp = (s & 0x00ffffff) | (*dstp & 0xff000000);}
			     ...
			     */
			    d = *dstp;
			    if(((d & 0x00FFFFFF) == gui_core->config.osd_trans) || ((d & 0x00FFFFFF) == gui_core->config.gui_trans))
			    	dalpha = (((d & 0xff000000) >> 24) * alpha >> 8) << 24;
			    else
				dalpha = d & 0xff000000;
			    s1 = s & 0xff00ff;
			    d1 = d & 0xff00ff;
			    ...
			    s &= 0xff00;
			    d &= 0xff00; d = (d + ((s - d) * alpha >> 8)) & 0xff00; *dstp = d1 | d | dalpha;}
			    }
			    ++srcp; ++dstp;}
			    , width) ;
		srcp += srcskip;
		...
		*(Uint64 *) load = 0x001F001F001F001FULL;
		...
		while (height--) {
			DUFFS_LOOP_QUATRO2( {
					   ...
					   s = (s | s << 16) & 0x07e0f81f;
					   d = (d | d << 16) & 0x07e0f81f;
					   d += (s - d) * alpha >> 5; d &= 0x07e0f81f; *dstp++ = d | d >> 16;}
					   , {
					   s = *srcp++; d = *dstp;
					   /*
					    * shift out the middle component (green) to
					   ...
					   d = (d | d << 16) & 0x07e0f81f;
					   d += (s - d) * alpha >> 5;
					   d &= 0x07e0f81f; *dstp++ = d | d >> 16; s = *srcp++; d = *dstp;
					   /*
					    * shift out the middle component (green) to
					    * the high 16 bits, and process all three RGB
					    * components at the same time.
		...
		*(Uint64 *) load = 0x001F001F001F001FULL;
		...
		while (height--) {
			DUFFS_LOOP_QUATRO2( {
					   ...
					   s = (s | s << 16) & 0x03e07c1f;
					   d = (d | d << 16) & 0x03e07c1f;
					   d += (s - d) * alpha >> 5; d &= 0x03e07c1f; *dstp++ = d | d >> 16;}
					   , {
					   s = *srcp++; d = *dstp;
					   /*
					    * shift out the middle component (green) to
					   ...
					   d = (d | d << 16) & 0x03e07c1f;
					   d += (s - d) * alpha >> 5;
					   d &= 0x03e07c1f; *dstp++ = d | d >> 16; s = *srcp++; d = *dstp;
					   /*
					    * shift out the middle component (green) to
					    * the high 16 bits, and process all three RGB
					    * components at the same time.
...
static void BlitARGBto565PixelAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    Uint32 s = *srcp; unsigned alpha = s >> 27;	/* downscale alpha to 5 bits */
			    /* FIXME: Here we special-case opaque alpha since the
			       compositioning used (>>8 instead of /255) doesn't handle
			       it correctly. Also special-case alpha=0 for speed?
			       Benchmark this! */
			    ...
			    *dstp = (s >> 8 & 0xf800) + (s >> 5 & 0x7e0)
			    + (s >> 3 & 0x1f);
                *dstp = SDL_Swap16(*dstp);}
			    else {
			    Uint32 d = SDL_Swap16(*dstp);
			    /*
			    ...
			    + (s >> 3 & 0x1f);
			    d = (d | d << 16) & 0x07e0f81f;
			    d += (s - d) * alpha >> 5; d &= 0x07e0f81f; *dstp = SDL_Swap16(d | d >> 16);}
			    }
			    srcp++; dstp++;}
			    , width) ;
		srcp += srcskip;
...
static void BlitARGBto555PixelAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    unsigned alpha; Uint32 s = *srcp; alpha = s >> 27;	/* downscale alpha to 5 bits */
			    /* FIXME: Here we special-case opaque alpha since the
			       compositioning used (>>8 instead of /255) doesn't handle
			       it correctly. Also special-case alpha=0 for speed?
			       Benchmark this! */
...
static void BlitNtoNSurfaceAlphaKey(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    ...
			    unsigned sB;
			    unsigned dR;
			    unsigned dG; unsigned dB; RETRIEVE_RGB_PIXEL(src, srcbpp, pixel); if (sA && pixel != ckey) {
...
static void BlitNtoNPixelAlpha(SDL_BlitInfo * info)
{
	...
	while (height--) {
		DUFFS_LOOP4( {
			    ...
			    unsigned dB;
			    unsigned sA;
			    unsigned dA; DISEMBLE_RGBA(src, srcbpp, srcfmt, pixel, sR, sG, sB, sA); if (sA) {
	...
	if (sf->Amask == 0) {
		if ((surface->flags & SDL_SRCCOLORKEY) == SDL_SRCCOLORKEY) {
			...
			case 4:
				if (sf->Rmask == df->Rmask
				    && sf->Gmask == df->Gmask
				    && sf->Bmask == df->Bmask
				    && (sf->Rmask | sf->Gmask | sf->Bmask) == 0xffffff && sf->BytesPerPixel == 4) {
#ifdef MMX_ASMBLIT
					if (SDL_HasMMX())
		...
		case 2:
			if (sf->BytesPerPixel == 4 && sf->Amask == 0xff000000
			    && sf->Gmask == 0xff00 && ((sf->Rmask == 0xff && df->Rmask == 0x1f)
		...
		case 4:
			if (sf->Amask == 0xff000000
			    && sf->Rmask == df->Rmask
			    && sf->Gmask == df->Gmask && sf->Bmask == df->Bmask && sf->BytesPerPixel == 4) {
#ifdef MMX_ASMBLIT
				if (SDL_Has3DNow())
