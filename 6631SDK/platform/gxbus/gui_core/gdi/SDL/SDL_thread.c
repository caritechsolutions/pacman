...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_thread.c,v 1.8 2004/01/04 16:49:18 slouken Exp $";
#endif
...
#ifdef DEBUG_THREADS
	gxlogd("Adding thread (%d already - %d max)\n", SDL_numthreads, SDL_maxthreads);
#endif
	if (SDL_numthreads == SDL_maxthreads) {
		threads = (SDL_Thread **) GxCore_Malloc((SDL_maxthreads + ARRAY_CHUNKSIZE) * (sizeof *threads));
		if (threads == NULL) {
			SDL_OutOfMemory();
			goto done;
		}
...
#ifdef DEBUG_THREADS
			gxlogd("Deleting thread (%d left - %d max)\n", SDL_numthreads, SDL_maxthreads);
#endif
		}
		SDL_mutexV(thread_lock);
	}
...
SDL_Thread *SDL_CreateThread(int (*fn) (void *), void *data)
{
	...
	thread = (SDL_Thread *) GxCore_Malloc(sizeof(*thread));
	if (thread == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
	...
	args = (thread_args *) GxCore_Malloc(sizeof(*args));
	if (args == NULL) {
		SDL_OutOfMemory();
		GxCore_Free(thread);
		return (NULL);
	...
	if (ret >= 0) {
		...
		/* Oops, failed.  Gotta GxCore_Free everything */
