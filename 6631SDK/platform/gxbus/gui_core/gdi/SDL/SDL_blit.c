...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_blit.c,v 1.12 2004/04/11 19:52:58 slouken Exp $";
#endif
...
static int SDL_SoftBlit(SDL_Surface * src, SDL_Rect * srcrect, SDL_Surface * dst, SDL_Rect * dstrect)
{
	...
	if (okay && srcrect->w && srcrect->h) {
		...
		info.s_pixels = (Uint8 *) src->pixels +
		    (Uint16) srcrect->y * src->pitch + (Uint16) srcrect->x * src->format->BytesPerPixel;
		info.s_width = srcrect->w;
		info.s_height = srcrect->h;
		info.s_skip = src->pitch - info.s_width * src->format->BytesPerPixel;
		info.d_pixels = (Uint8 *) dst->pixels +
		    (Uint16) dstrect->y * dst->pitch + (Uint16) dstrect->x * dst->format->BytesPerPixel;
...
int SDL_CalculateBlit(SDL_Surface * surface)
{
	...
	if (surface->map->identity) {
		...
		if ((surface->flags & SDL_HWSURFACE) == SDL_HWSURFACE) {
			...
			if (surface->map->dst->flags & SDL_HWSURFACE) {
				hw_blit_ok = gx_current_video->info.blit_hw;
			} else {
				hw_blit_ok = 0;
			}
			if (hw_blit_ok && (surface->flags & SDL_SRCCOLORKEY)) {
				hw_blit_ok = gx_current_video->info.blit_hw_CC;
			}
			if (hw_blit_ok && (surface->flags & SDL_SRCALPHA)) {
				hw_blit_ok = gx_current_video->info.blit_hw_A;
			}
		} else {
			/* We only support accelerated blitting to hardware */
			if (surface->map->dst->flags & SDL_HWSURFACE) {
				hw_blit_ok = gx_current_video->info.blit_sw;
			} else {
				hw_blit_ok = 0;
			}
			if (hw_blit_ok && (surface->flags & SDL_SRCCOLORKEY)) {
				hw_blit_ok = gx_current_video->info.blit_sw_CC;
			}
			if (hw_blit_ok && (surface->flags & SDL_SRCALPHA)) {
				hw_blit_ok = gx_current_video->info.blit_sw_A;
			}
		}
		if (hw_blit_ok) {
	...
	if (surface->flags & SDL_SRCALPHA && (surface->format->alpha != SDL_ALPHA_OPAQUE || surface->format->Amask)) {
		blit_index |= 2;
	}
	...
	if (surface->map->identity && blit_index == 0) {
		...
		if (surface->format->BitsPerPixel < 8) {
			...
			case 1:
				surface->map->sw_data->blit = SDL_CalculateBlit1(surface, blit_index);
				break;
			case 2:
			case 3:
			case 4:
				surface->map->sw_data->blit = SDL_CalculateBlitN(surface, blit_index);
				break;
			default:
	...
	if (surface->flags & SDL_RLEACCELOK && (surface->flags & SDL_HWACCEL) != SDL_HWACCEL) {

		if (surface->map->identity && (blit_index == 1 || (blit_index == 3 && !surface->format->Amask))) {
