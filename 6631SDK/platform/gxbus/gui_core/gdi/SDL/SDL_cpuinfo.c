...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_cpuinfo.c,v 1.16 2004/05/16 17:19:48 slouken Exp $";
#endif
...
static __inline__ int CPU_getCPUIDFeatures(void)
{
	...
	Set up for CPUID instruction
		    ...
		    xor eax, eax inc eax cpuid; Get family / model / stepping / features mov features, edx done:}
#endif
	return
		 features;
}
...
static __inline__ int CPU_getCPUIDFeaturesExt(void)
{
	...
	__asm {
		mov eax, 80000000 h;
		Query for extended functions cpuid; Get extended function limit cmp eax, 80000001 h jl done; Nope
