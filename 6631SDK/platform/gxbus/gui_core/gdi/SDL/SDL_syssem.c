...
#ifdef SAVE_RCSID
static char rcsid = "@(#) $Id: SDL_syssem.c,v 1.7 2004/01/04 16:49:19 slouken Exp $";
#endif
...
{
	SDL_sem *sem = (SDL_sem *) GxCore_Malloc(sizeof(SDL_sem));
	if (sem) {
...
SDL_sem *SDL_CreateSemaphore(Uint32 initial_value)
{
	...
	sem = (SDL_sem *) GxCore_Malloc(sizeof(*sem));
	if (sem == NULL) {
		SDL_OutOfMemory();
		return (NULL);
	}
