...
void PrintFormatNumber(void * ValuePtr, int Format, int ByteCount)
{
    ...
    for(n=0;n<16;n++){
        switch(Format){
            case FMT_SBYTE:
            case FMT_BYTE:      gxlogd("%02x",*(uchar *)ValuePtr); s=1;  break;
            case FMT_USHORT:    gxlogd("%d",Get16u(ValuePtr)); s=2;      break;
            case FMT_ULONG:
            case FMT_SLONG:     gxlogd("%d",Get32s(ValuePtr)); s=4;      break;
            case FMT_SSHORT:    gxlogd("%hd",(signed short)Get16u(ValuePtr)); s=2; break;
            case FMT_URATIONAL:
            case FMT_SRATIONAL:
               gxlogd("%d/%d",Get32s(ValuePtr), Get32s(4+(char *)ValuePtr));
               s = 8;
               break;

            case FMT_SINGLE:    gxlogd("%f",(double)*(float *)ValuePtr); s=8; break;
            case FMT_DOUBLE:    gxlogd("%f",*(double *)ValuePtr);        s=8; break;
            default:
                gxlogd("Unknown format %d:", Format);
                return;
        }
        ByteCount -= s;
        ...
        unsigned ExifLength, int NestingLevel)
{
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (ByteCount > 4){
            ...
            if (DumpExifMap){
                gxlogd("Map: %05d-%05d:   Data for tag %04x\n",OffsetVal, OffsetVal+ByteCount, Tag);
            }
        }else{
        ...
        if (Tag == TAG_MAKER_NOTE){
            if (ShowTags){
                gxlogd("%s    Maker note: ",IndentString);
            }
        ...
        if (ShowTags){
            ...
            for (a=0;;a++){
                if (a >= TAG_TABLE_SIZE){
                    gxlogd(IndentString);
                    gxlogd("    Unknown Tag %04x Value = ", Tag);
                    break;
                }
                if (TagTable[a].Tag == Tag){
                    gxlogd(IndentString);
                    gxlogd("    %s = ",TagTable[a].Desc);
                    break;
                }
            }
        ...
        switch(Tag){
            ...
            case TAG_WINXP_COMMENT:
                if (ImageInfo.Comments[0]){ // We already have a jpeg comment.
                    ...
                    if (ShowTags) gxlogd("Windows XP commend and other comment in header\n");
            ...
            case TAG_USERCOMMENT:
                if (ImageInfo.Comments[0]){ // We already have a jpeg comment.
                    // Already have a comment (probably windows comment), skip this one.
                    if (ShowTags) gxlogd("Multiple comments in exif header\n");
                    break; // Already have a windows comment, skip this one.
                }
            ...
            case TAG_EXIF_OFFSET:
                if (ShowTags) gxlogd("%s    Exif Dir:",IndentString);

            case TAG_INTEROP_OFFSET:
                if (Tag == TAG_INTEROP_OFFSET && ShowTags) gxlogd("%s    Interop Dir:",IndentString);
                {
            ...
            case TAG_GPSINFO:
                if (ShowTags) gxlogd("%s    GPS info dir:",IndentString);
                {
                    unsigned char * SubdirStart;
                    SubdirStart = OffsetBase + Get32u(ValuePtr);
                    if (SubdirStart < OffsetBase || SubdirStart > OffsetBase+ExifLength){
            ...
            if (Offset){
                ...
                if (SubdirStart > OffsetBase+ExifLength || SubdirStart < OffsetBase){
                    if (SubdirStart > OffsetBase && SubdirStart < OffsetBase+ExifLength+20){
                        ...
                        if (ShowTags) gxlogd("Thumbnail removed with Jhead 1.3 or earlier\n");
                    }else{
                        ErrNonfatal("Illegal subdirectory link in Exif header",0,0);
                    }
                }else{
                    if (SubdirStart <= OffsetBase+ExifLength){
                        if (ShowTags) gxlogd("%s    Continued directory ",IndentString);
                        ProcessExifDir(SubdirStart, OffsetBase, ExifLength, NestingLevel+1);
                    }
                }
    ...
    if (ThumbnailOffset){
        ...
        if (ThumbnailOffset <= ExifLength){
            if (ThumbnailSize > ExifLength-ThumbnailOffset){
                ...
                if (ShowTags) gxlogd("Thumbnail incorrectly placed in header\n");
...
void process_EXIF (unsigned char * ExifSection, unsigned int length)
{
    ...
    if (ShowTags){
        gxlogd("Exif header %d bytes long\n",length);
    }
    ...
    if (memcmp(ExifSection+8,"II",2) == 0){
        if (ShowTags) gxlogd("Exif section in Intel order\n");
        MotorolaOrder = 0;
    }else{
        if (memcmp(ExifSection+8,"MM",2) == 0){
            if (ShowTags) gxlogd("Exif section in Motorola order\n");
            MotorolaOrder = 1;
        }else{
    ...
    if (DumpExifMap){
        unsigned a,b;
        gxlogd("Map: %05d- End of exif\n",length-8);
        for (a=0;a<length-8;a+= 10){
            gxlogd("Map: %05d ",a);
            for (b=0;b<10;b++) gxlogd(" %02x",*(ExifSection+8+a+b));
            gxlogd("\n");
        }
    }
...
void create_EXIF(void)
{
...

    {
        ...
        unsigned char * NewBuf = GUI_MALLOC(DataWriteIndex);
        if (NewBuf == NULL){
...
{
#ifdef GUI_JPEG_INFO
    gxlogd("\"%s\"",ImageInfo.FileName);

    gxlogd(" %dx%d",ImageInfo.Width, ImageInfo.Height);

    if (ImageInfo.ExposureTime){
        if (ImageInfo.ExposureTime <= 0.5){
            gxlogd(" (1/%d)",(int)(0.5 + 1/ImageInfo.ExposureTime));
        }else{
            gxlogd(" (%1.1f)",ImageInfo.ExposureTime);
        }
    }

    if (ImageInfo.ApertureFNumber){
        gxlogd(" f/%3.1f",(double)ImageInfo.ApertureFNumber);
    }

    if (ImageInfo.FocalLength35mmEquiv){
        gxlogd(" f(35)=%dmm",ImageInfo.FocalLength35mmEquiv);
    }
