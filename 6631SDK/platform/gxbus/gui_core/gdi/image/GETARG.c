...
static int GAGetParmeters(int *Parameters[], int *ParamCount, char *CtrlStrCopy, char *Option, int *argc, char ***argv);
static int GAGetMultiParmeters(int *Parameters[], int *ParamCount, char *CtrlStrCopy, int *argc, char ***argv);
...
{
	int i, Error = FALSE, ParamCount = 0, *Parameters[MAX_PARAM];	/* Save here parameter addresses */
	...
	return GATestAllSatis(CtrlStrCopy, CtrlStr, &argc, &argv, Parameters, &ParamCount);
}
			  ...
			  char ***argv, int *Parameters[MAX_PARAM], int *ParamCount)
{
	...
	if (!ISCTRLCHAR(CtrlStr[i + 2])) {
		...
		*Parameters[(*ParamCount)++] = *argc;
		GAByteCopy((char *)Parameters[(*ParamCount)++], (char *)argv, sizeof(char *));
	}
			      ...
			      char *Option, char *CtrlStrCopy, char *CtrlStr, int *argc, char ***argv)
{
	...
	*Parameters[(*ParamCount)++] = 1;	/* Set boolean flag for that option */
	if (ISSPACE(CtrlStrCopy[i]))
		return OK;	/* Only a boolean flag is needed */
	...
	return GAGetParmeters(Parameters, ParamCount, &CtrlStrCopy[i], Option, argc, argv);
}
...
static int GAGetParmeters(int *Parameters[], int *ParamCount, char *CtrlStrCopy, char *Option, int *argc, char ***argv)
{
	...
	while (!(ISSPACE(CtrlStrCopy[i]))) {
		...
		case 'd':	/* Get signed integers */
			ScanRes = sscanf(*((*argv)++), "%d", (int *)Parameters[(*ParamCount)++]);
			break;
		case 'u':	/* Get unsigned integers */
			ScanRes = sscanf(*((*argv)++), "%u", (unsigned *)Parameters[(*ParamCount)++]);
			break;
		case 'x':	/* Get hex integers */
			ScanRes = sscanf(*((*argv)++), "%x", (int *)Parameters[(*ParamCount)++]);
			break;
		case 'o':	/* Get octal integers */
			ScanRes = sscanf(*((*argv)++), "%o", (int *)Parameters[(*ParamCount)++]);
			break;
		case 'D':	/* Get signed long integers */
			ScanRes = sscanf(*((*argv)++), "%ld", (long *)Parameters[(*ParamCount)++]);
			break;
		case 'U':	/* Get unsigned long integers */
			ScanRes = sscanf(*((*argv)++), "%lu", (unsigned long *)Parameters[(*ParamCount)++]);
			break;
		case 'X':	/* Get hex long integers */
			ScanRes = sscanf(*((*argv)++), "%lx", (long *)Parameters[(*ParamCount)++]);
			break;
		case 'O':	/* Get octal long integers */
			ScanRes = sscanf(*((*argv)++), "%lo", (long *)Parameters[(*ParamCount)++]);
			break;
		case 'f':	/* Get float number */
			ScanRes = sscanf(*((*argv)++), "%f", (float *)Parameters[(*ParamCount)++]);
		case 'F':	/* Get double float number */
			ScanRes = sscanf(*((*argv)++), "%lf", (double *)Parameters[(*ParamCount)++]);
			break;
...
static int GAGetMultiParmeters(int *Parameters[], int *ParamCount, char *CtrlStrCopy, int *argc, char ***argv)
{
	...
	do {
		...
		case 'd':	/* Format to read the parameters: */
			...
			ScanRes = sscanf(*((*argv)++), "%d", (int *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'u':
			TmpArray.IntArray[NumOfPrm] = (int *)MyMalloc(sizeof(int));
			ScanRes = sscanf(*((*argv)++), "%u", (unsigned int *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'o':
			TmpArray.IntArray[NumOfPrm] = (int *)MyMalloc(sizeof(int));
			ScanRes = sscanf(*((*argv)++), "%o", (int *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'x':
			TmpArray.IntArray[NumOfPrm] = (int *)MyMalloc(sizeof(int));
			ScanRes = sscanf(*((*argv)++), "%x", (int *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'D':
			TmpArray.LngArray[NumOfPrm] = (long *)MyMalloc(sizeof(long));
			ScanRes = sscanf(*((*argv)++), "%ld", (long *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'U':
			TmpArray.LngArray[NumOfPrm] = (long *)MyMalloc(sizeof(long));
			ScanRes = sscanf(*((*argv)++), "%lu", (unsigned long *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'O':
			TmpArray.LngArray[NumOfPrm] = (long *)MyMalloc(sizeof(long));
			ScanRes = sscanf(*((*argv)++), "%lo", (long *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'X':
			TmpArray.LngArray[NumOfPrm] = (long *)MyMalloc(sizeof(long));
			ScanRes = sscanf(*((*argv)++), "%lx", (long *)TmpArray.IntArray[NumOfPrm++]);
			break;
		case 'f':
			TmpArray.FltArray[NumOfPrm] = (float *)MyMalloc(sizeof(float));
			ScanRes = sscanf(*((*argv)++), "%f", (float *)TmpArray.LngArray[NumOfPrm++]);
			break;
		case 'F':
			TmpArray.DblArray[NumOfPrm] = (double *)MyMalloc(sizeof(double));
			ScanRes = sscanf(*((*argv)++), "%lf", (double *)TmpArray.LngArray[NumOfPrm++]);
			break;
		case 's':
			while ((*argc) && ((**argv)[0] != '-')) {
	...
	*(Ptemp) = (int *)NULL;	/* Close the block with NULL pointer */
	...
	*Parameters[(*ParamCount)++] = NumOfPrm;
	GAByteCopy((char *)Parameters[(*ParamCount)++], (char *)&Pmain, sizeof(char *));
	return NumOfPrm;
}
...
{
	gxlogf ( "Error in command line parsing - ");
	switch (Error) {
	case 0:;
		break;
	case CMD_ERR_NotAnOpt:
		gxlogf( "None option Found");
		break;
	case CMD_ERR_NoSuchOpt:
		gxlogf( "Undefined option Found");
		break;
	case CMD_ERR_WildEmpty:
		gxlogf( "Empty input for '!*?' seq.");
		break;
	case CMD_ERR_NumRead:
		gxlogf( "Failed on reading number");
		break;
	case CMD_ERR_AllSatis:
		gxlogf( "Fail to satisfy");
		break;
	}
	gxlogf( " - '%s'\n", GAErrorToken);
}
...
void GAPrintHowTo(char *CtrlStr)
{
	...
	while (i < strlen(CtrlStr)) {
		...
		case '%':
			...
			SpaceFlag = TRUE;
			while (!ISCTRLCHAR(CtrlStr[i]) && (i < strlen(CtrlStr)) && (!ISSPACE(CtrlStr[i])))
				if (SpaceFlag) {
					if (CtrlStr[i++] == SPACE_CHAR)
						gxlogf( " ");
					else
						gxlogf( " %c", CtrlStr[i - 1]);
					SpaceFlag = FALSE;
		...
		default:	/* Not checked, but must be last one! */
			gxlogf( " ");
			while (!ISSPACE(CtrlStr[i]) && (i < strlen(CtrlStr)) && !ISCTRLCHAR(CtrlStr[i]))
				gxlogf( "%c", CtrlStr[i++]);
			gxlogf( "\n");
			return;
		}
...
* this routine (no direct call to GUI_MALLOC). Dies if no memory.		   *
***************************************************************************/
static char *MyMalloc(unsigned size)
{
	char *p;

	if ((p = GUI_MALLOC(size)) != NULL)
		return p;

	gxlogf( "Not enough memory, exit\n");
	exit(2);
