...
static void process_SOFn (const uchar * Data, int marker)
{
    ...
    if (ShowTags){
        gxlogd("JPEG image is %uw * %uh, %d color components, %d bits per sample\n",
...
void CheckSectionsAllocated(void)
{
    ...
    if (SectionsRead >= SectionsAllocated){
        ...
        Sections = (Section_t *)GUI_REALLOC(Sections, sizeof(Section_t)*SectionsAllocated);
        if (Sections == NULL){
    ...
    for(;;){
        ...
        switch(marker){
            ...
            case M_EOI:   // in case it's a tables-only JPEG stream
                gxlogf ("No image in jpeg!\n");
                return FALSE;

            case M_COM: // Comment section
                if (HaveCom || ((ReadMode & READ_METADATA) == 0)){
                    // Discard this section.
		    ptr = Sections[--SectionsRead].Data;
                    GUI_FREE(ptr);
                }else{
            ...
            case M_JFIF:
                ...
                if (memcmp(Data+2, "JFIF\0",5)){
                    gxlogf ("Header missing JFIF marker\n");
                }
                if (itemlen < 16){
                    gxlogf ("Jfif header too short\n");
                    goto ignore;
                }
                ...
                if (ShowTags){
                    gxlogd("JFIF SOI marker: Units: %d ",ImageInfo.JfifHeader.ResolutionUnits);
                    switch(ImageInfo.JfifHeader.ResolutionUnits){
                        case 0: gxlogd("(aspect ratio)"); break;
                        case 1: gxlogd("(dots per inch)"); break;
                        case 2: gxlogd("(dots per cm)"); break;
                        default: gxlogd("(unknown)"); break;
                    }
                    gxlogd("  X-density=%d Y-density=%d\n",ImageInfo.JfifHeader.XDensity, ImageInfo.JfifHeader.YDensity);

                    if (Data[14] || Data[15]){
                        gxlogf ("Ignoring jfif header thumbnail\n");
                    }
                }
            ...
            case M_EXIF:
                ...
                if (ReadMode & READ_METADATA){
                    if (memcmp(Data+2, "Exif", 4) == 0){
                        ...
                        if (ShowTags){
                            gxlogd("Image cotains XMP section, %d bytes long\n", itemlen);
                            if (ShowTags){
		...
		ptr = Sections[--SectionsRead].Data;
            ...
            case M_IPTC:
                if (ReadMode & READ_METADATA){
                    if (ShowTags){
                        gxlogd("Image cotains IPTC section, %d bytes long\n", itemlen);
                    }
            ...
            default:
                ...
                if (ShowTags){
                    gxlogd("Jpeg section marker 0x%02x size %d\n",marker, itemlen);
                }
                break;
        }
    }
...
int SaveThumbnail(char * ThumbFileName)
{
    ...
    if (ImageInfo.ThumbnailOffset == 0 || ImageInfo.ThumbnailSize == 0){
        gxloge ("Image contains no thumbnail\n");
        return FALSE;
    }
...
int ReplaceThumbnail(const char * ThumbFileName)
{
    ...
    if (ImageInfo.ThumbnailOffset == 0 || ImageInfo.ThumbnailAtEnd == FALSE){
        ...
        gxloge ("Image contains no thumbnail to replace - add is not possible\n");
        return FALSE;
    }
    ...
    ExifSection->Data = (uchar *)GUI_REALLOC(ExifSection->Data, NewExifSize);
...
{
    if (Sections == NULL){
        Sections = (Section_t *)GUI_MALLOC(sizeof(Section_t)*5);
        SectionsAllocated = 5;
    }
