        ...
        unsigned ExifLength)
{
    ...
    for (de=0;de<NumDirEntries;de++){
        ...
        if (ByteCount > 4){
            ...
            if (DumpExifMap){
                gxlogd("Map: %05d-%05d:   Data for makernote tag %04x\n",OffsetVal, OffsetVal+ByteCount, Tag);
            }
        }else{
        ...
        if (ShowTags){
            // Show tag name
            gxlogd("            Canon maker tag %04x Value = ", Tag);
        }
...
void ShowMakerNoteGeneric(unsigned char * ValuePtr, int ByteCount)
{
    ...
    for (a=0;a<ByteCount;a++){
        ...
        gxlogd(" %02x",ValuePtr[a]);
    }
    gxlogd(" (%d bytes)", ByteCount);
    gxlogd("\n");
}
