...
static void si_parse_lib_nit_release_loop2(GxSubsystemSiParseNitLoop2* loop2)
{
    ...
    if(loop2)
    {
        ...
        if(loop2->terrestrial_delivery_system)
        {
            GxCore_Free(loop2->terrestrial_delivery_system);
        }
    }
    return;
}
...
static int32_t si_parse_lib_nit_parse_loop2_des(GxSubsystemSiParseNitLoop2* loop2,uint8_t* src,uint8_t*section_end,uint32_t len)
{
    ...
    uint32_t terrestrial_delivery_system_count = 0;
    uint8_t* p = src;
    uint32_t descriptor_length = 0;
    int32_t ret = 0;
    ...
    while(size > 0)
    {
        switch(p[0])
        {
            ...
            case SI_PARSE_LIB_TER_DELIVERY_SYSTEM_DES:
                terrestrial_delivery_system_count++;
                loop2->terrestrial_delivery_system = GxCore_Realloc(loop2->terrestrial_delivery_system,
                        sizeof(GxSubsystemSiParseTerrestrialDeliveryDes)*terrestrial_delivery_system_count);
                if(loop2->terrestrial_delivery_system == NULL)
                {
#ifdef GX_SI_PARSE_LIB_ERR_DBUG
                    SI_PARSE_LIB_ERR_PRINTF("parse nit no memery!!");
#endif  
                ...
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].descriptor_tag = TODATA0_7BIT(p[0]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].descriptor_length = TODATA0_7BIT(p[1]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].centre_frequency = TODATA0_31BIT(p[2],p[3],p[4],p[5]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].banwidth = TODATA5_7BIT(p[6]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].reserved_future_use = TODATA0_4BIT(p[6]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].constellation = TODATA6_7BIT(p[7]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].hierarchy_information = TODATA3_5BIT(p[7]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].code_rate_HP_stream = TODATA0_2BIT(p[7]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].code_rate_LP_stream = TODATA5_7BIT(p[8]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].guard_interval = TODATA3_4BIT(p[8]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].transmission_mode = TODATA1_2BIT(p[8]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].other_frequency_flag = TODATA0BIT(p[8]);
                loop2->terrestrial_delivery_system[terrestrial_delivery_system_count-1].reserved_future_use1 = TODATA0_31BIT(p[9],p[10],p[11],p[12]);
    ...
    loop2->terrestrial_delivery_system_count = terrestrial_delivery_system_count;
    return size;
