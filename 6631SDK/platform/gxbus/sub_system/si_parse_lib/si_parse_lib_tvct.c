...
static int32_t si_parse_lib_tvct_parse_loop(GxSubsystemSiParseVCT * vct , uint8_t * section,uint32_t size);
GxSubsystemSiParseVCT * GxSubsystm_SiParseLibTvctParse(uint8_t * section,uint32_t len);
...
GxSubsystemSiParseVCT * GxSubsystm_SiParseLibTvctParse(uint8_t * section,uint32_t len)
{
    ...
    {
        ret = si_parse_lib_tvct_parse_loop(vct,p,size);
        if(ret < 0)
            goto err;
        else
            size = ret;          
 ...
 *   
 *  @param       GxSubsystemSiParseVCT* vct:vct的存储空间，是通过GxSubsystm_SiParseLibTvctParse获取的
 *  
