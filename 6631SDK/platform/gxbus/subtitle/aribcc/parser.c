...
struct arib_parser_t
{
    ...
    uint32_t          i_num_languages;
    unsigned int      *psz_languages_code;
...
static void parse_data_unit_statement_body( arib_parser_t *p_parser, bs_t *p_bs,
                                            ...
                                            uint32_t i_data_unit_size )
{
    char* p_data_unit_data_byte = (char*) av_malloc(
...
//    p_parser->p_instance->p->i_drcs_num = 0;
    ...
    {
        for( i = 0; i < p_parser->p_drcs_data->i_NumberOfCode; i++ )
        {
            drcs_code_t* p_drcs_code = &p_parser->p_drcs_data->p_drcs_code[i];
            for( j = 0; j < p_drcs_code->i_NumberOfFont ; j++ )
            {
                drcs_font_data_t *p_drcs_font_data =  &p_drcs_code->p_drcs_font_data[j];
                av_free( p_drcs_font_data->p_drcs_pattern_data );
                av_free( p_drcs_font_data->p_drcs_geometric_data );
            }
            av_free( p_drcs_code->p_drcs_font_data );
        }
        av_free( p_parser->p_drcs_data->p_drcs_code );
        av_free( p_parser->p_drcs_data );
    }
    p_parser->p_drcs_data = (drcs_data_t*) av_mallocz(sizeof(drcs_data_t) );
    if( p_parser->p_drcs_data == NULL )
    {
        return;
    }
    ...
    p_parser->p_drcs_data->p_drcs_code = (drcs_code_t*) av_mallocz(
    ...
    for( i = 0; i < i_NumberOfCode; i++ )
    {
        ...
        p_drcs_code->p_drcs_font_data = (drcs_font_data_t*) av_mallocz(
        ...
        for( j = 0; j < i_NumberOfFont; j++ )
        {
            ...
            if( i_mode == 0x00 || i_mode == 0x01 )
            {
                ...
                drcs_pattern_data_t* p_drcs_pattern_data =
                    ...
                    (drcs_pattern_data_t*) av_mallocz( sizeof(drcs_pattern_data_t) );
                ...
                p_drcs_pattern_data->p_patternData = (int8_t*) av_malloc(
                            (i_width * i_height * i_bits_per_pixel / 8) * sizeof(int8_t) );
                if( p_drcs_pattern_data->p_patternData == NULL )
                {
                    return;
                }
#else
                int8_t *p_patternData = (int8_t*) av_malloc(
                            (i_width * i_height * i_bits_per_pixel / 8) * sizeof(int8_t) );
                ...
                for( k = 0; k < i_width * i_height * i_bits_per_pixel / 8; k++ )
                {
...
//                                   p_patternData );
            ...
            else
            {
                ...
                drcs_geometric_data_t* p_drcs_geometric_data =
                    ...
                    (drcs_geometric_data_t*) av_mallocz( sizeof(drcs_geometric_data_t) );
                ...
                p_drcs_geometric_data->p_geometricData = (int8_t*)
                    av_malloc( i_geometricData_length * sizeof(int8_t) );
                ...
                for( k = 0; k < i_geometricData_length ; k++ )
                {
...
static void parse_caption_management_data( arib_parser_t *p_parser, bs_t *p_bs )
{
    ...
    uint8_t i;
    p_parser->i_num_languages = bs_read( p_bs, 8 );
    if (p_parser->psz_languages_code) {
     av_free(p_parser->psz_languages_code);
     p_parser->psz_languages_code = NULL;
    }
    if (p_parser->i_num_languages) {
        p_parser->psz_languages_code = (unsigned int*) av_malloc(
                p_parser->i_num_languages * sizeof(unsigned int) );
    }
    for(i = 0; i < p_parser->i_num_languages; i++ )
    {
        ...
        p_parser->psz_languages_code[i] = bs_read( p_bs, 24 ); /* i_ISO_639_language_code */
...
{
    arib_parser_t *p_parser = (arib_parser_t *)av_mallocz( sizeof(*p_parser) );
    if ( !p_parser )
       return NULL;
    p_parser->p_instance = p_instance;
    gxlogd("arib parser was created\n" );
#if 0
	if ( p_instance->p->psz_base_path )
    {
//        load_drcs_conversion_table( p_parser->p_instance );
        gxlogd("could not load drcs conversion table\n" );
    }
#endif
    return p_parser;
}
...
{
    gxlogd("arib parser was destroyed\n");
    av_free( p_parser->psz_subtitle_data );
    av_free( p_parser );
}
