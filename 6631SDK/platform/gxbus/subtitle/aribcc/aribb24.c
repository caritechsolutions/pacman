...
{
    arib_instance_t *p_instance = (arib_instance_t *)av_mallocz(sizeof(*p_instance) );
    if ( !p_instance )
        return NULL;
    p_instance->p = (arib_instance_private_t *)av_mallocz(sizeof(*(p_instance->p)) );
    if (!p_instance->p)
    {
        av_free( p_instance );
        return NULL;
    ...
    p_instance->p->p_decoder = NULL;
    p_instance->p->p_parser  = NULL;
    return p_instance;
}
...
void arib_instance_destroy( arib_instance_t *p_instance )
{
    ...
    //av_free( p_instance->p->psz_base_path );
   // av_free( p_instance->p->psz_last_error );
