...
{
    arib_buf_region_t *p_region =
        (arib_buf_region_t*) av_mallocz(sizeof(arib_buf_region_t) );
    if( p_region == NULL )
    {
        return NULL;
    }
...
static int decoder_handle_korean_table( arib_decoder_t *decoder, int c )
{
    unsigned int uc;
    uc = decoder_Korean_table[c];
    return decoder_push( decoder, uc );
}
...
{
    gxlogd("could not decode ARIB string:");
    while( start < end )
    {
        gxlogd("%02x ", *start++ );
    }
...
void arib_initialize_decoder_c_profile( arib_decoder_t* decoder )
{
    ...
    decoder->handle_g2 = decoder_handle_korean_table;
...
{
    arib_decoder_t *p_decoder = (arib_decoder_t *)av_mallocz(sizeof( *p_decoder ) );
    if ( !p_decoder )
        return NULL;
    p_decoder->p_instance = p_instance;
    gxlogd("arib decoder was created\n" );
    return p_decoder;
}
