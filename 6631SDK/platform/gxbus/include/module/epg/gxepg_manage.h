...
#define GX_BUS_EPG_EVENT_TOO_SMALL   (0x7ffffff7)
...
typedef struct {
#ifdef EPG_MALLOC
	GxBusEpgMemPoolCell  name;                ///< event名字
	GxBusEpgMemPoolCell  detaile_description; ///< event详细描述
#else
	GxBusEpgMemPoolCell* name;                ///< event名字的起始指针
	GxBusEpgMemPoolCell* detaile_description; ///< event详细描述的起始指针
#endif
	uint16_t event_id;                        ///< service id ts id和event id唯一的确定一个event
...
typedef struct {
	uint8_t use_flag;                       ///< 该头部是否被使用 1--被使用 0--空闲
	uint16_t ts_id;                          ///< 和service_id一起用以确定一个service
	uint16_t service_id;                     ///< 和ts_id一起用以确定一个service
	uint16_t reference_service_id;           ///< 和ts_id一起用以确定一个service
	uint16_t original_id;                    ///< 用来确定一个service
	...
	uint16_t parental_rating_num;            ///< 父母锁信息的数量
	GxBusEpgChannelEtt* channel_ett;         ///< 存储channel ett信息
	uint16_t channel_ett_num;                ///< channel ett的数量
	uint8_t detaile_description_ver;        ///< 详细描述的版本号
} GxBusEpgServiceHead;

typedef struct {
	uint16_t event_id;                        ///< service id ts id和event id唯一的确定一个event
	uint16_t reference_event_id;              ///< service id ts id和event id唯一的确定一个event
	uint16_t service_id;
	uint16_t ts_id;
#ifdef EPG_MALLOC
	GxBusEpgMemPoolCell name;                ///< event名字
...
typedef struct {
	...
	uint16_t service_id;  ///< 和ts_id一起用以确定一个service
	uint16_t original_id; ///< 用来确定一个service
	uint16_t pos;         ///< service头数组的下标
} GxBusEpgEventCurUseService;


/*添加epg信息时的结构体*/
typedef struct {
	uint16_t ts_id;                          ///< 和service_id一起确定一个节目
	uint16_t service_id;                     ///< 和ts_id一起确定一个节目
	uint16_t reference_service_id;           ///< 和ts_id一起用以确定一个service
	uint16_t original_id;                    ///< 用来确定一个service
	uint16_t event_id;                       ///< 和service id ts id original_id一起确定一个event
	uint16_t reference_event_id;             ///< service id ts id和event id唯一的确定一个event
	...
	uint16_t parental_rating_num;            ///< 父母锁信息的数量，如果没有父母锁信息请置为0
	GxBusEpgChannelEtt* channel_ett;         ///< 存储channel ett信息，如果没有channel ett请置为NULL
	uint16_t channel_ett_num;                ///< channel ett的数量，如果没有channel ett请置为0
	uint8_t ver;
 ...
 *
 * @param uint32_t event_pos:event的pos
 *        GxBusEpgEventHead* event_head:保存头信息的空间,由调用者申请
 *        int32_t event_per_size:申请的event_head的大小
 *
 * @return  GX_BUS_EPG_OK:执行正常
 *          GX_BUS_EPG_PARAMETER_ERR:传入的参数不正确,无法匹配到正确值
 *          GX_BUS_EPG_NOT_LOCK:epg没有上锁
 ...
 *        GxBusEpgEventHead* event:保存头信息的空间
