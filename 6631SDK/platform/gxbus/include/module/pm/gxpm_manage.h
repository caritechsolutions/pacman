...
typedef enum {
	...
	GXBUS_PM_SAT_LNBF,               //for Multi-Point LNBF #359333
} GxBusPmSatLnbType;
...
typedef enum {
	GXBUS_PM_SAT_UNICABLE_OFF = 0,
	GXBUS_PM_SAT_UNICABLE,
	GXBUS_PM_SAT_UNICABLE2,
}GxBusPmSatUnicableType;
...
typedef struct {
    uint8_t server_name[MAX_SAT_NAME];
...
typedef enum {
	...
	GXBUS_PM_PROG_RESERVED_TV,
	GXBUS_PM_PROG_RESERVED_RADIO,
...
typedef struct {
	...
	GxBusPmDataProgDefinition definition    : 1; //清晰度标记
...
typedef struct {
	...
	uint32_t  pip_main_tp_id : 16;              // 0~15 bit 表示开启小画面时大画面锁定的tp id，16bit 表示是否只过滤当前tuner 0-false 1-true
 ...
 * @return     	0:该节目不存在
 ...
 -1:失败
 */
//增加一个tp id 再增加原始网络id 导致搜台时需要增加?娲⒉问?
...
status_t GxBus_PmGetNetServerUrlByIndex(uint32_t *index_arry, char **server_url_arry, uint32_t num);
...
/*
 * 回调函数，view过滤节目的时候，用户自定义的回调函数原型  add 20240718
 *
 * @brief prog : 节目的数据类型
 *        sys  : view 模式
 *        mode : 有些模式,sys结构体中并没有，目前0:不用关心，sys中有体现  1: 代表HD模式. 其它：未定义
 *
 * @return  0 : 成功　　其它：　失败
 *
*/
typedef int32_t (*GxBus_PmProgCheckCallback)(GxBusPmDataProg* prog,GxBusPmViewInfo* sys,int mode);
/*
 * 设置回调函数，view过滤节目的时候，通过回调函数过滤相关的数据 能在任何模式下都能使用 add 20240718
 *
 * @brief callback 回调函数
 *
 * @return  0
 ...
 * @return  正常数字:返回符合条件的prog数量
*/
uint32_t GxBus_PmProgNumGetByViewInfo(GxBusPmViewInfo*);
/**
