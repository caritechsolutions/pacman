...
typedef status_t (*GxSearchExtFinish)(void);
 ...
 * 说明：app为了能实现搜台功能，
 * 就需要填写如上的一个结构体，
 * 然后发送PMServiceMsgID_ CableScanNetStart消息，并且带上填写好的结构体
 */
typedef struct {
		GxSearchType scan_type;               ///< 选择何种搜索方式
		GxSearchTVRadio tv_radio;             ///< 选择搜索tv节目还是radio节目还是全搜
...
typedef struct {
	...
	GxSearchExtend*  ext;             ///< 搜台扩展过滤
	bool        only_filter_ext;      ///< 频点只进行扩展过滤
	uint32_t    ext_num;              ///< 扩展过滤的数量
	GxSearchTimeOut time_out;         ///< 各张表的超时时间
	uint32_t demux_id;                ///< 使用哪个demux
...
// pdmx
typedef int (*PDMXExit)(int tuner, void* params);
typedef int (*PDMXInitParams)(int tuner, int demux_id, void* params);
typedef int (*PDMXSetParams)(int tuner, int ts_source, int demux_id, void* params);
typedef int (*PDMXGetParams)(void* params);
typedef int (*PDMXGetTsSize)(void);
typedef int (*PDMXActiveFlag)(void);
typedef struct _PDMXSearchClass{
    PDMXInitParams init;
    PDMXExit       exit;
    PDMXSetParams  set_params;
    PDMXGetParams  get_params;
    PDMXGetTsSize  get_ts_size;
    PDMXActiveFlag get_active_flag;
}PDMXSearchClass;

typedef struct _PDMXSignalInfoClass{
    unsigned short longitude;//if west, (long = 3600 - long*10), sunch as 7w, longitude = 3600-70 = 3530
    unsigned short fre_m;
    unsigned short sym_k;
    unsigned char  pol;
}PDMXSignalInfoClass;
...
// multistream
typedef int32_t (*MultiStreamInit)(int32_t tuner);
typedef int32_t (*MultiStreamExit)(void);
typedef int32_t (*MultiStreamGetParams)(void *params);
typedef int32_t (*MultiStreamSetParams)(int32_t tuner);
typedef bool    (*MultiStreamIncreaseTsIndex)(void);
typedef bool    (*MultiStreamGetActiveFlag)(void);

typedef struct _MultiStreamSearchClass{
	MultiStreamInit            init;
	MultiStreamExit            exit;
	MultiStreamGetParams       get_params;
	MultiStreamSetParams       set_params;
	MultiStreamIncreaseTsIndex increase_ts_index;
	MultiStreamGetActiveFlag   get_active_flag;
}MultiStreamSearchClass;

int32_t GxSearchRegistMultiStream(MultiStreamSearchClass *multistream_ops);
void    GxSearchUnregistMultiStream(void);

//plsn
typedef bool (*PlsnSet)(int32_t tuner, uint32_t *tp_pls_n);

typedef struct _PlsnSearchClass{
	PlsnSet  plsn_set;
}PlsnSearchClass;
...
typedef enum {
    ...
    GX_NIT_SW_FILTER,
    GX_SW_FILTER_ALL,
} Gx_Search_Table_Type;

typedef void (*FilterSetParams)(Gx_Search_Table_Type table_type);
typedef status_t (*FilterCheckParams)(Gx_Search_Table_Type table_type);

typedef struct {
    uint32_t pat_filter : 8;
    uint32_t pmt_filter : 8;
    uint32_t sdt_filter : 8;
    uint32_t nit_filter : 8;
    FilterSetParams set_params;
    FilterCheckParams check_params;
} FilterSearchClass;
