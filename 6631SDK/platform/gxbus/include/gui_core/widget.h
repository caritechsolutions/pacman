...
#define WIDGET_IS_BOXITEM(pwidget)   ((pwidget) && (pwidget->classname) && (strcasecmp(pwidget->classname, "boxitem") == 0))
#define WIDGET_IS_WINDOW(pwidget)    ((pwidget) && (pwidget->classname) && (strcasecmp(pwidget->classname, "window") == 0))
#define WIDGET_ISNOT_WINDOW(pwidget) ((pwidget) && (pwidget->classname) && (strcasecmp(pwidget->classname, "window")))
#define WIDGET_IS_ROOT(pwidget)      ((pwidget) && (pwidget == gui.config.root_widget))
#define WIDGET_ISNOT_ROOT(pwidget)   ((pwidget) && (pwidget != gui.config.root_widget))
...
typedef struct _GuiWidgetOps {
    ...
    int (*active)(GuiWidget *self);
    int (*draw)(GuiWidget *self);          // 将图片资源清空( 指针赋空 )
    int (*unactive)(GuiWidget *self);
    int (*gotfocus)(GuiWidget *self);      // 重新聚焦后
