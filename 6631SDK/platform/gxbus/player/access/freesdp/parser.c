...
fsdp_error_t fsdp_parse(const char* text_description, fsdp_description_t * dsc)
{
  ...
  /***************************************************************************/
	...
	if (!strncmp(p, "o=", 2)) {
		...
		dsc->o_username = av_strdup(fsdp_buf[0]);
		dsc->o_session_id = av_strdup(fsdp_buf[1]);
		dsc->o_announcement_version = av_strdup(fsdp_buf[2]);
		...
		dsc->o_address = av_strdup(fsdp_buf[5]);
	} else {
	...
	if (!strncmp(p, "s=", 2)) {
		...
		dsc->s_name = av_strdup(longfsdp_buf);
	} else {
	...
	if (!strncmp(p, "i=", 2)
	    && sscanf(p, "i=%" MLFLENS "[^\r\n]", longfsdp_buf)) {
		dsc->i_information = av_strdup(longfsdp_buf);
		NEXT_LINE(p);
	} else {
	...
	if (!strncmp(p, "u=", 2)
		...
		dsc->u_uri = av_strdup(longfsdp_buf);
		NEXT_LINE(p);
	} else {
		/* (optional) uri absent*/
	}
	...
	if (dsc->emails_count > 0) {
		...
		dsc->emails = av_calloc(j, sizeof(const char* ));
		for (j = 0; j < dsc->emails_count; j++) {
			sscanf(p, "e=%" MLFLENS "[^\r\n]", longfsdp_buf);
			/* TODO? check valid email-address.*/
			dsc->emails[j] = av_strdup(longfsdp_buf);
			NEXT_LINE(p);
		}
	}
	...
	if (dsc->phones_count > 0) {
		dsc->phones = av_calloc(j, sizeof(const char* ));
		for (j = 0; j < dsc->phones_count; j++) {
			sscanf(p, "p=%" MLFLENS "[^\r\n]", longfsdp_buf);
			/* TODO? check valid phone-number.*/
			dsc->phones[j] = av_strdup(longfsdp_buf);
			NEXT_LINE(p);
		}
	}
	...
	result = fsdp_parse_c(&p, &(dsc->c_network_type), &(dsc->c_address_type), &(dsc->c_address));
	if (FSDPE_OK != result)
		return result;

	/* `b=' lines (bandwidth information) [optional]*/
	/* b=<modifier>:<bandwidth-value>*/
	result = fsdp_parse_b(&p, &(dsc->bw_modifiers), &(dsc->bw_modifiers_count));
	if (FSDPE_OK != result)
		return result;
	...
	dsc->time_periods = av_calloc(dsc->time_periods_count, sizeof(fsdp_time_period_t* ));
	index = 0;
	for (j = 0; j < dsc->time_periods_count; j++) {
		...
		dsc->time_periods[j] = av_calloc(1, sizeof(fsdp_time_period_t));
		...
		if (h > 0) {
			...
			dsc->time_periods[j]->repeats = av_calloc(h, sizeof(fsdp_repeat_t* ));
			for (h = 0; h < dsc->time_periods[j]->repeats_count; h++) {
				...
				*/
				if (sscanf(p, "r=%10s %10s %" MLFLENS "[^\r\n]",
					...
					dsc->time_periods[j]->repeats[h] = av_calloc(1, sizeof(fsdp_repeat_t));
					repeat = dsc->time_periods[j]->repeats[h];
					/* get interval, duration and list of offsets*/
					result = fsdp_repeat_time_to_uint(fsdp_buf[0], &(repeat->interval));
					if (result == FSDPE_OK) {
						result = fsdp_repeat_time_to_uint(fsdp_buf[1], &(repeat->duration));
						if (result == FSDPE_OK) {
							unsigned int k = 1;
							...
							repeat->offsets = av_calloc(k, sizeof(time_t));
							i = longfsdp_buf;
							for (k = 0; (k < repeat->offsets_count)
							     && (result == FSDPE_OK); k++) {
								result =
								    fsdp_repeat_time_to_uint(i, &(repeat->offsets[k]));
								i = strchr(i, ' ');
								if (NULL != i)
									i++;
							}
	...
	if (!strncmp(p, "z=", 2)) {
		if (sscanf(p, "z=%" MLFLENS "[^\r\n]", longfsdp_buf)) {
			...
			dsc->timezone_adj = av_strdup(longfsdp_buf);
			NEXT_LINE(p);
		} else {
	...
	result = fsdp_parse_k(&p, &(dsc->k_encryption_method), &(dsc->k_encryption_content));
	if (result != FSDPE_OK)
		return result;
	...
	while (!strncmp(p, "a=", 2)) {
		...
		if (sscanf(p, "a=%9[^:\r\n]:%" MSFLENS "[^\r\n]", fsdp_buf[0], fsdp_buf[1]) == 2) {
			...
			if (!strncmp(fsdp_buf[0], "cat", 3))
				dsc->a_category = av_strdup(fsdp_buf[1]);
			else if (!strncmp(fsdp_buf[0], "keywds", 6))
				dsc->a_keywords = av_strdup(fsdp_buf[1]);
			else if (!strncmp(fsdp_buf[0], "tool", 4))
				dsc->a_keywords = av_strdup(fsdp_buf[1]);
			else if (!strncmp(fsdp_buf[0], "rtpmap", 6))
				fsdp_parse_rtpmap(&(dsc->a_rtpmaps), &(dsc->a_rtpmaps_count), fsdp_buf[1]);
				...
				dsc->a_charset = av_strdup(fsdp_buf[1]);
			else if (!strncmp(fsdp_buf[0], "sdplang", 7)) {
				if (NULL == dsc->a_sdplangs) {
					dsc->a_sdplangs_count = 0;
					dsc->a_sdplangs = av_calloc(SDPLANGS_MAX_COUNT, sizeof(char* ));
				}
				if (dsc->a_sdplangs_count < SDPLANGS_MAX_COUNT) {
					dsc->a_sdplangs[dsc->a_sdplangs_count] = av_strdup(fsdp_buf[1]);
					dsc->a_sdplangs_count++;
				}
			} else if (!strncmp(fsdp_buf[0], "lang", 4)) {
				if (NULL == dsc->a_langs) {
					dsc->a_langs_count = 0;
					dsc->a_langs = av_calloc(SDPLANGS_MAX_COUNT, sizeof(char* ));
				}
				if (dsc->a_langs_count < SDPLANGS_MAX_COUNT) {
					dsc->a_langs[dsc->a_langs_count] = av_strdup(fsdp_buf[1]);
					dsc->a_langs_count++;
				}
			} else if (!strncmp(fsdp_buf[0], "control", 7)) {
				if (NULL == dsc->a_controls) {
					dsc->a_controls_count = 0;
					dsc->a_controls = av_calloc(SDPCONTROLS_MAX_COUNT, sizeof(char* ));
				}
				if (dsc->a_controls_count < SDPCONTROLS_MAX_COUNT) {
					dsc->a_controls[dsc->a_controls_count] = av_strdup(fsdp_buf[1]);
					dsc->a_controls_count++;
				}
			} else if (!strncmp(fsdp_buf[0], "range", 5)) {
				if (dsc->a_range)
					av_free(dsc->a_range);
				dsc->a_range = av_strdup(fsdp_buf[1]);
			} else {
				...
				*longfsdp_buf = '\0';
				...
				if (NULL == dsc->unidentified_attributes) {
					...
					dsc->unidentified_attributes =
					    av_calloc(UNIDENTIFIED_ATTRIBUTES_MAX_COUNT, sizeof(char* ));
				}
				if (dsc->unidentified_attributes_count < UNIDENTIFIED_ATTRIBUTES_MAX_COUNT) {
					dsc->unidentified_attributes
					    [dsc->unidentified_attributes_count] = av_strdup(longfsdp_buf);
  ...
  /***************************************************************************/
	...
	if (dsc->media_announcements_count == 0) {
	...
	} else {		/* dsc->media_announcements_count > 0*/
		dsc->media_announcements = av_calloc(j, sizeof(fsdp_media_announcement_t* ));
		for (j = 0; j < dsc->media_announcements_count; j++) {
			...
			if (sscanf(p, "m=%11s %8s %7s %" MLFLENS "[^\r\n]",
				   fsdp_buf[0], fsdp_buf[1], fsdp_buf[2], longfsdp_buf) != 4) {
				return FSDPE_INVALID_MEDIA;
			} else {
				dsc->media_announcements[j] = av_calloc(1, sizeof(fsdp_media_announcement_t));
					...
					return FSDPE_UNKNOWN_MEDIA_TRANSPORT;
				{
					...
					media->formats = av_calloc(k, sizeof(char* ));
					s = longfsdp_buf;
					for (k = 0; k < media->formats_count; k++) {
						char* space = strchr(s, ' ');
						if (NULL != space)
							*space = '\0';
						media->formats[k] = av_strdup(s);
						s = space + 1;
					}
				}
				NEXT_LINE(p);
			...
			if (!strncmp(p, "i=", 2)
			    && sscanf(p, "i=%" MLFLENS "[^\r\n]", longfsdp_buf)) {
				media->i_title = av_strdup(longfsdp_buf);
				NEXT_LINE(p);
			} else {
				/* (optional) information absent*/
			}
			...
			result = fsdp_parse_c(&p, &(media->c_network_type),
					      &(media->c_address_type), &(media->c_address));
			if (result != FSDPE_OK)
				return result;

			/* `b=' lines (bandwidth information) [optional]*/
			/* b=<modifier>:<bandwidth-value>*/
			result = fsdp_parse_b(&p, &(media->bw_modifiers), &(media->bw_modifiers_count));
			if (FSDPE_OK != result)
				return result;
			...
			/* k=<method>
			   k=<method>:<encryption key>*/
			result = fsdp_parse_k(&p, &(media->k_encryption_method), &(media->k_encryption_content));
			if (result != FSDPE_OK)
				return result;
				...
				if (sscanf(p, "a=%9[^:\r\n]:%" MLFLENS "[^\r\n]", fsdp_buf[0], longfsdp_buf) == 2) {
					...
					if (!strncmp(fsdp_buf[0], "ptime", 5))
						...
						fsdp_parse_rtpmap(&(media->a_rtpmaps),
								  &(media->a_rtpmaps_count), longfsdp_buf);
						...
						if (NULL == dsc->a_sdplangs) {
							...
							media->a_sdplangs = av_calloc(SDPLANGS_MAX_COUNT, sizeof(char* ));
						}
						if (media->a_sdplangs_count < SDPLANGS_MAX_COUNT) {
							media->a_sdplangs[dsc->a_sdplangs_count] = av_strdup(longfsdp_buf);
							media->a_sdplangs_count++;
						}
					} else if (!strncmp(fsdp_buf[0], "lang", 4)) {
						if (NULL == dsc->a_langs) {
							media->a_langs_count = 0;
							media->a_langs = av_calloc(SDPLANGS_MAX_COUNT, sizeof(char* ));
						}
						if (media->a_langs_count < SDPLANGS_MAX_COUNT) {
							media->a_langs[dsc->a_langs_count] = av_strdup(longfsdp_buf);
							media->a_langs_count++;
						}
					} else if (!strncmp(fsdp_buf[0], "control", 7)) {
						if (NULL == media->a_controls) {
							...
							media->a_controls =
							    av_calloc(SDPCONTROLS_MAX_COUNT, sizeof(char* ));
						}
						if (media->a_controls_count < SDPCONTROLS_MAX_COUNT) {
							media->a_controls[media->a_controls_count] =
							    av_strdup(longfsdp_buf);
							media->a_controls_count++;
						}
					} else if (!strncmp(fsdp_buf[0], "range", 5)) {
						if (media->a_range)
							av_free(media->a_range);
						media->a_range = av_strdup(fsdp_buf[1]);
						...
						if (NULL == media->a_fmtps) {
							...
							media->a_fmtps = av_calloc(SDPLANGS_MAX_COUNT, sizeof(char* ));
						}
						if (media->a_fmtps_count < SDPLANGS_MAX_COUNT) {
							media->a_fmtps[media->a_fmtps_count] = av_strdup(longfsdp_buf);
							media->a_fmtps_count++;
						}
						...
						opts =
						    sscanf(longfsdp_buf, "%lu %2s %3s %" MSFLENS "s",
							   &wuint[0], fsdp_buf[0], fsdp_buf[1], fsdp_buf[2]);
						if (opts >= 1) {
						...
						*fsdp_buf[1] = '\0';
						...
						strncat(fsdp_buf[1], longfsdp_buf,
							MAXSHORTFIELDLEN - strlen(fsdp_buf[1]) - 1);
	     ...
	     fsdp_address_type_t*  atype, fsdp_connection_address_t * address)
{
	...
	if (!strncmp(*p, "c=", 2)) {
		if (sscanf(*p, "c=%2s %3s %" MSFLENS "s", fsdp_buf[0], fsdp_buf[1], fsdp_buf[2])) {
			if (!strncmp(fsdp_buf[0], "IN", 2)) {
			...
			}
			{
				...
				if (NULL == slash) {
					address->address = av_strdup(fsdp_buf[2]);
...
static fsdp_error_t fsdp_parse_b(const char* *p, fsdp_bw_modifier_t ** bw_modifiers, unsigned int *bw_modifiers_count)
{
	...
	*bw_modifiers = av_calloc(i, sizeof(fsdp_bw_modifier_t));
	...
	while (i > 0) {
		...
		if (2 == sscanf(*p, "b=%20[^:\r\n]:%lu", fsdp_buf, &wuint)) {
			if (!strncmp(fsdp_buf, "CT", 2))
				...
				(*bw_modifiers)[index].b_mod_type = FSDP_BW_MOD_TYPE_APPLICATION_SPECIFIC;
			else if (!strncmp(fsdp_buf, "RS", 2))
				(*bw_modifiers)[index].b_mod_type = FSDP_BW_MOD_TYPE_RTCP_SENDERS;
			else if (!strncmp(fsdp_buf, "RR", 2))
				(*bw_modifiers)[index].b_mod_type = FSDP_BW_MOD_TYPE_RTCP_RECEIVERS;
			else {
				(*bw_modifiers)[index].b_mod_type = FSDP_BW_MOD_TYPE_UNKNOWN;
				(*bw_modifiers)[index].b_unknown_bw_modt = (char* )av_strdup(fsdp_buf);
			}
		...
		if (sscanf(*p, "k=prompt")) {
			...
			*content = NULL;
			...
			if (sscanf(*p, "k=%6[^:\r\n]:%" MLFLENS "s", fsdp_buf, longfsdp_buf)) {
				...
				*content = av_strdup(longfsdp_buf);
				NEXT_LINE(*p);
			}
		}
	}
...
static fsdp_error_t fsdp_parse_rtpmap(fsdp_rtpmap_t* ** rtpmap, unsigned int *counter, const char *value)
{
	...
	if (0 ==* counter) {
		*counter = 0;
		*rtpmap = av_calloc(MEDIA_RTPMAPS_MAX_COUNT, sizeof(fsdp_rtpmap_t* ));
	}
	if (*counter < MEDIA_RTPMAPS_MAX_COUNT) {
		...
		char fsdp_buf[MAXSHORTFIELDLEN];
		char longfsdp_buf[MAXLONGFIELDLEN];
		map[c] = av_calloc(1, sizeof(fsdp_rtpmap_t));

		/* a=rtpmap:<payload type> <encoding name>/<clock rate>[/<encoding
		   parameters]>*/
		if (2 == sscanf(value, "%s %s", fsdp_buf, longfsdp_buf)) {
			char* slash1;
			map[c]->pt = av_strdup(fsdp_buf);
			...
			if (NULL == slash1) {
				...
				*slash1 = '\0';
				slash1++;
				map[c]->encoding_name = av_strdup(longfsdp_buf);
				slash2 = strchr(slash1, '/');
				if (NULL != slash2) {
					*slash2 = '\0';
					slash2++;
					map[c]->parameters = av_strdup(slash2);
				}
...
unsigned long int
fsdp_get_period_repeat_interval(const fsdp_description_t*  dsc, unsigned int index, unsigned int rindex)
{
	if ((!dsc) || (index >= dsc->time_periods_count))
		return 0;
...
unsigned long int
fsdp_get_period_repeat_duration(const fsdp_description_t*  dsc, unsigned int index, unsigned int rindex)
{
	if ((!dsc) || (index >= dsc->time_periods_count))
		return 0;
...
const char* fsdp_get_control(const fsdp_description_t * dsc, unsigned int index)
{
...
const fsdp_media_description_t* fsdp_get_media(const fsdp_description_t * dsc, unsigned int index)
{
...
unsigned int fsdp_get_media_control_count(const fsdp_media_description_t*  mdsc)
{
	if (!mdsc)
		return 0;
...
char* fsdp_get_media_control(const fsdp_media_description_t * mdsc, unsigned int index)
{
