...
{
	fsdp_description_t* result = av_malloc(sizeof(fsdp_description_t));

	result->version = 0;
	result->o_username = result->o_session_id = result->o_announcement_version = NULL;
	...
	for (i = 0; i < dsc->media_announcements_count; i++) {
		...
		for (j = 0; j < dsc->media_announcements[i]->bw_modifiers_count; j++) {
			if (FSDP_BW_MOD_TYPE_UNKNOWN == dsc->media_announcements[i]->bw_modifiers[j].b_mod_type)
				safe_free(dsc->media_announcements[i]->bw_modifiers[j].b_unknown_bw_modt);
		}
		...
		for (j = 0; j < dsc->media_announcements[i]->a_rtpmaps_count; j++) {
			...
			safe_free(dsc->media_announcements[i]->a_rtpmaps[j]->encoding_name);
		...
		safe_free(dsc->media_announcements[i]->c_address.address);
		safe_free(dsc->media_announcements[i]->a_rtcp_address);
