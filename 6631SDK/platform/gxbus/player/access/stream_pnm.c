...
extern void fixup_network_stream_cache(GxStream*  stream);

typedef struct gx_stream_pnm {
	GxStream parent;
...
static int pnm_get_chunk(pnm_t*  p, unsigned int max, unsigned int *chunk_type, char *data, int *need_response)
{
	...
	switch (*chunk_type) {
			...
			*need_response = 0;
			...
			while (1) {
				...
				if (*ptr == 'X') {	/* checking for server message*/
					gxlogf("input_pnm: got a message from server:\n");
					if (max < 1)
						return -1;
					rm_read(p->s, ptr + 2, 1);
					max = -1;
		...
		case CONT_TAG:
			if (chunk_size > max || chunk_size < PREAMBLE_SIZE) {
				gxlogf("error: max chunk size exceded (max was 0x%04x)\n", max);
#ifdef LOG
...
static void pnm_send_request(pnm_t*  p, uint32_t bandwidth)
{
	...
	c += pnm_write_chunk(PNA_CLIENT_CHALLANGE, strlen(pnm_challenge), pnm_challenge, &p->buffer[c]);
	c += pnm_write_chunk(PNA_CLIENT_CAPS, PNM_CLIENT_CAPS_SIZE, pnm_client_caps, &p->buffer[c]);
	...
	c += pnm_write_chunk(PNA_TIMESTAMP, strlen(pnm_timestamp), pnm_timestamp, &p->buffer[c]);
	c += pnm_write_chunk(PNA_BANDWIDTH, 4, (const char* )&pnm_default_bandwidth, &p->buffer[c]);
	...
	c += pnm_write_chunk(PNA_GUID, strlen(pnm_guid), pnm_guid, &p->buffer[c]);
	c += pnm_write_chunk(PNA_TWENTYFOUR, PNM_TWENTYFOUR_SIZE, pnm_twentyfour, &p->buffer[c]);
	...
	*need_response = 0;
	...
	while (1) {
		if (HEADER_SIZE - size <= 0) {
			gxlogf("input_pnm: header buffer overflow. exiting\n");
			return 0;
		}
	...
	if (!prop_hdr) {
		gxlogf("input_pnm: error while parsing headers.\n");
		return 0;
	}
...
static int pnm_calc_stream(pnm_t*  p)
{
	 ...
	 */
	if (p->seq_current[0] == p->seq_num[0])
		str0 = 1;
	if (p->seq_current[0] == p->seq_num[2])
		str1 = 1;
	...
	gxlogf("input_pnm: wow, something very nasty happened in pnm_calc_stream\n");
	return 2;
}
...
static int pnm_get_stream_chunk(pnm_t*  p)
{
	...
	if (p->buffer[0] == 0x62) {
		...
		if (n < 8)
			return 0;
#ifdef LOG
		gxlogf("input_pnm: had to seek 8 bytes on 0x62\n");
#endif
	}
	...
	if (p->buffer[0] == 'X') {
		...
		p->buffer[size + 3] = 0;
		gxlogf("input_pnm: got message from server while reading stream:\n%s\n", &p->buffer[3]);
		return -1;
	}
	if (p->buffer[0] == 'F') {
		gxlogf("input_pnm: server error.\n");
...
#ifdef LOG
	if (n)
		gxlogf("input_pnm: had to seek %i bytes to next chunk\n", n);
#endif

	/* check for 'Z's*/
	if ((p->buffer[0] != 0x5a) || (p->buffer[7] != 0x5a)) {
		gxlogf("input_pnm: bad boundaries\n");
		hexdump(p->buffer, 8);
		return 0;
	}
	...
	if (fof1 != fof2) {
		gxlogf("input_pnm: frame offsets are different: 0x%04x 0x%04x\n", fof1, fof2);
		return 0;
	}
...
static pnm_t* pnm_connect(int fd, char *path)
{
	...
	pnm_t* p = av_malloc(sizeof(pnm_t));
	int need_response = 0;

	p->path = av_strdup(path);
	p->s = fd;

	pnm_send_request(p, pnm_available_bandwidths[10]);
	if (!pnm_get_headers(p, &need_response)) {
		gxlogf("input_pnm: failed to set up stream\n");
		av_free(p->path);
		av_free(p);
		return NULL;
	}
...
#ifdef LOG
			gxlogf("input_pnm: %d of %d bytes provided\n", len - to_copy, len);
#endif
			if (retval < 0)
				return retval;
			else
...
#ifdef LOG
	gxlogf("input_pnm: %d bytes provided\n", len);
#endif
...
static void close_s(GxStream*  stream)
{
	GxStreamPnm* s = (GxStreamPnm *) (stream);
	pnm_close(s->streaming_ctrl->data);
}

static int pnm_streaming_read(int fd, char* buffer, int size, GxStreamingCtrl * stream_ctrl)
{
	return pnm_read(stream_ctrl->data, buffer, size);
}

static int read_s(GxStream*  stream, uint8_t * buffer, size_t size)
{
	GxStreamPnm* s = (GxStreamPnm *) (stream);
	return pnm_read(s->streaming_ctrl->data, buffer, size);
}

static int open_s(GxStream*  s, int mode)
{

	GxStreamPnm* stream = (GxStreamPnm *) (s);
	int fd;
	pnm_t* pnm;
	GxURL* url;

	gxlogf("GX_STREAM_PNM, URL: %s\n", stream->parent.url);
	...
	fd = connect_2_server(stream->streaming_ctrl->url->hostname,
			stream->streaming_ctrl->url->port ? stream->streaming_ctrl->url->port : 7070, 1, s->interrupt_cbk);
	...
	fixup_network_stream_cache(&stream->parent);
	return GX_PLAYER_OK;
...
GxStreamClass gx_stream_pnm = {
	._inherit = {
		._inherit = {
			.name    = "Streampnm",
			.parent  = &gx_streambase,
			.size    = sizeof(GxStreamPnm),
			.version = {7, 2},
		},
	},
	.protocols = {"pnm", NULL},
	DEF_AUTHOR("Stream","pnm","No description","L.F","No comment"),
