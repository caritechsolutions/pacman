				   ...
				   int port, int* redir, uint32_t bandwidth, char *user, char *pass)
{
	...
	rtsp_session = av_malloc(sizeof(rtsp_session_t));
	rtsp_session->s = NULL;
...
//connect:
	*redir = 0;
	...
	if (!rtsp_session->s) {
		gxlogd("rtsp_session: failed to connect to server %s\n", path);
		av_free(rtsp_session);
		return NULL;
	}

	/* looking for server type*/
	if (rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_SERVER))
		server = av_strdup(rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_SERVER));
	else {
		if (rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_REAL))
			server = av_strdup(RTSP_SERVER_TYPE_REAL);
		else
			server = av_strdup(RTSP_SERVER_TYPE_UNKNOWN);
	}
	if (strstr(server, RTSP_SERVER_TYPE_REAL) || strstr(server, RTSP_SERVER_TYPE_HELIX)) {
		...
		if (!h) {
			...
			if (rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_LOCATION)) {
				av_free(mrl_line);
				mrl_line = av_strdup(rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_LOCATION));
				gxlogd("rtsp_session: redirected to %s\n", mrl_line);
...
//      goto connect; /** shudder* i made a design mistake somewhere */
				...
				gxlogd("rtsp_session: session can not be established.\n");
				rtsp_close(rtsp_session->s);
				av_free(server);
				av_free(rtsp_session);
				return NULL;
		...
		if (!strncmp(h->streams[0]->mime_type, "application/vnd.rn-rmadriver", h->streams[0]->mime_type_size) ||
			...
			gxlogd("smil-over-realrtsp playlist, switching to raw rdt mode\n");
		} else {
			...
			rtsp_session->real_session->recv =
			    xbuffer_copyin(rtsp_session->real_session->recv, 0,
					   rtsp_session->real_session->header, rtsp_session->real_session->header_len);

			rtsp_session->real_session->recv_size = rtsp_session->real_session->header_len;
		}
		...
		if (!strstr(public, RTSP_METHOD_DESCRIBE)
			...
			gxlogd("Remote server does not meet minimal RTSP 1.0 compliance.\n");
			rtsp_close(rtsp_session->s);
			av_free(server);
			av_free(mrl_line);
			av_free(rtsp_session);
...
int rtsp_session_resume(rtsp_session_t* this)
{
	if(this->real_session)
		real_rtsp_resume(this->s);
	else if(this->rtp_session)
		rtp_session_resume(this->s, this->rtp_session);
	return 0;
}

int rtsp_session_pause(rtsp_session_t* this)
{
	if(this->real_session)
		real_rtsp_pause(this->s);
	else if(this->rtp_session)
		rtp_session_pause(this->s, this->rtp_session);
	return 0;
}
...
{
	if(this->real_session){
		rest_real_rtsp_session(this->real_session);
		this->real_session->recv_read = 0;
		this->real_session->recv_size = 0;
		real_rtsp_seek(this->s,start_time);
	}else if(this->rtp_session){
		init_rtp_cache();
		rtp_session_seek(this->s, this->rtp_session, (int64_t)start_time);
	}
	return 0;
}
...
{
	return real_rtsp_get_answer(this->s);
}

int rtsp_session_send_alive(rtsp_session_t* this)
{
	if(this->real_session){
		real_rtsp_send_alive(this->s);
	}else if(this->rtp_session){
...
int rtsp_session_read(rtsp_session_t*  this, char *data, int len, int time_over)
{
	if (this->real_session) {
		int to_copy = len;
		char* dest = data;
		char* source = (char *)(this->real_session->recv + this->real_session->recv_read);
		...
		while (to_copy > fill) {
			...
			if (this->real_session->recv_size == 0) {
#ifdef LOG
			//	gxlogd("librtsp: %d of %d bytes provided\n", len - to_copy, len);
#endif
				return len - to_copy;
			}
		}
...
#ifdef LOG
	//	gxlogd("librtsp: 0x%x bytes provided\n", len);
#endif
		...
		l = read_rtp_from_server(this->rtp_session->rtp_socket, data, len, time_over);
...
int64_t rtsp_get_duration(rtsp_session_t* session)
{
	if(session->rtp_session){
		return rtp_session_get_duration(session->rtp_session);
	}
	return -1;
}
