...
#include "../freesdp/common_sdp.h"
#include "../freesdp/parser_sdp.h"
...
#define RTCP_SEND_FREQUENCY (60*1000)
...
static struct rtp_rtsp_session_t* rtp_session_new(void)
{
	struct rtp_rtsp_session_t* st = NULL;

	st = av_malloc(sizeof(struct rtp_rtsp_session_t));

	st->rtp_socket = -1;
	st->rtcp_socket = -1;
	st->control_url = NULL;
	st->last_time = GxStream_GetTimeMs();
	st->duration = -1;
...
void rtp_session_free(struct rtp_rtsp_session_t* st)
{
	...
	if (st->control_url)
		av_free(st->control_url);
	av_free(st);
}

static void rtp_session_set_fd(struct rtp_rtsp_session_t* st, int rtp_sock, int rtcp_sock)
{
	if (!st)
		return;
...
static int parse_port(const char* line, const char *param, int *rtp_port, int *rtcp_port)
{
	char* parse1;
	char* parse2;
	char* parse3;

	char* line_copy = av_strdup(line);
...
static char* parse_destination(const char *line)
{
	char* parse1;
	char* parse2;
...
static int rtcp_connect(int client_port, int server_port, const char* server_hostname)
{
	...
	hints.ai_next = NULL;
	if(getaddrinfo(server_hostname, NULL, &hints, &res)){
		gxlogd("udp: %s error, getaddrinfo return null\n",__FUNCTION__);
		closesocket(s);
		return -1;
	}
	...
	memcpy(&sin, res->ai_addr, sizeof(struct sockaddr));
	sin.sin_family = AF_INET;
...
static int rtp_connect(char* hostname, int port)
{
	struct sockaddr_in sin;
	struct timeval tv;
	int err, err_len;
	...
	if (setsockopt(s, SOL_SOCKET, SO_RCVBUF, &rxsockbufsz, sizeof(rxsockbufsz)))
		gxlogd("Couldn't set receive socket buffer size\n");
	...
	if ((ntohl(sin.sin_addr.s_addr) >> 28) == 0xe) {
		...
		if (setsockopt(s, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mcast, sizeof(mcast))) {
			gxlogd("IP_ADD_MEMBERSHIP failed\n");
			close(s);
			return -1;
		}
	}
	...
	if (err < 0) {
		gxlogd("Select failed: %s\n", strerror(errno));
		close(s);
		return -1;
	} else if (err == 0) {
		gxlogd("Timeout! No data from host %s\n", hostname);
		close(s);
		return -1;
	}
...
#else
	inet_pton(AF_INET, addr, (char*)&sin.sin_addr);
#endif
#else
	sin.sin_addr.s_addr = htonl(INADDR_ANY);
#endif
...
struct rtp_rtsp_session_t* rtp_setup_and_play(rtsp_t * rtsp_session)
{
	...
	sdp = (char* )av_malloc(content_length + 1);
	if (rtsp_read_data(rtsp_session, (unsigned char*)sdp, content_length) <= 0) {
		av_free(sdp);
		return NULL;
	}
	...
	if (fsdp_get_media_count(dsc) != 1) {
		gxlogd("A single media stream only is supported atm.\n");
		fsdp_description_delete(dsc);
		return NULL;
	}
	...
	*     use range provided by server in SDP or start now if empty */
	...
	if (fsdp_get_media_type(med_dsc) != FSDP_MEDIA_VIDEO && fsdp_get_media_type(med_dsc) != FSDP_MEDIA_AUDIO) {
		fsdp_description_delete(dsc);
		return NULL;
	}
	...
	if (!strstr(fsdp_get_media_format(med_dsc, 0), RTSP_MEDIA_CONTAINER_MPEG_TS)) {
		fsdp_description_delete(dsc);
		return NULL;
	}
	...
	gxlogf("RTP Port from SDP appears to be: %d\n", client_rtp_port);
	gxlogf("RTCP Port from SDP appears to be: %d\n", client_rtcp_port);
	...
	if (fsdp_get_media_address(med_dsc))
		server_addr = av_strdup(fsdp_get_media_address(med_dsc));
	else if (fsdp_get_global_conn_address(dsc)) {
		/* no control for media: try global one instead*/
		server_addr = av_strdup(fsdp_get_global_conn_address(dsc));
	}
	...
	if (fsdp_get_media_control(med_dsc, 0))
		rtp_session->control_url = av_strdup(fsdp_get_media_control(med_dsc, 0));
	fsdp_description_delete(dsc);
	...
	snprintf(temp_buf, MAX_LENGTH,
		 ...
		 is_multicast ? RTSP_MULTICAST_PORT : RTSP_UNICAST_CLIENT_PORT, client_rtp_port, client_rtcp_port);
	gxlogd("RTSP Transport: %s\n", temp_buf);
	...
	if (!strncmp(rtp_session->control_url, "rtsp://", 7))	/* absolute URL*/
		statut = rtsp_request_setup(rtsp_session, rtp_session->control_url, NULL);
	else			/* relative URL*/
		statut = rtsp_request_setup(rtsp_session, NULL, rtp_session->control_url);
	...
	*      the real address and port to connect to */
	...
	if (is_multicast)
		parse_port(answer, RTSP_MULTICAST_PORT, &client_rtp_port, &client_rtcp_port);
	else {
		parse_port(answer, RTSP_UNICAST_CLIENT_PORT, &client_rtp_port, &client_rtcp_port);
		parse_port(answer, RTSP_UNICAST_SERVER_PORT, &server_rtp_port, &server_rtcp_port);
	}

	/* now check network settings as determined by server*/
	if (rtsp_destination)
		destination = av_strdup(rtsp_destination);
	else
		destination = parse_destination(answer);
	if (!destination)
		destination = av_strdup(server_addr);
	av_free(server_addr);
	...
	gxlogd("RTSP Destination: %s\n", destination);
	gxlogd("Client RTP port : %d\n", client_rtp_port);
	gxlogd("Client RTCP port : %d\n", client_rtcp_port);
	gxlogd("Server RTP port : %d\n", server_rtp_port);
	gxlogd("Server RTCP port : %d\n", server_rtcp_port);
...
int rtp_session_resume(rtsp_t* s, struct rtp_rtsp_session_t* st)
{
	...
	rtsp_request_resume(s, NULL);
	rtsp_get_respone_answers(s);
	return 0;
}

int rtp_session_pause(rtsp_t* s, struct rtp_rtsp_session_t* st)
{
	rtsp_request_pause(s,NULL);
	rtsp_get_respone_answers(s);
	return 0;
}
...
int rtp_session_seek(rtsp_t* s, struct rtp_rtsp_session_t* st, int64_t start_ms)
{
	...
	if((start_ms < 0) || (start_ms > st->duration*1000))
		return 0;
