 ...
 *
 *  librtmp is av_free software; you can redistribute it and/or modify
 *  it under the terms of the GNU Lesser General Public License as
 *  published by the av_free Software Foundation; either version 2.1,
 ...
 *  the av_free Software Foundation, Inc., 51 Franklin Street, Fifth Floor,
...
AMF_AddProp(AMFObject *obj, const AMFObjectProperty *prop)
{
  if (!(obj->o_num & 0x0f))
    obj->o_props =
      av_realloc(obj->o_props, (obj->o_num + 16) * sizeof(AMFObjectProperty));
...
{
  if (!(cd->cd_num & 0x0f))
    cd->cd_props = av_realloc(cd->cd_props, (cd->cd_num + 16) * sizeof(AVal));
