...
#endif
extern void av_md5_sum(uint8_t *dst, const uint8_t *src, const int len);
...
static int select_mlti_data(const char* mlti_chunk, int mlti_size, int selection, char **out)
{
	...
	if ((mlti_chunk[0] != 'M')
	    ...
	    || (mlti_chunk[3] != 'I')) {
#ifdef RTSP_LOG
		gxlogd("libreal: MLTI tag not detected, copying data\n");
#endif
	...
	if (codec >= numrules) {
		gxlogd("realrtsp: codec index >= number of codecs. %i %i\n", codec, numrules);
		return 0;
	}
...
static rmff_header_t* real_parse_sdp(char *data, char **stream_rules, uint32_t bandwidth)
{
	...
	header = av_calloc(1, sizeof(rmff_header_t));

	header->fileheader = rmff_new_fileheader(4 + desc->stream_count);
	header->cont = rmff_new_cont(desc->title, desc->author, desc->copyright, desc->abstract);
	header->data = rmff_new_dataheader(0, 0);
	header->streams = av_calloc(1, sizeof(rmff_mdpr_t* ) * (desc->stream_count + 1));
#ifdef RTSP_LOG
	gxlogd("number of streams: %u\n", desc->stream_count);
#endif
	...
	for (i = 0; i < desc->stream_count; i++) {
...
#ifdef RTSP_LOG
		gxlogd("calling asmrp_match with:\n%s\n%u\n", desc->stream[i]->asm_rule_book, bandwidth);
#endif
		...
		for (j = 0; j < n; j++) {
#ifdef RTSP_LOG
			gxlogd("asmrp rule match: %u for stream %u\n", rulematches[j], desc->stream[i]->stream_id);
#endif
		...
		if (!desc->stream[i]->mlti_data) {
			...
			len =
			    select_mlti_data(desc->stream[i]->mlti_data, desc->stream[i]->mlti_data_size,
					     rulematches[0], &buf);

		header->streams[i] = rmff_new_mdpr(desc->stream[i]->stream_id,
						   ...
						   desc->stream[i]->stream_name, desc->stream[i]->mime_type, len, buf);

		duration = GXMAX(duration, desc->stream[i]->duration);
		max_bit_rate += desc->stream[i]->max_bit_rate;
		avg_bit_rate += desc->stream[i]->avg_bit_rate;
		max_packet_size = GXMAX(max_packet_size, desc->stream[i]->max_packet_size);
		if (avg_packet_size)
	...
	header->prop = rmff_new_prop(max_bit_rate,
				     avg_bit_rate,
				     max_packet_size,
				     avg_packet_size, 0, duration, 0, 0, 0, desc->stream_count, desc->flags);
...
int real_get_rdt_chunk(rtsp_t*  rtsp_session, char **buffer, int rdt_rawdata)
{
	...
	if (header[0] != 0x24) {
		gxlogd("realrtsp: rdt chunk not recognized: got 0x%02x\n", header[0]);
		return 0;
	}
		...
		if (header[6] == 0x06) {	// eof packet
			...
			*/
			if (flags1 & 0x7c)	// ignore eof for streams with id != 0
				return 0;
			gxlogd("realrtsp: Stream EOF detected\n");
			return -1;
		}
		header[0] = header[5];
		header[1] = header[6];
	...
	gxlogf("ts: %u, size: %u, flags: 0x%02x, unknown values: 0x%06x 0x%02x 0x%02x\n",
	...
	*buffer = xbuffer_ensure_size(*buffer, 12 + size);
	if (rdt_rawdata) {
		n = rtsp_read_data(rtsp_session,(unsigned char*)*buffer, size - 12);
		return (n <= 0) ? 0 : n;
	}
	rmff_dump_pheader(&ph,* buffer);
	size -= 12;
	n = rtsp_read_data(rtsp_session, (unsigned char*)(*buffer) + 12, size);
...
rmff_header_t* real_setup_and_get_header(rtsp_t * rtsp_session, uint32_t bandwidth, char *username, char *password)
{
	...
	/* get challenge*/
	challenge1 = av_strdup(rtsp_search_answers(rtsp_session, "RealChallenge1"));
#ifdef RTSP_LOG
	gxlogd("real: Challenge1: %s\n", challenge1);
#endif
	...
	if (status == 401) {
		int authlen;
		char* authreq, *b64_authstr = NULL;
		char* authstr = NULL;

		if (authfield) {
			gxlogd("realrtsp: authorization failed, check your credentials\n");
			goto autherr;
		}
		if (!(authreq = rtsp_search_answers(rtsp_session, "WWW-Authenticate"))) {
			gxlogd("realrtsp: 401 but no auth request, aborting\n");
			goto autherr;
		}
		if (!username) {
			gxlogd("realrtsp: auth required but no username supplied\n");
			goto autherr;
		}
		if (!strstr(authreq, "Basic")) {
			gxlogd("realrtsp: authenticator not supported (%s)\n", authreq);
			goto autherr;
		}
		authlen = strlen(username) + (password ? strlen(password) : 0) + 2;
		authstr = av_malloc(authlen);
		sprintf(authstr, "%s:%s", username, password ? password : "");
		authfield = av_malloc(authlen*  2 + 22);
		strcpy(authfield, "Authorization: Basic ");
		b64_authstr = av_base64_encode(authstr, authlen, (const uint8_t *)(authfield + 21), authlen*2);
		av_free(authstr);
		if (!b64_authstr) {
			gxlogd("realrtsp: base64 output overflow, this should never happen\n");
			goto autherr;
		}
	...
	if (status < 200 || status > 299) {
		...
		if (alert) {
			gxlogd("realrtsp: got message from server:\n%s\n", alert);
		}
	...
	if (!rtsp_search_answers(rtsp_session, "Content-length"))
		gxlogd("real: got no Content-length!\n");
	else
		size = atoi(rtsp_search_answers(rtsp_session, "Content-length"));

	// as size is unsigned this also catches the case (size < 0)
	if (size > MAX_DESC_BUF) {
		gxlogd("realrtsp: Content-length for description too big (> %uMB)!\n", MAX_DESC_BUF / (1024*  1024));
		xbuffer_free(buf);
		return NULL;
	}

	if (!rtsp_search_answers(rtsp_session, "ETag"))
		gxlogd("realrtsp: got no ETag!\n");
	else
		session_id = av_strdup(rtsp_search_answers(rtsp_session, "ETag"));

#ifdef RTSP_LOG
	gxlogd("real: Stream description size: %u\n", size);
#endif

	description = av_malloc(size + 1);

	if (rtsp_read_data(rtsp_session,(unsigned char*)description, size) <= 0) {
		buf = xbuffer_free(buf);
		return NULL;
	}
	description[size] = 0;
...
int real_rtsp_resume(rtsp_t * rtsp_session)
{
	char* buf = xbuffer_init(256);

	sprintf(buf,"%s-","Range: npt=");
	rtsp_schedule_field(rtsp_session, buf);
	rtsp_request_resume(rtsp_session, NULL);
	buf = xbuffer_free(buf);
	return 0;
}

int real_rtsp_pause(rtsp_t* rtsp_session)
{
	rtsp_request_pause(rtsp_session,NULL);
	return 0;
}

int real_rtsp_seek(rtsp_t* rtsp_session, uint64_t start_ms)
{
	char* buf = xbuffer_init(256);
	int s_ss=0, s_ms=0;
	...
	s_ss = start_ms/1000;
	s_ms = start_ms%1000;
	sprintf(buf, s_ms ? "%s%d.%d-" : "%s%d-", "Range: npt=", s_ss, s_ms);
	rtsp_schedule_field(rtsp_session, buf);
	rtsp_request_seek(rtsp_session, NULL);
	buf = xbuffer_free(buf);
	return 0;
}

int real_rtsp_send_alive(rtsp_t* rtsp_session)
{
	rtsp_request_options(rtsp_session,NULL);
	return 0;
}

int real_rtsp_get_answer(rtsp_t* rtsp_session)
{
	return rtsp_get_answers(rtsp_session);
}
...
struct real_rtsp_session_t* init_real_rtsp_session(void)
{
	...
	real_rtsp_session = av_malloc(sizeof(struct real_rtsp_session_t));
...
void free_real_rtsp_session(struct real_rtsp_session_t* real_session)
{
	...
	av_free(real_session);
}

void rest_real_rtsp_session(struct real_rtsp_session_t* real_session)
{
	if (!real_session)
		return;
	xbuffer_reset(real_session->recv);
	real_session->rdteof = 0;
	real_session->rdt_rawdata = 0;
}
