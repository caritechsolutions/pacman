...
static int rmff_dump_mdpr(rmff_mdpr_t*  mdpr, char *buffer, int bufsize)
{
	...
	if (!(bufsize > RMFF_MDPRHEADER_SIZE + mdpr->stream_name_size + mdpr->mime_type_size &&
	      (unsigned)bufsize - RMFF_MDPRHEADER_SIZE - mdpr->stream_name_size - mdpr->mime_type_size >
...
static int rmff_dump_cont(rmff_cont_t*  cont, char *buffer, int bufsize)
{
	...
	return RMFF_CONTHEADER_SIZE + cont->title_len + cont->author_len + cont->copyright_len + cont->comment_len;
}
...
int rmff_dump_header(rmff_header_t*  h, char *buffer, int max)
{
	...
	return written;
      ...
      buftoosmall:
	gxlogd("rmff_dumpheader: buffer too small, aborting. Please report\n");
	return -1;
}
...
static rmff_fileheader_t* rmff_scan_fileheader(const char *data)
{
	...
	rmff_fileheader_t* fileheader = av_malloc(sizeof(rmff_fileheader_t));
	...
	if (fileheader->object_version != 0) {
		gxlogd("warning: unknown object version in .RMF: 0x%04x\n", fileheader->object_version);
	}
...
static rmff_mdpr_t* rmff_scan_mdpr(const char *data)
{
	...
	mdpr->stream_name = av_malloc(mdpr->stream_name_size + 1);
	...
	mdpr->mime_type = av_malloc(mdpr->mime_type_size + 1);
	...
	mdpr->type_specific_data = av_malloc(mdpr->type_specific_len);
...
static rmff_cont_t* rmff_scan_cont(const char *data)
{
	...
	rmff_cont_t* cont = av_malloc(sizeof(rmff_cont_t));
	int pos;
	...
	cont->copyright = av_malloc(cont->copyright_len + 1);
	...
	cont->comment = av_malloc(cont->comment_len + 1);
...
rmff_header_t* rmff_scan_header(const char *data)
{
	...
	rmff_header_t* header = av_malloc(sizeof(rmff_header_t));
	...
	if (chunk_type != RMF_TAG) {
		gxlogd("rmff: not an real media file header (.RMF tag not found).\n");
		av_free(header);
		return NULL;
	}
	...
	header->streams = av_malloc(sizeof(rmff_mdpr_t* ) * (header->fileheader->num_headers));
	...
	for (i = 1; i < header->fileheader->num_headers; i++) {
		...
		if (ptr[0] == 0) {
			gxlogd("rmff: warning: only %d of %d header found.\n", i, header->fileheader->num_headers);
			break;
		}
...
rmff_header_t* rmff_scan_header_stream(int fd)
{
	...
	do {
		...
		chunk_type = AV_RB32(buf + index);
		index += 4;
		chunk_size = AV_RB32(buf + index);
		index += 4;
		...
		case PROP_TAG:
			...
			break;
		default:
			gxlogd("rmff_scan_header_stream: unknown chunk");
...
rmff_prop_t* rmff_new_prop(uint32_t max_bit_rate,
			   ...
			   uint32_t duration,
			   uint32_t preroll,
			   uint32_t index_offset, uint32_t data_offset, uint16_t num_streams, uint16_t flags)
{
...
rmff_mdpr_t* rmff_new_mdpr(uint16_t stream_number,
			   ...
			   const char* mime_type, uint32_t type_specific_len, const char *type_specific_data)
{
...
rmff_cont_t* rmff_new_cont(const char *title, const char *author, const char *copyright, const char *comment)
{
	...
	if (title) {
		...
		cont->title = av_strdup(title);
	}
	if (author) {
		cont->author_len = strlen(author);
		cont->author = av_strdup(author);
	}
	if (copyright) {
		cont->copyright_len = strlen(copyright);
		cont->copyright = av_strdup(copyright);
	}
	if (comment) {
		cont->comment_len = strlen(comment);
		cont->comment = av_strdup(comment);
	}
...
rmff_data_t* rmff_new_dataheader(uint32_t num_packets, uint32_t next_data_header)
{
	...
	rmff_data_t* data = av_malloc(sizeof(rmff_data_t));
...
void rmff_print_header(rmff_header_t*  h)
{
	...
	if (!h) {
		gxlogd("rmff_print_header: NULL given\n");
		return;
	}
	if (h->fileheader) {
		gxlogd("\nFILE:\n");
		gxlogd("file version      : %d\n", h->fileheader->file_version);
		gxlogd("number of headers : %d\n", h->fileheader->num_headers);
	}
	if (h->cont) {
		gxlogd("\nCONTENT:\n");
		gxlogd("title     : %s\n", h->cont->title);
	...
	if (h->prop) {
		gxlogd("\nSTREAM PROPERTIES:\n");
		gxlogd("bit rate (max/avg)    : %i/%i\n", h->prop->max_bit_rate, h->prop->avg_bit_rate);
		gxlogd("packet size (max/avg) : %i/%i bytes\n", h->prop->max_packet_size, h->prop->avg_packet_size);
		gxlogd("packets       : %i\n", h->prop->num_packets);
		gxlogd("duration      : %i ms\n", h->prop->duration);
		gxlogd("pre-buffer    : %i ms\n", h->prop->preroll);
		gxlogd("index offset  : %i bytes\n", h->prop->index_offset);
		gxlogd("data offset   : %i bytes\n", h->prop->data_offset);
		gxlogd("media streams : %i\n", h->prop->num_streams);
		gxlogd("flags         : ");
		if (h->prop->flags & PN_SAVE_ENABLED)
			gxlogd("save_enabled ");
		if (h->prop->flags & PN_PERFECT_PLAY_ENABLED)
			gxlogd("perfect_play_enabled ");
		if (h->prop->flags & PN_LIVE_BROADCAST)
			gxlogd("live_broadcast ");
		gxlogd("\n");
	}
	stream = h->streams;
	if (stream) {
...
void rmff_fix_header(rmff_header_t*  h)
{
	...
	if (!h) {
		gxlogd("rmff_fix_header: fatal: no header given.\n");
		return;
	}

	if (!h->streams) {
		gxlogd("rmff_fix_header: warning: no MDPR chunks\n");
	} else {
		streams = h->streams;
		while (*streams) {
			num_streams++;
	...
	if (h->prop) {
		if (h->prop->size != 50) {
#ifdef RTSP_LOG
			gxlogd("rmff_fix_header: correcting prop.size from %i to %i\n", h->prop->size, 50);
#endif
			h->prop->size = 50;
		}
		if (h->prop->num_streams != num_streams) {
#ifdef RTSP_LOG
			gxlogd("rmff_fix_header: correcting prop.num_streams from %i to %i\n", h->prop->num_streams,
			       num_streams);
#endif
		...
		gxlogd("rmff_fix_header: warning: no PROP chunk.\n");

	if (h->cont) {
		num_headers++;
		header_size += h->cont->size;
	} else
		gxlogd("rmff_fix_header: warning: no CONT chunk.\n");

	if (!h->data) {
#ifdef RTSP_LOG
		gxlogd("rmff_fix_header: no DATA chunk, creating one\n");
#endif
		h->data = av_malloc(sizeof(rmff_data_t));
	...
	if (!h->fileheader) {
#ifdef RTSP_LOG
		gxlogd("rmff_fix_header: no fileheader, creating one");
#endif
		h->fileheader = av_malloc(sizeof(rmff_fileheader_t));
...
#ifdef RTSP_LOG
		gxlogd("rmff_fix_header: setting num_headers from %i to %i\n", h->fileheader->num_headers, num_headers);
#endif
		h->fileheader->num_headers = num_headers;
	}
	...
	if (h->prop) {
...
#ifdef RTSP_LOG
			gxlogd("rmff_fix_header: setting prop.data_offset from %i to %i\n", h->prop->data_offset,
			       header_size);
#endif
		...
		if (h->prop->num_packets == 0) {
			int p =
			    (int)(h->prop->avg_bit_rate / 8.0*  (h->prop->duration / 1000.0) /
				  h->prop->avg_packet_size);
#ifdef RTSP_LOG
			gxlogd("rmff_fix_header: assuming prop.num_packets=%i\n", p);
#endif
			h->prop->num_packets = p;
		}
		if (h->data->num_packets == 0) {
#ifdef RTSP_LOG
			gxlogd("rmff_fix_header: assuming data.num_packets=%i\n", h->prop->num_packets);
#endif
			h->data->num_packets = h->prop->num_packets;
		}
#ifdef RTSP_LOG
		gxlogd("rmff_fix_header: assuming data.size=%i\n", h->prop->num_packets*  h->prop->avg_packet_size);
#endif
...
void rmff_free_header(rmff_header_t*  h)
{
	...
	if (h->fileheader)
		av_free(h->fileheader);
	if (h->prop)
		av_free(h->prop);
	if (h->data)
		av_free(h->data);
	if (h->cont) {
		av_free(h->cont->title);
		av_free(h->cont->author);
		av_free(h->cont->copyright);
		av_free(h->cont->comment);
		av_free(h->cont);
	}
	if (h->streams) {
		...
		while (*s) {
			av_free((*s)->stream_name);
			av_free((*s)->mime_type);
			av_free((*s)->type_specific_data);
			av_free(*s);
			s++;
		}
		av_free(h->streams);
