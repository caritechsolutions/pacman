...
static void asmrp_string(asmrp_t*  p)
{
	...
	while ((p->ch != '"') && (p->ch >= 32)) {
		...
		if (l < ASMRP_MAX_ID - 1)
			...
			gxlogd("error: string too long, ignoring char %c.\n", p->ch);
...
static void asmrp_identifier(asmrp_t*  p)
{
	...
	while (((p->ch >= 'A') && (p->ch <= 'z'))
		...
		if (l < ASMRP_MAX_ID - 1)
			...
			gxlogd("error: identifier too long, ignoring char %c.\n", p->ch);
...
static int asmrp_set_id(asmrp_t*  p, char *s, int v)
{
	...
	if (i < 0) {
		if (p->sym_tab_num == ASMRP_MAX_SYMTAB - 1) {
			gxlogd("sym_tab overflow, ignoring identifier %s\n", s);
			return 0;
		}
		i = p->sym_tab_num;
		p->sym_tab_num++;
		p->sym_tab[i].id = av_strdup(s);
...
static int asmrp_operand(asmrp_t*  p)
{
	...
	case ASMRP_SYM_DOLLAR:
		...
		if (i < 0) {
			gxlogd("error: unknown identifier %s\n", p->str);
		} else
			ret = p->sym_tab[i].v;
	...
	default:
		gxlogd("syntax error, $ number or ( expected\n");
	}

#ifdef RTSP_LOG
	gxlogd("operand done, =%d\n", ret);
#endif
...
static void asmrp_assignment(asmrp_t*  p)
{
	...
	if ((p->sym != ASMRP_SYM_NUM) && (p->sym != ASMRP_SYM_STRING)
	    && (p->sym != ASMRP_SYM_ID)) {
		gxlogd("error: number or string expected\n");
		return;
	}
	asmrp_get_sym(p);

#ifdef RTSP_LOG
	gxlogd("assignment done\n");
#endif
}
...
static int asmrp_eval(asmrp_t*  p, int *matches)
{
	...
	while (p->sym != ASMRP_SYM_EOF) {
		...
		if (asmrp_rule(p)) {
#ifdef RTSP_LOG
			gxlogd("rule #%d is true\n", rule_num);
#endif
			if (num_matches < MAX_RULEMATCHES - 1)
				matches[num_matches++] = rule_num;
			else{
				gxlogd("Ignoring matched asm rule %d, too many matched rules.\n", rule_num);
				return -1;
			}
		}
