...
static char* b64_decode(const char *in, char *out, int *size)
{
	...
	k = 0;
	 ...
	 /*CONSTANTCONDITION*/ for (j = 0; j < strlen(in); j += 4) {
		char a[4], b[4];
		...
		for (i = 0; i < 4; i++) {
			...
			if (dtable[c] & 0x80) {
				gxlogd("Illegal character '%c' in input.\n", c);
//        exit(1);
				return NULL;
			}
			a[i] = (char)c;
...
static sdpplin_stream_t* sdpplin_parse_stream(char **data)
{
	...
	sdpplin_stream_t* desc = av_calloc(1, sizeof(sdpplin_stream_t));
	...
	if (filter(*data, "m=", &buf)) {
		desc->id = av_strdup(buf);
	} else {
		gxlogd("sdpplin: no m= found.\n");
		av_free(desc);
		xbuffer_free(buf);
		return NULL;
	}
	*data = nl(*data);
	...
	while (*data &&* *data && *data[0] != 'm') {
#ifdef RTSP_LOG
		{
			...
			buf[len] = 0;
			gxlogd("libreal: sdpplin_stream: '%s'\n", buf);
		}
#endif
		...
		if (filter(*data, "a=StreamName:string;", &buf)) {
			desc->stream_name = av_strdup(buf);
		...
		if (filter(*data, "a=OpaqueData:buffer;", &buf)) {
			...
			desc->mlti_data = av_malloc(desc->mlti_data_size);
			memcpy(desc->mlti_data, decoded, desc->mlti_data_size);
			handled = 1;
			*data = nl(*data);
#ifdef RTSP_LOG
			gxlogd("mlti_data_size: %i\n", desc->mlti_data_size);
#endif
		}

		if (filter(*data, "a=ASMRuleBook:string;", &buf)) {
			desc->asm_rule_book = av_strdup(buf);
			handled = 1;
			*data = nl(*data);
		}
		...
		if (!handled) {
			...
			buf = xbuffer_copyin(buf, 0,* data, len + 1);
			buf[len] = 0;
			gxlogd("libreal: sdpplin_stream: not handled: '%s'\n", buf);
#endif
			*data = nl(*data);
		}
	}
	...
	if (!got_mimetype) {
		gxlogd("libreal: sdpplin_stream: no mimetype\n");
		desc->mime_type = av_strdup("audio/x-pn-realaudio");
		desc->mime_type_size = strlen(desc->mime_type);
		if (desc->stream_id)
			gxlogd("libreal: sdpplin_stream: implicit mimetype for stream_id != 0, weird.\n");
	}
...
sdpplin_t* sdpplin_parse(char *data)
{
	...
	sdpplin_t* desc = av_calloc(1, sizeof(sdpplin_t));
	char* buf = xbuffer_init(32);
	char* decoded = xbuffer_init(32);
	int handled;
	int len;
	...
	while (data &&* data) {
		...
		if (filter(data, "m=", &buf)) {
...
#ifdef RTSP_LOG
			gxlogd("got data for stream id %u\n", stream->stream_id);
#endif
			if (desc->stream && (stream->stream_id >= 0) && (stream->stream_id < desc->stream_count))
				...
				gxlogd("sdpplin: got 'm=', but 'a=StreamCount' is still unknown.\n");
				if (stream->stream_id == 0) {
					desc->stream_count = 1;
					desc->stream = av_malloc(sizeof(sdpplin_stream_t* ));
		...
		if (filter(data, "a=Copyright:buffer;", &buf)) {
			decoded = b64_decode(buf, decoded, &len);
			desc->copyright = av_strdup(decoded);
			handled = 1;
			data = nl(data);
		}
		...
		if (filter(data, "a=Abstract:buffer;", &buf)) {
			decoded = b64_decode(buf, decoded, &len);
			desc->abstract = av_strdup(decoded);
			handled = 1;
			data = nl(data);
		}
		...
		if (filter(data, "a=StreamCount:integer;", &buf)) {
			...
			desc->stream = av_malloc(sizeof(sdpplin_stream_t* ) * desc->stream_count);
			handled = 1;
			data = nl(data);
		}
		...
		if (!handled) {
			...
			buf = xbuffer_copyin(buf, 0, data, len + 1);
			buf[len] = 0;
			gxlogd("libreal: sdpplin: not handled: '%s'\n", buf);
#endif
			data = nl(data);
		}
	}
	...
	for (i = 0; i < description->stream_count; i++) {
		if (description->stream[i]) {
			if (description->stream[i]->stream_name)
				av_free(description->stream[i]->stream_name);
			if (description->stream[i]->mime_type)
				av_free(description->stream[i]->mime_type);
			if (description->stream[i]->mlti_data)
				av_free(description->stream[i]->mlti_data);
			if (description->stream[i]->asm_rule_book)
				av_free(description->stream[i]->asm_rule_book);
			av_free(description->stream[i]);
		}
	}

	if (description->stream_count)
		av_free(description->stream);
	if (description->title)
		av_free(description->title);
	if (description->author)
		av_free(description->author);
	if (description->copyright)
		av_free(description->copyright);
	if (description->abstract)
		av_free(description->abstract);
