...
{
	uint8_t* data = av_calloc(1, chunk_size + XBUFFER_HEADER_SIZE);
...
void *xbuffer_reset(void* buf)
{
	xbuffer_header_t* header = (xbuffer_header_t *) (buf - XBUFFER_HEADER_SIZE);
	memset(buf, 0, header->chunk_size);
	return buf;
}
...
void* xbuffer_free(void *buf)
{
	...
	av_free(((uint8_t* ) buf) - XBUFFER_HEADER_SIZE);
...
void* xbuffer_copyin(void *buf, int index, const void *data, int len)
{
	...
	if(buf == NULL){
		gxlogd("malloc error index %d, len %d\n", index, len);
		return 0;
	}
...
void* xbuffer_ensure_size(void *buf, int size)
{
	...
	if (xbuf->size < size) {
		...
		xbuf->size = new_size;
		buf = ((uint8_t* ) av_realloc(((uint8_t *) buf) - XBUFFER_HEADER_SIZE,
