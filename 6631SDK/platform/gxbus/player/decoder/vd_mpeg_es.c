...
static inline void idct_row(int16_t*  const block)
{
	...
	/* shortcut*/
	if (likely(!(block[1] | ((int32_t* ) block)[1] | ((int32_t *) block)[2] | ((int32_t *) block)[3]))) {
...
static void* (*av_malloc_hook) (unsigned size, mpeg2_alloc_t reason) = NULL;
static int (*free_hook) (void* buf) = NULL;

void* mpeg2_av_malloc(unsigned size, mpeg2_alloc_t reason)
{
	char* buf;

	if (av_malloc_hook) {
		buf = (char* )av_malloc_hook(size, reason);
		if (buf)
			return buf;
	}

	if (size) {
		buf = (char* )av_malloc(size + 63 + sizeof(void **));
		if (buf) {
			char* align_buf;
...
int mpeg2_header_picture(mpeg2dec_t*  mpeg2dec)
{
	...
	if (type == PIC_FLAG_CODING_TYPE_P || type == PIC_FLAG_CODING_TYPE_B) {
		...
		decoder->f_motion.f_code[0] = (((buffer[3] << 1) | (buffer[4] >> 7)) & 7) - 1;
...
static int sequence_ext(mpeg2dec_t*  mpeg2dec)
{
	...
	sequence->display_width = sequence->picture_width += ((buffer[1] << 13) | (buffer[2] << 5)) & 0x3000;
	sequence->display_height = sequence->picture_height += (buffer[2] << 7) & 0x3000;
	...
	sequence->frame_period = sequence->frame_period*  ((buffer[5] & 31) + 1) / (((buffer[5] >> 2) & 3) + 1);
...
static int sequence_display_ext(mpeg2dec_t*  mpeg2dec)
{
	...
	flags = ((sequence->flags & ~SEQ_MASK_VIDEO_FORMAT) | ((buffer[0] << 4) & SEQ_MASK_VIDEO_FORMAT));
	if (buffer[0] & 1) {
...
int mpeg2_header_sequence(mpeg2dec_t*  mpeg2dec)
{
	...
	if (buffer[7] & 2) {
		...
		for (i = 0; i < 64; i++)
			mpeg2dec->new_quantizer_matrix[0][mpeg2_scan_norm[i]] = default_intra_quantizer_matrix[i];

	if (buffer[7] & 1)
		for (i = 0; i < 64; i++)
			mpeg2dec->new_quantizer_matrix[1][mpeg2_scan_norm[i]] = buffer[i + 8];
	else
...
mpeg2_state_t mpeg2_seek_header(mpeg2dec_t*  mpeg2dec)
{
	...
	return (mpeg2dec->code ? mpeg2_parse_header(mpeg2dec) : mpeg2_header_picture_start(mpeg2dec));
}
...
mpeg2_state_t mpeg2_header_picture_start(mpeg2dec_t*  mpeg2dec)
{
	...
	mpeg2dec->state = ((mpeg2dec->state != STATE_SLICE_1ST) ? STATE_PICTURE : STATE_PICTURE_2ND);
	picture->flags = 0;
		...
		*picture = mpeg2dec->new_picture;
		...
		if (decoder->coding_type != B_TYPE) {
			...
			if (!low_delay + !mpeg2dec->convert)
				mpeg2dec->info.discard_fbuf = mpeg2dec->fbuf[!low_delay + !mpeg2dec->convert];
		}
		if (mpeg2dec->convert) {
			...
			if (!mpeg2dec->convert_start) {
				...
				mpeg2dec->decoder.convert_id =
				    mpeg2_av_malloc(mpeg2dec->convert_id_size, MPEG2_ALLOC_CONVERT_ID);
				...
				mpeg2dec->yuv_buf[0][0] = (uint8_t* ) mpeg2_av_malloc(y_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[0][1] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[0][2] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[1][0] = (uint8_t* ) mpeg2_av_malloc(y_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[1][1] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[1][2] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				y_size = decoder->stride_frame*  32;
				uv_size = y_size >> (2 - mpeg2dec->decoder.chroma_format);
				mpeg2dec->yuv_buf[2][0] = (uint8_t* ) mpeg2_av_malloc(y_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[2][1] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				mpeg2dec->yuv_buf[2][2] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
			}
			if (!mpeg2dec->custom_fbuf) {
				while (mpeg2dec->alloc_index < 3) {
					...
					fbuf->id = NULL;
					fbuf->buf[0] =
					    (uint8_t* ) mpeg2_av_malloc(convert_init.buf_size[0], MPEG2_ALLOC_CONVERTED);
					fbuf->buf[1] =
					    (uint8_t* ) mpeg2_av_malloc(convert_init.buf_size[1], MPEG2_ALLOC_CONVERTED);
					fbuf->buf[2] =
					    (uint8_t* ) mpeg2_av_malloc(convert_init.buf_size[2], MPEG2_ALLOC_CONVERTED);
				}
			...
			while (mpeg2dec->alloc_index < 3) {
				...
				fbuf->buf[0] = (uint8_t* ) mpeg2_av_malloc(y_size, MPEG2_ALLOC_YUV);
				fbuf->buf[1] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
				fbuf->buf[2] = (uint8_t* ) mpeg2_av_malloc(uv_size, MPEG2_ALLOC_YUV);
			}
			mpeg2_set_fbuf(mpeg2dec, (decoder->coding_type == B_TYPE));
		}
	} else {
...
static inline int get_motion_delta(mpeg2_decoder_t*  const decoder, const int f_code)
{
	int delta;
	int sign;
	const MVtab* tab;
...
#define bit_ptr (decoder->bitstream_ptr)
	...
	NEEDBITS(bit_buf, bits, bit_ptr);
	motion_x = (motion->pmv[0][0] + (get_motion_delta(decoder, motion->f_code[0]) << motion->f_code[1]));
	motion_x = bound_motion_vector(motion_x, motion->f_code[0] + motion->f_code[1]);
	motion->pmv[0][0] = motion_x;

	NEEDBITS(bit_buf, bits, bit_ptr);
	motion_y = (motion->pmv[0][1] + (get_motion_delta(decoder, motion->f_code[0]) << motion->f_code[1]));
...
static void motion_dummy(mpeg2_decoder_t*  const decoder, motion_t * const motion, mpeg2_mc_fct * const *const table)
{
}
		     ...
		     uint8_t*  forward_fbuf[3], uint8_t * backward_fbuf[3])
{
	...
	decoder->slice_uv_stride = decoder->slice_stride >> (2 - decoder->chroma_format);
...
mpeg2_state_t mpeg2_header_slice_start(mpeg2dec_t*  mpeg2dec)
{
	...
	if (!(mpeg2dec->nb_decode_slices))
		...
		mpeg2dec->convert_start(decoder->convert_id, mpeg2dec->fbuf[0], mpeg2dec->picture, mpeg2dec->info.gop);
		...
		if (mpeg2dec->decoder.coding_type == B_TYPE)
			...
			mpeg2_init_fbuf(&(mpeg2dec->decoder),
					mpeg2dec->yuv_buf[mpeg2dec->yuv_index ^ 1],
					mpeg2dec->yuv_buf[mpeg2dec->yuv_index], mpeg2dec->yuv_buf[mpeg2dec->yuv_index]);
		...
		mpeg2_init_fbuf(&(mpeg2dec->decoder), mpeg2dec->fbuf[0]->buf,
				mpeg2dec->fbuf[b_type + 1]->buf, mpeg2dec->fbuf[b_type]->buf);
	}
...
mpeg2dec_t* mpeg2es_init(void)
{
	mpeg2dec_t* mpeg2dec;

	mpeg2_accel(MPEG2_ACCEL_DETECT);

	mpeg2dec = (mpeg2dec_t* ) mpeg2_av_malloc(sizeof(mpeg2dec_t), MPEG2_ALLOC_MPEG2DEC);
	if (mpeg2dec == NULL)
		return NULL;
	...
	mpeg2dec->chunk_buffer = (uint8_t* ) mpeg2_av_malloc(BUFFER_SIZE + 4, MPEG2_ALLOC_CHUNK);
...
static int mpeg_es_open(GxAVCodec*  codec)
{
	GxStreamVideoHeader* header = NULL;
	mpeg2dec_t* mpeg2dec;
	vd_libmpeg2_ctx_t* context;
	int accel;

	header = (GxStreamVideoHeader* ) (codec->header);

	accel = 0;

	mpeg2_accel(accel);

	mpeg2dec = mpeg2es_init();
	...
	context = av_calloc(1, sizeof(vd_libmpeg2_ctx_t));
...
static int mpeg_es_control(GxAVCodec*  codec, int query, void* args)
{
	return (1);
}

void mpeg2context_close(mpeg2dec_t*  mpeg2dec)
{
	mpeg2_header_state_init(mpeg2dec);
...
static int mpeg_es_close(GxAVCodec*  codec)
{
	int i;
	vd_libmpeg2_ctx_t* context = codec->priv;

	mpeg2dec_t* mpeg2dec = context->mpeg2dec;

	if (mpeg2dec->pending_buffer)
		av_free(mpeg2dec->pending_buffer);
	mpeg2dec->decoder.convert = NULL;
	mpeg2dec->decoder.convert_id = NULL;
	mpeg2context_close(mpeg2dec);

	for (i = 0; i < 3; i++) {
		if (context->quant_store[i])
			av_free(context->quant_store[i]);
	}
	if (codec->priv)
		av_free(codec->priv);
...
{
	decoder->bitstream_buf = (start[0] << 24) | (start[1] << 16) | (start[2] << 8) | start[3];
...
static void motion_fr_conceal(mpeg2_decoder_t*  const decoder)
{
	...
	NEEDBITS(bit_buf, bits, bit_ptr);
	tmp = (decoder->f_motion.pmv[0][0] + get_motion_delta(decoder, decoder->f_motion.f_code[0]));
	tmp = bound_motion_vector(tmp, decoder->f_motion.f_code[0]);
	decoder->f_motion.pmv[1][0] = decoder->f_motion.pmv[0][0] = tmp;

	NEEDBITS(bit_buf, bits, bit_ptr);
	tmp = (decoder->f_motion.pmv[0][1] + get_motion_delta(decoder, decoder->f_motion.f_code[1]));
	...
	while (1) {
		...
		if (macroblock_modes & MACROBLOCK_INTRA) {
			...
			if (likely(decoder->chroma_format == 0)) {
				slice_intra_DCT(decoder, 1, decoder->dest[1] + (offset >> 1), decoder->uv_stride);
				slice_intra_DCT(decoder, 2, decoder->dest[2] + (offset >> 1), decoder->uv_stride);
				if (decoder->coding_type == D_TYPE) {
					NEEDBITS(bit_buf, bits, bit_ptr);
					DUMPBITS(bit_buf, bits, 1);
				}
				...
				slice_intra_DCT(decoder, 1, dest_u + DCT_offset + 8, DCT_stride);
				slice_intra_DCT(decoder, 2, dest_v + DCT_offset + 8, DCT_stride);
			}
		} else {
			...
			if (macroblock_modes & MACROBLOCK_PATTERN) {
				...
				if (likely(decoder->chroma_format == 0)) {
					...
					if (coded_block_pattern & 2)
						slice_non_intra_DCT(decoder, 0, dest_y + 8, DCT_stride);
					...
					if (coded_block_pattern & 16)
						slice_non_intra_DCT(decoder, 1,
								    decoder->dest[1] + (offset >> 1), DCT_stride);
					if (coded_block_pattern & 32)
						slice_non_intra_DCT(decoder, 2,
								    decoder->dest[2] + (offset >> 1), DCT_stride);
					if (coded_block_pattern & (2 << 30))
						slice_non_intra_DCT(decoder, 1,
								    decoder->dest[1] + DCT_offset, DCT_stride);
					if (coded_block_pattern & (1 << 30))
						slice_non_intra_DCT(decoder, 2,
								    decoder->dest[2] + DCT_offset, DCT_stride);
				} else {
					int offset;
					...
					if (coded_block_pattern & (32 << 26))
						slice_non_intra_DCT(decoder, 1, dest_u + DCT_offset, DCT_stride);
					if (coded_block_pattern & (16 << 26))
						slice_non_intra_DCT(decoder, 2, dest_v + DCT_offset, DCT_stride);
					if (coded_block_pattern & (8 << 26))
						slice_non_intra_DCT(decoder, 1, dest_u + 8, DCT_stride);
					if (coded_block_pattern & (4 << 26))
						slice_non_intra_DCT(decoder, 2, dest_v + 8, DCT_stride);
					if (coded_block_pattern & (2 << 26))
						slice_non_intra_DCT(decoder, 1, dest_u + DCT_offset + 8, DCT_stride);
					if (coded_block_pattern & (1 << 26))
						slice_non_intra_DCT(decoder, 2, dest_v + DCT_offset + 8, DCT_stride);
				}
			}
		...
		do {
			if (decoder->quant_store) {
				if (decoder->picture_structure == TOP_FIELD)
					...
					decoder->quant_store[2*  decoder->quant_stride * (decoder->v_offset >> 4)
							     + decoder->quant_stride + (decoder->offset >> 4)] =
			...
			if (decoder->offset == decoder->width) {
				do {	/* just so we can use the break statement*/
					if (decoder->convert) {
						decoder->convert(decoder->convert_id, decoder->dest, decoder->v_offset);
						if (decoder->coding_type == B_TYPE)
							break;
					}
					decoder->dest[0] += decoder->slice_stride;
		...
		if (mba_inc) {
			...
			if (decoder->coding_type == P_TYPE) {
				do {
					MOTION_CALL(decoder->motion_parser[0], MACROBLOCK_MOTION_FORWARD);
					NEXT_MACROBLOCK;
				} while (--mba_inc);
			} else {
				do {
...
void vf_mpi_clear(GxImage*  mpi, int x0, int y0, int w, int h)
{
	int y;
	if (mpi->flags & MP_IMGFLAG_PLANAR) {
		...
		if (x0 == 0 && w == mpi->width) {
			...
			memset(mpi->planes[1] + mpi->stride[1]*  (y0 >> mpi->chroma_y_shift), 128,
			       mpi->stride[1]*  (h >> mpi->chroma_y_shift));
			memset(mpi->planes[2] + mpi->stride[2]*  (y0 >> mpi->chroma_y_shift), 128,
			       mpi->stride[2]*  (h >> mpi->chroma_y_shift));
		} else
			for (y = y0; y < y0 + h; y += 2) {
...
static inline GxImage* new_mp_image(int w, int h)
{
	GxImage* mpi;
	mpi = &tmp_mpi;
	if (!mpi)
		return NULL;	// error!
	memset(mpi, 0, sizeof(GxImage));
	mpi->width = mpi->w = w;
	mpi->height = mpi->h = h;
	return mpi;
}
...
#ifdef IMGFMT_YUY2
static inline void mp_image_setfmt(GxImage*  mpi, unsigned int out_fmt)
{
	...
	if (out_fmt == IMGFMT_ZRMJPEGNI || out_fmt == IMGFMT_ZRMJPEGIT || out_fmt == IMGFMT_ZRMJPEGIB) {
		mpi->bpp = 0;
		return;
	}
...
//      mpi->flags|=MP_IMGFLAG_PLANAR;
	...
	gxlogf ( "mp_image: unknown out_fmt: 0x%X\n", out_fmt);
	mpi->bpp = 0;
}
#endif

GxImage* vf_get_image(GxStreamVideoHeader * sh, int mp_imgtype, int mp_imgflag, int w, int h)
{
	GxImage* mpi = NULL;
	int w2;
	vf_image_context_t imgctx = { {0, 0}, {0}, {0}, 0 };

#ifdef MP_DEBUG
	assert(w == -1 || w >= sh->disp_w);
	assert(h == -1 || h >= sh->disp_h);
	assert(sh->disp_w > 0);
	assert(sh->disp_h > 0);
#endif
	...
	case MP_IMGTYPE_STATIC:
		if (!imgctx.static_images[0])
			imgctx.static_images[0] = new_mp_image(w2, h);
		mpi = imgctx.static_images[0];
		break;
	case MP_IMGTYPE_TEMP:
		if (!imgctx.temp_images[0])
			imgctx.temp_images[0] = new_mp_image(w2, h);
	...
	case MP_IMGTYPE_IP:
		if (!imgctx.static_images[imgctx.static_idx])
			imgctx.static_images[imgctx.static_idx] = new_mp_image(w2, h);
	...
	if (mpi) {
		...
		if (mpi->width != w2 || mpi->height != h) {
			//      gxlogf("vf.c: MPI parameters changed!  %dx%d -> %dx%d   \n", mpi->width,mpi->height,w2,h);
			if (mpi->flags & MP_IMGFLAG_ALLOCATED) {
				if (mpi->width < w2 || mpi->height < h) {
					// need to re-allocate buffer memory:
					av_free(mpi->planes[0]);
					mpi->flags &= ~MP_IMGFLAG_ALLOCATED;
					gxlogf("vf.c: have to REALLOCATE buffer memory :(\n");
				}
				//      } else {
			}
			{
		...
		if (!mpi->bpp)
			mp_image_setfmt(mpi, 0x32315659);
		if (!(mpi->flags & MP_IMGFLAG_ALLOCATED) && mpi->type > MP_IMGTYPE_EXPORT) {
			...
			if (!(mpi->flags & MP_IMGFLAG_DIRECT)) {
				...
				if (mp_imgflag & MP_IMGFLAG_PREFER_ALIGNED_STRIDE) {
					int align = (mpi->flags & MP_IMGFLAG_PLANAR && mpi->flags & MP_IMGFLAG_YUV) ? (8 << mpi->chroma_x_shift) - 1 : 15;	// -- maybe FIXME
					w2 = ((w + align) & (~align));
					if (mpi->width != w2) {
				...
				if (mpi->imgfmt == IMGFMT_IF09) {
					mpi->planes[0] =
					    memalign(64,
						     mpi->bpp*  mpi->width * (mpi->height + 2) / 8 +
						     mpi->chroma_width*  mpi->chroma_height);
					/* export delta table*/
					mpi->planes[3] =
					    mpi->planes[0] + (mpi->width*  mpi->height) +
					    2*  (mpi->chroma_width * mpi->chroma_height);
				} else
			...
			if (!(mpi->flags & MP_IMGFLAG_TYPE_DISPLAYED)) {
				gxlogf
				    ("(imgfmt: %x, planes: %p,%p,%p strides: %d,%d,%d, chroma: %dx%d, shift: h:%d,v:%d)\n",
				     ...
				     mpi->stride[0], mpi->stride[1], mpi->stride[2], mpi->chroma_width,
				     mpi->chroma_height, mpi->chroma_x_shift, mpi->chroma_y_shift);
...
// returns NULL or allocated GxImage*
// Note: buffer allocation may be moved to mpcodecs_config_vo() later...
GxImage* mpcodecs_get_image(GxStreamVideoHeader * sh, int mp_imgtype, int mp_imgflag, int w, int h)
{
	GxImage* mpi = vf_get_image(sh, mp_imgtype, mp_imgflag, w, h);
	mpi->x = mpi->y = 0;
	return mpi;
}
...
void mpeg2_set_buf(mpeg2dec_t*  mpeg2dec, uint8_t * buf[3], void *id)
{
	...
	if (mpeg2dec->custom_fbuf) {
		...
		mpeg2_set_fbuf(mpeg2dec, (mpeg2dec->decoder.coding_type == PIC_FLAG_CODING_TYPE_B));
...
//        gxlogf("libmpeg2: FBUF 0:%p 1:%p 2:%p\n",
//          mpeg2dec->fbuf[0]->buf[0],mpeg2dec->fbuf[1]->buf[0],mpeg2dec->fbuf[2]->buf[0]);
}

static int mpeg_es_decode(GxAVCodec*  codec, void **outdata, int *outdata_size, uint8_t * buf, int buf_size)
{
	vd_libmpeg2_ctx_t* context = codec->priv;
	mpeg2dec_t* mpeg2dec = context->mpeg2dec;
	const mpeg2_info_t* info = mpeg2_info(mpeg2dec);
	int drop_frame, framedrop = 1 & 3;
	GxStreamVideoHeader* header = codec->header;
	...
	while (1) {
		...
		unsigned long pw, ph;
		GxImage* mpi_new ;
		GxStreamVideoHeader* sh = (GxStreamVideoHeader*)codec->header;
		...
		case STATE_PICTURE:
			...
			// get_buffer "callback":
			mpi_new = mpcodecs_get_image((GxStreamVideoHeader* ) (header),
						     MP_IMGTYPE_IPB,
			...
			mpi_new->qstride = info->sequence->width >> 4;
			{
				...
				*p = av_realloc(*p, mpi_new->qstride*  (info->sequence->height >> 4));
				mpi_new->qscale =* p;
			}
			...
			if (mpi_new->flags & MP_IMGFLAG_DRAW_CALLBACK && !(mpi_new->flags & MP_IMGFLAG_DIRECT)) {
		...
		case STATE_INVALID_END:
			...
			if (info->display_fbuf) {
				//GxImage* mpi = info->display_fbuf->id;
				if (mpeg2dec->pending_length == 0) {
					mpeg2dec->pending_length = mpeg2dec->buf_end - mpeg2dec->buf_start;
					mpeg2dec->pending_buffer =
					    av_realloc(mpeg2dec->pending_buffer, mpeg2dec->pending_length);
					...
					mpeg2dec->pending_length = mpeg2dec->buf_end - mpeg2dec->buf_start;
					memmove(mpeg2dec->pending_buffer, mpeg2dec->buf_start,
						mpeg2dec->pending_length);
					mpeg2dec->pending_buffer =
					    av_realloc(mpeg2dec->pending_buffer, mpeg2dec->pending_length + buf_size);
					memcpy(mpeg2dec->pending_buffer + mpeg2dec->pending_length, buf, buf_size);
					mpeg2dec->pending_length += buf_size;
				}
				*outdata = info->display_fbuf->id;
				*outdata_size = sizeof(GxImage);
				//              gxlogd ( "pending = %d\n", mpeg2dec->pending_length);
				return 0;
			}
...
VdESBuffer  vd_es_buffer;

#define FILE_WRITE_BYTE(fp, ch)    fputc(ch, fp)
#define FILE_WRITE_WORD(fp, ch)	   fputc(ch & 0x00ff, fp);\
							   fputc((ch >> 8) & 0x00ff, fp)
#define FILE_WRITE_DWORD(fp, ch)   fputc((ch) & 0x00ff, fp);\
							   fputc(((ch) >> 8) & 0x00ff, fp);\
							   fputc(((ch)>> 16) & 0x00ff, fp);\
							   fputc(((ch) >> 24) & 0x00ff, fp)

uint8_t* table_rV[256];
uint8_t* table_gU[256];
int32_t  table_gV[256];
uint8_t* table_bU[256];

static const int Inverse_Table[4] = { 104597, 132201, 25675, 53279 };
...
static int vd_mpeg_es_run(GxMediaFilter*  filter);

void  yuv2rgb_c_open_tables(const int inv_table[4]) {
	int i = 0;
...
char* recorde_frame_data(uint8_t * src[], int stride[], int w, int h, int x, int y) {
	static int count;
	int i, j, k;
	int length = 0;
	char filename[128] = { 0 };
	FILE* fp = NULL;
	static char bmpbuf[720*  600 * 3] = { 0 };
	char* p1 = NULL, *p2 = NULL;

	if (w < 0 || h < 0)
		return (0);

	yuv2rgb_c_open_tables(Inverse_Table);

	snprintf(filename, sizeof(filename), "bmp/%04d.bmp", count);
	fp = fopen(filename, "w");
	if (fp == NULL)
		exit(0);
	fputs("BM", fp);

	gxlogf("*************Generate BMP file %s***************\n", filename);

	length = w*  h * 3;
	FILE_WRITE_DWORD(fp, length + 0x36);
	FILE_WRITE_DWORD(fp, 0);
	FILE_WRITE_DWORD(fp, 0x36);
	FILE_WRITE_DWORD(fp, 0x28);
	FILE_WRITE_DWORD(fp, w);
	FILE_WRITE_DWORD(fp, h);
	FILE_WRITE_WORD(fp, 1);
	FILE_WRITE_WORD(fp, 0x18);
	FILE_WRITE_DWORD(fp, 0);

	length = w*  h * 3;
	FILE_WRITE_DWORD(fp, length);
	FILE_WRITE_DWORD(fp, 0);
	FILE_WRITE_DWORD(fp, 0);
	FILE_WRITE_DWORD(fp, 0);
	FILE_WRITE_DWORD(fp, 0);
	...
	for (i = 0; i < h; i += 2) {
		...
		if (w & 4) {
			...
			Y = py_1[2*  0];
			FILE_WRITE_BYTE(fp, b[Y] >> 16);
			FILE_WRITE_BYTE(fp, g[Y] >> 8);
			FILE_WRITE_BYTE(fp, r[Y]);
			Y = py_1[2*  0 + 1];
			FILE_WRITE_BYTE(fp, b[Y] >> 16);
			FILE_WRITE_BYTE(fp, g[Y] >> 8);
			FILE_WRITE_BYTE(fp, r[Y]);
	...
	fwrite(bmpbuf, w*  h * 3, 1, fp);
	fclose(fp);
	count++;
	return bmpbuf;
}

void vd_mpeg_es_run_thread(void*  filter)
{
	void* decoded_frame = NULL;
	int drop_frame = 0;
	GxImage* mpi;
	GxStreamVideoHeader* sh_video = NULL;
	GxVideoCodec* vide_codec = (GxVideoCodec*)filter;
	int in_size;
	...
	sh_video = (GxStreamVideoHeader* ) (vide_codec->parent.header);
	...
	while (1) {
		...
		if(in_pin == NULL)
			continue;
		vfifo  = in_pin->source->fifo;
		len = GxFifo_GetLength(vfifo);

		GxFifo_Read(vfifo,vd_es_buffer.buffer + vd_es_buffer.size,len);
		vd_es_buffer.size += len;

		do{
				pos_bak = vd_es_buffer.pos;
				in_size = decoder_read_frame(vd_es_buffer.buffer, &sh_video->frametime, &start, sh_video->fps);

				if (in_size < 0 || (in_size > MAX_PACK_BYTES)) {
					vd_es_buffer.pos = pos_bak;
					break;
				}

				if (start) {
					decoded_frame = GxVideoDecoder_Decode(vide_codec, vd_es_buffer.buffer+pos_bak, in_size, drop_frame, sh_video->pts);
					vd_es_buffer.size -= (vd_es_buffer.pos-1);
				}
				mpi = (GxImage* ) decoded_frame;
				if (mpi)
					 recorde_frame_data(mpi->planes, mpi->stride, mpi->w, mpi->h, mpi->x, mpi->y);

				if(GxFifo_GetLength(vfifo) == 0 &&
					in_pin->source->filter->status== GX_MFT_STATE_STOPPED){
					GxMediaFilter_Stop(filter);
					return  ;
				}
		}while(in_size >0);

		if(vd_es_buffer.pos !=0)
			memcpy(vd_es_buffer.buffer,vd_es_buffer.buffer+ vd_es_buffer.pos,vd_es_buffer.size);
		vd_es_buffer.pos  = 0;
...
static int vd_mpeg_es_run(GxMediaFilter*  filter)
{
	...
	if (GxCore_ThreadCreate("vd_mpeg_es_thread", &d->run_pthread, vd_mpeg_es_run_thread, filter,10240,GXOS_DEFAULT_PRIORITY) == 0) {
...
GxAVCodecClass gx_decoder_mpeg_es = {
	._inherit = {
		._inherit = {
			.name    = "VideoDecoderMPEG_ES",
	...
	.name    = "VideoDecoderMPEG_ES",
	.format  = {0x10000001, 0x10000002, 0x3267706d,0},
	.open    = mpeg_es_open,
	.close   = mpeg_es_close,
	.control = mpeg_es_control,
	.decode  = mpeg_es_decode,
};
