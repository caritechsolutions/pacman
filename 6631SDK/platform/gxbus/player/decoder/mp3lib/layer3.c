...
*/
static int III_get_side_info(struct III_sideinfo* si, int stereo, int ms_stereo, int sfreq, int single, int lsf)
{
	int ch, gr;
		...
		for (ch = 0; ch < stereo; ch++) {
			...
			if (get1bit()) {	/* window switch flag */
				...
				if (2*  gr_info->big_values > bandInfo[sfreq].shortIdx[12])
					gxlogd ( "L3: BigValues too large, doesn't make sense %d %d\n",
...
#ifdef L3_DEBUG
					gxlogd ( "Blocktype == 0 and window-switching == 1 not allowed.\n");
#endif
					return 0;
				}
	...
	if (gr_info->block_type == 2) {
		...
		*/
		...
		if (gr_info->mixed_block_flag) {
			...
			/* max[3] not doublely needed in this case*/
			m = map[sfreq][1];
			me = mapend[sfreq][1];
		}
...
//  backbits(num);
	...
	if (part2remain > 0)
		...
		//gxlogf ( "mpg123: Can't rewind stream by %d bits!\n", -part2remain);
		return 1;	/* -> error*/
	}
	return 0;
}
...
//      gxlogd("III_i_st: tab1=%p tab2=%p tab=%d ms=%d  \n", tab1, tab2, tab, ms_stereo);
