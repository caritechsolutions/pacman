...
void gx_msg_init(void);
int gx_msg_test(int mod, int lev);

#ifdef __GNUC__
void gx_msg(int mod, int lev, const char *format, ... ) __attribute__ ((format (printf, 3, 4)));
#   ifdef MP_DEBUG
#      define gx_dbg(mod,lev, args... ) gx_msg(mod, lev, ## args )
#   else
#      define gx_dbg(mod,lev, args... ) /* only useful for developers */
#   endif
#else // not GNU C
void gx_msg(int mod, int lev, const char *format, ... );
#   ifdef MP_DEBUG
#      define gx_dbg(mod,lev, ... ) gx_msg(mod, lev, __VA_ARGS__)
#   else
#      define gx_dbg(mod,lev, ... ) /* only useful for developers */
#   endif
#endif
#else
#define gx_msg(mod,lev,format,...)	do{}while(0)
#define gx_dbg(mod,lev, ... )		do{}while(0)
#endif
