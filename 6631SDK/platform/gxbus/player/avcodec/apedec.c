...
int ape_decode_init(AVCodecContext *avctx)
{
	...
	avctx->priv_data = av_mallocz(sizeof(APEContext));
	s = avctx->priv_data;
	...
	case 24:
		...
		break;
	default:
		av_log(avctx, AV_LOG_ERROR, "%d bits per coded sample", s->bps);
		...
		AVPacket *avpkt)
{
	...
	if(!s->samples){
		...
		if (!nblocks || nblocks > INT_MAX / 2 / sizeof(*s->decoded_buffer) - 8) {
			av_log(avctx, AV_LOG_ERROR, "Invalid sample count: %d.\n",
					nblocks);
			return AVERROR_INVALIDDATA;
		}
	...
	av_fast_malloc(&s->decoded_buffer, &s->decoded_size, decoded_buffer_size, 0);
	if (!s->decoded_buffer)
		return AVERROR(ENOMEM);
...
void ape_flush(AVCodecContext *avctx)
{
	APEContext *s = avctx->priv_data;
	s->samples= 0;
}
