...
static void dv_close_context(PayloadContext *data)
{
    ffio_free_dyn_buf(&data->buf);
}

static int dv_sdp_parse_fmtp_config(AVFormatContext *s,
                                            AVStream *stream,
...
static int dv_parse_sdp_line(AVFormatContext *ctx, int st_index,
