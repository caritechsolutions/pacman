...
static int rtmp_http_send_cmd(URLContext *h, const char *cmd)
{
    ...
    if ((ret = url_read(rt->stream, &c, 1)) < 0)
        return ret;
...
static int rtmp_http_write(URLContext *h, uint8_t *buf, int size)
{
    RTMP_HTTPContext *rt = h->priv_data;
...
static int rtmp_http_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    do {
        ret = url_read(rt->stream, buf + off, size);
        ...
        if (!ret || ret == AVERROR_EOF) {
            ...
            if (rt->out_size > 0) {
                ...
                if ((ret = rtmp_http_write(h, (uint8_t *)"", 1)) < 0)
                    return ret;
...
static int rtmp_http_close(URLContext *h)
{
    ...
    if (rt->initialized) {
        ...
        if ((ret = rtmp_http_write(h, (uint8_t *)"", 1)) == 1)
...
static int rtmp_http_open(URLContext *h, const char *uri, int flags)
{
    ...
    if ((ret = url_alloc(&rt->stream, url, AVIO_FLAG_READ_WRITE)) < 0)
        goto fail;
    ...
    av_opt_set_bin(rt->stream->priv_data, "post_data", (uint8_t *)"", 1, 0);
    ...
    for (;;) {
        ret = url_read(rt->stream, (unsigned char *)(rt->client_id + off), sizeof(rt->client_id) - off);
        if (!ret || ret == AVERROR_EOF)
            break;
        if (ret < 0)
            goto fail;
...
URLProtocol gx_ffrtmphttp_protocol = {
    .name           = "ffrtmphttp",
