...
static int mmsh_open_internal(URLContext *h, const char *uri, int flags, int timestamp, int64_t pos)
{
    ...
    av_url_split(NULL, 0, NULL, 0,
        host, sizeof(host), &port, path, sizeof(path), (char *)mmsh->location);
    if (port<0)
        port = 80; // default mmsh protocol port
    ff_url_join(httpname, sizeof(httpname), "http", NULL, host, port, "%s", path);

    if (url_alloc(&mms->mms_hd, httpname, AVIO_FLAG_READ) < 0) {
        return AVERROR(EIO);
    }
    ...
    if ((err = url_alloc(&mms->mms_hd, httpname, AVIO_FLAG_READ)) < 0) {
        goto fail;
    }
...
static int handle_chunk_type(MMSHContext *mmsh)
{
    ...
    case CHUNK_TYPE_STREAM_CHANGE:
        ...
        if ((res = get_http_header_data(mmsh))) {
            gxloge ("Stream changed! Failed to get new header!\n");
            return res;
        }
        break;
    case CHUNK_TYPE_DATA:
                        ...
                        int64_t timestamp, int flags)
{
    ...
    h->priv_data = mmsh;
    ret= mmsh_open_internal(h, (char *)mmsh_old->location, 0, FFMAX(timestamp, 0), 0);
    if(ret>=0){
