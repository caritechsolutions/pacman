...
#include "../avutil/lfg.h"
#include "gx_common.h"
#include "../avutil/md5.h"
...
#define ENABLE_RTMPE 0
//#define ENABLE_RTMPT 1

#define RTMP_STREAM_FILE_DEBUG 0
#if RTMP_STREAM_FILE_DEBUG
    static char* stream_http_pvr_file = "/mnt/usb01/01_rtmp.flv"; /*ecos*/
    //static char* stream_http_pvr_file = "/media/sda1/01_rtmp.flv"; /*linux*/
#endif
...
extern uint64_t av_double2int(double f);
...
static int rtmp_send_packet(RTMPContext *rt, RTMPPacket *pkt, int track)
{
    ...
    if (pkt->type == RTMP_PT_INVOKE && track) {
        ...
        if ((ret = ff_amf_read_string(&gbc, (uint8_t *)name, sizeof(name), &len)) < 0) {
            goto fail;
        }
        ...
        while(fourcc_data - rt->enhanced_codecs < fourcc_str_len) {
            ...
            if (!strncmp(fourcc_data, "hvc1", 4) ||
                ...
                !strncmp(fourcc_data, "vp09", 4)) {
                    ...
                    gxloge ("Unsupported codec fourcc, %.*s\n", 4, fourcc_data);
...
static int read_connect(URLContext *s, RTMPContext *rt)
{
    ...
    for (;;) {
        ...
        if (pkt.type == RTMP_PT_CHUNK_SIZE) {
            ...
            gxlogf ("received abort message\n");
            ff_rtmp_packet_destroy(&pkt);
            return AVERROR_UNKNOWN;
        } else if (pkt.type == RTMP_PT_BYTES_READ) {
            gxlogf ("received acknowledgement\n");
            ...
            gxlogf ("Unknown control message type (%d)\n", pkt.type);
        }
        ff_rtmp_packet_destroy(&pkt);
    }
    ...
    if (ff_amf_read_string(&gbc, (uint8_t *)command, sizeof(command), &stringlen)) {
        gxlogf ("Unable to read command string\n");
        ff_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    if (strcmp(command, "connect")) {
        gxlogf ("Expecting connect, got %s\n", command);
        ff_rtmp_packet_destroy(&pkt);
        return AVERROR_INVALIDDATA;
    }
    ret = ff_amf_read_number(&gbc, &seqnum);
    if (ret)
        gxlogf ("SeqNum not found\n");
    ...
    ret = ff_amf_get_field_value(gbc.buffer,
                                 ...
                                 (uint8_t *)"app", (uint8_t *)tmpstr, sizeof(tmpstr));
    if (ret)
        gxlogf ("App field not found in connect\n");
    if (!ret && (0 != strcmp((char*)tmpstr, rt->app)))
        gxlogf ("App field don't match up: %s <-> %s\n", tmpstr, rt->app);
    ff_rtmp_packet_destroy(&pkt);
...
static int gen_play(URLContext *s, RTMPContext *rt)
{
    ...
    gxlogf ("Sending play command for '%s'\n", rt->playpath);
...
static int gen_pong(URLContext *s, RTMPContext *rt, RTMPPacket *ppkt)
{
    ...
    if (ppkt->size < 6) {
        gxlogf ("Too short ping packet (%d)\n", ppkt->size);
        return AVERROR_INVALIDDATA;
    }
                                      ...
                                      uint8_t *buf)
{
    ...
    if (rt->swfhash_len != 32) {
        gxlogf ("Hash of the decompressed SWF file is not 32 bytes long.\n");
        return AVERROR(EINVAL);
    }
    ...
    if ((ret = ff_rtmp_calc_digest((uint8_t *)rt->swfhash, 32, 0, buf, 32, p)) < 0)
        return ret;
...
static int rtmp_calc_swfhash(URLContext *s)
{
    ...
    ret = url_open(&stream, rt->swfverify, URL_RDWR, &s->url_context_options);
    if (ret < 0) {
        return ret;
    }

    if ((in_size = url_seek(stream, 0, AVSEEK_SIZE)) < 0) {
        ret = AVERROR(EIO);
        goto fail;
    }
    ...
    if ((ret = url_read_complete(stream, in_data, in_size)) < 0)
        goto fail;
    ...
    if (!memcmp(in_data, "CWS", 3)) {
        ...
        *in_data = 'F'; // magic stuff
        ...
        swfdata = out_data;
#else
        gxlogf ("Zlib is required for decompressing the SWF player file.\n");
        ret = AVERROR(EINVAL);
        goto fail;
#endif
    } else {
    ...
    if ((ret = ff_rtmp_calc_digest(swfdata, swfsize, 0,
                                   (uint8_t *)"Genuine Adobe Flash Player 001", 30,
                                   (uint8_t *)swfhash)) < 0)
        goto fail;

    /* Set SWFVerification parameters. */
    av_opt_set_bin(rt, "rtmp_swfhash", (uint8_t *)swfhash, 32, 0);
    rt->swfsize = swfsize;
...
static int rtmp_handshake(URLContext *s, RTMPContext *rt)
{
    ...
    if ((ret = url_write(rt->stream, tosend,
                           RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) {
        gxlogf ("Cannot write RTMP handshake request, ret=%d\n", ret);
        return ret;
    }

    if ((ret = url_read_complete(rt->stream, serverdata,
                                   RTMP_HANDSHAKE_PACKET_SIZE + 1)) < 0) {
        gxlogf ("Cannot read RTMP handshake response\n");
        return ret;
    }

    if ((ret = url_read_complete(rt->stream, clientdata,
...
#endif
        if ((ret = url_write(rt->stream, serverdata + 1,
                               RTMP_HANDSHAKE_PACKET_SIZE)) < 0)
            return ret;
                                  ...
                                  int size)
{
    ...
    inoutsize = url_read_complete(rt->stream, (unsigned char *)arraydata,
                                    RTMP_HANDSHAKE_PACKET_SIZE);
    if (inoutsize <= 0)
        return AVERROR(EIO);
    if (inoutsize != RTMP_HANDSHAKE_PACKET_SIZE) {
        gxlogf ("Erroneous Message size %d not following standard\n", (int)inoutsize);
        return AVERROR(EINVAL);
    }
                               ...
                               uint32_t second_int, char *arraydata, int size)
{
    ...
    inoutsize = url_write(rt->stream, (unsigned char *)arraydata,
                            RTMP_HANDSHAKE_PACKET_SIZE);
    if (inoutsize != RTMP_HANDSHAKE_PACKET_SIZE) {
        gxlogf ( "Unable to write answer\n");
        return AVERROR(EIO);
    }
...
static int rtmp_server_handshake(URLContext *s, RTMPContext *rt)
{
    ...
    char hs_c1[RTMP_HANDSHAKE_PACKET_SIZE];
    char hs_s1[RTMP_HANDSHAKE_PACKET_SIZE];
    uint32_t zeroes;
    uint32_t temp       = 0;
    int randomidx       = 0;
    int inoutsize       = 0;
    ...
    inoutsize = url_read_complete(rt->stream, buffer, 1);       // Receive C0
    if (inoutsize <= 0) {
        gxlogf ( "Unable to read handshake\n");
        return AVERROR(EIO);
    }
    // Check Version
    if (buffer[0] != 3) {
        gxlogf ( "RTMP protocol version mismatch\n");
        return AVERROR(EIO);
    }
    if (url_write(rt->stream, buffer, 1) <= 0) {                 // Send S0
        gxlogf ( "Unable to write answer - RTMP S0\n");
        return AVERROR(EIO);
    }
    /* Receive C1 */
    ret = rtmp_receive_hs_packet(rt, &hs_epoch, &zeroes, hs_c1,
                                 RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        gxlogf ("RTMP Handshake C1 Error\n");
        return ret;
    }
    /* Send S1 */
    ...
    for (randomidx = 8; randomidx < (RTMP_HANDSHAKE_PACKET_SIZE);
         randomidx += 4)
        AV_WB32(hs_s1 + randomidx, gx_av_get_random_seed());

    ret = rtmp_send_hs_packet(rt, hs_my_epoch, 0, hs_s1,
                              RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        gxlogf ( "RTMP Handshake S1 Error\n");
        return ret;
    }
    /* Send S2 */
    ret = rtmp_send_hs_packet(rt, hs_epoch, 0, hs_c1,
                              RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        gxlogf ("RTMP Handshake S2 Error\n");
        return ret;
    }
    /* Receive C2 */
    ret = rtmp_receive_hs_packet(rt, &temp, &zeroes, (char *)buffer,
                                 RTMP_HANDSHAKE_PACKET_SIZE);
    if (ret) {
        gxlogf ( "RTMP Handshake C2 Error\n");
        return ret;
    }
    if (temp != hs_my_epoch)
        gxlogf ("Erroneous C2 Message epoch does not match up with C1 epoch\n");
    if (memcmp(buffer + 8, hs_s1 + 8, RTMP_HANDSHAKE_PACKET_SIZE - 8))
        gxlogf ("Erroneous C2 Message random does not match up\n");
...
static int handle_chunk_size(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (pkt->size < 4) {
        gxlogf ("Too short chunk size change packet (%d)\n", pkt->size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (rt->in_chunk_size <= 0) {
        gxlogf ("Incorrect chunk size %d\n", rt->in_chunk_size);
        return AVERROR_INVALIDDATA;
    }
    gxlogf ("New incoming chunk size = %d\n", rt->in_chunk_size);
...
static int handle_user_control(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (pkt->size < 2) {
        gxlogf ("Too short user control packet (%d)\n", pkt->size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (t == 6) { // PingRequest
        ...
        if (rt->swfsize) {
            ...
            gxlogf ("Ignoring SWFVerification request.\n");
        }
    }
...
static int handle_set_peer_bw(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (pkt->size < 4) {
        gxlogf ( "Peer bandwidth packet is less than 4 bytes long (%d)\n",  pkt->size);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (rt->max_sent_unacked <= 0) {
        gxlogf ("Incorrect set peer bandwidth %d\n", rt->max_sent_unacked);
        return AVERROR_INVALIDDATA;

    }
    gxlogf ("Max sent, unacked = %d\n", rt->max_sent_unacked);
...
static int handle_window_ack_size(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (pkt->size < 4) {
        gxlogf ("Too short window acknowledgement size packet (%d)\n", pkt->size);
        return AVERROR_INVALIDDATA;
    }

    rt->receive_report_size = AV_RB32(pkt->data);
    if (rt->receive_report_size <= 0) {
        gxlogf ("Incorrect window acknowledgement size %d\n", rt->receive_report_size);
        return AVERROR_INVALIDDATA;
    }
    gxlogf ("Window acknowledgement size = %d\n", rt->receive_report_size);
...
static int do_adobe_auth(RTMPContext *rt, char *user, const char *salt,
                         char *opaque, char *challenge)
{
    uint8_t hash[16];
    ...
    snprintf(challenge2, sizeof(challenge2), "%08x", gx_av_get_random_seed());
    av_md5_init(md5);
    av_md5_update(md5, (uint8_t *)user, strlen(user));
    av_md5_update(md5, (uint8_t *)salt, strlen(salt));
    av_md5_update(md5, (uint8_t *)rt->password, strlen(rt->password));
    av_md5_final(md5, hash);
    av_base64_encode(hashstr, sizeof(hashstr), hash,
                     sizeof(hash));
    av_md5_init(md5);
    av_md5_update(md5, (uint8_t *)hashstr, strlen(hashstr));
    if (opaque)
        av_md5_update(md5, (uint8_t *)opaque, strlen(opaque));
    else if (challenge)
        av_md5_update(md5, (uint8_t *)challenge, strlen(challenge));
    av_md5_update(md5, (uint8_t *)challenge2, strlen(challenge2));
    av_md5_final(md5, hash);
    av_base64_encode(hashstr, sizeof(hashstr), hash,
                     sizeof(hash));
...
static int do_llnw_auth(RTMPContext *rt, const char *user, const char *nonce)
{
    ...
    snprintf(cnonce, sizeof(cnonce), "%08x", gx_av_get_random_seed());

    av_md5_init(md5);
    av_md5_update(md5, (uint8_t *)user, strlen(user));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)realm, strlen(realm));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)rt->password, strlen(rt->password));
    av_md5_final(md5, hash);
    ...
    av_md5_init(md5);
    av_md5_update(md5, (uint8_t *)method, strlen(method));
    av_md5_update(md5, (uint8_t *)":/", 2);
    av_md5_update(md5, (uint8_t *)rt->app, strlen(rt->app));
    if (!strchr(rt->app, '/'))
        av_md5_update(md5, (uint8_t *)"/_definst_", strlen("/_definst_"));
    av_md5_final(md5, hash);
    ff_data_to_hex(hashstr2, hash, 16, 1);
    hashstr2[32] = '\0';

    av_md5_init(md5);
    av_md5_update(md5, (uint8_t *)hashstr1, strlen(hashstr1));
    av_md5_update(md5, (uint8_t *)":", 1);
    if (nonce)
        av_md5_update(md5, (uint8_t *)nonce, strlen(nonce));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)nc, strlen(nc));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)cnonce, strlen(cnonce));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)qop, strlen(qop));
    av_md5_update(md5, (uint8_t *)":", 1);
    av_md5_update(md5, (uint8_t *)hashstr2, strlen(hashstr2));
    av_md5_final(md5, hash);
    ff_data_to_hex(hashstr1, hash, 16, 1);
...
static int handle_connect_error(URLContext *s, const char *desc)
{
    ...
    if (!rt->username[0] || !rt->password[0]) {
        gxlogd("No credentials set\n");
        return AVERROR_UNKNOWN;
    }

    if (strstr(desc, "?reason=authfailed")) {
        gxloge ("Incorrect username/password\n");
        return AVERROR_UNKNOWN;
    } else if (strstr(desc, "?reason=nosuchuser")) {
        gxloge ("Incorrect username\n");
        return AVERROR_UNKNOWN;
    }
...
static int handle_invoke_error(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (!ff_amf_get_field_value(pkt->data + 9, data_end,
                                (uint8_t *)"description", (uint8_t *)tmpstr, sizeof(tmpstr))) {
        if (tracked_method && (!strcmp(tracked_method, "_checkbw")      ||
            ...
            ret = handle_connect_error(s, (char *)tmpstr);
            if (!ret) {
                rt->do_reconnect = 1;
...
static int send_invoke_response(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (ff_amf_read_string(&gbc, (uint8_t *)command, sizeof(command),
                           &stringlen)) {
    ...
    if (!strcmp(command, "FCPublish") ||
        ...
        ret = ff_amf_read_string(&gbc, (uint8_t *)filename,
                                 sizeof(filename), &stringlen);
        if (ret) {
            if (ret == AVERROR(EINVAL))
                gxlogf ("Unable to parse stream name - name too long?\n");
            else
                gxlogf ("Unable to parse stream name\n");
            return ret;
        }
        // check with url
        if (s->filename) {
            ...
            if (!pchar) {
                gxlogf ( "Unable to find / in url %s, bad format\n", s->filename);
                pchar = s->filename;
            }
            pchar++;
            if (strcmp(pchar, filename))
                gxlogf ("Unexpected stream %s, expecting %s\n", filename, pchar);
        }
    ...
    if (!strcmp(command, "FCPublish")) {
        if ((ret = ff_rtmp_packet_create(&spkt, RTMP_SYSTEM_CHANNEL,
                                         RTMP_PT_INVOKE, 0,
                                         RTMP_PKTDATA_DEFAULT_SIZE)) < 0) {
            gxlogf ("Unable to create response packet\n");
            return ret;
        }
        pp = spkt.data;
...
static int read_number_result(RTMPPacket *pkt, double *number)
{
    ...
    if (ff_amf_read_string(&gbc, strbuffer, sizeof(strbuffer), &stringlen))
        return AVERROR_INVALIDDATA;
    if (strcmp((char *)strbuffer, "_result"))
        return AVERROR_INVALIDDATA;
...
static int handle_invoke_result(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (!strcmp(tracked_method, "connect")) {
        ...
        if (read_number_result(pkt, &stream_id)) {
            gxlogf ("Unexpected reply on connect()\n");
        } else {
        ...
        if (read_number_result(pkt, &rt->duration)) {
            gxlogf ("Unexpected reply on getStreamLength()\n");
        }
    }
...
static int handle_invoke_status(URLContext *s, RTMPPacket *pkt)
{
    ...
    t = ff_amf_get_field_value(ptr, data_end, (uint8_t *)"level", (uint8_t *)tmpstr, sizeof(tmpstr));
    if (!t && !strcmp(tmpstr, "error")) {
        t = ff_amf_get_field_value(ptr, data_end,
                                   (uint8_t *)"description", (uint8_t *)tmpstr, sizeof(tmpstr));
        if (t || !tmpstr[0])
            t = ff_amf_get_field_value(ptr, data_end, (uint8_t *)"code", (uint8_t *)tmpstr, sizeof(tmpstr));
        if (!t)
...
static int handle_notify(URLContext *s, RTMPPacket *pkt)
{
    ...
    if (!strcmp((char *)commandbuffer, "onMetaData")) {
        ...
        if (bytestream2_get_byte(&gbc) == AMF_DATA_TYPE_MIXEDARRAY) {
            ...
            while (bytestream2_get_bytes_left(&gbc) > 3) {
                if (ff_amf_get_string(&gbc, (uint8_t *)statusmsg, sizeof(statusmsg), &stringlen))
                    return AVERROR_INVALIDDATA;
    ...
    if (!strcmp((char *)commandbuffer, "@setDataFrame")) {
        skip = gbc.buffer - pkt->data;
        ret = ff_amf_read_string(&gbc, (uint8_t *)statusmsg, sizeof(statusmsg), &stringlen);
        if (ret < 0)
            return AVERROR_INVALIDDATA;
    }
...
static int rtmp_parse_result(URLContext *s, RTMPContext *rt, RTMPPacket *pkt)
{
    ...
    case RTMP_PT_BYTES_READ:
        gxlogf ("received bytes read report\n");
        break;
    ...
    case RTMP_PT_NOTIFY:
        ...
        break;
    default:
        gxlogf ("Unknown packet type received 0x%02X\n", pkt->type);
        break;
    }
    return 0;
}
...
static int handle_metadata(RTMPContext *rt, RTMPPacket *pkt)
{
    ...
    if (p != rt->flv_data + rt->flv_size) {
        gxlogf ("Incomplete flv packets in  RTMP_PT_METADATA packet\n");
...
#if RTMP_STREAM_FILE_DEBUG
    if(rt->rtmp_file_fd) {
        fclose(rt->rtmp_file_fd);
        rt->rtmp_file_fd = NULL;
    }
#endif
    if (!rt->is_input) {
        rt->flv_data = NULL;
...
static int inject_fake_duration_metadata(RTMPContext *rt)
{
    ...
    bytestream_put_buffer(&p, (const uint8_t *)"onMetaData", 10);
    ...
    bytestream_put_buffer(&p, (const uint8_t *)"duration", 8);
...
typedef struct {
    char proto[8]; // 8 byte
    char hostname[256]; // 256 byte
    char path[1024]; // 1KB
    char auth[100]; // 100 byte
    char fname_buffer[1024]; // 1KB
    char buf[2048]; // 2KB
}rtmp_open_param;
...
{
    RTMPContext *rt = s->priv_data;
    char *old_app, *qmark, *n, *fname;
    int port = -1, ret = -1;
    AVDictionary **opts = NULL;
    AVDictionaryEntry *e = NULL;
    rtmp_open_param *url_param = av_mallocz(sizeof(rtmp_open_param));
    ...
    av_url_split(url_param->proto, sizeof(url_param->proto), url_param->auth, sizeof(url_param->auth), url_param->hostname, sizeof(url_param->hostname), &port,
                 url_param->path, sizeof(url_param->path), s->filename);

    n = strchr(url_param->path, ' ');
    if (n) {
        *n = '\0'; // Trim not supported part
    }

    if (url_param->auth[0]) {
        char *ptr = strchr(url_param->auth, ':');
        if (ptr) {
            av_strlcpy(rt->username, url_param->auth, sizeof(rt->username));
            av_strlcpy(rt->password, ptr + 1, sizeof(rt->password));
        }
    }

    if (rt->listen && strcmp(url_param->proto, "rtmp")) {
        gxlogf ("rtmp_listen not available for %s\n",  url_param->proto);
        av_free(url_param);
        return AVERROR(EINVAL);
    }
    if (!strcmp(url_param->proto, "rtmpt") || !strcmp(url_param->proto, "rtmpts")) {
#if defined(CONFIG_STREAM_FFRTMPHTTP)
        if (!strcmp(url_param->proto, "rtmpts"))
            av_dict_set(opts, "ffrtmphttp_tls", "1", 1);

        /* open the http tunneling connection */
        ff_url_join(url_param->buf, sizeof(url_param->buf), "ffrtmphttp", NULL, url_param->hostname, port, NULL);
#endif
    } else if (!strcmp(url_param->proto, "rtmps")) {
        /* open the tls connection */
        if (port < 0)
            port = RTMPS_DEFAULT_PORT;
        ff_url_join(url_param->buf, sizeof(url_param->buf), "tls", NULL, url_param->hostname, port, NULL);
    } else if (!strcmp(url_param->proto, "rtmpe") || (!strcmp(url_param->proto, "rtmpte"))) {
#if ENABLE_RTMPE
        if (!strcmp(url_param->proto, "rtmpte"))
            av_dict_set(opts, "ffrtmpcrypt_tunneling", "1", 1);

        /* open the encrypted connection */
        ff_url_join(url_param->buf, sizeof(url_param->buf), "ffrtmpcrypt", NULL, url_param->hostname, port, NULL);
        rt->encrypted = 1;
#endif
    } else {
        ...
        if (rt->listen)
            ff_url_join(url_param->buf, sizeof(url_param->buf), "tcp", NULL, url_param->hostname, port,
                        "?listen&listen_timeout=%d", rt->listen_timeout * 1000);
        else
            ff_url_join(url_param->buf, sizeof(url_param->buf), "tcp", NULL, url_param->hostname, port, NULL);
    }

reconnect:
    ret = url_open(&rt->stream, url_param->buf, URL_RDWR, &s->url_context_options);
    if (ret < 0) {
        av_free(url_param);
        return ret;
    }
    ...
    qmark = strchr(url_param->path, '?');
    if (qmark && strstr(qmark, "slist=")) {
        char* amp;
        // After slist we have the playpath, the full path is used as app
        av_strlcpy(rt->app, url_param->path + 1, APP_MAX_LENGTH);
        fname = strstr(url_param->path, "slist=") + 6;
        // Strip any further query parameters from fname
        amp = strchr(fname, '&');
        if (amp) {
            av_strlcpy(url_param->fname_buffer, fname, FFMIN(amp - fname + 1,
                                                  sizeof(url_param->fname_buffer)));
            fname = url_param->fname_buffer;
        }
    } else if (!strncmp(url_param->path, "/ondemand/", 10)) {
        fname = url_param->path + 10;
        memcpy(rt->app, "ondemand", 9);
    } else {
        char *next = url_param->path[0] ? url_param->path + 1 : url_param->path;
        char *p = strchr(next, '/');
        if (!p) {
            if (old_app) {
            ...
            if (!fname || (c && c < fname)) {
                fname = p + 1;
                av_strlcpy(rt->app, url_param->path + 1, FFMIN(p - url_param->path, APP_MAX_LENGTH));
            } else {
                fname++;
                av_strlcpy(rt->app, url_param->path + 1, FFMIN(fname - url_param->path - 1, APP_MAX_LENGTH));
            }
        }
    }
    ...
    if (!rt->playpath) {
        ...
        if (fname) {
            ...
            if (!strchr(fname, ':') && len >= 4 &&
                (!strcmp(fname + len - 4, ".f4v") || !strcmp(fname + len - 4, ".mp4"))) {
    ...
    if (!rt->tcurl) {
        ...
        ff_url_join(rt->tcurl, TCURL_MAX_LENGTH, url_param->proto, NULL, url_param->hostname,  port, "/%s", rt->app);
    }
    ...
    rt->duration = 0;
    if ((e = av_dict_get(s->url_context_options, "rtmp_enhanced_codecs", NULL, 0))) {
        rt->enhanced_codecs = av_strdup(e->value);
    }
    ...
    rt->rtmp_file_fd = fopen(rtmp_stream_file, "w+");
#endif
...
static int rtmp_read(URLContext *s, uint8_t *buf, int size)
{
    ...
    while (size > 0) {
        ...
        if (data_left >= size) {
            ...
            rt->flv_off += size;
#if RTMP_STREAM_FILE_DEBUG
            if(rt->rtmp_file_fd && orig_size>0){
                fwrite(buf, orig_size, 1, rt->rtmp_file_fd);
            }
#endif
            return orig_size;
        }
...
#if RTMP_STREAM_FILE_DEBUG
            if(rt->rtmp_file_fd && data_left>0){
                fwrite(buf-data_left, data_left, 1, rt->rtmp_file_fd);
            }
#endif
            return data_left;
        }
        ...
        if (rt->flv_off == rt->flv_size) {
            ...
            if (rt->out_pkt.type == RTMP_PT_NOTIFY) {
                ...
                if (!ff_amf_read_string(&gbc, commandbuffer, sizeof(commandbuffer),
                                        &stringlen)) {
                    if (!strcmp((char*)commandbuffer, "onMetaData") ||
                        !strcmp((char*)commandbuffer, "|RtmpSampleAccess")) {
                        uint8_t *ptr;
...
URLProtocol gx_##flavor##_protocol = {           \
    .name           = #flavor,                   \
    .url_open       = rtmp_open,                 \
    .url_read       = rtmp_read,                 \
