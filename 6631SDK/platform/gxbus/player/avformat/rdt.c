...
void
ff_rdt_calc_response_and_checksum(char *response, int response_size, char *chksum,
...
#define XOR_TABLE_SIZE 37
    ...
    /* add tail */
    av_strlcpy (response + 32, "01d0a8e3", (response_size-31));
...
rdt_load_mdpr (PayloadContext *rdt, AVStream *st, int rule_nr)
{
    ...
    if (!rdt->mlti_data)
        return -1;
    ffio_init_context(&pb, (unsigned char*)rdt->mlti_data, rdt->mlti_data_size, 0,
                  NULL, NULL, NULL, NULL);
    tag = avio_rl32(&pb);
    ...
    if (ff_rm_read_mdpr_codecdata(rdt->rmctx, &pb, st, rdt->rmst[st->index], size, NULL) < 0)
       return -1;
#else
	gxlogd("Warning : not RMVB decoder\n");
#endif
...
#else
		res = -1;
		gxlogd("Warning : not RMVB decoder %d\n", seq);
#endif
        pos = avio_tell(&pb);
        if (res < 0)
            return res;
...
extern int ff_copy_whiteblacklists(AVFormatContext *dst, const AVFormatContext *src);
static int rdt_init(AVFormatContext *s, int st_index, PayloadContext *rdt)
{
    int ret;
