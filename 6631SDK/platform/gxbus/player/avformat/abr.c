...
static float abr_get_avg_tpt(ABRContext* ac) {
    float avg_tpt = 0;
    int i = 0, tpt_size = 0;
    for (i = 0; i < ac->max_history_len; ++i) {
...
static int abr_get_stream_simplethroughput(ABRContext* ac, int* bandwidth, int size) {
    int best_idx = -1, best_bw = -1, least_idx = -1, least_bw = INT_MAX, cur_bw, i = 0;
...
#if 0
    int rate_min = INT_MAX, rate_max = 0, rate_min_idx, rate_max_idx, best_bw, best_idx = -1, rc, i;
    for (i = 0; i < size; ++i) {
...
void abr_add_metric(ABRContext* ac, float tpt, int buffer_max, int buffer_r, int buffer_c) {
    ...
    if (!ac->throughput_history) {
        ac->throughput_history = (float*)av_mallocz((sizeof(float) * ac->max_history_len));
        for (i = 0; i < ac->max_history_len; ++i) {
    ...
    for (i = ac->max_history_len - 1; i >= 0; --i) {
        if (ac->throughput_history[i] != -1) {
            last_empty_pos = i + 1;
            break;
        }
    ...
    if (last_empty_pos != ac->max_history_len) {
        ...
        for (i = 0; i < ac->max_history_len - 1; ++i) {
...
int abr_get_stream(ABRContext* ac, int* bandwidth, int size) {
    switch (ac->algorithm) {
        case AlwaysFirst:
            return abr_get_stream_alwaysfirst(ac, bandwidth, size);
        case SimpleThroughput:
            return abr_get_stream_simplethroughput(ac, bandwidth, size);
        case BBA_0:
            return abr_get_stream_bba(ac, bandwidth, size);
        default:
            return 0;
    }
}
