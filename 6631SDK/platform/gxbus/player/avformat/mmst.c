...
static int mms_put_utf16(MMSContext *mms, const uint8_t *src)
{
    ...
    len = avio_put_str16le(&bic, (char *)src);
    if (len < 0)
        return len;
...
static int send_protocol_select(MMSTContext *mmst)
{
    ...
    if ((ret = mms_put_utf16(mms, (uint8_t *)data_string)) < 0)
        return ret;
    return send_command_packet(mmst);
}
...
static int send_media_file_request(MMSTContext *mmst)
{
    ...
    bytestream_put_le32(&mms->write_out_ptr, 0);
    bytestream_put_le32(&mms->write_out_ptr, 0);
    if ((ret = mms_put_utf16(mms, (uint8_t *)(mmst->path + 1))) < 0) // +1 for skip "/"
        return ret;
    ...
    for(;;) {
        ...
        if(AV_RL32(mms->in_buffer + 4)==0xb00bface) {
            ...
            if (length_remaining < 0
                ...
                gxlogd ("Data length %d is invalid or too large (max=%"SIZE_SPECIFIER")\n",
            ...
            if(read_result != length_remaining) {
                gxlogd ("Failed to read packet data of size %d: %d (%s)\n",
                       length_remaining, read_result,
                       read_result < 0 ? strerror(AVUNERROR(read_result)) :
                           "The server closed the connection");
                return read_result < 0 ? read_result : AVERROR(EIO);
            ...
            if(packet_id_type == mmst->header_packet_id) {
                ...
                gxlogd ("packet id type %d is old.", packet_id_type);
                continue;
            }
        }
                              ...
                              const MMSSCPacketType expect_type)
{
    ...
    if ((type = get_tcp_server_response(mmst)) != expect_type) {
        gxlogd ("Corrupt stream (unexpected packet type 0x%x, expected 0x%x)\n",
               type, expect_type);
        return AVERROR_INVALIDDATA;
    } else {
        return 0;
...
static int mms_open(URLContext *h, const char *uri, int flags)
{
    ...
    err = url_open(&mms->mms_hd, tcpname, URL_RDWR, &h->url_context_options);
    if (err)
        goto fail;
    ...
    if((mmst->incoming_flags != 0X08) && (mmst->incoming_flags != 0X0C)) {
        gxloge( "The server does not support MMST (try MMSH or RTSP)\n");
        err = AVERROR(EINVAL);
        goto fail;
    }
    err = ff_mms_asf_header_parser(mms);
    if (err) {
        gxloge ("asf header parsed failed!\n");
        goto fail;
    }
    mms->header_parsed = 1;
    ...
    gxlogd ("Leaving open (success)\n");
    return 0;
fail:
    mms_close(h);
    gxlogd("Leaving open (failure: %d)\n", err);
    return err;
}
...
static int mms_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    do {
        if(mms->asf_header_read_size < mms->asf_header_size) {
            ...
            if (err == 0) {
                if(mms->remaining_in_len>mms->asf_packet_len) {
                    gxlogd ("Incoming pktlen %d is larger than ASF pktsize %d\n",
                    ...
                    result = ff_mms_read_data(mms, buf, size);
                    if (result == 0) {
                        gxlogd ("Read ASF media packet size is zero!\n");
                        break;
                    }
                }
            } else {
                gxlogd ("read packet error!\n");
                break;
            }
        }
