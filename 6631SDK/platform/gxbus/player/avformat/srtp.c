...
int ff_srtp_decrypt(struct SRTPContext *s, uint8_t *buf, int *lenptr)
{
...
#else /*use openssl hmac function*/
    HMAC_Init(s->hmac, rtcp ? s->rtcp_auth : s->rtp_auth, sizeof(s->rtp_auth), EVP_sha1());
    HMAC_Update(s->hmac, buf, len - hmac_size);
#endif
    if (!rtcp) {
    ...
    HMAC_Final(s->hmac, hmac, &hmac_len);
#endif
...
#else/*ssl hmac*/
    HMAC_Init(s->hmac, rtcp ? s->rtcp_auth : s->rtp_auth, sizeof(s->rtp_auth), EVP_sha1());
    HMAC_Update(s->hmac, out, buf + len - out);
#endif
    if (!rtcp) {
        uint8_t rocbuf[4];
