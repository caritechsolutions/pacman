...
static RTPDynamicProtocolHandler *rtp_dynamic_protocol_handler_list[] = {
    /* rtp */
...
RTPDynamicProtocolHandler *ff_rtp_handler_iterate(void **opaque)
{
    uintptr_t i = (uintptr_t)*opaque;
    RTPDynamicProtocolHandler *r = rtp_dynamic_protocol_handler_list[i];
                             ...
                             int len)
{
    ...
    while (len >= 4) {
        ...
        case RTCP_SR:
            ...
            s->last_rtcp_reception_time = gx_av_gettime_relative();
                                  ...
                                  AVIOContext *avio, int count)
{
    ...
    if (s->last_rtcp_ntp_time == AV_NOPTS_VALUE) {
        ...
        uint32_t delay_since_last = av_rescale(gx_av_gettime_relative() - s->last_rtcp_reception_time,
                                               65536, AV_TIME_BASE);
    ...
    if ((len > 0) && buf) {
        ...
        gxlogd ("sending %d bytes of RR\n", len);
        result = url_write(fd, buf, len);
        gxlogd ("result from ffurl_write: %d\n", result);
        av_free(buf);
    }
    return 0;
}
                                   ...
                                   int payload_type, int queue_size)
{
    ...
    gxlogd ("setting jitter buffer size to %d\n", s->queue_size);
                                     ...
                                     const uint8_t *buf, int len)
{
    ...
    if (!rtp_valid_packet_in_sequence(&s->statistics, seq)) {
        gxlogd ("RTP: PT=%02x: bad cseq %04x expected=%04x\n", payload_type, seq, ((s->seq + 1) & 0xffff));
        //s->seq = seq;
        return -1;
    }
...
static int enqueue_packet(RTPDemuxContext *s, uint8_t *buf, int len)
{
    ...
    RTPPacket **cur = &s->queue, *packet = NULL;
    ...
    if (!packet) {
		gxloge ("rtpdec.c file. malloc fail fail.\n");
        return AVERROR(ENOMEM);
    }
    memset (packet, 0, sizeof(*packet));
    packet->recvtime = gx_av_gettime_relative();
    packet->seq      = seq;
    packet->len      = len;
    packet->buf      = buf;
    packet->next     = *cur;
...
static int rtp_parse_queued_packet(RTPDemuxContext *s, AVPacket *pkt)
{
    ...
    if (!has_next_packet(s)) {
        gxlogd ("missed %d packets..[b_seq:0x%x,cur_seq:0x%x]\n", s->queue->seq - s->seq - 1, s->queue->seq, s->seq);
    }
                                ...
                                uint8_t **bufptr, int len)
{
	...
	if (debug_rtp_packt) {
		RTPStatistics *ss = &s->statistics;
		uint16_t tmp_seq = seq & 0x3ff;

		if (ss->debug_last_seq == -1)
			ss->debug_last_seq = tmp_seq;
		if (tmp_seq < ss->debug_last_seq) {
			gxlogi("---> %p: %d %d %d, %.2f\n",
					ss, ss->debug_last_seq, tmp_seq,
					ss->debug_packet_num, (ss->debug_packet_num *100.0)/(1<<10));
			ss->debug_packet_num = 0;
		}
		ss->debug_last_seq = tmp_seq;
		ss->debug_packet_num++;
	}
    ...
    if (s->st) {
        int64_t received = gx_av_gettime_relative();
        uint32_t arrival_ts = av_rescale_q(received, AV_TIME_BASE_Q, s->st->time_base);
        timestamp = AV_RB32(buf + 4);
    ...
    if ((s->seq == 0 && !s->queue) || s->queue_size <= 1) {
        ...
        if ((lost_packet < 0) || (lost_packet > 1))
            gxlogd ("rtpdec.c.cur_seq:%d..pre_seq:%d..D-value:%d.\n", seq, s->seq, lost_packet);
        if ((s->seq == 0 && !s->queue) || (lost_packet > 0) || (abs(lost_packet)>32768))
        ...
        if (diff <= 0) {
            ...
            gxlogd ("rtpdec.c Dropping old packet received too late.cur_seq:%d..pre_seq:%d..D-value:%d.\n", seq, s->seq, diff);
            rv = -1;
        } else if (diff <= 1) {
            /* Correct packet */
            rv = rtp_parse_packet_internal(s, pkt, buf, len);
            ...
            gxlogd ("rtpdec.c Still missing some packet.cur_seq:%d..pre_seq:%d..D-value:%d.queue_len=%d..\n", seq, s->seq, diff, s->queue_len);
                                    ...
                                    const char *attr, const char *value))
{
    ...
    if (!(value = av_malloc(value_size))) {
        gxloge ("not mem.malloc fail fail\n");
        return AVERROR(ENOMEM);
    }
