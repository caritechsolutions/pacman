...
static void get_tag(AVFormatContext *s, const char *key, int type, int len)
{
    ...
    if (type == 0) {         // UTF16-LE
        ...
        snprintf(value, len, "%lld", num);
    } else {
        av_log(s, AV_LOG_DEBUG, "Unsupported value type %d in tag %s.\n", type, key);
        goto finish;
    }
    if (*value)
        av_dict_set((AVDictionary**)&s->metadata, key, value, 0);
finish:
    av_freep(&value);
...
static int asf_read_stream_properties(AVFormatContext *s, int64_t size)
{
    ...
    if (type == AVMEDIA_TYPE_AUDIO) {
        ...
        //gxlogd("Descrambling: ps:%d cs:%d ds:%d s:%d  sd:%d\n",
        ...
        if (st->codec->extradata_size && (st->codec->bits_per_coded_sample <= 8)) {
            ...
            for (i = 0; i < FFMIN(st->codec->extradata_size, AVPALETTE_SIZE)/4; i++)
                asf_st->palette[i] = bswap_32(((uint32_t*)st->codec->extradata)[i]);
#else
...
static int asf_read_marker(AVFormatContext *s, int64_t size)
{
    ...
    for(i=0;i<count;i++){
        ...
        if ((ret = avio_get_str16le(pb, name_len * 2, name, sizeof(name))) < name_len)
            avio_skip(pb, name_len - ret);
        new_chapter(s, i, (AVRational){1, 10000000}, pres_time, AV_NOPTS_VALUE, name );
    }
...
static int asf_read_header(AVFormatContext *s,  AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        ff_get_guid(pb, &g);
        gsize = avio_rl64(pb);
        av_dlog(s, "%08lld: ", gpos);
        print_guid(&g);
        av_dlog(s, "  size=0x%lld\n", gsize);
        ...
        if (!ff_guidcmp(&g, &ff_asf_file_header)) {
            ...
            if (!s->keylen) {
                if (!ff_guidcmp(&g, &ff_asf_content_encryption)) {
                    ...
                    av_dict_set((AVDictionary**)&s->metadata, "encryption", "ASF Extended Content Encryption", 0);
        ...
        if(avio_tell(pb) != gpos + gsize)
            av_log(s, AV_LOG_DEBUG, "gpos mismatch our pos=%lld, end=%lld\n", avio_tell(pb)-gpos, gsize);
    ...
    for(i=0; i<128; i++){
        ...
        if(stream_num>=0){
            ...
            } else if ((asf->dar[0].num > 0) && (asf->dar[0].den > 0) && (st->codec->codec_type==CODEC_TYPE_VIDEO)) // Use ASF container value if the stream doesn't AR set.
                av_reduce(&st->sample_aspect_ratio.num,
                          &st->sample_aspect_ratio.den,
...
//                    if (iso6392)
//                        av_dict_set((AVDictionary**)&st->metadata, "language", iso6392, 0);
	                  gxlogd("language %s\n", primary_tag);
                }
            }
        }
...
static int ff_asf_get_packet(AVFormatContext *s, AVIOContext *pb)
{
    ...
    if (rsize > packet_length - padsize) {
        ...
        av_log(s, AV_LOG_ERROR,
               "invalid packet header length %d for pktlen %d-%d at %lld\n",
...
//gxlogd("key:%d stream:%d seq:%d offset:%d replic_size:%d\n", asf->packet_key_frame, asf->stream_index, asf->packet_seq, //asf->packet_frag_offset, asf->packet_replic_size);
...
//            av_log(s, AV_LOG_DEBUG, "\n");
    ...
    asf->packet_size_left -= rsize;
    //gxlogd("___objsize____  %d   %d    rs:%d\n", asf->packet_obj_size, asf->packet_frag_offset, rsize);
...
static int ff_asf_parse_packet(AVFormatContext *s, AVIOContext *pb, AVPacket *pkt)
{
    ...
    for (;;) {
        ...
        if (asf->packet_size_left < FRAME_HEADER_SIZE
            ...
            //gxlogd("PacketLeftSize:%d  Pad:%d Pos:%lld\n", asf->packet_size_left, asf->packet_padsize, avio_tell(pb));
            assert(ret>=0);
            /* fail safe */
            avio_skip(pb, ret);
        ...
        if (asf->packet_replic_size == 1) {
            ...
            //gxlogd("COMPRESS size  %d  %d  %d   ms:%d\n", asf->packet_obj_size, asf->packet_frag_timestamp, asf->packet_size_left, asf->packet_multi_size);
        }
...
//gxlogd("new packet: stream:%d key:%d packet_key:%d audio:%d size:%d\n",
...
//s->streams[asf->stream_index]->codec->codec_type == AVMEDIA_TYPE_AUDIO, asf->packet_obj_size);
        ...
        /* read data */
        //gxlogd("READ PACKET s:%d  os:%d  o:%d,%d  l:%d   DATA:%p\n",
              ...
              if(asf_st->pkt.size != asf_st->ds_packet_size * asf_st->ds_span){
                ...
                if (newdata) {
                    ...
                    while (offset < asf_st->pkt.size) {
                        ...
                        //gxlogd("off:%d  row:%d  col:%d  idx:%d\n", off, row, col, idx);
...
//            assert((asf_st->packet_pos - s->data_offset) % s->packet_size == 0);
    ...
    *ppos= pos;
//gxlogd("found keyframe at %lld stream %d stamp:%lld\n", *ppos, stream_index, pts);
...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
...

    {
        ...
        pct=avio_rl32(s->pb);
        ict=avio_rl32(s->pb);
        av_log(s, AV_LOG_DEBUG, "itime:0x%lld, pct:%d, ict:%d\n",itime,pct,ict);

        for (i=0;i<ict;i++){
            int pktnum, pktct;
            int64_t pos, index_pts;

            pktnum=avio_rl32(s->pb);
            pktct =avio_rl16(s->pb);
            pos      = s->data_offset + s->packet_size*(int64_t)pktnum;
            index_pts= FFMAX(av_rescale(itime, i, 10000) - asf->hdr.preroll, 0);
            if(pos != last_pos){
...
static int asf_read_seek(AVFormatContext *s, int stream_index, int64_t pts, int flags)
{
    ...
    if(s->pb) {
        int ret = url_seek_time(s->pb, stream_index, pts, flags);
        if(ret >= 0)
            asf_reset_header(s);
