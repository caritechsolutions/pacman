...
{
    uint8_t *dst;
    int encoding, dict_flags = 0;//AV_DICT_DONT_OVERWRITE | AV_DICT_DONT_STRDUP_VAL;
    unsigned genre;
    ...
    if (dst) {
        av_dict_set(metadata, key, (char*)dst, dict_flags);
        av_freep(&dst);
    }
}
                      ...
                      AVDictionary **metadata)
{
    ...
    if (decode_str(s, pb, encoding, (uint8_t**)(&text), &taglen) < 0)
        goto error;
                      ...
                      AVDictionary **metadata)
{
    ...
    if (decode_str(s, pb, encoding, (uint8_t**)(&dst), &taglen) < 0) {
...
static void read_chapter(AVFormatContext *s, AVIOContext *pb, int len, const char *ttag, ID3v2ExtraMeta **extra_meta, int isv34)
{
    ...
    metadata_conv(&chap->meta, NULL, ff_id3v2_34_metadata_conv);
    metadata_conv(&chap->meta, NULL, ff_id3v2_4_metadata_conv);
    ...
    while (len >= taghdrlen) {
        ...
        /* check for text tag or supported special meta tag */
            ...
            if (unsync || tunsync || tcomp) {
                av_fast_malloc(&buffer, &buffer_size, tlen, 0);
                if (!buffer) {
                    av_log(s, AV_LOG_ERROR, "Failed to alloc %d bytes\n", tlen);
                    goto seek;
                }
                                ...
                                ID3v2ExtraMeta **extra_meta, int64_t max_search_size)
{
    ...
    metadata_conv(metadata, NULL, ff_id3v2_34_metadata_conv);
    metadata_conv(metadata, NULL, id3v2_2_metadata_conv);
    metadata_conv(metadata, NULL, ff_id3v2_4_metadata_conv);
    merge_date(metadata);
}
...
int ff_id3v2_parse_apic(AVFormatContext *s, ID3v2ExtraMeta **extra_meta)
{
    ...
    for (cur = *extra_meta; cur; cur = cur->next) {
        ...
        if (apic->description[0])
            av_dict_set(&st->metadata, "title", (char *)apic->description, 0);
...
int ff_id3v2_parse_chapters(AVFormatContext *s, ID3v2ExtraMeta **extra_meta)
{
    ...
    for (i = 0; i < num_chapters; i++) {
        ...
        chap = chapters[i];
        chapter = avpriv_new_chapter(s, i, time_base, chap->start, chap->end, (char *)chap->element_id);
        if (!chapter)
            continue;
