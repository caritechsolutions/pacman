...
int ff_rtp_get_codec_info(AVCodecContext *par, int payload_type)
{
    int i = 0;
...
int ff_rtp_get_codec_type(int *pcodec_type, int payload_type)
{
    int i = 0;
    for (i = 0; rtp_payload_types[i].pt >= 0; i++)
        if (rtp_payload_types[i].pt == payload_type) {
            if (rtp_payload_types[i].codec_id != AV_CODEC_ID_NONE) {
                *pcodec_type = rtp_payload_types[i].codec_type;
                return 0;
            }
        }
    return -1;
...
int ff_rtp_get_payload_type(AVFormatContext *fmt, AVCodecContext *par, int idx)
{
    int i;
