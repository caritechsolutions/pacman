...
#define RECVBUF_SIZE (1 * RTP_MAX_PACKET_LENGTH*3/4)
...
extern int ffurl_connect(URLContext *uc, AVDictionary **options);
...
static int queue_init(RTSPStream *st)
{
    if (!st->rtp_queue_inited) {
        unsigned short queue_max = (32 * (st->is_video ? 24 : (st->codec_type == AVMEDIA_TYPE_DATA?24:1)));
        st->rtp_queue       = av_mallocz(queue_max * sizeof(RTSPStreamQueue));
		if (!st->rtp_queue) {
			gxlogd ("rtsp.c..malloc fail..%d.%d..\n", queue_max, sizeof(RTSPStreamQueue));
			return -1;
		}
		st->rtp_queue_count = 0;
		st->rtp_queue_rd    = 0;
		st->rtp_queue_max   = queue_max;
		st->rtp_queue_inited = 1;
	}

	return 0;
}

static int queue_unint(RTSPStream *st)
{
	if (st->rtp_queue_inited) {
		av_free(st->rtp_queue);
		st->rtp_queue        = NULL;
		st->rtp_queue_rd     = 0;
		st->rtp_queue_count  = 0;
		st->rtp_queue_max    = 0;
		st->rtp_queue_inited = 0;
	}

	return 0;
}

static int queue_read(RTSPStream *st, unsigned char *buf, unsigned int buf_size)
{
	unsigned int   offset = 0;
	unsigned int   length = 0;
	unsigned char *buffer = NULL;

	if (st->rtp_queue_count == 0)
		return 0;

	offset = st->rtp_queue_rd;
	buffer = st->rtp_queue[offset].buf;
	length = st->rtp_queue[offset].len - st->rtp_queue[offset].rd;
	length = (buf_size > length) ? length : buf_size;
	memcpy(buf, buffer + st->rtp_queue[offset].rd, length);
	st->rtp_queue[offset].rd += length;
	if (st->rtp_queue[offset].rd >= st->rtp_queue[offset].len) {
		st->rtp_queue[offset].len = 0;
		st->rtp_queue[offset].rd  = 0;
		st->rtp_queue_count--;
		st->rtp_queue_rd++;
		if (st->rtp_queue_rd >= st->rtp_queue_max)
			st->rtp_queue_rd = 0;
	}
...
static void queue_get_buf(RTSPStream *st, unsigned char **buf, unsigned int *buf_size)
{
	unsigned short offset = 0;

	if (st->rtp_queue_count >= st->rtp_queue_max) {
		*buf      = NULL;
		*buf_size = 0;
		return;
	}

	offset = (st->rtp_queue_rd + st->rtp_queue_count) % st->rtp_queue_max;
	*buf      = st->rtp_queue[offset].buf;
	*buf_size = MAX_QUEUE_SIZE;

	return;
}

static void queue_add_buf(RTSPStream *st, unsigned int size)
{
	unsigned short offset = 0;

	offset = (st->rtp_queue_rd + st->rtp_queue_count) % st->rtp_queue_max;
	st->rtp_queue[offset].rd  = 0;
	st->rtp_queue[offset].len = size;
	st->rtp_queue_count++;
	if (st->rtp_queue_count > st->rtp_queue_max) {
		gxloge("rtp queue up overflow..\n");
		st->rtp_queue_count = st->rtp_queue_max;
	}

	return;
}

static int queue_is_empty(RTSPState *rt)
{
	int i = 0;
...
static int queue_printf_count(RTSPState *rt, int line)
{
	return 0;
	int i = 0;
	...
	for (i = 0; i < rt->nb_rtsp_streams; i++) {
		...
		gxlogi("[%d] %s - %d\n", line, rtsp_st->is_video ? "v" : "a", rtsp_st->rtp_queue_count);
	}
...
{
    AVCodecContext *par = st ? st->codec : NULL;
    if (!handler)
        return;
    if (par)
...
#if 0
extern int ff_wms_parse_sdp_a_line(AVFormatContext *s, const char *p);
#endif
                           ...
                           int letter, const char *buf)
{
    ...
    case 'm':
        ...
        if (codec_type == AVMEDIA_TYPE_UNKNOWN /*|| !(rt->media_type_mask & (1 << codec_type)) || rt->nb_rtsp_streams >= s->max_streams*/) {
            s1->skip_media = 1;
            return;
        }
        ...
        if (!rtsp_st)
            return;
		rtsp_st->is_video = (codec_type == AVMEDIA_TYPE_VIDEO) ? 1 : 0;
        rtsp_st->codec_type = codec_type;
        rtsp_st->stream_index = -1;
        dynarray_add(&rt->rtsp_streams, &rt->nb_rtsp_streams, rtsp_st);
...
{
#define SDP_INFO_MAX_LENGTH 16384
    const char *p = NULL;
    int letter = 0, i = 0;
    ...
    if (!(buf = av_malloc(sizeof(char)*SDP_INFO_MAX_LENGTH))) {
        return AVERROR(ENOMEM);
    }
    ...
    for (;;) {
        ...
        q = buf;
        while (*p != '\n' && *p != '\r' && *p != '\0') {
            if ((q - buf) < (SDP_INFO_MAX_LENGTH*sizeof(char)) - 1)
                *q++ = *p;
            p++;
        }
        *q = '\0';
...
int ff_rtsp_open_transport_ctx(AVFormatContext *s, RTSPStream *rtsp_st)
{
    ...
    if ((e = av_dict_get(s->url_options, "rtp_queue_size", NULL, 0))) {
        reordering_queue_size = atoi(e->value);
    }
                        ...
                        RTSPState *rt, const char *method)
{
    ...
    if (av_stristart(p, "Session:", &p)) {
        ...
        if (method && !strcmp(method, "DESCRIBE")) {
            if (0 != strncmp(rt->control_uri, p, strlen(p)-1)) {
...
typedef struct {
    char buf[3072];
    char buf1[1024];
    char base64buf[AV_BASE64_SIZE(1024)];
}rtsp_read_reply_param;
                       ...
                       int return_on_interleaved_data, const char *method)
{
    ...
    rtsp_read_reply_param *reply_param = av_mallocz(sizeof(rtsp_read_reply_param));
    if (!reply_param) {
...
start:
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (ch == '$' && q == reply_param->buf) {
                ...
                if ((q - reply_param->buf) < sizeof(reply_param->buf) - 1)
                    *q++ = ch;
            }
        }
        *q = '\0';

        {
            ...
            if (debug_network_stream)
                gxlogi_raw_l("rtsp.c line=%s\n", reply_param->buf);
        }

        /* test if last line */
        if (reply_param->buf[0] == '\0')
            break;
        p = reply_param->buf;
        if (line_count == 0) {
            /* get reply code */
            get_word(reply_param->buf1, sizeof(reply_param->buf1), &p);
            if (!strncmp(reply_param->buf1, "RTSP/", 5)) {
                get_word(reply_param->buf1, sizeof(reply_param->buf1), &p);
                reply->status_code = atoi(reply_param->buf1);
                av_strlcpy(reply->reason, p, sizeof(reply->reason));
            } else {
                av_strlcpy(reply->reason, reply_param->buf1, sizeof(reply->reason)); // method
                get_word(reply_param->buf1, sizeof(reply_param->buf1), &p); // object
                request = 1;
            }
        } else {
    ...
    if (content_length > 0) {
        ...
        url_read_complete(rt->rtsp_hd, content, content_length);
    ...
    if (request) {
        memset (reply_param->buf1, 0, sizeof(reply_param->buf1));
        const char* ptr = reply_param->buf1;

        if (!strcmp(reply->reason, "OPTIONS")) {
            snprintf(reply_param->buf, sizeof(reply_param->buf1), "RTSP/1.0 200 OK\r\n");
            if (reply->seq)
                av_strlcatf(reply_param->buf, sizeof(reply_param->buf1), "CSeq: %d\r\n", reply->seq);
            if (reply->session_id[0])
                av_strlcatf(reply_param->buf, sizeof(reply_param->buf1), "Session: %s\r\n",
                                              reply->session_id);
        } else {
            snprintf(reply_param->buf1, sizeof(reply_param->buf1), "RTSP/1.0 501 Not Implemented\r\n");
        }
        av_strlcat(reply_param->buf1, "\r\n", sizeof(reply_param->buf1));

        if (rt->control_transport == RTSP_MODE_TUNNEL) {
            av_base64_encode(reply_param->base64buf, sizeof(reply_param->base64buf), (const uint8_t *)(reply_param->buf1), strlen(reply_param->buf1));
            ptr = reply_param->base64buf;
        }
        url_write(rt->rtsp_hd_out, (unsigned char *)ptr, strlen(ptr));

        rt->last_cmd_time = gx_av_gettime_relative();
    ...
    if (rt->seq != reply->seq) {
        gxlogd ("CSeq %d expected, %d received.\n", rt->seq, reply->seq);
    }
...
typedef struct {
     char buf[RTSP_MAX_URL_SIZE];
     char base64buf[AV_BASE64_SIZE(RTSP_MAX_URL_SIZE)];
     char *out_buf;
}rtsp_content_param;
...
{
    RTSPState *rt = s->priv_data;
    rtsp_content_param *url_param = av_mallocz(sizeof(rtsp_content_param));
    AVDictionaryEntry *e = NULL;
    if (!url_param)
        return AVERROR(ENOMEM);
    ...
    /* Add in RTSP headers */
    url_param->out_buf = url_param->buf;
    rt->seq++;
    snprintf(url_param->buf, sizeof(url_param->buf), "%s %s RTSP/1.0\r\n", method, url);
    if (headers)
        av_strlcat(url_param->buf, headers, sizeof(url_param->buf));
    av_strlcatf(url_param->buf, sizeof(url_param->buf), "CSeq: %d\r\n", rt->seq);
    e = av_dict_get(s->url_options, "user_agent", NULL, 0);
    if (e) {
        if (rt->user_agent)
            av_free(rt->user_agent);
        rt->user_agent = av_strdup(e->value);
    }
    if (!rt->user_agent)
        rt->user_agent = av_strdup(LIBAVFORMAT_IDENT);
    av_strlcatf(url_param->buf, sizeof(url_param->buf), "User-Agent: %s\r\n",  rt->user_agent);
    if (rt->session_id[0] != '\0' && (!headers ||
        !strstr(headers, "\nIf-Match:"))) {
        av_strlcatf(url_param->buf, sizeof(url_param->buf), "Session: %s\r\n", rt->session_id);
    }
    if (rt->auth[0]) {
        char *str = ff_http_auth_create_response(&rt->auth_state,
                                                 rt->auth, url, method);
        if (str)
            av_strlcat(url_param->buf, str, sizeof(url_param->buf));
        av_free(str);
    }
    if (send_content_length > 0 && send_content)
        av_strlcatf(url_param->buf, sizeof(url_param->buf), "Content-Length: %d\r\n", send_content_length);
    av_strlcat(url_param->buf, "\r\n", sizeof(url_param->buf));

    /* base64 encode rtsp if tunneling */
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        av_base64_encode(url_param->base64buf, sizeof(url_param->base64buf), (const uint8_t *)(url_param->buf), strlen(url_param->buf));
        url_param->out_buf = url_param->base64buf;
    }

    {
        ...
        GxPlayer_SystemGet(PSYS_DEBUG_NETWORK_STREAM, &debug_network_stream);
        if (debug_network_stream)
            gxlogi_raw_l("rtsp.c Sending:\n%s\n", url_param->buf);
    }

    url_write(rt->rtsp_hd_out, (unsigned char *)(url_param->out_buf), strlen(url_param->out_buf));
    if (send_content_length > 0 && send_content) {
        if (rt->control_transport == RTSP_MODE_TUNNEL) {
...
int ff_rtsp_send_cmd(AVFormatContext *s, const char *method, const char *url,
                     ...
                     unsigned char **content_ptr)
{
    return ff_rtsp_send_cmd_with_content(s, method, url, headers, reply, content_ptr, NULL, 0);
}
                              ...
                              int lower_transport, const char *real_challenge)
{
    ...
    port_off = gx_av_get_random_seed() % ((rt->rtp_port_max - rt->rtp_port_min)/2);
    /* even random offset */
    ...
    for (j = rt->rtp_port_min + port_off, i = 0; i < rt->nb_rtsp_streams; ++i) {
        ...
        if (lower_transport == RTSP_LOWER_TRANSPORT_UDP) {
            ...
            while (j <= rt->rtp_port_max) {
                ...
                if (rtsp_st->stream_index >= 0) {
                    ...
                    if (st && (AVMEDIA_TYPE_AUDIO == st->codec->codec_type)) {
                        av_dict_set(&opts, "stream_type", "audio_rtp", 0);
                    }
                }

                err = url_open(&rtsp_st->rtp_handle, buf, URL_RDWR, &opts);
                if (opts) {
                    av_dict_free(&opts);
                    opts = NULL;
                }
            ...
            gxlogd("Unable to open an input RTP port\n");
            err = AVERROR(EIO);
            goto fail;
        ...
        have_port:
            ...
            av_strlcat(transport, rt->server_type == RTSP_SERVER_SATIP ? ";" : "/UDP;", sizeof(transport));
            if (rt->server_type != RTSP_SERVER_REAL)
        ...
        if (CONFIG_RTPDEC && i == 0 && rt->server_type == RTSP_SERVER_REAL) {
            ...
            ff_rdt_calc_response_and_checksum(real_res, sizeof(real_res)-1, real_csum, real_challenge);
        ...
        case RTSP_LOWER_TRANSPORT_UDP_MULTICAST: {
            ...
            err = url_open(&rtsp_st->rtp_handle, url, URL_RDWR, NULL);
            if (err < 0) {
                err = AVERROR_INVALIDDATA;
                goto fail;
            }
...
void ff_rtsp_close_connections(AVFormatContext *s)
{
    ...
    if (rt->rtsp_hd_out != rt->rtsp_hd)
        ffurl_close(rt->rtsp_hd_out);
    ffurl_close(rt->rtsp_hd);
    rt->rtsp_hd = rt->rtsp_hd_out = NULL;
}

#define RTSP_OFFSET 1024
typedef struct {
     char proto[RTSP_OFFSET/8];
     char host[RTSP_OFFSET];
     char path[RTSP_OFFSET];
     char tcpname[RTSP_OFFSET];
     char cmd[2*RTSP_OFFSET];
     char auth[RTSP_OFFSET/8];
     char real_challenge[RTSP_OFFSET/16];
}rtsp_url_param;
...
int ff_rtsp_connect(AVFormatContext *s)
{
    ...
    int lower_transport = 0;
    int default_port = RTSP_DEFAULT_PORT;
    int https_tunnel = 0;
    struct sockaddr_storage peer;
    socklen_t peer_len = sizeof(peer);
    AVDictionaryEntry *e = NULL;
    rtsp_url_param *url_param = av_mallocz(sizeof(rtsp_url_param));
    if (!url_param) {
        return AVERROR(ENOENT);
    }
    ...
    if ((e = av_dict_get(s->url_options, "rtsp_transport", NULL, 0))) {
        if (0 == strcasecmp(e->value, "tcp"))
            rt->lower_transport_mask = 1 << RTSP_LOWER_TRANSPORT_TCP;
        else if (0 == strcasecmp(e->value, "udp"))
            rt->lower_transport_mask = 1 << RTSP_LOWER_TRANSPORT_UDP;
        else if (0 == strcasecmp(e->value, "udp_multicast"))
...
redirect:
    ...
    if (!strcmp(url_param->proto, "rtsps")) {
        ...
        rt->lower_transport_mask = 1 << RTSP_LOWER_TRANSPORT_TCP;
    } else if (!strcmp(url_param->proto, "satip")) {
        av_strlcpy(url_param->proto, "rtsp", sizeof(url_param->proto));
        rt->server_type = RTSP_SERVER_SATIP;
    }

    if (url_param->auth[0]) {
        av_strlcpy(rt->auth, url_param->auth, sizeof(rt->auth));
    }
    if (port < 0)
        port = default_port;
     ...
     * stripped out. */
    ff_url_join(rt->control_uri, sizeof(rt->control_uri), url_param->proto, NULL, url_param->host, port, "%s", url_param->path);
    ...
    if (rt->control_transport == RTSP_MODE_TUNNEL) {
        ...
        ff_url_join(httpname, sizeof(httpname), https_tunnel ? "https" : "http", url_param->auth, url_param->host, port, "%s", url_param->path);
        snprintf(sessioncookie, sizeof(sessioncookie), "%08x%08x", gx_av_get_random_seed(), gx_av_get_random_seed());

        /* GET requests */
        if (url_alloc(&rt->rtsp_hd, httpname, AVIO_FLAG_READ) < 0) {
            err = AVERROR(EIO);
            goto fail;
        }
        ...
        /* POST requests */
        if (url_alloc(&rt->rtsp_hd_out, httpname, AVIO_FLAG_WRITE) < 0 ) {
            err = AVERROR(EIO);
            goto fail;
        }
        ...
        ff_url_join(url_param->tcpname, sizeof(url_param->tcpname), lower_rtsp_proto, NULL, url_param->host, port, "?timeout=%d", rt->stimeout);
#ifndef LINUX_OS
        av_dict_set_int(&s->url_options, "ff_recvbuf_size", 32*1024, 0);
#endif
        ret = url_open(&rt->rtsp_hd, url_param->tcpname, URL_RDWR, &s->url_options);
        if (ret < 0) {
            err = ret;
            goto fail;
        }
    ...
    if (!getpeername(tcp_fd, (struct sockaddr*) &peer, &peer_len)) {
        getnameinfo((struct sockaddr*) &peer, peer_len, url_param->host, sizeof(url_param->host), NULL, 0, NI_NUMERICHOST);
    }
    ...
    for (rt->server_type = RTSP_SERVER_RTP;;) {
        ...
        if (rt->server_type == RTSP_SERVER_REAL)
            av_strlcat(url_param->cmd,
                       ...
                       sizeof(url_param->cmd));
        ff_rtsp_send_cmd(s, "OPTIONS", rt->control_uri, url_param->cmd, reply, NULL);
        ...
        if (rt->server_type != RTSP_SERVER_REAL && reply->real_challenge[0]) {
            ...
            snprintf (url_param->real_challenge, sizeof(url_param->real_challenge)-1,"%s", reply->real_challenge);
        }
        break;
    }
    ...
    do {
        lower_transport = log2_tab[lower_transport_mask & ~(lower_transport_mask - 1)];
        ...
        err = ff_rtsp_make_setup_request(s, url_param->host, port, lower_transport,
                                 rt->server_type == RTSP_SERVER_REAL ? url_param->real_challenge : NULL);
        if (err < 0)
            goto fail;
    ...
    rt->lower_transport_type = lower_transport;
    av_strlcpy(rt->real_challenge, url_param->real_challenge, sizeof(rt->real_challenge));
    rt->state = RTSP_STATE_IDLE;
 ...
 fail:
    ...
    if (reply->status_code >=300 && reply->status_code < 400 && s->iformat) {
        ...
        s->url = av_strdup(reply->location);
        if (!s->url) {
            err = AVERROR(ENOMEM);
            goto fail2;
        }
    ...
    if (rt->rtsp_flags & RTSP_FLAG_LISTEN) {
        if (rt->state == RTSP_STATE_STREAMING) {
            if (!ff_rtsp_parse_streaming_commands(s))
                ...
                gxloge ("Unable to answer to TEARDOWN\n");
        } else
            return 0;
    } else {
        RTSPMessageHeader reply;
...
//#define DEBUG_SOCKRT_BUF
#ifdef DEBUG_SOCKRT_BUF
...
udp_complete:
	if (rt->is_udp_cache) {
		for (i = 0; i < rt->nb_rtsp_streams; i++) {
			rtsp_st = rt->rtsp_streams[i];
			if (0 > queue_init(rtsp_st)) {
				return AVERROR(ENOMEM);
			}
			ret = queue_read(rtsp_st, buf, buf_size);
			if (ret > 0) {
				*prtsp_st = rtsp_st;
				return ret;
			}
	...
	if (!p) {
		...
		if (!p) {
			gxloge ("rtsp.c file. malloc fail fail fail fail.\n");
			return AVERROR(ENOMEM);
		}

		if (rt->rtsp_hd) {
			p[rt->max_p].fd = url_get_file_handle(rt->rtsp_hd);
		...
		for (i = 0; i < rt->nb_rtsp_streams; i++) {
			rtsp_st = rt->rtsp_streams[i];
			if (rtsp_st->rtp_handle) {
				if ((ret = ffurl_get_multi_file_handle(rtsp_st->rtp_handle, &fds, &fdsnum))) {
					return ret;
				}
				if (fdsnum != 2) {
	...
	for (;;) {
		if (wait_end && (wait_end - gx_av_gettime_relative() < 0)) {
			if (rt->is_udp_cache) {
				if (!queue_is_empty(rt)) {
					queue_printf_count(rt, __LINE__);
					goto udp_complete;
				}
			}
		...
		if (n > 0) {
			...
			for (i = 0; i < rt->nb_rtsp_streams; i++) {
				...
				if (rtsp_st->rtp_handle) {
					...
					unsigned int   tmp_size = buf_size;

					if (rt->is_udp_cache) {
						queue_get_buf(rtsp_st, &tmp_buf, &tmp_size);
						if (!tmp_buf) {
							queue_printf_count(rt, __LINE__);
							goto udp_complete;
						}
					...
					if ((p[j].revents & POLLIN) || (p[j+1].revents & POLLIN)) {
						p[j].revents = 0;
						p[j+1].revents = 0;
						ret = url_read(rtsp_st->rtp_handle, tmp_buf, tmp_size);
						if (ret > 0) {
#ifdef DEBUG_SOCKRT_BUF
							static int rtsp_count = 0;

							if (rtsp_count > 512) {
								int k = 0;
								gxlogi("-----------------------------\n");
								gxlogi("%d, %s, %d\n", p[j].fd, (rtsp_st->is_video) ? "video" : "audio", ret);
								for (k = 0; k < rt->max_p; k++) {
									cyg_file *fp = cyg_fp_get(p[k].fd);
									register struct socket *so = (struct socket *)fp->f_data;

									gxlogi("%d -- %lu \t%lu\r\n",
											p[k].fd, so->so_rcv.sb_cc, so->so_rcv.sb_max_cc);
									cyg_fp_free(fp);
								}
								rtsp_count = 0;
							} else
...
#endif
		} else if (rt->is_udp_cache && n == 0 && !queue_is_empty(rt)) {
			queue_printf_count(rt, __LINE__);
			goto udp_complete;
		} else if (n == 0 && ++timeout_cnt >= MAX_TIMEOUTS) {
			gxlogi("%s %d: recv timeout\n", __func__, __LINE__);
			return AVERROR(ETIMEDOUT);
		} else if (n < 0 && errno != EINTR) {
			gxlogi("%s %d: recv error\n", __func__, __LINE__);
                       ...
                       int64_t wait_end)
{
    ...
    case RTSP_LOWER_TRANSPORT_UDP_MULTICAST:
        ...
        //if (s->nobuffer&AVFMT_FLAG_NOBUFFER)
        //    break;
    ...
    case RTSP_LOWER_TRANSPORT_CUSTOM:
        if (first_queue_st && rt->transport == RTSP_TRANSPORT_RTP &&
            wait_end && wait_end < gx_av_gettime_relative())
            len = AVERROR(EAGAIN);
...
extern int avpriv_mpegts_parse_packet(MpegTSContext *ts, AVPacket *pkt, const uint8_t *buf, int len);
            ...
            if (rtpctx->first_rtcp_ntp_time != AV_NOPTS_VALUE) {
                ...
                for (i = 0; i < rt->nb_rtsp_streams; i++) {
                    ...
                    if (rtpctx2 && st && st2 &&
                        ...
                        rtpctx2->rtcp_ts_offset = av_rescale_q(rtpctx->rtcp_ts_offset, st->time_base, st2->time_base);
                    }
                }
			#if 0
...
static int sdp_probe(AVProbeData *p1)
{
    ...
    while (p < p_end && *p != '\0') {
        if (sizeof("c=IN IP") - 1 < p_end - p && av_strstart((char *)p, "c=IN IP", NULL))
...
static int sdp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    /* XXX: better loading */
    content = av_mallocz(SDP_MAX_SIZE);
    if (!content)
        return AVERROR(ENOMEM);
    ...
    rt->is_udp_cache = (s->nobuffer&AVFMT_FLAG_NOBUFFER)?0:1;
...
static int rtp_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    int ret, port, payload_type = 0, codec_type = 0;
    URLContext* in = NULL;
    struct sockaddr_storage addr;
    AVIOContext pb;
    ...
    if (!(recvbuf = av_mallocz(RTP_MAX_PACKET_LENGTH)))
        return AVERROR(ENOMEM);
    ...
    rt->pkt_size = 1472;
    opts = map_to_opts(rt);
    ret = url_open(&in, s->url, URL_RDWR, &opts);
    if (ret < 0) {
        goto fail;
    }

    while (1) {
        ret = url_read(in, recvbuf, RTP_MAX_PACKET_LENGTH);
        if (ret == AVERROR(EAGAIN))
            continue;
        if (ret < 0)
            goto fail;
        if (ret < 12) {
            gxloge ("Received too short packet,[%s,%d]\n", __func__, __LINE__);
            continue;
        }

        if ((recvbuf[0] & 0xc0) != 0x80) {
            gxloge ("Unsupported RTP version packet received,[%s,%d]\n", __func__, __LINE__);
            continue;
        }
    ...
    ffurl_close(in);
    in = NULL;
    if (ff_rtp_get_codec_type(&codec_type, payload_type)) {
        gxloge("Unable to receive RTP payload type %d without an SDP file describing it\n", payload_type);
        ret = AVERROR_INVALIDDATA;
        goto fail;
    }
    if (codec_type != AVMEDIA_TYPE_DATA) {
        gxloge ("Guessing on RTP content - if not received properly you need an SDP file describing it\n");
    }
    ...
    snprintf(sdp, sizeof(sdp),
             ...
             codec_type == AVMEDIA_TYPE_DATA  ? "application" :
