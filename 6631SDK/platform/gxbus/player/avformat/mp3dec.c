...
#include "avcodec.h"
#include "../avcodec/mpegaudiodecheader.h"
#include "id3v1.h"
...
#define MP3_ID3v2_DEFAULT_MAGIC "ID3"
...
/* mp3 read */
int ff_mp3_id3v2_match(const uint8_t *buf, const char *magic)
{
...
int ff_mp3_id3v2_tag_len(const uint8_t *buf)
{
...
static int mp3_read_probe(AVProbeData *p)
{
    ...
    int whole_used = 0;
    int frames, ret;
    int framesizes = 0, max_framesizes = 0;
    uint32_t header;
    ...
    } else if(ff_mp3_id3v2_match(buf0, MP3_ID3v2_DEFAULT_MAGIC) && 2*ff_mp3_id3v2_tag_len(buf0) >= p->buf_size) {
...
extern int ff_replaygain_export_raw(AVStream *st, int32_t tg, uint32_t tp, int32_t ag, uint32_t ap);
...
static int mp3_parse_vbr_tags(AVFormatContext *s, AVStream *st, int64_t base)
{
    ...
    if (mp3->frames)
        st->duration = av_rescale_q(mp3->frames, (AVRational){spf, c.sample_rate}, st->time_base);
...
static int mp3_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    pos = avio_tell(s->pb);
    if(mp3->filesize > MP3_ID3v1_TAG_SIZE && pos < mp3->filesize)
...
static int mp3_seek(AVFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    int64_t best_pos;
    int fast_seek = 1;//s->flags & AVFMT_FLAG_FAST_SEEK;
    int64_t filesize = mp3->header_filesize;

    if (filesize <= 0) {
        int64_t size = avio_size(s->pb);
        if (size > 0 && size > s->data_offset)
            filesize = size - s->data_offset;
    }
    ...
    if (mp3->xing_toc && (mp3->usetoc || (fast_seek && !mp3->is_cbr))) {
        ...
        ie->pos       = av_rescale(timestamp, filesize, st->duration) + s->data_offset;
    } else {
    ...
    if (mp3->is_cbr && ie == &ie1 && mp3->frames) {
        ...
        ie1.timestamp = frame_duration * av_rescale(best_pos - s->data_offset, mp3->frames, mp3->header_filesize);
    }
