...
int ff_get_extradata(AVCodecContext *par, AVIOContext *pb, int size)
{
    int ret = ff_alloc_extradata(par, size);
    if (ret < 0)
        return ret;
    ret = get_buffer(pb, par->extradata, size);
    if (ret < 0) {
...
static int wav_parse_xma2_tag(AVFormatContext *s, int64_t size, AVStream *st)
{
    ...
    st->codec->codec_id   = CODEC_ID_XMA2;
    st->need_parsing = AVSTREAM_PARSE_FULL_RAW;
...
#if 0
    av_channel_layout_uninit(&st->codec->ch_layout);
    st->codec->ch_layout.order       = AV_CHANNEL_ORDER_UNSPEC;
    st->codec->ch_layout.nb_channels = channels;

    if (st->codec->ch_layout.nb_channels <= 0 || st->codec->sample_rate <= 0)
        return AVERROR_INVALIDDATA;
#endif
    avpriv_set_pts_info(st, 64, 1, st->codec->sample_rate);

    avio_seek(pb, -size, SEEK_CUR);
    if ((ret = ff_get_extradata(st->codec, pb, size)) < 0)
        return ret;
                                        ...
                                        int length)
{
    ...
    if ((ret = get_buffer(s->pb, (unsigned char *)temp, length)) < 0)
        return ret;
...
static int wav_parse_bext_tag(AVFormatContext *s, int64_t size)
{
    ...
    snprintf(temp, sizeof(temp), "%lld", time_reference);
    ...
    if (size > 602) {
        ...
        if ((ret = get_buffer(s->pb, (unsigned char *)coding_history, size)) < 0) {
...
static int wav_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for (;;) {
        ...
        case MKTAG('i', 'd', '3', ' '): {
            ...
            break;
        case MKTAG('c', 'u', 'e', ' '):
            if (size >= 4 && got_fmt && st->codec->sample_rate > 0) {
                AVRational tb = {1, st->codec->sample_rate};
...
break_loop:
    ...
    if (   st->codec->bit_rate > 0 && data_size > 0
        ...
        && sample_count > 0 && st->codec->ch_layout.nb_channels > 1
        && sample_count % st->codec->ch_layout.nb_channels == 0) {
        if (fabs(8.0 * data_size * st->codec->ch_layout.nb_channels * st->codec->sample_rate /
            sample_count /st->codec->bit_rate - 1.0) < 0.3)
            sample_count /= st->codec->ch_layout.nb_channels;
    }

    if (data_size > 0 && sample_count && st->codec->ch_layout.nb_channels &&
        (data_size << 3) / sample_count / st->codec->ch_layout.nb_channels > st->codec->bits_per_coded_sample  + 1) {
    ...
    if (!sample_count || av_get_exact_bits_per_sample(st->codec->codec_id) > 0)
        if (   st->codec->ch_layout.nb_channels
            && data_size
            && av_get_bits_per_sample(st->codec->codec_id)
            && wav->data_end <= avio_size(pb))
            sample_count = (data_size << 3)
                                  /
                (st->codec->ch_layout.nb_channels * (uint64_t)av_get_bits_per_sample(st->codec->codec_id));

    if (sample_count)
        st->duration = sample_count;

    if (st->codec->codec_id == AV_CODEC_ID_PCM_S32LE &&
        st->codec->block_align == st->codec->ch_layout.nb_channels * 4 &&
        st->codec->bits_per_coded_sample == 32 &&
        st->codec->extradata_size == 2 &&
        AV_RL16(st->codec->extradata) == 1) {
        st->codec->codec_id = AV_CODEC_ID_PCM_F16LE;
        st->codec->bits_per_coded_sample = 16;
    } else if (st->codec->codec_id == AV_CODEC_ID_PCM_S24LE &&
               st->codec->block_align == st->codec->ch_layout.nb_channels * 4 &&
               st->codec->bits_per_coded_sample == 24) {
        st->codec->codec_id = AV_CODEC_ID_PCM_F24LE;
    } else if (st->codec->codec_id == AV_CODEC_ID_XMA1 ||
               st->codec->codec_id == AV_CODEC_ID_XMA2) {
        st->codec->block_align = 2048;
    } else if (st->codec->codec_id == AV_CODEC_ID_ADPCM_MS && st->codec->ch_layout.nb_channels > 2 &&
               st->codec->block_align < INT_MAX / st->codec->ch_layout.nb_channels) {
        st->codec->block_align *= st->codec->ch_layout.nb_channels;
    }
#endif
    av_metadata_conv(s, NULL, wav_metadata_conv);
    av_metadata_conv(s, NULL, ff_riff_info_conv);
    set_spdif(s, wav);
    return 0;
}
    ...
    while (!avio_feof(pb)) {
        ...
        if (!memcmp(guid, ff_w64_guid_fmt, 16)) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (chunk_size == SIZE_MAX || (filesize >= 0 && (chunk_size > filesize)))
                    return AVERROR_INVALIDDATA;
