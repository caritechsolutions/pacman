...
redo:
	...
	if(es_type == STREAM_TYPE_VIDEO_MPEG1){
        ...
        codec_id = CODEC_ID_DVD_NAV;
    } else if (startcode >= 0x1c0 && startcode <= 0x1df) {
        type     = CODEC_TYPE_AUDIO;
        if (m->sofdec > 0) {
            codec_id = CODEC_ID_ADPCM_ADX;
            // Auto-detect AC-3
            request_probe = 50;
        } else {
...
skip:
    ...
    if (st->codec->codec_id == CODEC_ID_PCM_MULAW) {
        ...
        st->codec->channel_layout = 0x00000004;
...
found:
	...
	if (startcode >= 0xa0 && startcode <= 0xaf) {
		if (lpcm_header_len == 6 && st->codec->codec_id == CODEC_ID_MLP) {
			if (len < 6)
				goto skip;
			avio_skip(s->pb, 6);
			len -=6;
	...
	//gxlogf("%d: pts=%0.3f dts=%0.3f size=%d\n",
		...
		int64_t *ppos, int64_t pos_limit)
{
	...
	//gxlogf("pos=0x%x, dts=0x%x, %0.3f\n", (unsigned int)pos, (unsigned int)dts, dts / 90000.0);
	*ppos = pos;
	return dts;
}

extern void av_read_frame_flush(AVFormatContext*  s);
static int mpegps_read_seek(AVFormatContext* s, int stream_index, int64_t timestamp, int flags)
{
#define PS_READ_SEEK_TIMEOUT_SEC (3)
	int64_t npos, pos, offset, pts=-1, start_time = s->start_time/AV_TIME_BASE;
	AVPacket pkt;
	int i, max_search = 0;
	AVStream *st;
	...
	do {
		...
		if(time_tmp.seconds-time.seconds>PS_READ_SEEK_TIMEOUT_SEC){
			gxlogf("%s, mpegps_read_packe time out !\n", __func__);
			avio_seek(s->pb, npos, SEEK_SET);
			goto RET;
		}
		pkt.pts = -1;
		url_fseek(s->pb, pos, SEEK_SET);
		while(pkt.pts == -1 || pkt.pts == pts || pkt.pos == -1 || pkt.stream_index != i ) {
			if(mpegps_read_packet(s, &pkt) < 0){
				gxloge("%s, mpegps_read_packe failed !\n", __func__);
				avio_seek(s->pb, npos, SEEK_SET);
				goto RET;
			}
			av_free_packet(&pkt);
