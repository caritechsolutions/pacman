...
#include "../avutil/parseutils.h"
#include "../avutil/opt.h"
#include "../avcodec/xiph.h"
...
static char *extradata2psets(AVFormatContext *s, AVCodecContext *par)
{
    char *psets, *p;
    const uint8_t *r;
...
static char *extradata2psets_hevc(AVCodecContext *par)
{
    char *psets;
    uint8_t *extradata = par->extradata;
    int extradata_size = par->extradata_size;
...
static char *extradata2config(AVFormatContext *s, AVCodecContext *par)
{
    char *config;
...
static char *xiph_extradata2config(AVFormatContext *s, AVCodecContext *par)
{
    uint8_t *config;
    char *encoded_config;
...
static int latm_context2profilelevel(AVCodecContext *par)
{
    /* MP4A-LATM
...
static char *latm_context2config(AVFormatContext *s, AVCodecContext *par)
{
    /* MP4A-LATM
     * The RTP payload format specification is described in RFC 3016
     * The encoding specifications are provided in ISO/IEC 14496-3 */
                        ...
                        int port, int ttl, AVFormatContext *fmt)
{
    ...
    payload_type = ff_rtp_get_payload_type(fmt, st->codec, idx);
