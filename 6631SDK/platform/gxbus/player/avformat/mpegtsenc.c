	...
	for(i = 0; i < s->nb_streams; i++) {
		...
		*q++ = stream_type;
		...
		case CODEC_TYPE_SUBTITLE:
			{
				...
				if (st->codec->codec_id == CODEC_ID_DVB_SUBTITLE) {
					...
					*len_ptr = q - len_ptr - 1;
				} else if (st->codec->codec_id == CODEC_ID_DVB_TELETEXT) {
		...
		case CODEC_TYPE_DATA:
			if (st->codec->codec_id == CODEC_ID_SMPTE_KLV) {
				*q++ = 0x05; /* MPEG-2 registration descriptor */
				*q++ = 4;
				*q++ = 'K';
				*q++ = 'L';
...
static void mpegts_prefix_m2ts_header(AVFormatContext *s)
{
	...
	if (ts->m2ts_mode) {
		...
		put_buffer(s->pb, (unsigned char *) &tp_extra_header,
...
static int mpegts_write_header(AVFormatContext *s)
{
	...
	title = av_dict_get((AVDictionary*)s->metadata, "service_name", NULL, 0);
	if (!title)
		title = av_dict_get((AVDictionary*)s->metadata, "title", NULL, 0);
	service_name = title ? title->value : DEFAULT_SERVICE_NAME;
	provider = av_dict_get((AVDictionary*)s->metadata, "service_provider", NULL, 0);
	...
	if (ts->m2ts_mode == -1) {
		if (s->url && av_match_ext(s->url, "m2ts")) {
			ts->m2ts_mode = 1;
		} else {
			ts->m2ts_mode = 0;
		}
		...
		int64_t pts, int64_t dts, int key)
{
	...
	uint8_t buf[TS_PACKET_SIZE];
	uint8_t *q;
	int val, is_start, len, header_len, write_pcr, is_dvb_subtitle = 0, is_dvb_teletext, flags;
	int afc_len, stuffing_len;
	int64_t pcr = -1; /* avoid warning */
	int64_t delay = av_rescale(s->max_delay, 90000, AV_TIME_BASE);
	int force_pat = st->codec->codec_type == CODEC_TYPE_VIDEO && key && !ts_st->prev_payload_key;
		...
		*q++ = 0x10 | ts_st->cc; // payload indicator + CC
		...
		if (is_start) {
			...
			*q++ = len;
			...
			if (st->codec->codec_type == CODEC_TYPE_SUBTITLE || st->codec->codec_type == CODEC_TYPE_DATA)
				val |= 0x04;
			*q++ = val;
			*q++ = flags;
			*q++ = header_len;
...
static int mpegts_write_packet_internal(AVFormatContext *s, AVPacket *pkt)
{
	...
	if (st->codec->codec_id == CODEC_ID_H264) {
		...
		if ((AV_RB16(pkt->data) & 0xfff0) != 0xfff0) {
			...
			pkt2.data = pkt->data;
			pkt2.size = pkt->size;
			ret = url_open_dyn_buf(&ts_st->amux->pb);
			if (ret < 0)
				return AVERROR(ENOMEM);

			ret = av_write_frame(ts_st->amux, &pkt2);
			if (ret < 0) {
				url_close_dyn_buf(ts_st->amux->pb, &data);
				ts_st->amux->pb = NULL;
				av_free(data);
				return ret;
			}
			size = url_close_dyn_buf(ts_st->amux->pb, &data);
			ts_st->amux->pb = NULL;
			buf = data;
		}
	}
	...
	if (st->codec->codec_type != CODEC_TYPE_AUDIO || size > ts->pes_payload_size) {
...
static void gx_mpegts_write_pat(MpegTSWrite *ts)
{
	MpegTSService *service;
	uint8_t data[1012], *q;
	int i;
	...
	mpegts_write_section1(&ts->pat, PAT_TID, 123, 0, 0, 0,
			data, q - data);
}

static void gx_mpegts_write_pmt(MpegTSService *service)
{
	uint8_t data[1012], *q, *desc_length_ptr, *program_info_length_ptr;
	int val, stream_type, i;
	...
	for(i = 0; i < service->nb_es; i++) {
		MpegES *es =&service->es[i];
		switch(es->codec_id) {
			case CODEC_ID_MPEG1VIDEO:
			case CODEC_ID_MPEG2VIDEO:
				stream_type = STREAM_TYPE_VIDEO_MPEG2;
			...
			case CODEC_ID_H264:
				...
				break;
			case CODEC_ID_H265:
				stream_type = STREAM_TYPE_VIDEO_H265;
				break;
			case CODEC_ID_AVS:
				stream_type = STREAM_TYPE_VIDEO_AVS;
				break;
			case CODEC_ID_MP1:
			case CODEC_ID_MP2:
			case CODEC_ID_MP3:
		...
		*q++ = stream_type;
		put16(&q, 0xe000 | es->pid);
		desc_length_ptr = q;
		q += 2; /* patched after */

		/* write optional descriptors here */

		if(es->codec_id==CODEC_ID_EAC3){
			*q++=0x7a; // EAC3 descriptor see A038 DVB SI
			*q++=1; // 1 byte, all flags sets to 0
			*q++=0; // omit all fields...
		} else if (es->codec_id == CODEC_ID_DRA) {
			*q++=0x5;
			*q++=0x4;
			*q++='D';
			*q++='R';
...
void write_packet(struct MpegTSSection *s, const uint8_t *packet)
{
	memcpy(s->opaque + s->len, packet, TS_PACKET_SIZE);
	s->len += TS_PACKET_SIZE;
}

void MpegTsPMTCreate(char* url,  char** retBuf, unsigned int * retLen, int default_pmt_pid)
{
	int v_id = -1;
	int a_id = -1;
	int pcr_id = -1;
	...
	ts.pat.pid = 0;
	ts.pat.opaque = patbuf;
	ts.pat.write_packet = write_packet;

	GxUrl_GetItem(url,GX_URL_KEY_VPID, &v_id );
	GxUrl_GetItem(url,GX_URL_KEY_APID, &a_id );
	GxUrl_GetItem(url,GX_URL_KEY_PCRPID, &pcr_id);
	GxUrl_GetItem(url,GX_URL_KEY_PMTPID, &pmt_id);
	GxUrl_GetItem(url,GX_URL_KEY_VCODEC, &vcodec);
	GxUrl_GetItem(url,GX_URL_KEY_ACODEC, &acodec);

	if(pmt_id == -1)
		pmt_id = default_pmt_pid;

	service.sid = pmt_id;
	service.pcr_pid = pcr_id;
	service.pmt.pid = pmt_id;
	service.pmt.opaque = pmtbuf;
	service.pmt.write_packet = write_packet;

	if(a_id >0 && a_id < 0x1fff)
	{
		service.es[service.nb_es].pid = a_id;
		switch(acodec)
		{
			case GX_URL_VAL_ACODEC_MPG1 :
			case GX_URL_VAL_ACODEC_MPG2 :
				service.es[service.nb_es].codec_id = CODEC_ID_MP2;
				break;
			case GX_URL_VAL_ACODEC_AC3:
				service.es[service.nb_es].codec_id = CODEC_ID_AC3;
				break;
			case GX_URL_VAL_ACODEC_EAC3:
				service.es[service.nb_es].codec_id = CODEC_ID_EAC3;
				break;
			case GX_URL_VAL_ACODEC_LATM:
				service.es[service.nb_es].codec_id = CODEC_ID_AAC_LATM;
				break;
			case GX_URL_VAL_ACODEC_ATDS:
				service.es[service.nb_es].codec_id = CODEC_ID_AAC;
				break;
			case GX_URL_VAL_ACODEC_DRA1:
				service.es[service.nb_es].codec_id = CODEC_ID_DRA;
				break;
			default:
				service.es[service.nb_es].codec_id = CODEC_ID_MP2;
				break;
	...
	if(v_id >0 && v_id < 0x1fff)
	{
		service.es[service.nb_es].pid = v_id;
		switch(vcodec)
		{
			case GX_URL_VAL_VCODEC_MPG2 :
				service.es[service.nb_es].codec_id = CODEC_ID_MPEG2VIDEO;
				break;
			case GX_URL_VAL_VCODEC_AVS:
				service.es[service.nb_es].codec_id = CODEC_ID_AVS;
				break;
			case GX_URL_VAL_VCODEC_H264:
				service.es[service.nb_es].codec_id = CODEC_ID_H264;
				break;
			case GX_URL_VAL_VCODEC_H265:
				service.es[service.nb_es].codec_id = CODEC_ID_H265;
				break;
			case GX_URL_VAL_VCODEC_MPG4:
				service.es[service.nb_es].codec_id = CODEC_ID_MPEG4;
				break;
			default:
				service.es[service.nb_es].codec_id = CODEC_ID_MPEG2VIDEO;
				break;
	...
	gx_mpegts_write_pat(&ts);
	gx_mpegts_write_pmt(&service);

	* retBuf =av_malloc(ts.pat.len + service.pmt.len);
	memcpy(*retBuf, patbuf, ts.pat.len);
	memcpy(*retBuf + ts.pat.len, pmtbuf, service.pmt.len);

	*retLen = ts.pat.len + service.pmt.len;
	av_free(ts.services);
}

void MpegTsPMTDestroy(char* pmt)
{
	av_free(pmt);
}
