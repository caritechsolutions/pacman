...
#include "../avcodec/ac3_parser_internal.h"
...
int ff_hls_senc_parse_audio_setup_info(AVStream *st, HLSAudioSetupInfo *info)
{
    ...
    st->codec->codec_tag = info->codec_tag;

    if (st->codec->codec_id == AV_CODEC_ID_AAC)
        return 0;

    if (st->codec->codec_id != AV_CODEC_ID_AC3 && st->codec->codec_id != AV_CODEC_ID_EAC3)
    ...
    if (st->codec->codec_id == AV_CODEC_ID_AC3) {
        ...
        st->codec->sample_rate       = ac3hdr->sample_rate;
        st->codec->channels          = ac3hdr->channels;
        st->codec->channel_layout    = ac3hdr->channel_layout;
        st->codec->bit_rate          = ac3hdr->bit_rate;
        ...
        st->codec->sample_rate = eac3_sample_rate_tab[fscod];
...
#endif
        st->codec->bit_rate = data_rate*1000;
    }
