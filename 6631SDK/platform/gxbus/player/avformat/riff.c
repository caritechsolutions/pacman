...
const AVCodecTag ff_codec_wav_tags[] = {
 ...
 //   { CODEC_ID_G723_1,          0x0042 },
  ...
  //  { CODEC_ID_IAC,             0x0402 },
    ...
    { CODEC_ID_AAC_LATM,        MKTAG('M', 'T', 'A', 'L') },
    { CODEC_ID_AC3,             0x2000 },
    { CODEC_ID_DTS,             0x2001 },
    { CODEC_ID_EAC3,            0x2002 },
...
#if 0
{
	...
	codec->codec_id = ff_wav_codec_get_id(id, codec->bits_per_sample);
...
int ff_read_riff_info(AVFormatContext *s, int64_t size)
{
    ...
    while ((cur = avio_tell(pb)) >= 0 && cur <= end - 8 /* = tag + size */) {
        ...
        av_dict_set((AVDictionary **)&s->metadata, key, value, AV_DICT_DONT_STRDUP_VAL);
    }
