...
void ff_amf_write_string(uint8_t **dst, char *str)
{
...
void ff_amf_write_string2(uint8_t **dst, char *str1, char *str2)
{
    int len1 = 0, len2 = 0;
    if (str1)
        len1 = strlen(str1);
    ...
    bytestream_put_buffer(dst, (const uint8_t *)str1, len1);
    bytestream_put_buffer(dst, (const uint8_t *)str2, len2);
}
                      ...
                      int strsize, int *length)
{
    ...
    if (readsize != stringlen) {
        gxlogd ("Unable to read as many bytes as AMF string signaled\n");
    }
    str[readsize] = '\0';
                                      ...
                                      int *nb_prev_pkt, uint8_t hdr)
{
    ...
    if (prev_pkt[channel_id].read && size != prev_pkt[channel_id].size) {
        gxloge ("RTMP packet size mismatch %d != %d\n", size, prev_pkt[channel_id].size);
    ...
    if (url_read_complete(h, p->data + p->offset, toread) != toread) {
                         ...
                         int *nb_prev_pkt)
{
    ...
    if ((ret = url_write(h, pkt_hdr, p - pkt_hdr)) < 0)
        return ret;
    written = p - pkt_hdr + pkt->size;
    while (off < pkt->size) {
        int towrite = FFMIN(chunk_size, pkt->size - off);
        if ((ret = url_write(h, pkt->data + off, towrite)) < 0)
            return ret;
        off += towrite;
        if (off < pkt->size) {
            uint8_t marker = 0xC0 | pkt->channel_id;
            if ((ret = url_write(h, &marker, 1)) < 0)
                return ret;
            written++;
            if (pkt->ts_field == 0xFFFFFF) {
                uint8_t ts_header[4];
                AV_WB32(ts_header, timestamp);
                if ((ret = url_write(h, ts_header, 4)) < 0)
                    return ret;
                written += 4;
            }
        }
                           ...
                           const uint8_t *name, uint8_t *dst, int dst_size)
{
    ...
    for (;;) {
        ...
        if (size == namelen && !memcmp(gb->buffer-size, name, namelen)) {
            ...
            case AMF_DATA_TYPE_NUMBER:
                snprintf((char*)dst, dst_size, "%g", av_int2double(bytestream2_get_be64(gb)));
                break;
            case AMF_DATA_TYPE_BOOL:
                snprintf((char*)dst, dst_size, "%s", bytestream2_get_byte(gb) ? "true" : "false");
                break;
            case AMF_DATA_TYPE_STRING:
...
void ff_rtmp_packet_dump(void *ctx, RTMPPacket *p)
{
    if (p->type == RTMP_PT_INVOKE || p->type == RTMP_PT_NOTIFY) {
        ...
        gxlogd ("Window acknowledgement size = %d\n", AV_RB32(p->data));
    } else if (p->type == RTMP_PT_SET_PEER_BW) {
        gxlogd ("Set Peer BW = %d\n", AV_RB32(p->data));
