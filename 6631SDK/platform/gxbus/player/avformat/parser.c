...
AVCodecParserContext* av_parser_init(int codec_id)
{
	...
	for (parser = av_first_parser; parser != NULL; parser = parser->next) {
		if (parser->codec_ids[0] == codec_id ||
		    ...
		    parser->codec_ids[3] == codec_id || parser->codec_ids[4] == codec_id)
			goto found;
	}
	return NULL;
      found:
		    ...
		    uint8_t* * poutbuf, int *poutbuf_size, const uint8_t * buf, int buf_size, int64_t pts, int64_t dts)
{
	...
	if (buf_size == 0) {
		...
		if (s->fetch_timestamp) {
			...
			s->last_dts = dts;
			s->last_offset = 0;
			s->cur_frame_pts[k] = s->cur_frame_dts[k] = AV_NOPTS_VALUE;
		}
	}
...
int ff_combine_frame(ParseContext*  pc, int next, const uint8_t ** buf, int *buf_size)
{
#if 0
	if (pc->overread) {
		gxlogf("overread %d, state:%X next:%d index:%d o_index:%d\n", pc->overread, pc->state, next, pc->index,
		       pc->overread_index);
	...
	if (next == END_NOT_FOUND) {
		void *ptmp = av_fast_realloc(pc->buffer, &pc->buffer_size, (*buf_size) + pc->index + FF_INPUT_BUFFER_PADDING_SIZE);
		if (ptmp == NULL)
			return -1;
	...
	*buf_size = pc->overread_index = pc->index + next;
	...
	/* append to buffer*/
	if (pc->index) {
		void * ptmp = av_fast_realloc(pc->buffer, &pc->buffer_size, next + pc->index + FF_INPUT_BUFFER_PADDING_SIZE);
		if (ptmp == NULL)
			return -1;
...
int hevc_split(AVCodecContext * avctx, const uint8_t *buf, int buf_size)
{
    int i;
    uint32_t state = -1;
    int has_ps = 0;
