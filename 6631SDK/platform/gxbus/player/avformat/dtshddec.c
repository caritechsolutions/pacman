...
static int dtshd_read_header(AVFormatContext *s, AVFormatParameters * ap)
{
    DTSHDDemuxContext *dtshd = s->priv_data;
    AVIOContext *pb = s->pb;
    uint64_t chunk_type, chunk_size;
    int64_t duration, data_start = 0;
    AVStream *st;
    int ret;
    char *value;
    ...
    for (;;) {
        ...
        case AUPR_HDR:
            if (chunk_size < 21)
                return AVERROR_INVALIDDATA;
            avio_skip(pb, 3);
            st->codec->sample_rate = avio_rb24(pb);
            ...
            avio_skip(pb, 5);
            st->codec->channels = ff_dca_count_chs_for_mask(avio_rb16(pb));
            st->codec->initial_padding = avio_rb16(pb);
