...
static int ogg_write_page(AVFormatContext *s, OGGPage *page, int extra_flags)
{
    ...
    if (ret < 0)
        return ret;
   // init_checksum(pb, crc04C11DB7_update, 0);
                           ...
                           int header)
{
    ...
    if ((st->codec->codec_id == AV_CODEC_ID_THEORA || st->codec->codec_id == AV_CODEC_ID_VP8) && !header &&
...
static int ogg_build_flac_headers(AVCodecContext *par,
...
static int ogg_build_speex_headers(AVCodecContext *par,
                                   OGGStreamContext *oggstream, int bitexact,
                                   AVDictionary **m)
{
    uint8_t *p;
...
static int ogg_build_opus_headers(AVCodecContext *par,
                                  OGGStreamContext *oggstream, int bitexact,
                                  AVDictionary **m)
{
    uint8_t *p;
    ...
    if (!p)
        return AVERROR(ENOMEM);
    oggstream->header[1] = p;
    bytestream_put_buffer(&p, (const uint8_t*)"OpusTags", 8);
                                 ...
                                 OGGStreamContext *oggstream, int bitexact)
{
    ...
    bytestream_put_buffer(&p, (const uint8_t*)"VP80", 4); // Identifier
...
static int ogg_init(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        } else if (st->codec->codec_id == AV_CODEC_ID_VP8) {
