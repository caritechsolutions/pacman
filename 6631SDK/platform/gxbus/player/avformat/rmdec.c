...
const unsigned char sipr_subpk_size[4] = { 29, 19, 37, 20 };
...
RMStream *rm_alloc_rmstream (void)
{
...
void rm_free_rmstream (RMStream *rms)
{
                                     ...
                                     AVStream *st, RMStream *ast, int read_all)
{
    ...
    if (version == 3) {
        ...
        if (version == 5) {
            get_be32(pb);
            get_buffer(pb, (unsigned char*)buf, 4);
            buf[4] = 0;
        } else {
        ...
        case CODEC_ID_RA_288:
            ...
            if(ast->audio_framesize >= UINT_MAX / sub_packet_h){
                gxlogf("ast->audio_framesize * sub_packet_h too large\n");
                return -1;
            }
        ...
        case CODEC_ID_SIPR:
            ...
            if(codecdata_length + FF_INPUT_BUFFER_PADDING_SIZE <= (unsigned)codecdata_length){
                gxlogf("codecdata_length too large\n");
                return -1;
            }
            ...
            if (st->codec->codec_id == CODEC_ID_SIPR) {
                ...
                if(sub_packet_size <= 0){
                    gxlogf("sub_packet_size is invalid\n");
                    return -1;
                }
            ...
            if(ast->audio_framesize >= UINT_MAX / sub_packet_h){
                gxlogf("rm->audio_framesize * sub_packet_h too large\n");
                return -1;
            }
...
int
rm_read_mdpr_codecdata (AVFormatContext *s, ByteIOContext *pb,
                           AVStream *st, RMStream *rst, int codec_data_size)
{
    ...
    if (v == MKTAG(0xfd, 'a', 'r', '.')) {
        ...
        fail1:
            gxlogf("Unsupported video codec\n");
            goto skip;
        }
...
static int rm_read_header_old(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    if (!st)
        return -1;
    st->priv_data = rm_alloc_rmstream();
...
static int rm_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        case MKTAG('M', 'D', 'P', 'R'):
            ...
            st->priv_data = rm_alloc_rmstream();
            if (rm_read_mdpr_codecdata(s, s->pb, st, st->priv_data,
                                          get_be32(pb)) < 0)
                return -1;
            break;
        case MKTAG('D', 'A', 'T', 'A'):
            ...
            if(state == MKBETAG('I', 'N', 'D', 'X')){
                ...
                if (len == 20)
                    ...
                    len = expected_len;
                     ...
                     gxlogf("Index size %d (%d pkts) is wrong, should be %d.\n",
                ...
                goto skip;
                       ...
                       gxlogf("DATA tag in middle of chunk, file may be broken.\n");
            }
 ...
 */
void rm_reorder_sipr_data(uint8_t *buf, int sub_packet_h, int framesize)
{
...
int
rm_parse_packet (AVFormatContext *s, ByteIOContext *pb,
                    AVStream *st, RMStream *ast, int len, AVPacket *pkt,
                    int *seq, int flags, int64_t timestamp)
{
    RMDemuxContext *rm = s->priv_data;

//TODO: comparison between 'enum CodecType' and 'enum AVMediaType'
    if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
        ...
        if ((st->codec->codec_id == CODEC_ID_RA_288) ||
            ...
            if (++(ast->sub_packet_cnt) < h)
                return -1;
            if (st->codec->codec_id == CODEC_ID_SIPR)
                rm_reorder_sipr_data(ast->pkt.data, h, w);
...
int
rm_retrieve_cache (AVFormatContext *s, ByteIOContext *pb,
...
static int rm_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (;;) {
        if (rm->audio_pkt_cnt) {
            ...
            rm_retrieve_cache(s, s->pb, st, st->priv_data, pkt);
            flags = 0;
        } else {
            if (rm->old_format) {
                RMStream *ast;
            ...
            res = rm_parse_packet (s, s->pb, st, st->priv_data, len, pkt,
                                      &seq, flags, timestamp);
...
static int rm_read_close(AVFormatContext *s)
{
    ...
    for (i=0;i<s->nb_streams;i++)
        rm_free_rmstream(s->streams[i]->priv_data);
                               ...
                               int64_t *ppos, int64_t pos_limit)
{
    ...
    for(;;){
        ...
        if((flags&2) && (seq&0x7F) == 1){
//            av_log(s, AV_LOG_DEBUG, "%d %d-%d %lld %d\n", flags, stream_index2, stream_index, dts, seq);
