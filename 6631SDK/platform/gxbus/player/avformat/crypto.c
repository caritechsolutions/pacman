...
#define MAX_BUFFER_BLOCKS 150//257,consistent with stream_crypto.h
#define BLOCKSIZE 16
...
typedef struct CryptoContext {
    ...
    int flags;
    int eof;
    uint8_t key[BLOCKSIZE*2];
    int keylen;
    uint8_t iv[BLOCKSIZE*2];
    int ivlen;
    uint8_t *decrypt_key;
    int decrypt_keylen;
    uint8_t *decrypt_iv;
...
static int crypto_open2(URLContext *h, const char *uri, int flags)
{
    ...
    if ((e = av_dict_get(h->url_context_options, "key", NULL, 0))) {
        ff_hex_to_data(c->key, e->value);
        c->keylen = BLOCKSIZE;//strlen(((char*)c->key));
    }

    if ((e = av_dict_get(h->url_context_options, "iv", NULL, 0))) {
        ff_hex_to_data(c->iv, e->value);
        c->ivlen = BLOCKSIZE;//strlen(((char*)c->iv));
    }

    if (!av_strstart(uri, "crypto+", &nested_url) &&
        !av_strstart(uri, "crypto:", &nested_url)) {
        gxloge ("Unsupported url[%s]\n", uri);
        ret = AVERROR(EINVAL);
        goto err;
    }

    if (flags & AVIO_FLAG_READ) {
        if ((ret = set_aes_arg(h, &c->decrypt_key, &c->decrypt_keylen, c->key, c->keylen, "decryption key")) < 0)
            goto err;
        if ((ret = set_aes_arg(h, &c->decrypt_iv, &c->decrypt_ivlen, c->iv, c->ivlen, "decryption IV")) < 0)
            goto err;
    }

    if (flags & AVIO_FLAG_WRITE) {
        if ((ret = set_aes_arg(h, &c->encrypt_key, &c->encrypt_keylen, c->key, c->keylen, "encryption key")) < 0)
        if (ret < 0)
            goto err;
        if ((ret = set_aes_arg(h, &c->encrypt_iv, &c->encrypt_ivlen, c->iv, c->ivlen, "encryption IV")) < 0)
            goto err;
    }

    if ((ret = url_open(&c->hd, nested_url, flags, &(h->url_context_options))) < 0) {
        if (c->decrypt_key)
            av_free(c->decrypt_key);
        if (c->decrypt_iv)
            av_free(c->decrypt_iv);
        if (c->encrypt_key)
            av_free(c->encrypt_key);
        if (c->encrypt_iv)
            av_free(c->encrypt_iv);
        goto err;
    }
    if (flags & AVIO_FLAG_READ) {
...
retry:
    ...
    while (c->indata - c->indata_used < 2*BLOCKSIZE) {
        int n = url_read(c->hd, c->inbuffer + c->indata, sizeof(c->inbuffer) - c->indata);
        if (n <= 0) {
            c->eof = 1;
            break;
        }
...
static int64_t crypto_seek(URLContext *h, int64_t pos, int whence)
{
    ...
    case SEEK_CUR:
        ...
        break;
    case SEEK_END: {
        int64_t newpos = url_seek( c->hd, pos, AVSEEK_SIZE );
        if (newpos < 0) {
            return newpos;
        }
        pos = newpos - pos;
    ...
    newpos = url_seek( c->hd, c->position, SEEK_SET );
    if (newpos < 0) {
        return newpos;
    }
    ...
    if (pos - c->position) {
        unsigned char buff[BLOCKSIZE*2]; // maximum size of pos-c->position
        ...
        if (len != 0) {
            gxloge ("Crypoto:discard read did not get all.\n");
            return AVERROR(EINVAL);
        }
    }
...
static int crypto_write(URLContext *h, unsigned char *buf, int size)
{

    CryptoContext *c = h->priv_data;
    int ret = 0, total_size = 0, blocks = 0, pad_len = 0, out_size = 0;
    ...
    if (out_size) {
        av_fast_malloc(&c->write_buf, (int *)(&(c->write_buf_size)), out_size, 0);
        ...
        ret = url_write(c->hd, c->write_buf, out_size);
        if (ret < 0)
            return ret;
...
URLProtocol gx_crypto_protocol = {
    .name            = "crypto",
    .url_open       = crypto_open2,
