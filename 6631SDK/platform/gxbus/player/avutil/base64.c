                              ...
                              const char *encoded_ref)
{
    ...
    if (!av_base64_encode(encoded, MAX_ENCODED_SIZE, data, data_size)) {
        gxlogd("Failed: cannot encode the input data\n");
        return 1;
    }
    if (encoded_ref && strcmp(encoded, encoded_ref)) {
        gxlogd("Failed: encoded string differs from reference\n"
               "Encoded:\n%s\nReference:\n%s\n", encoded, encoded_ref);
        return 1;
    }

    if ((data2_size = av_base64_decode(data2, encoded, max_data2_size)) != data_size) {
        gxlogd("Failed: cannot decode the encoded string\n"
               "Encoded:\n%s\n", encoded);
        return 1;
    }
    if ((data2_size = av_base64_decode(data2, encoded, data_size)) != data_size) {
        gxlogd("Failed: cannot decode with minimal buffer\n"
               "Encoded:\n%s\n", encoded);
        return 1;
    }
    if (memcmp(data2, data, data_size)) {
        gxlogd("Failed: encoded/decoded data differs from original data\n");
        return 1;
    }
    if (av_base64_decode(NULL, encoded, 0) != 0) {
        gxlogd("Failed: decode to NULL buffer\n");
        return 1;
    }
    if (strlen(encoded)) {
...
int main(int argc, char ** argv)
{
    ...
    gxlogd("Encoding/decoding tests\n");
