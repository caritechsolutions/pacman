...
const int av_md5_size = sizeof(struct AVMD5);
...
{
#ifdef CONFIG_HAVE_WOLFSSL
	wc_Md5Update(&ctx->md5_ctx, src, len);
#endif
#ifdef CONFIG_HAVE_OPENSSL
	MD5Update(&ctx->md5_ctx, (const void *)src, (size_t)len);
#endif
}
...
{
#ifdef CONFIG_HAVE_WOLFSSL
	wc_Md5Final(&ctx->md5_ctx, dst);
#endif
#ifdef CONFIG_HAVE_OPENSSL
	MD5Final(dst, &ctx->md5_ctx);
#endif
}

#else
#define av_bswap32(x) bswap_32(x)
#define av_bswap64(x) bswap_64(x)
...
{
    int i;
    //uint64_t finalcount = av_bswap64(ctx->len << 3);
