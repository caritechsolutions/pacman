...
int av_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
	...
	if (decrypt)
		ret = AES_set_decrypt_key(key, key_bits, &a->aes);
	else
		ret = AES_set_encrypt_key(key, key_bits, &a->aes);
                  ...
                  int count, uint8_t *iv, int decrypt)
{
	if (iv) {
		/*CBC encrypt*/
		AES_cbc_encrypt(src, dst, count * 16, &a->aes, iv, (decrypt ? 0 : 1));
	} else {
		/*ECB encrypt*/
		unsigned int i = 0;

		for (i = 0; i < count; i++)
			AES_ecb_encrypt(src + i * 16, dst + i * 16, &a->aes, (decrypt ? 0 : 1));
	}
...
#if CONFIG_SMALL
    uint32_t enc_multbl[1][256];
    uint32_t dec_multbl[1][256];
#else
                  ...
                  int count, uint8_t *iv, int decrypt)
{
    while (count--) {
        ...
        if (decrypt) {
            aes_crypt(a, 0, a->inv_sbox, a->dec_multbl);
            if (iv) {
            ...
            aes_crypt(a, 2, a->sbox, a->enc_multbl);
            addkey_d(dst, &a->state[0], &a->round_key[0]);
            if (iv)
                memcpy(iv, dst, 16);
        }
...
int av_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
    ...
    if (!a->enc_multbl[FF_ARRAY_ELEMS(a->enc_multbl)-1][FF_ARRAY_ELEMS(a->enc_multbl[0])-1]) {
        j = 1;
        for (i = 0; i < 255; i++) {
        ...
        init_multbl2(a->dec_multbl, (const int[4]) { 0xe, 0x9, 0xd, 0xb },
                     log8, alog8, a->inv_sbox);
        init_multbl2(a->enc_multbl, (const int[4]) { 0x2, 0x1, 0x1, 0x3 },
                     log8, alog8, a->sbox);
    }
    ...
    if (decrypt) {
        for (i = 1; i < rounds; i++) {
            av_aes_block tmp[3];
            tmp[2] = a->round_key[i];
            subshift(&tmp[1], 0, a->sbox);
            mix(tmp, a->dec_multbl, 1, 3);
            a->round_key[i] = tmp[0];
        }
    } else {
