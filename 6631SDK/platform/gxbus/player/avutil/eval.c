    ...
    switch (e->type) {
        ...
        default: {
            ...
            switch (e->type) {
                ...
                case e_gcd: return e->value * ff_gcd(d,d2);
...
static int parse_primary(AVExpr **e, Parser *p)
{
    ...
    p->s= strchr(p->s, '(');
    if (p->s==NULL) {
        gxlogd("Undefined constant or missing '(' in '%s'\n", s0);
        p->s= next;
        av_expr_free(d);
    ...
    if (p->s[0] != ')') {
        gxlogd("Missing ')' or too many args in '%s'\n", s0);
        av_expr_free(d);
        return AVERROR(EINVAL);
    }
    p->s++; // ")"
                  ...
                  int log_offset, void *log_ctx)
{
    ...
    *wp++ = 0;
    ...
    if (*p.s) {
        av_expr_free(e);
        gxlogd("Invalid chars '%s' at the end of expression '%s'\n", p.s, s0);
        ret = AVERROR(EINVAL);
        goto end;
    }
    if (!verify_expr(e)) {
