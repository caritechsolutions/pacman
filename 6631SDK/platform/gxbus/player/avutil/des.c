...
int main(void) {
    ...
    for (i = 0; i < 1000; i++) {
        ...
        if (ct != data) {
            gxlogd("Test 2 failed\n");
            return 1;
        }
    }
#ifdef GENTABLES
    gxlogd("static const uint32_t S_boxes_P_shuffle[8][64] = {\n");
    for (i = 0; i < 8; i++) {
        gxlogd("    {");
        for (j = 0; j < 64; j++) {
