...
const char *av_get_string(void *obj, const char *name, const AVOption **o_out, char *buf, int buf_len)
{
	...
	switch (o->type) {
		...
		case AV_OPT_TYPE_INT64:     snprintf(buf, buf_len, "%lld", *(int64_t*)dst);break;
...
int av_opt_get(void *obj, const char *name, int search_flags, uint8_t **out_val)
{
	...
	if (!o || !target_obj || (o->offset<=0 && o->type != AV_OPT_TYPE_CONST))
		return AVERROR(EINVAL);/*AVERROR_OPTION_NOT_FOUND;*/
	...
	switch (o->type) {
		...
		case AV_OPT_TYPE_INT64:     ret = snprintf(buf, sizeof(buf), "%lld", *(int64_t*)dst);break;
...
void av_opt_free(void *obj)
{
    ...
    while ((o = av_opt_next(obj, o))) {
        ...
        case AV_OPT_TYPE_BINARY:
            tmp = (uint8_t *)obj;
            if ((*(tmp + o->offset)))
                av_freep(tmp + o->offset);
            break;

        case AV_OPT_TYPE_DICT:
            tmp = (uint8_t *)obj;
            av_dict_free((AVDictionary **)(tmp + o->offset));
            break;
        default:
            break;
        }
...
int av_opt_set_dict(void *obj, AVDictionary **options)
{
	...
	while ((t = av_dict_get(*options, "", t, AV_DICT_IGNORE_SUFFIX))) {
		...
		if (ret == AVERROR_OPTION_NOT_FOUND)
			...
			gxlogd ("Error setting option %s to value %s.\n", t->key, t->value);
			break;
		}
		ret = 0;
	}
...
int av_opt_copy(void *dst, void *src)
{
    ...
    while ((o = av_opt_next(src, o))) {
        void *field_dst = (char *)dst + o->offset;
        void *field_src = (char *)src + o->offset;
        char **field_dst8 = (char **)field_dst;
        char **field_src8 = (char **)field_src;
