...
#define UINT32_TYPE_MAX 0x1fffffff
...
AVEncryptionInfo *av_encryption_info_alloc(uint32_t subsample_count, uint32_t key_id_size, uint32_t iv_size)
{
    ...
    info->subsamples = av_mallocz(subsample_count*sizeof(*info->subsamples));
...
uint8_t *av_encryption_info_add_side_data(const AVEncryptionInfo *info, size_t *size)
{
    ...
    if (UINT32_TYPE_MAX - FF_ENCRYPTION_INFO_EXTRA < info->key_id_size ||
        UINT32_TYPE_MAX - FF_ENCRYPTION_INFO_EXTRA - info->key_id_size < info->iv_size ||
        (UINT32_TYPE_MAX - FF_ENCRYPTION_INFO_EXTRA - info->key_id_size - info->iv_size) / 8 < info->subsample_count) {
        return NULL;
    }
    ...
    uint32_t system_id_size, uint32_t num_key_ids, uint32_t key_id_size, uint32_t data_size)
{
    ...
    info->key_ids = key_id_size ? av_mallocz(num_key_ids * sizeof(*info->key_ids)) : NULL;
    ...
    const uint8_t *side_data, size_t side_data_size)
{
    ...
    AVEncryptionInitInfo *ret = NULL, *info = NULL, *temp_info = NULL;
    ...
    for (i = 0; i < init_info_count; i++) {
        ...
        // UINT32_TYPE_MAX + UINT32_TYPE_MAX + UINT32_TYPE_MAX * UINT32_TYPE_MAX == UINT64_MAX
...
uint8_t *av_encryption_init_info_add_side_data(const AVEncryptionInitInfo *info, size_t *side_data_size)
{
    ...
    for (cur_info = info; cur_info; cur_info = cur_info->next) {
        ...
        if (init_info_count == UINT32_TYPE_MAX || temp_side_data_size > UINT32_TYPE_MAX) {
            return NULL;
        }
        init_info_count++;
        ...
        if (cur_info->num_key_ids) {
            ...
            if (temp_side_data_size > UINT32_TYPE_MAX) {
                return NULL;
            }
        }
    }
