...
HX_RESULT rm_unpack_rule_map(BYTE* * ppBuf,
			     UINT32*  pulLen,
			     rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree, void* pUserMem, rm_rule_map * pMap)
{
	HX_RESULT retVal = HXR_FAIL;

	if (ppBuf && pulLen && fpMalloc && fpFree && pMap &&* ppBuf && *pulLen >= 2) {
...
void rm_cleanup_rule_map(rm_free_func_ptr fpFree, void* pUserMem, rm_rule_map * pMap)
{
...
HX_RESULT rm_unpack_multistream_hdr(BYTE* * ppBuf,
				    UINT32*  pulLen,
				    rm_av_malloc_func_ptr fpMalloc,
				    rm_free_func_ptr fpFree, void* pUserMem, rm_multistream_hdr * hdr)
{
	HX_RESULT retVal = HXR_FAIL;

	if (ppBuf && pulLen && fpMalloc && fpFree && hdr &&* ppBuf && *pulLen >= 4) {
		/* Unpack the multistream members*/
		hdr->ulID = rm_unpack32(ppBuf, pulLen);
		/* Unpack the rule to substream map*/
		retVal = rm_unpack_rule_map(ppBuf, pulLen, fpMalloc, fpFree, pUserMem, &hdr->rule2SubStream);
		if (retVal == HXR_OK) {
			if (*pulLen >= 2) {
...
void rm_cleanup_multistream_hdr(rm_free_func_ptr fpFree, void* pUserMem, rm_multistream_hdr * hdr)
{
	if (hdr) {
