...
char* copy_string(void *pUserMem, rm_av_malloc_func_ptr fpMalloc, const char *pszStr)
{
	char* pRet = HXNULL;
	...
	if (fpMalloc && pszStr) {
		...
		if (pRet) {
			/* Copy the string*/
			strncpy(pRet, pszStr, strlen(pszStr));
		}
	}
...
void free_string(void* pUserMem, rm_free_func_ptr fpFree, char **ppszStr)
{
