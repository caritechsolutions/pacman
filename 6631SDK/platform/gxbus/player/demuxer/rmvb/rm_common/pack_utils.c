...
HX_RESULT rm_unpack_string(BYTE* * ppBuf,
			   UINT32*  pulLen,
			   UINT32 ulStrLen,
			   char* *ppStr, void *pUserMem, rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree)
{
...
HX_RESULT rm_unpack_buffer(BYTE* * ppBuf,
			   UINT32*  pulLen,
			   UINT32 ulBufLen,
			   BYTE* * ppUnPackBuf, void *pUserMem, rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_unpack_array(BYTE* * ppBuf,
			  ...
			  UINT32 ulNumElem,
			  UINT32 ulElemSize,
			  void* *ppArr, void *pUserMem, rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree)
{
	HX_RESULT retVal = HXR_FAIL;

	if (ppBuf && pulLen &&* pulLen >= ulNumElem * ulElemSize && ppArr && fpMalloc && fpFree) {
		/* Clear the return value*/
		retVal = HXR_OK;
