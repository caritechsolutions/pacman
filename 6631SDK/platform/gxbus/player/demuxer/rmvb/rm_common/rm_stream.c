...
HXBOOL rm_stream_is_realaudio_mimetype(const char* pszStr)
{
	...
	if (pszStr) {
		if (!strcmp(pszStr, REALAUDIO_MIME_TYPE) || !strcmp(pszStr, REALAUDIO_ENCRYPTED_MIME_TYPE)) {
			bRet = TRUE;
		}
	}
...
HXBOOL rm_stream_is_realvideo_mimetype(const char* pszStr)
{
	...
	if (pszStr) {
		if (!strcmp(pszStr, REALVIDEO_MIME_TYPE) || !strcmp(pszStr, REALVIDEO_ENCRYPTED_MIME_TYPE)) {
			bRet = TRUE;
		}
	}
...
HXBOOL rm_stream_is_realevent_mimetype(const char* pszStr)
{
	...
	if (pszStr) {
		if (!strcmp(pszStr, REALEVENT_MIME_TYPE) ||
		    ...
		    !strcmp(pszStr, SYNCMM_MIME_TYPE) || !strcmp(pszStr, SYNCMM_ENCRYPTED_MIME_TYPE)) {
			bRet = TRUE;
		}
	}
...
HXBOOL rm_stream_is_real_mimetype(const char* pszStr)
{
	return rm_stream_is_realaudio_mimetype(pszStr) ||
	    rm_stream_is_realvideo_mimetype(pszStr) || rm_stream_is_realevent_mimetype(pszStr);
}

HX_RESULT rm_stream_get_property_int(rm_stream_header*  hdr, const char *pszStr, UINT32 * pulVal)
{
	HX_RESULT retVal = HXR_FAIL;

	if (hdr && pszStr && pulVal && hdr->pProperty && hdr->ulNumProperties) {
		UINT32 i = 0;
		for (i = 0; i < hdr->ulNumProperties; i++) {
			rm_property* pProp = &hdr->pProperty[i];
			if (pProp->ulType == RM_PROPERTY_TYPE_UINT32 &&
			    pProp->pName && !strcmp(pszStr, (const char* )pProp->pName)) {
...
HX_RESULT rm_stream_get_property_buf(rm_stream_header*  hdr, const char *pszStr, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;

	if (hdr && pszStr && ppBuf && pulLen && hdr->pProperty && hdr->ulNumProperties) {
		UINT32 i = 0;
		for (i = 0; i < hdr->ulNumProperties; i++) {
			rm_property* pProp = &hdr->pProperty[i];
...
HX_RESULT rm_stream_get_property_str(rm_stream_header*  hdr, const char *pszStr, char **ppszStr)
{
	HX_RESULT retVal = HXR_FAIL;

	if (hdr && pszStr && ppszStr && hdr->pProperty && hdr->ulNumProperties) {
		UINT32 i = 0;
		for (i = 0; i < hdr->ulNumProperties; i++) {
			rm_property* pProp = &hdr->pProperty[i];
