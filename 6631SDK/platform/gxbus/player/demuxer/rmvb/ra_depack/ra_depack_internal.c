...
unsigned char RA_Parameter[1000];
unsigned char is_aac;
unsigned char is_cook;
unsigned char* AUDIO_USED_SDRAM_PARA_ADDRESS;
 ...
 *  @param fpMalloc user-defined av_malloc() implementation
 *  @return an error code, signalling success or failure.
 *  @see reverseBitstream
 */
int newBitstream(struct BITSTREAM* *ppBitstream, int nBits,
                 void* pUserMem, rm_av_malloc_func_ptr fpMalloc)
{
...
unsigned char Output(UINT32 data ,UINT32 type,UINT32 addr)//type == 0 8bit;type == 1 16bit;type == 2 32bit
{
	if(type == 0)
	{
		RA_Parameter[addr] = (unsigned char)(data & 0xff);
	}
	else if(type == 1)
	{
		RA_Parameter[addr+1]   = (unsigned char)(data & 0xff);
		RA_Parameter[addr] = (unsigned char)((data >> 8)& 0xff);
	}
	else if(type == 2)
	{
		RA_Parameter[addr+3]	 = (unsigned char)(data & 0xff);
		RA_Parameter[addr+2] = (unsigned char)((data >> 8)& 0xff);
		RA_Parameter[addr+1] = (unsigned char)((data >> 16)& 0xff);
		RA_Parameter[addr+0] = (unsigned char)((data >> 24)& 0xff);
	}

	return 1;
}

unsigned int Endian_B2L(unsigned int temp)
{
	unsigned int temp_32;
	unsigned int temp_24;
	unsigned int temp_16;
	unsigned int temp_8;
	unsigned int temp_return;

	temp_32 = (temp>>24)&0xff;
	temp_24 = (temp>>16)&0xff;
	temp_16 = (temp>>8)&0xff;
	temp_8 = (temp)&0xff;
	temp_return = temp_8<<24 | temp_16<<16  | temp_24<<8 | temp_32;
	return temp_return;

}
// LIUFEI-END

void* ra_depacki_av_malloc(ra_depack_internal * pInt, UINT32 ulSize)
{
	void* pRet = HXNULL;
...
HX_RESULT ra_depacki_init(ra_depack_internal*  pInt, rm_stream_header * hdr)
{
	...
	if (pInt && hdr) {
		...
		if (pInt->bHasEndTime && pInt->ulStreamDuration > pInt->ulEndTime) {
		...
		if (retVal == HXR_OK) {
			...
			retVal = ra_depacki_unpack_rule_map(pInt, &pInt->rule2Flag, &pTmp, &ulTmp);
			if (retVal == HXR_OK) {
...
HX_RESULT ra_depacki_unpack_rule_map(ra_depack_internal*  pInt, rm_rule_map * pMap, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt) {
		retVal = rm_unpack_rule_map(ppBuf, pulLen, pInt->fpMalloc, pInt->fpFree, pInt->pUserMem, pMap);
	}

	return retVal;
}

HX_RESULT ra_depacki_unpack_multistream_hdr(ra_depack_internal*  pInt, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt) {
		retVal = rm_unpack_multistream_hdr(ppBuf, pulLen,
						   pInt->fpMalloc, pInt->fpFree, pInt->pUserMem, &pInt->multiStreamHdr);
	}
                                        ...
                                        UINT32              ulLen)
{
	...
	UINT32	i;
	//UINT32	addr;
	UINT32	ra8lbr_major_version;
	UINT32	ra8lbr_minor_version;
	UINT32	num_samples;
	UINT32	num_regions;
	//UINT32	delay;
	UINT32	cplStart;
	...
	UINT32	body_type;
    HX_RESULT retVal = HXR_FAIL;
	input_info_header	*para	=	(input_info_header*)AUDIO_USED_SDRAM_PARA_ADDRESS;
	input_info_header_aac* para_aac = (input_info_header_aac*)AUDIO_USED_SDRAM_PARA_ADDRESS;
    ...
    if (pInt && pBuf && ulLen >= 4)
    {
        ...
        */
        ...
        pInt->pSubStreamHdr = (ra_substream_hdr*) ra_depacki_av_malloc(pInt, ulSize);
        if (pInt->pSubStreamHdr)
        {
            ...
            for (i = 0; i < pInt->multiStreamHdr.ulNumSubStreams && retVal == HXR_OK; i++)
            {
                ...
                if (ulLen >= ulSize)
                {
                    /* Now unpack an substream header*///huanglei for test
                    ...
                    if (retVal == HXR_OK)
                    {
                        ...
                        */
                        ...
                        else
                        {
                            ...
                            pHdr->pIBuffer = (BYTE*) ra_depacki_av_malloc(pInt, pHdr->ulSuperBlockSize);
                            if (pHdr->pIBuffer)
                            {
                                /* Zero out the buffer*/
                                memset(pHdr->pIBuffer, 0, pHdr->ulSuperBlockSize);
                                /* Allocate the de-interleaved buffer*/
                                pHdr->pDBuffer = (BYTE*) ra_depacki_av_malloc(pInt, pHdr->ulSuperBlockSize);
                                if (pHdr->pDBuffer)
                                {
                                    /* Zero out the buffer*/
                                    ...
                                    pHdr->pIPresentFlags = (UINT32*) ra_depacki_av_malloc(pInt, ulTmp);
                                    if (pHdr->pIPresentFlags)
                                    {
                                        /* Zero out the flags*/
                                        memset(pHdr->pIPresentFlags, 0, ulTmp);
                                        /* Allocate the de-interleaved flags*/
                                        pHdr->pDPresentFlags = (UINT32*) ra_depacki_av_malloc(pInt, ulTmp);
	...
	//输出音频参数"huanglei"
	if(pInt->pSubStreamHdr->ulCodecID == 0x636F6F6B)//cook
	{
		is_aac = 0;
		is_cook =1;
		para->ulSuperBlockSize = Endian_B2L(pInt->pSubStreamHdr->ulSuperBlockSize);
		para->dBlockDuration_inte = Endian_B2L((UINT32)pInt->pSubStreamHdr->dBlockDuration);
		para->dBlockDuration_frac = Endian_B2L((UINT32)((pInt->pSubStreamHdr->dBlockDuration - pInt->pSubStreamHdr->dBlockDuration/1)*1024*1024));
		para->ulInterleaveFactor = Endian_B2L(pInt->pSubStreamHdr->ulInterleaveFactor);
		para->ulInterleaveBlockSize = Endian_B2L(pInt->pSubStreamHdr->ulInterleaveBlockSize);
		para->ulInterleaverID = Endian_B2L(pInt->pSubStreamHdr->ulInterleaverID);
		para->ulNumCodecFrames = Endian_B2L(pInt->pSubStreamHdr->ulNumCodecFrames);
		para->ulCodecFrameSize = Endian_B2L(pInt->pSubStreamHdr->ulCodecFrameSize);
		para->rule2Flag[0] = Endian_B2L(pInt->rule2Flag.pulMap[0]);
		para->rule2Flag[1] =Endian_B2L(pInt->rule2Flag.pulMap[1]);
		for(i=0;i<pInt->pSubStreamHdr->ulNumCodecFrames;i++)
		{
			para->pulGENRPattern[i] = Endian_B2L(pInt->pSubStreamHdr->pulGENRPattern[i]);
		}
		for(i=0;i<pInt->pSubStreamHdr->ulNumCodecFrames;i++)
		{
			para->pulGENRBlockNum[i] = Endian_B2L(pInt->pSubStreamHdr->pulGENRBlockNum[i]);
		}
		for(i=0;i<pInt->pSubStreamHdr->ulNumCodecFrames;i++)
		{
			para->pulGENRBlockOffset[i] = Endian_B2L(pInt->pSubStreamHdr->pulGENRBlockOffset[i]);
		}
		para->sampRate	=	Endian_B2L(pInt->pSubStreamHdr->ulSampleRate);
		para->nChannels	=	Endian_B2L(pInt->pSubStreamHdr->ulChannels);
		if( pInt->pSubStreamHdr->pOpaqueData != NULL)
		{
			ra8lbr_major_version = pInt->pSubStreamHdr->pOpaqueData[0];
			ra8lbr_minor_version = pInt->pSubStreamHdr->pOpaqueData[3];
			num_samples = ((pInt->pSubStreamHdr->pOpaqueData[4]<<8) & 0xff00) | pInt->pSubStreamHdr->pOpaqueData[5];
			num_regions  = ((pInt->pSubStreamHdr->pOpaqueData[6]<<8) & 0xff00) | pInt->pSubStreamHdr->pOpaqueData[7];
			if((ra8lbr_major_version>=1) && (ra8lbr_minor_version>=3))
			{
				cplStart = ((pInt->pSubStreamHdr->pOpaqueData[12]<<8) & 0xff00) | pInt->pSubStreamHdr->pOpaqueData[13];
				cplQBits = ((pInt->pSubStreamHdr->pOpaqueData[14]<<8) & 0xff00) | pInt->pSubStreamHdr->pOpaqueData[15];
			}
			else
			{
				cplStart = 0;
			...
			para->nSamples  =	Endian_B2L(num_samples);
			para->nRegions	=	Endian_B2L(num_regions);
			para->cplStart	=	Endian_B2L(cplStart);
			para->cplQbits	=	Endian_B2L(cplQBits);
		}

	}
	else if((pInt->pSubStreamHdr->ulCodecID == 0x72616163)||(pInt->pSubStreamHdr->ulCodecID == 0x72616370))//raac
	{
		is_aac = 1;
		is_cook =0;
		para_aac->dBlockDuration_inte = Endian_B2L((UINT32)pInt->pSubStreamHdr->dBlockDuration);
		para_aac->dBlockDuration_frac = Endian_B2L((UINT32)((pInt->pSubStreamHdr->dBlockDuration - pInt->pSubStreamHdr->dBlockDuration/1)*1024*1024));

		if( pInt->pSubStreamHdr->pOpaqueData != NULL)
		{
			pData = (UCHAR* )pInt->pSubStreamHdr->pOpaqueData;
			body_type = pInt->pSubStreamHdr->pOpaqueData[0];
			inBuf = pData + 1;
			numBytes = pInt->pSubStreamHdr->ulOpaqueDataSize - 1;
			para_aac->body_type	= Endian_B2L(body_type);
			if (body_type == 2) /* MP4 Audio Specific Config Data*/
		    	{
		        	numBits = numBytes*8;

		        	if (newBitstream(&pBs, numBits, pInt->pUserMem, pInt->fpMalloc))
		            	return HXR_FAIL;

		       	feedBitstream(pBs, (const UCHAR* )inBuf, numBits);
		        	setAtBitstream(pBs, 0, 1);
		       	result = ga_config_get_data(pBs, &configData);
		        	deleteBitstream(pBs, pInt->pUserMem, pInt->fpFree);
		       	if (result != HXR_OK) /* config data error*/
		        	...
		        	frameInfo.nChans = configData.numChannels;
		        	frameInfo.sampRateCore =  configData.samplingFrequency;
				...
				para_aac->nChans = Endian_B2L(frameInfo.nChans);
				para_aac->sampRateCore = Endian_B2L(frameInfo.sampRateCore);
				para_aac->profile = Endian_B2L(frameInfo.profile);
		   	 }
			else
			{
				para_aac->nChans = 0;
				para_aac->sampRateCore = 0;
				para_aac->profile = 0;
			}
		}
				...
				if (ulLen >= ulSize) {
					...
					if (retVal == HXR_OK) {
						...
						*/
						...
						if (pHdr->ulCodecFrameSize) {
							pHdr->ulNumCodecFrames = pHdr->ulInterleaveBlockSize*
							    pHdr->ulInterleaveFactor / pHdr->ulCodecFrameSize;
						}
						/* Compute the ms per block*/
						if (pHdr->ulBytesPerMin) {
							pHdr->dBlockDuration = ((double)pHdr->ulInterleaveBlockSize)*
							    60000.0 / ((double)pHdr->ulBytesPerMin);
						}
						/* Compute the superblock size and time*/
						pHdr->ulSuperBlockSize = pHdr->ulInterleaveBlockSize*
						    pHdr->ulInterleaveFactor;
						...
						if (pHdr->bIsVBR) {
							...
							*/
							...
							/* Allocate the interleaved buffer*/
							pHdr->pIBuffer =
							    (BYTE* ) ra_depacki_malloc(pInt, pHdr->ulSuperBlockSize);
							if (pHdr->pIBuffer) {
								/* Zero out the buffer*/
								memset(pHdr->pIBuffer, 0, pHdr->ulSuperBlockSize);
								/* Allocate the de-interleaved buffer*/
								pHdr->pDBuffer =
								    (BYTE* ) ra_depacki_malloc(pInt,
											       pHdr->ulSuperBlockSize);
								if (pHdr->pDBuffer) {
									/* Zero out the buffer*/
									memset(pHdr->pDBuffer, 0,
									       pHdr->ulSuperBlockSize);
									/* Allocate the interleaved flags*/
									ulTmp =
									    pHdr->ulInterleaveFactor*  sizeof(UINT32);
									pHdr->pIPresentFlags =
									    (UINT32* ) ra_depacki_malloc(pInt, ulTmp);
									if (pHdr->pIPresentFlags) {
										/* Zero out the flags*/
										memset(pHdr->pIPresentFlags, 0, ulTmp);
										/* Allocate the de-interleaved flags*/
										pHdr->pDPresentFlags =
										    (UINT32* ) ra_depacki_malloc(pInt,
														 ulTmp);
										if (pHdr->pDPresentFlags) {
											/* Null out the memory*/
											memset(pHdr->pDPresentFlags, 0,
											       ulTmp);
											/* Clear the return value*/
											retVal = HXR_OK;
											/* If this is GENR interleaving, then init the tables*/
											if (pHdr->ulInterleaverID ==
											    RA_INTERLEAVER_GENR) {
												retVal =
												    ra_depacki_init_genr
												    (pInt, i);
											}
										}
									}
...
HX_RESULT ra_depacki_unpack_substream_hdr(ra_depack_internal*  pInt, BYTE * pBuf, UINT32 ulLen, ra_substream_hdr * pHdr)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT ra_depacki_unpack_raformat3(ra_depack_internal*  pInt, BYTE * pBuf, UINT32 ulLen, ra_substream_hdr * pHdr)
{
	return HXR_NOTIMPL;
}

HX_RESULT ra_depacki_unpack_raformat4(ra_depack_internal*  pInt, BYTE * pBuf, UINT32 ulLen, ra_substream_hdr * pHdr)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pBuf && ulLen >= 63 && pHdr) {
		...
		/* Sanity check*/
		if (pHdr->usRAFormatVersion == 4 && pHdr->usRAFormatRevision == 0) {
...
HX_RESULT ra_depacki_unpack_raformat5(ra_depack_internal*  pInt, BYTE * pBuf, UINT32 ulLen, ra_substream_hdr * pHdr)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pBuf && ulLen >= 68 && pHdr) {
		...
		pHdr->usRAFormatRevision = rm_unpack16(&pBuf, &ulLen);
		/* Sanity check*/
		if (pHdr->usRAFormatVersion == 5 && pHdr->usRAFormatRevision == 0) {
			pHdr->usHeaderBytes = rm_unpack16(&pBuf, &ulLen);
			pHdr->usFlavorIndex = rm_unpack16(&pBuf, &ulLen);
			pHdr->ulGranularity = rm_unpack32(&pBuf, &ulLen);
			pHdr->ulTotalBytes = rm_unpack32(&pBuf, &ulLen);
			...
			pHdr->ulNumCodecFrames = pHdr->ulInterleaveFactor*
			    pHdr->ulInterleaveBlockSize / pHdr->ulCodecFrameSize;
			/* Clear the return value*/
			retVal = HXR_OK;
			/*
			...
			*/
			if (pHdr->ucScatterType) {
				...
				pHdr->pulInterleavePattern = ra_depacki_av_malloc(pInt, ulSize);
			...
			if (retVal == HXR_OK) {
				...
				if (ulLen >= 4) {
					...
					if (ulLen >= pHdr->ulOpaqueDataSize) {
						/* Allocate the buffer*/
						pHdr->pOpaqueData = ra_depacki_av_malloc(pInt, pHdr->ulOpaqueDataSize);
						if (pHdr->pOpaqueData) {
...
HX_RESULT ra_depacki_get_format_info(ra_depack_internal*  pInt, UINT32 ulSubStream, ra_format_info * pInfo)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pInfo && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		/* Init local variables*/
		...
		if (pHdr) {
			...
			pInfo->pOpaqueData = copy_buffer(pInt->pUserMem,
							 pInt->fpMalloc, pHdr->pOpaqueData, pHdr->ulOpaqueDataSize);
...
UINT32 ra_depacki_rule_to_flags(ra_depack_internal*  pInt, UINT32 ulRule)
{
	...
	if (pInt && pInt->rule2Flag.pulMap && ulRule < pInt->rule2Flag.ulNumRules) {
	...
	if (pInt && pPacket) {
		...
		if (!pPacket->ucLost) {
			...
			if (pInt->bStreamSwitchable) {
				...
				*/
				if (pInt->pSubStreamHdr && pInt->multiStreamHdr.ulNumSubStreams) {
...
HX_RESULT ra_depacki_add_vbr_packet(ra_depack_internal*  pInt, UINT32 ulSubStream, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT ra_depacki_add_non_vbr_packet(ra_depack_internal*  pInt, UINT32 ulSubStream, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pPacket && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		/* Init local variables*/
		HXBOOL bKey = FALSE;
			...
			if (pHdr->bHasKeyTime && ulPacketTime >= pHdr->ulKeyTime) {
				...
				*/
				if (!pHdr->bKnowIfAdjustNeeded) {
					...
					*/
					if (pHdr->bHasLastPacket && !pHdr->lastPacket.ucLost && !bKey) {
				...
				if (pHdr->bKnowIfAdjustNeeded && pHdr->bAdjustTimestamps && !bKey) {
				...
				if (ulBlockNum < pHdr->ulInterleaveFactor &&
					...
					memcpy(pHdr->pIBuffer + ulBlockNum*  pHdr->ulInterleaveBlockSize,
					       pPacket->pData, pHdr->ulInterleaveBlockSize);
...
HX_RESULT ra_depacki_parse_vbr_packet(ra_depack_internal*  pInt,
				      rm_packet*  pPacket,
				      UINT32*  pulNumAU,
				      HXBOOL*  pbFragmented, UINT32 * pulAUSize, UINT32 * pulAUFragSize)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pPacket && pulNumAU && pbFragmented &&
	    pulAUSize && pulAUFragSize && pPacket->pData && pPacket->usDataLen) {
		/* Init local variables*/
					    ...
					    UINT32 ulSubStream, UINT32 ulFirstStartTime, UINT32 ulLastEndTime)
{
	...
	if (pInt && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams && ulLastEndTime > ulFirstStartTime) {
		...
		for (i = 0; i < ulNumLossPackets && HX_SUCCEEDED(retVal); i++) {
			...
			retVal = ra_depacki_send_block(pInt, ulSubStream, HXNULL, 0, ulTime, 0);	/* Flags of 0 indicate loss*/
		}
	}
				...
				UINT32 ulSubStream, BYTE*  pBuf, UINT32 ulLen, UINT32 ulTime, UINT32 ulFlags)
{
	...
	if (pInt && pInt->fpAvail) {
		...
		ra_block* pBlock = ra_depacki_av_malloc(pInt, sizeof(ra_block));
		if (pBlock) {
			/* Zero out the memory*/
			...
			/* Copy the buffer*/
			if (pBuf && ulLen) {
				pBlock->pData = copy_buffer(pInt->pUserMem, pInt->fpMalloc, pBuf, ulLen);
				if (pBlock->pData) {
					...
					UINT32 ulSubStream, rm_packet*  pPacket, UINT32 ulAUSize, UINT32 ulAUFragSize)
{
	...
	if (pInt && pInt->pSubStreamHdr && pPacket && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		UINT32 ulPacketSize = (UINT32) pPacket->usDataLen;
		UINT32 ulPacketOffset = (ulPacketSize >= ulAUFragSize ? ulPacketSize - ulAUFragSize : 0);
		/* Get the substream header*/
		ra_substream_hdr* pHdr = &pInt->pSubStreamHdr[ulSubStream];
		/* Clear the return value*/
		retVal = HXR_OK;
			...
			if (HX_SUCCEEDED(retVal)) {
				...
				if (pHdr->ulFragBufferOffset + ulAUFragSize <= pHdr->ulFragBufferSize &&
					...
					memcpy(pHdr->pFragBuffer + pHdr->ulFragBufferOffset,
					       pPacket->pData + ulPacketOffset, ulAUFragSize);
					...
					if (pHdr->ulFragBufferOffset >= pHdr->ulFragBufferAUSize) {
						...
						retVal = ra_depacki_send_block(pInt, ulSubStream,
									       ...
									       pHdr->ulFragBufferTime, 0xFFFFFFFF);
		...
		*/
		...
		for (i = 0; i < ulNumAU && HX_SUCCEEDED(retVal); i++) {
			...
			if (ulBufOffset + 1 < ulLen) {
				...
				ulAUSize = rm_unpack16_nse(pBuf + ulBufOffset, ulLen - ulBufOffset);
				...
				if (ulBufOffset + ulAUSize <= ulLen) {
					...
					retVal = ra_depacki_send_block(pInt,
								       ...
								       pBuf + ulBufOffset,
								       ulAUSize,
								       pPacket->ulTime + ulTSOffset, 0xFFFFFFFF);
					if (retVal == HXR_OK) {
...
HX_RESULT ra_depacki_init_frag_buffer(ra_depack_internal*  pInt, ra_substream_hdr * pHdr)
{
	...
	if (pInt && pHdr && !pHdr->pFragBuffer) {
		...
		pHdr->pFragBuffer = ra_depacki_av_malloc(pInt, INITIAL_FRAG_BUFFER_SIZE);
		if (pHdr->pFragBuffer) {
			/* Zero out the buffer*/
...
HX_RESULT ra_depacki_resize_frag_buffer(ra_depack_internal*  pInt, ra_substream_hdr * pHdr, UINT32 ulNewSize)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pHdr && pHdr->pFragBuffer) {
		/* Allocate a new buffer*/
		BYTE* pNewBuf = ra_depacki_av_malloc(pInt, ulNewSize);
		if (pNewBuf) {
			/* Copy the old buffer*/
			...
			memset(pNewBuf + pHdr->ulFragBufferOffset, 0, ulNewSize - pHdr->ulFragBufferOffset);
			/* Free the old buffer*/
...
HX_RESULT ra_depacki_deinterleave_send(ra_depack_internal*  pInt, UINT32 ulSubStream)
{
	...
	if (pInt && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams && pInt->fpAvail) {
		/* Init local variables*/
		UINT32 i = 0;
		...
		if (retVal == HXR_OK) {
			...
			for (i = 0; i < pHdr->ulBlockCount && HX_SUCCEEDED(retVal); i++) {
				...
				if (ulTimestamp <= pInt->ulStreamDuration) {
					...
					pBlock = (ra_block* ) ra_depacki_av_malloc(pInt, sizeof(ra_block));
					if (pBlock) {
						/* Alloc and copy the buffer*/
						pBlock->pData = copy_buffer(pInt->pUserMem,
									    pInt->fpMalloc,
									    pHdr->pDBuffer +
									    i*  pHdr->ulInterleaveBlockSize,
...
HX_RESULT ra_depacki_deinterleave(ra_depack_internal*  pInt, UINT32 ulSubStream)
{
	...
	if (pInt && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
...
HX_RESULT ra_depacki_deinterleave_sipr(ra_depack_internal*  pInt, UINT32 ulSubStream)
{
	...
	if (pInt && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		...
		ra_substream_hdr* pHdr = &pInt->pSubStreamHdr[ulSubStream];
		/* Sanity check*/
		if (pHdr->pIBuffer && pHdr->pDBuffer && pHdr->pIPresentFlags && pHdr->pDPresentFlags) {
			/*
			...
			*/
			...
			/* Copy the flags*/
			memcpy(pHdr->pDPresentFlags, pHdr->pIPresentFlags, pHdr->ulInterleaveFactor*  sizeof(UINT32));
			/* Do the de-interleave*/
			RASL_DeInterleave((char* )pHdr->pDBuffer,
					  pHdr->ulSuperBlockSize, pHdr->usFlavorIndex, pHdr->pDPresentFlags);
			/* Clear the return value*/
			retVal = HXR_OK;
		}
	}
...
HX_RESULT ra_depacki_init_genr(ra_depack_internal*  pInt, UINT32 ulSubStream)
{
	...
	if (pInt && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		...
		if (pHdr->ulCodecFrameSize && pHdr->ulNumCodecFrames) {
			...
			ulSize = pHdr->ulNumCodecFrames*  sizeof(UINT32);
			pHdr->pulGENRBlockNum = (UINT32* ) ra_depacki_av_malloc(pInt, ulSize);
			if (pHdr->pulGENRBlockNum) {
				pHdr->pulGENRBlockOffset = (UINT32* ) ra_depacki_av_malloc(pInt, ulSize);
				if (pHdr->pulGENRBlockOffset) {
					pHdr->pulGENRPattern = (UINT32* ) ra_depacki_av_malloc(pInt, ulSize);
					if (pHdr->pulGENRPattern) {
						/* Initialize the block num and offset arrays*/
						for (ulBlockIndx = 0; ulBlockIndx < pHdr->ulInterleaveFactor;
						     ulBlockIndx++) {
							for (ulFrameIndx = 0; ulFrameIndx < ulFramesPerBlock;
							     ulFrameIndx++) {
						...
						if (pHdr->pulInterleavePattern) {
							...
							memcpy(pHdr->pulGENRPattern,
							       pHdr->pulInterleavePattern, ulSize);
							/*
							...
							*/
							if (pHdr->ulInterleaveFactor == 1) {
								...
								while (ulCount < pHdr->ulNumCodecFrames) {
									pHdr->pulGENRPattern[ulCount] =
									    ulBlockIndx*  ulFramesPerBlock +
									    ulFrameIndx;
									ulCount++;
									ulBlockIndx += 2;
...
HX_RESULT ra_depacki_deinterleave_genr(ra_depack_internal*  pInt, UINT32 ulSubStream)
{
	...
	if (pInt && pInt->pSubStreamHdr && ulSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
		...
		if (pHdr->pIBuffer && pHdr->pDBuffer && pHdr->pIPresentFlags && pHdr->pDPresentFlags) {
			...
			memset(pHdr->pDPresentFlags, 0xFF, pHdr->ulInterleaveFactor*  sizeof(UINT32));
			...
			for (fo = 0; fo < pHdr->ulNumCodecFrames; fo++) {
				...
				memcpy(pHdr->pDBuffer + fo*  pHdr->ulCodecFrameSize,
				       pHdr->pIBuffer + fi*  pHdr->ulCodecFrameSize, pHdr->ulCodecFrameSize);
