		...
		for (fi = 0; fi < RASL_NFRAMES*  RASL_NBLOCKS; fi++) {
			...
			*/
			if (fo > fi) {	/* do the swap if needed*/
				...
				ra_bitcopy((unsigned char* )toPtr, ulToBufSize, (unsigned char *)fromPtr, ulFromBufSize,
					   bitOffsetTo, bitOffsetFrom, nCodeBits);
...
void ra_bitcopy(unsigned char* toPtr,
		unsigned long ulToBufSize,
		unsigned char* fromPtr, unsigned long ulFromBufSize, int bitOffsetTo, int bitOffsetFrom, int numBits)
{
	unsigned char* pToLimit = toPtr + ulToBufSize;

	int bofMod8, botMod8, nbMod8, eightMinusBotMod8, eightMinusBofMod8, i, iMax;
	unsigned char rightInword, leftInword,* byteOffsetFrom, *byteOffsetTo, alignWord, endWord;
...

	{
		...
		case 1:
			...
			{	// shift bits as necessary
				*byteOffsetTo = (*byteOffsetTo & mask[0]) + ((*byteOffsetFrom & mask[0]) << 4);
