/** **** BEGIN LICENSE BLOCK *****
*  Source last modified: $Id: ra_depack.c,v 1.1.1.1.2.1 2005/05/04 18:21:33 hubbe Exp $
*
...
ra_depack* ra_depack_create(void *pAvail, ra_block_avail_func_ptr fpAvail, void *pUserError, rm_error_func_ptr fpError)
{
	return ra_depack_create2(pAvail,
				 fpAvail,
				 pUserError, fpError, HXNULL, rm_memory_default_av_malloc, rm_memory_default_free);
}
...
ra_depack* ra_depack_create2(void *pAvail,
			     ...
			     void* pUserMem, rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree)
{
...
HX_RESULT ra_depack_get_codec_init_info(ra_depack*  pDepack, UINT32 ulSubStream, ra_format_info ** ppInfo)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pDepack && ppInfo) {
		...
		pInfo = ra_depacki_av_malloc(pInt, ulSize);
		if (pInfo) {
			/* NULL out the memory*/
