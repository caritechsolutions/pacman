...
void* rv_depacki_av_malloc(rv_depack_internal * pInt, UINT32 ulSize)
{
	void* pRet = HXNULL;
...
HX_RESULT rv_depacki_unpack_rule_map(rv_depack_internal*  pInt, rm_rule_map * pMap, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rv_depacki_unpack_multistream_hdr(rv_depack_internal*  pInt, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rv_depacki_unpack_opaque_data(rv_depack_internal*  pInt, BYTE * pBuf, UINT32 ulLen)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pBuf && ulLen >= 4) {
		...
		*/
		...
		pInt->pSubStreamHdr = (rv_format_info* ) rv_depacki_av_malloc(pInt, ulSize);
		if (pInt->pSubStreamHdr) {
			...
			for (i = 0; i < pInt->multiStreamHdr.ulNumSubStreams && retVal == HXR_OK; i++) {
				...
				retVal = rv_depacki_unpack_format_info(pInt, &pInt->pSubStreamHdr[i], &pBuf, &ulLen);
			}
		}
	}
...
void rv_depacki_cleanup_format_info(rv_depack_internal*  pInt, rv_format_info * pInfo)
{
...
HX_RESULT rv_depacki_unpack_format_info(rv_depack_internal*  pInt,
					rv_format_info*  pInfo, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pInfo && ppBuf &&* ppBuf && pulLen && *pulLen >= 26) {
		...
		if (*pulLen >= pInfo->ulOpaqueDataSize) {
			...
			pInfo->pOpaqueData = (BYTE* ) rv_depacki_av_malloc(pInt, pInfo->ulOpaqueDataSize);
	...
	if (pInt && hdr) {
		...
		if (pInt->bStreamSwitchable) {
			...
			if (retVal == HXR_OK) {
				...
				pInt->bIgnoreSubStream = (HXBOOL* ) rv_depacki_av_malloc(pInt, ulSize);
				if (pInt->bIgnoreSubStream) {
					...
					pTmp = (HXBOOL* ) rv_depacki_av_malloc(pInt, ulSize);
					if (pTmp) {
						/* NULL out the array*/
...
HX_RESULT rv_depacki_copy_format_info(rv_depack_internal*  pInt, rv_format_info * pSrc, rv_format_info * pDst)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pSrc && pDst) {
		...
		pDst->pOpaqueData = copy_buffer(pInt->pUserMem,
						pInt->fpMalloc, pSrc->pOpaqueData, pSrc->ulOpaqueDataSize);
		if (pDst->pOpaqueData) {
			/* Clear the return value*/
			retVal = HXR_OK;
		}
...
HX_RESULT rv_depacki_add_packet(rv_depack_internal*  pInt, rm_packet * pPacket)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pPacket) {
		...
		if (!pInt->bStreamSwitchable || !pInt->bIgnoreSubStream[ulSubStream]) {
			...
			while (ulLen && retVal == HXR_OK) {
				...
				if (retVal == HXR_OK) {
					...
					case RVFrameTypeWhole:
						retVal =
						    rv_depacki_handle_one_frame(pInt, &pBuf, &ulLen, pPacket, &hdr);
						break;
					}
				}
			}
...
UINT32 rv_depacki_rule_to_substream(rv_depack_internal*  pInt, UINT32 ulRule)
{
	...
	if (pInt &&
	    pInt->multiStreamHdr.rule2SubStream.pulMap && ulRule < pInt->multiStreamHdr.rule2SubStream.ulNumRules) {
...
HX_RESULT rv_depacki_parse_frame_header(rv_depack_internal*  pInt,
					BYTE* * ppBuf, UINT32 * pulLen, rm_packet * pPacket, rv_frame_hdr * pFrameHdr)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && ppBuf && pulLen && pFrameHdr &&* pulLen >= 2 &&
	   * ppBuf >= pPacket->pData && *ppBuf < pPacket->pData + pPacket->usDataLen) {
		/* Initialize local variables*/
		UINT32 ulTmp = 0;
		HXBOOL bTmp = FALSE;
		...
		case RVFrameTypeLastPartial:
			...
			retVal = rv_depacki_read_14_or_30(ppBuf, pulLen,
							  &pFrameHdr->bBrokenUpByUs, &pFrameHdr->ulFrameSize);
			if (retVal == HXR_OK) {
					...
					if (*pulLen >= 1) {
						...
						if (pFrameHdr->eType == RVFrameTypePartial) {
							...
							*/
							...
							pFrameHdr->ulPartialFrameOffset =
							    pFrameHdr->ulFrameSize - ulTmp;
						}
					} else {
						retVal = HXR_FAIL;
					}
		...
		case RVFrameTypeWhole:
			...
			pFrameHdr->ulFrameSize = ((UINT32) pPacket->usDataLen) - pFrameHdr->ulHeaderSize;
			pFrameHdr->ulTimestamp = pPacket->ulTime;
		...
		case RVFrameTypeMultiple:
			...
			*/
			...
			retVal = rv_depacki_read_14_or_30(ppBuf, pulLen, &bTmp, &pFrameHdr->ulFrameSize);
			if (retVal == HXR_OK) {
				/* For multiple frames, there is no partial size or offset*/
				pFrameHdr->ulPartialFrameOffset = 0;
				pFrameHdr->ulPartialFrameSize = 0;
				/* Unpack the timestamp*/
				retVal = rv_depacki_read_14_or_30(ppBuf, pulLen, &bTmp, &pFrameHdr->ulTimestamp);
				if (retVal == HXR_OK) {
					...
					if (pInt->bHasRelativeTimeStamps) {
						...
						*/
						if (ulOrigPacketTimeStamp > RM_MAX_UINT14 &&
						    ...
						    */
						    !(((RM_MAX_UINT30 - ulOrigPacketTimeStamp) +
						       pFrameHdr->ulTimestamp) <= 60000UL)) {
						...
						*/
						if (pFrameHdr->ulTimestamp - pPacket->ulTime >
						    MAX_INTERNAL_TIMESTAMP_DELTA) {
							pFrameHdr->ulTimestamp = pPacket->ulTime;
						}
					}
...
HX_RESULT rv_depacki_read_14_or_30(BYTE* * ppBuf, UINT32 * pulLen, HXBOOL * pbHiBit, UINT32 * pulValue)
{
	HX_RESULT retVal = HXR_FAIL;
				    ...
				    BYTE* * ppBuf, UINT32 * pulLen, rm_packet * pPacket, rv_frame_hdr * pFrameHdr)
{
	...
	if (pInt && ppBuf && pulLen && pPacket && pFrameHdr &&* ppBuf) {
		...
		*/
		if (pInt->pCurFrame && pInt->pCurFrame->ulTimestamp != pPacket->ulTime) {
		...
		if (retVal == HXR_OK && !pInt->pCurFrame) {
			...
			retVal = rv_depacki_create_frame(pInt, pPacket, pFrameHdr, &pInt->pCurFrame);
		}
		if (retVal == HXR_OK) {
			/* Set the return value*/
			retVal = HXR_FAIL;
			   ...
			   * pulLen >= pFrameHdr->ulPartialFrameSize &&
				...
				*/
				for (i = pFrameHdr->ulPacketNum;
				     ...
				     !pInt->pCurFrame->pSegment[i].bIsValid; i++) {
		...
		if (retVal == HXR_OK) {
			...
			if (retVal == HXR_OK) {
				...
				if (*pulLen >= pInt->pCurFrame->ulDataLen) {
					/* Copy the data*/
					memcpy(pInt->pCurFrame->pData,* ppBuf, pInt->pCurFrame->ulDataLen);
	...
	if (pInt && pInt->fpAvail) {
		...
		if (pInt->pCurFrame) {
			...
			if (bDoSend) {
				...
				retVal = pInt->fpAvail(pInt->pAvail, pInt->ulActiveSubStream, pInt->pCurFrame);
...
void rv_depacki_cleanup_frame(rv_depack_internal*  pInt, rv_frame ** ppFrame)
{
...
HX_RESULT rv_depacki_create_frame(rv_depack_internal*  pInt,
				  rm_packet*  pPacket, rv_frame_hdr * pFrameHdr, rv_frame ** ppFrame)
{
	HX_RESULT retVal = HXR_FAIL;

	if (pInt && pPacket && pFrameHdr && ppFrame && pFrameHdr->ulFrameSize && pFrameHdr->ulNumPackets) {
		/* Init local variables*/
		...
		pFrame = rv_depacki_av_malloc(pInt, sizeof(rv_frame));
		if (pFrame) {
			/* NULL out the memory*/
			memset(pFrame, 0, sizeof(rv_frame));
			/* Allocate enough space for the data*/
			pFrame->pData = rv_depacki_av_malloc(pInt, pFrameHdr->ulFrameSize);
			if (pFrame->pData) {
				/* NULL out the memory*/
				...
				pFrame->pSegment = rv_depacki_av_malloc(pInt, ulSize);
				if (pFrame->pSegment) {
					/*
					...
					*/
					if (pFrameHdr->eType == RVFrameTypeMultiple && pFrameHdr->ulHeaderOffset) {
					...
					if (pInt->bCreatedFirstFrame) {
						...
						*/
						if (pFrameHdr->ulSeqNum < pInt->ulLastSeqNumIn) {
							/* We rolled over*/
							pInt->ulLastSeqNumOut +=
							    pFrameHdr->ulSeqNum + 256 - pInt->ulLastSeqNumIn;
						} else {
							pInt->ulLastSeqNumOut +=
							    pFrameHdr->ulSeqNum - pInt->ulLastSeqNumIn;
						}
