...
rv_depack* rv_depack_create(void *pAvail, rv_frame_avail_func_ptr fpAvail, void *pUserError, rm_error_func_ptr fpError)
{
...
UINT32 rv_depack_get_codec_4cc(rv_depack*  pDepack)
{
	...
	if (pDepack) {
		/* Get the internal struct*/
		rv_depack_internal* pInt = (rv_depack_internal *) pDepack;
		if (pInt->pSubStreamHdr && pInt->ulActiveSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
			/* Copy the subMOF tag*/
...
HX_RESULT rv_depack_get_codec_init_info(rv_depack*  pDepack, rv_format_info ** ppInfo)
{
	...
	if (pDepack && ppInfo) {
		...
		if (pInt->pSubStreamHdr && pInt->ulActiveSubStream < pInt->multiStreamHdr.ulNumSubStreams) {
			...
			*ppInfo = rv_depacki_av_malloc(pInt, sizeof(rv_format_info));
			if (*ppInfo) {
				/* NULL out the memory*/
