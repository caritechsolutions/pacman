...
typedef struct
{
	int ulSuperBlockSize;
    int dBlockDuration_inte;
    int dBlockDuration_frac;
	int ulInterleaveFactor;
	int ulInterleaveBlockSize;
	int ulInterleaverID;
	int ulNumCodecFrames;
	int ulCodecFrameSize;
	int rule2Flag[2];	//判断keyframe的映射表，长度为2
	int pulGENRPattern[100];	//一个superblock最大有100帧
	int pulGENRBlockNum[100];
	int pulGENRBlockOffset[100];
	int nSamples;
	int sampRate;
	int nRegions;
	int nChannels;
	int cplStart;
	int cplQbits;
} input_info_header;		//从外面传递来的header信息
...
typedef struct _DecodeControlINFO
{
   unsigned int audio_or32_clk;
   int sync_frame;
   int dec_volume;
   void* info_header;
