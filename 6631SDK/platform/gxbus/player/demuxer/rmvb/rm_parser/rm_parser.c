...
rm_parser* rm_parser_create(void *pError, rm_error_func_ptr fpError)
{
	return rm_parser_create2(pError, fpError, HXNULL, rm_memory_default_av_malloc, rm_memory_default_free);
}

rm_parser* rm_parser_create2(void *pError,
			     rm_error_func_ptr fpError,
			     void* pMem, rm_av_malloc_func_ptr fpMalloc, rm_free_func_ptr fpFree)
{
	rm_parser* pRet = HXNULL;

	if (fpMalloc) {
		/* Allocate space for the rm_parser_internal struct
		*  by using the passed-in av_malloc function
		*/
...
HX_RESULT rm_parser_init_stdio(rm_parser*  pParser, FILE * fp)
{
...
HX_RESULT rm_parser_init_io(rm_parser*  pParser, void *pUserRead, rm_read_func_ptr fpRead, rm_seek_func_ptr fpSeek)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parser_get_file_properties(rm_parser*  pParser, rm_property ** ppProp, UINT32 * pulNumProps)
{
	HX_RESULT retVal = HXR_FAIL;
...
void rm_parser_destroy_properties(rm_parser*  pParser, rm_property ** ppProp, UINT32 * pulNumProps)
{
...
HX_RESULT rm_parser_get_stream_header(rm_parser*  pParser, UINT32 ulStreamNum, rm_stream_header ** ppHdr)
{
	HX_RESULT retVal = HXR_FAIL;
...
void rm_parser_destroy_stream_header(rm_parser*  pParser, rm_stream_header ** ppHdr)
{
...
HX_RESULT rm_parser_get_packet(rm_parser*  pParser, rm_packet ** ppPacket)
{
	HX_RESULT retVal = HXR_FAIL;
...
void rm_parser_destroy_packet(rm_parser*  pParser, rm_packet ** ppPacket)
{
...
HX_RESULT rm_parser_seek(rm_parser*  pParser, UINT32 ulTime)
{
	HX_RESULT retVal = HXR_FAIL;
