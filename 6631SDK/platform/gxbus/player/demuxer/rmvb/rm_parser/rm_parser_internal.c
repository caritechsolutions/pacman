...
static const char* g_pszRuleBook[] = {
	...
	"Marker=0,AverageBandwidth=%lu,TimeStampDelivery=TRUE;" "Marker=1,AverageBandwidth=0,TimeStampDelivery=TRUE;",
	/* Non-real-datatype VBR streams when max_bit_rate == 0 && avg_bit_rate == 0*/
	"Marker=0,TimeStampDelivery=TRUE;" "Marker=1,TimeStampDelivery=TRUE;",
	/* CBR RealVideo*/
	...
	/* RealEvent streams*/
	"TimeStampDelivery=T,priority=10,PNMKeyFrameRule=T;" "TimeStampDelivery=T,priority=10,PNMNonKeyFrameRule=T;",
	...
	"Marker=0,priority=5,averagebandwidth=%lu;" "Marker=1,priority=5,averagebandwidth=0;"
};
		...
		if (ulLen >= 12) {
			...
			if (ulLen >= (UINT32) pInt->contHdr.title_sz + 2) {
				rm_unpack_string(&pBuf, &ulLen, pInt->contHdr.title_sz,
						 &pInt->contHdr.title, pInt->pUserMem, pInt->fpMalloc, pInt->fpFree);
...
HX_RESULT rm_parseri_unpack_media_props_hdr(rm_parser_internal*  pInt, struct rm_media_props_hdr *h)
{
	HX_RESULT retVal = HXR_FAIL;
...
void rm_parseri_cleanup_media_props_hdr(rm_parser_internal*  pInt, struct rm_media_props_hdr *h)
{
	if (pInt && h) {
...
HX_RESULT rm_parseri_unpack_name_value_map(rm_parser_internal*  pInt,
					   struct rm_name_value_map* pMap, BYTE ** ppBuf, UINT32 * pulLen)
{
	HX_RESULT retVal = HXR_FAIL;
...
void rm_parseri_cleanup_name_value_map(rm_parser_internal*  pInt, struct rm_name_value_map *h)
{
	if (pInt && h) {
...
void rm_parseri_cleanup_all_name_value_maps(rm_parser_internal*  pInt, struct rm_logical_stream_hdr *h)
{
	if (pInt && h && h->props) {
...
HX_RESULT rm_parseri_unpack_logical_stream_hdr(rm_parser_internal*  pInt,
					       struct rm_media_props_hdr* mh, struct rm_logical_stream_hdr *h)
{
	HX_RESULT retVal = HXR_FAIL;
		...
		if (ulLen >= 8) {
			...
			if (ulLen >= (UINT32) 6*  h->num_physical_streams + 2) {
				...
				if (ulLen >= (UINT32) h->num_rules*  2 + 2) {
					...
					if (ulSize) {
						h->props = (struct rm_name_value_map* )rm_parseri_av_malloc(pInt, ulSize);
						if (h->props) {
							/* NULL out the memory*/
							...
							for (i = 0; i < h->num_props && retVal == HXR_OK; i++) {
								retVal =
								    rm_parseri_unpack_name_value_map(pInt, &h->props[i],
												     &pBuf, &ulLen);
							}
						}
					} else {
...
HX_RESULT rm_parseri_unpack_all_logical_stream_hdrs(rm_parser_internal*  pInt)
{
	...
	if (pInt) {
		...
		if (ulNumLogical) {
			...
			pInt->pLogicalStreamHdr = (struct rm_logical_stream_hdr* )rm_parseri_av_malloc(pInt, ulSize);
						...
						*/
						...
						if (retVal == HXR_OK) {
							...
							rm_parseri_error(pInt, retVal,
									 "Failed to parse logical stream header.");
						}
					}
				}
			}
		...
		if (retVal == HXR_OK) {
			...
			if (rm_parseri_is_logical_fileinfo_present(pInt, &ulIndx)) {
				...
				pInt->pLogicalFileInfo =
				    (struct rm_logical_stream_hdr* )rm_parseri_av_malloc(pInt, ulSize);
				if (pInt->pLogicalFileInfo) {
					...
					if (retVal != HXR_OK) {
						/* Call back to the error interface*/
						rm_parseri_error(pInt, retVal,
								 "Failed to parse logical-fileinfo header.");
					}
				} else {
...
void rm_parseri_cleanup_logical_stream_hdr(rm_parser_internal*  pInt, struct rm_logical_stream_hdr *h)
{
	if (pInt && h) {
...
HX_RESULT rm_parseri_allocate_media_props_hdrs(rm_parser_internal*  pInt)
{
	...
	if (pInt) {
		...
		*/
		...
		pInt->pMediaPropsHdr = (struct rm_media_props_hdr* )rm_parseri_av_malloc(pInt, ulSize);
...
HXBOOL rm_parseri_is_logical_stream_mime_type(const char* pszMimeType)
{
	...
	if (pszMimeType) {
		if (!rm_parseri_is_logical_fileinfo_mime_type(pszMimeType)) {
			if (!strncmp(pszMimeType, RM_PARSER_LOGICAL_PREFIX, strlen(RM_PARSER_LOGICAL_PREFIX))) {
				bRet = TRUE;
			}
		}
	}
...
HX_RESULT rm_parseri_read_next_packet_header(rm_parser_internal*  pInt, struct rm_pkt_hdr * pPktHdr)
{
	HX_RESULT retVal = HXR_UNEXPECTED;
...
HX_RESULT rm_parseri_setup_interleaved_streams(rm_parser_internal*  pInt)
{
	...
	if (pInt && pInt->pMediaPropsHdr && pInt->ulNumMediaPropsHdrs) {
		...
		HXBOOL* pInterleaved = (HXBOOL *) rm_parseri_av_malloc(pInt, ulSize);
		if (pInterleaved) {
			...
			*/
			...
			if (ulNumInterleaved) {
				...
				pTmp = (struct rm_media_props_hdr* )rm_parseri_av_malloc(pInt, ulSize);
				if (pTmp) {
					...
					*/
					if ((pInt->fileHdr.version == 0 || pInt->fileHdr.version == 1) &&
						...
						*/
						HXBOOL bDone = FALSE;
						UINT32 i = 0;
						for (i = 0; i < pInt->fileHdr.num_headers && !bDone && retVal == HXR_OK;
						     i++) {
							...
							if (retVal == HXR_OK) {
								...
								case RM_PROPERTIES_OBJECT:
									{
										...
										if (retVal == HXR_OK) {
											...
											retVal =
											    rm_parseri_allocate_media_props_hdrs
											    (pInt);
											if (retVal == HXR_OK) {
								...
								case RM_MEDIA_PROPERTIES_OBJECT:
									{
										...
										*/
										UINT32 ulStreamNum =
										    ...
										    (pInt);
										if (ulStreamNum <
										    pInt->ulNumMediaPropsHdrsAlloc) {
											/* Unpack the media properties header*/
											retVal =
											    rm_parseri_unpack_media_props_hdr
											    (pInt,
											     &pInt->pMediaPropsHdr
											     [ulStreamNum]);
											...
											if (retVal == HXR_OK) {
												...
												retVal =
												    HXR_CORRUPT_FILE;
												/* Call back to the error interface*/
												rm_parseri_error(pInt,
														 retVal,
														 "Failed to parse media props header - possibly a corrupt file.");
											}
										} else {
											...
											/* Call back to the error interface*/
											rm_parseri_error(pInt, retVal,
													 "Illegal stream number - possibly a corrupt file.");
										}
									}
									break;
								case RM_CONTENT_OBJECT:
									{
										retVal =
										    rm_parseri_unpack_content_hdr(pInt);
										if (retVal != HXR_OK) {
											/* Call back to the error interface*/
											rm_parseri_error(pInt, retVal,
							...
							if (retVal == HXR_OK) {
								...
								*/
								...
								if (retVal == HXR_OK) {
									...
									retVal =
									    rm_parseri_create_stream_structures(pInt);
									if (retVal == HXR_OK) {
										/*
										...
										*/
										...
										if (retVal == HXR_OK) {
											...
											retVal =
											    rm_parseri_create_all_stream_headers
											    (pInt);
										}
									}
								} else {
...
HX_RESULT rm_parseri_get_file_properties(rm_parser_internal*  pInt, rm_property * pProp, UINT32 ulNumProp)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pProp && ulNumProp) {
		...
		retVal = rm_parseri_set_rm_property(pInt, &pProp[ulIndex++], "StreamCount",
						    RM_PROPERTY_TYPE_UINT32, (void* )pInt->ulNumStreams, 0);

		if (retVal == HXR_OK) {
			retVal = rm_parseri_set_rm_property(pInt, &pProp[ulIndex++], "Flags",
							    RM_PROPERTY_TYPE_UINT32, (void* )pInt->propHdr.flags, 0);
		}
		if (retVal == HXR_OK && rm_parseri_has_real_data_type(pInt)) {
...
HX_RESULT rm_parseri_set_rm_property(rm_parser_internal*  pInt, rm_property * pProp,
				     const char* pszName, UINT32 ulType, void *pValue, UINT32 ulValueLen)
{
	HX_RESULT retVal = HXR_FAIL;
	...
	if (pInt && pProp && pszName) {
		...
		pProp->pName = (char* )rm_parseri_av_malloc(pInt, strlen(pszName) + 1);
		if (pProp->pName) {
			/* Copy the string*/
			...
			case RM_PROPERTY_TYPE_BUFFER:
				{
					pProp->pValue = (BYTE* ) rm_parseri_av_malloc(pInt, ulValueLen);
					if (pProp->pValue) {
						/* Copy the string*/
			...
			case RM_PROPERTY_TYPE_CSTRING:
				{
					...
					pProp->pValue = (BYTE* ) rm_parseri_av_malloc(pInt, ulLen);
					if (pProp->pValue) {
						/* Copy the string*/
...
HX_RESULT rm_parseri_create_stream_structures(rm_parser_internal*  pInt)
{
	...
	if (pInt && pInt->ulNumStreams) {
		...
		*/
		...
		pInt->pulStreamNumMap = (UINT32* ) rm_parseri_av_malloc(pInt, ulSize);
		if (pInt->pulStreamNumMap) {
			...
			*/
			...
			pInt->pStreamInfo = (struct rm_stream_info* )rm_parseri_av_malloc(pInt, ulSize);
			if (pInt->pStreamInfo) {
				/* Zero out the array*/
				...
				for (i = 0; i < pInt->ulNumStreams && retVal == HXR_OK; i++) {
					...
					pInt->pStreamInfo[i].seekTable.pEntry =
					    (struct rm_seek_table_entry* )rm_parseri_av_malloc(pInt, ulSize);
					if (pInt->pStreamInfo[i].seekTable.pEntry) {
						...
						pInt->pStreamInfo[i].seekTable.ulTimeGranularity =
						    ulIndexTimeGranularity;
					...
					*/
					for (i = 0; i < pInt->dataHdr.num_pkts && retVal == HXR_OK; i++) {
						...
						if (retVal == HXR_OK) {
							...
							UINT32 ulStreamNum =
							    rm_parseri_translate_stream_number(pInt, hdr.stream_num);
							if (ulStreamNum != RM_NO_STREAM_SET) {
								/* Save the first timestamp of each stream*/
								if (!pInt->
								    pStreamInfo[ulStreamNum].bSeenFirstPacketTimeStamp)
								{
									pInt->pStreamInfo[ulStreamNum].ulLastTimeStamp =
									    hdr.timestamp;
									pInt->
									    pStreamInfo
									    [ulStreamNum].bSeenFirstPacketTimeStamp =
									    TRUE;
									/* Update the min first timestamp*/
									if (hdr.timestamp < pInt->ulMinFirstPacketTime) {
										pInt->ulMinFirstPacketTime =
										    hdr.timestamp;
									}
								}
								...
								*/
								if (j == pInt->ulNumStreams
								    || pInt->ulMinFirstPacketTime == 0) {
									/* Re-init the last timestamp for each stream*/
									for (j = 0; j < pInt->ulNumStreams; j++) {
										pInt->pStreamInfo[j].ulLastTimeStamp =
										    0xFFFFFFFF;
...
HX_RESULT rm_parseri_unpack_pkt_hdr(rm_parser_internal*  pInt, struct rm_pkt_hdr * pPktHdr)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parseri_unpack_index_hdr(rm_parser_internal*  pInt, struct rm_index_hdr * hdr)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parseri_unpack_index_rec(rm_parser_internal*  pInt, struct rm_index_rec * rec)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parseri_read_next_index_rec(rm_parser_internal*  pInt, struct rm_index_rec * rec)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parseri_create_all_stream_headers(rm_parser_internal*  pInt)
{
	...
	if (pInt && pInt->ulNumStreams) {
		...
		pInt->pStreamHdr = (rm_stream_header* ) rm_parseri_av_malloc(pInt, ulSize);
		if (pInt->pStreamHdr) {
			/* NULL out the memory*/
...
HX_RESULT rm_parseri_create_stream_header(rm_parser_internal*  pInt, UINT32 i, rm_stream_header * hdr)
{
	...
	if (pInt && pInt->pMediaPropsHdr && i < pInt->ulNumStreams && hdr) {
		...
		*  the properties
		*/
		ulNumProps = rm_parseri_count_set_stream_header_props(pInt, i, hdr, HXNULL, 0);
		if (ulNumProps) {
			/* Compute the size of the array*/
			ulSize = ulNumProps*  sizeof(rm_property);
			/* Allocate the array*/
			hdr->pProperty = (rm_property* ) rm_parseri_av_malloc(pInt, ulSize);
			if (hdr->pProperty) {
				/* NULL out the array*/
...
UINT32 rm_parseri_count_set_stream_header_props(rm_parser_internal*  pInt, UINT32 i,
						rm_stream_header*  hdr, rm_property * pProp, UINT32 ulNumProps)
{
	UINT32 ulRet = 0;
	...
	if (pInt && i < pInt->ulNumStreams && hdr) {
		...
		if (pInt->pStreamInfo[i].bIsRealAudio ||
		    pInt->pStreamInfo[i].bIsRealVideo || pInt->pStreamInfo[i].bIsRealEvent) {
			if (pProp) {
		...
		if (pInt->pMediaPropsHdr[i].duration > pInt->propHdr.duration) {
			if (pProp) {
				rm_parseri_set_rm_property(pInt, &pProp[ulRet], "EndTime",
							   RM_PROPERTY_TYPE_UINT32, (void* )pInt->propHdr.duration, 0);
			}
			ulRet++;
		}
		/* Set/count the "EndOneRuleEndAll" property*/
		if (pInt->pStreamInfo[i].bIsRealAudio || pInt->pStreamInfo[i].bIsRealVideo) {
			if (pProp) {
		...
		if (pProp) {
			rm_parseri_set_rm_property(pInt, &pProp[ulRet], "Duration",
						   RM_PROPERTY_TYPE_UINT32, (void* )ulDuration, 0);
		}
		ulRet++;
		...
		if (pInt->pStreamInfo[i].bIsRealVideo || pInt->pStreamInfo[i].bIsRealAudio) {
			...
			if (pInt->pStreamInfo[i].bIsRealVideo) {
				...
				*/
				...
				dPreDecBufSize = (((HXDOUBLE) ulPreDecBufTime)* 
						  ((HXDOUBLE) pInt->pMediaPropsHdr[i].max_bit_rate) + 7999.0) / 8000.0;
		...
		if (pProp) {
			rm_parseri_set_rm_property(pInt, &pProp[ulRet], "Preroll",
						   RM_PROPERTY_TYPE_UINT32, (void* )ulPreroll, 0);
		}
		ulRet++;
		...
		if (ulOldPreroll) {
			if (pProp) {
				rm_parseri_set_rm_property(pInt, &pProp[ulRet], "ActualPreroll",
							   RM_PROPERTY_TYPE_UINT32, (void* )ulOldPreroll, 0);
			}
			ulRet++;
		}
		...
		if (ulStreamingPreroll) {
			if (pProp) {
				rm_parseri_set_rm_property(pInt, &pProp[ulRet], "StreamingPreroll",
							   RM_PROPERTY_TYPE_UINT32, (void* )ulStreamingPreroll, 0);
			}
			ulRet++;
		}
		...
		if (ulPreDecBufSize) {
			if (pProp) {
				rm_parseri_set_rm_property(pInt, &pProp[ulRet], "X-PreDecBufSize",
							   RM_PROPERTY_TYPE_UINT32, (void* )ulPreDecBufSize, 0);
			}
			ulRet++;
		}
		...
		if (bHasPreDataProps) {
			dPreData = ((HXDOUBLE) ulPreroll)*  ((HXDOUBLE) pInt->pMediaPropsHdr[i].max_bit_rate) / 8000.0;
			ulPreData = (UINT32) (dPreData + 0.5);
			if (ulPreData) {
				if (pProp) {
					rm_parseri_set_rm_property(pInt, &pProp[ulRet], "PreData",
								   RM_PROPERTY_TYPE_UINT32, (void* )ulPreData, 0);
		...
		if (pInt->pStreamInfo[i].bIsRealVideo ||
		    pInt->pStreamInfo[i].bIsRealEvent || pInt->pMediaPropsHdr[i].type_spec_sz) {
			if (pProp) {
				if (pInt->pStreamInfo[i].bIsRealVideo) {
					/*
				...
				} else if (pInt->pStreamInfo[i].bIsRealEvent || pInt->pMediaPropsHdr[i].type_spec_sz) {
					/*
...
char* rm_parseri_create_asm_rulebook(rm_parser_internal * pInt, HXBOOL bIsRealVideo,
				     ...
				     UINT32 ulMaxBitRate, UINT32 ulAvgBitRate, HXBOOL*  pbHasPreDataProps)
{
	char* pRet = HXNULL;
	...
	if (pInt && pbHasPreDataProps) {
		...
		pRet = (char* )rm_parseri_av_malloc(pInt, RM_PARSER_RULE_BOOK_BUFFER_SIZE);
		if (pRet) {
			...
			if (bIsRealVideo) {
				...
				if (bIsVBR) {
					if (ulMaxBitRate) {
						...
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[0], 9, ulAvgBitRate, ulMaxBitRate, 5);
					} else {
						/* VBR RealVideo with max_bit_rate == 0*/
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[1], 9, ulAvgBitRate, 5);
					}
					/* Set the PreData flag*/
					*pbHasPreDataProps = TRUE;
				} else {
					/* CBR video*/
					snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[5], ulAvgBitRate, ulAvgBitRate,
				...
				if (bIsRealEvent) {
					/* RealEvent stream*/
					snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[6],0,0);
				} else if (bIsVBR) {
					/* Most likely an audio stream*/
					if (ulMaxBitRate) {
						/* VBR RealAudio with max_bit_rate != 0*/
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[0], 5, ulAvgBitRate, ulMaxBitRate, 5);
					} else {
						/* VBR RealAudio with max_bit_rate == 0*/
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[1], 5, ulAvgBitRate, 5);
					}
					/* Set the PreData flag*/
					*pbHasPreDataProps = TRUE;
				} else {
					/* CBR RealAudio*/
					snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[7], ulAvgBitRate);
				}
			} else {
				/* Non-RealAudio/RealVideo/RealEvent streams*/
				if (bIsVBR) {
					if (ulMaxBitRate) {
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[2], ulAvgBitRate, ulMaxBitRate);
					} else if (ulAvgBitRate) {
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[3], ulAvgBitRate);
					} else {
						snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[4],0,0);
					}
				} else {
					/* CBR Non-RealAudio/RealVideo/RealEvent streams*/
					snprintf(pRet, RM_PARSER_RULE_BOOK_BUFFER_SIZE, g_pszRuleBook[8], ulAvgBitRate);
				}
			}
		}
	}
	...
	if (pInt && ppPacket) {
		...
		if (pInt->ulCurFileOffset < pInt->propHdr.data_offset + pInt->dataHdr.size) {
			...
			if (HX_SUCCEEDED(retVal)) {
				...
				if (pInt->pStreamInfo[ulStreamNum].bIsRealVideo) {
					...
					*/
					if (hdr.timestamp != ulLastPacketTime &&
					    (hdr.flags & HX_KEYFRAME_FLAG ||
					     ulLastRule == HX_KEYFRAME_RULE || ulLastRule == HX_THINNING_RULE)) {
					...
					/* Assign the ASM rule*/
					ulASMRule =
					    (hdr.flags & HX_KEYFRAME_FLAG ? HX_KEYFRAME_RULE : HX_NONKEYFRAME_RULE);
					...
					*/
					...
					if (pInt->pStreamInfo[ulStreamNum].bIsRealEvent ||
						...
						ulASMRule =
						    (hdr.flags & HX_KEYFRAME_FLAG ? HX_KEYFRAME_RULE :
						     HX_NONKEYFRAME_RULE);
					} else {
						/*
				...
				pPacket = (rm_packet* ) rm_parseri_av_malloc(pInt, sizeof(rm_packet));
				if (pPacket) {
					/* NULL out the packet*/
					...
					pPacket->pData = (BYTE* ) rm_parseri_av_malloc(pInt, ulDataSize);
					if (pPacket->pData) {
						/* Read the data into the buffer*/
						ulBytesRead =
						    rm_parseri_file_read_buffer(pInt, pPacket->pData, ulDataSize);
						if (ulBytesRead == ulDataSize) {
							/* Assign the packet parameters*/
							pPacket->flag	= hdr.flags;
...
UINT32 rm_parseri_file_read(rm_parser_internal*  pInt, UINT32 ulBytesToRead, UINT32 ulReadBufferOffset)
{
	...
	if (pInt && pInt->fpRead) {
		...
		if (rv == HXR_OK) {
			...
			ulRet = pInt->fpRead(pInt->pUserRead, pInt->pReadBuffer + ulReadBufferOffset, ulBytesToRead);
...
void rm_parseri_file_seek(rm_parser_internal*  pInt, UINT32 ulOffset, UINT32 ulOrigin)
{
	if (pInt && pInt->fpSeek) {
		...
		if (ulOrigin == HX_SEEK_ORIGIN_SET) {
			...
			*/
			rm_parseri_error(pInt, HXR_NOT_SUPPORTED,
					 "File seek with end origin attempted - not suported.");
		}
	}
}
...
HX_RESULT rm_parseri_copy_stream_header(rm_parser_internal*  pInt, UINT32 i, rm_stream_header * pHdr)
{
	...
	if (pInt && pHdr && i < pInt->ulNumStreams && pInt->pStreamHdr) {
		...
		pHdr->pMimeType = copy_string(pInt->pUserMem, pInt->fpMalloc, pInt->pStreamHdr[i].pMimeType);
		pHdr->pStreamName = copy_string(pInt->pUserMem, pInt->fpMalloc, pInt->pStreamHdr[i].pStreamName);
		pHdr->pOpaqueData = copy_buffer(pInt->pUserMem, pInt->fpMalloc,
						pInt->pStreamHdr[i].pOpaqueData, pInt->pStreamHdr[i].ulOpaqueDataLen);
		if ((!pInt->pStreamHdr[i].pMimeType || pHdr->pMimeType) &&
			...
			if (pInt->pStreamHdr[i].ulNumProperties && pInt->pStreamHdr[i].pProperty) {
				/* Allocate space for the property array*/
				ulSize = pInt->pStreamHdr[i].ulNumProperties*  sizeof(rm_property);
				pHdr->pProperty = (rm_property* ) rm_parseri_av_malloc(pInt, ulSize);
				if (pHdr->pProperty) {
					/* NULL out the array*/
					...
					for (j = 0; j < pInt->pStreamHdr[i].ulNumProperties && retVal == HXR_OK; j++) {
						retVal = rm_parseri_set_rm_property(pInt, &pHdr->pProperty[j],
										    ...
										    pProperty[j].ulType,
										    (void* )pInt->
										    pStreamHdr[i].pProperty[j].pValue,
										    pInt->pStreamHdr[i].
										    pProperty[j].ulValueLen);
					}
					if (retVal == HXR_OK) {
...
HX_RESULT rm_parseri_update_time_range(rm_parser_internal*  pInt, UINT32 ulStreamNum, UINT32 ulTime)
{
	HX_RESULT retVal = HXR_FAIL;
					    ...
					    UINT32*  pulFoundTime, UINT32 * pulFoundOffset)
{
	...
	if (pInt && pulFoundTime && pulFoundOffset && pInt->ulNumStreams && pInt->pStreamInfo) {
		...
		for (i = 0; i < pInt->ulNumStreams; i++) {
			...
			if (pInfo) {
				...
				*/
				if (HX_FAILED(retVal) || (HX_SUCCEEDED(retVal) && status == HXR_OK)) {
					retVal = status;
				}
			}
		}
				       ...
				       UINT32*  pulFoundTime, UINT32 * pulFoundOffset)
{
	...
	if (pTable && pulFoundTime && pulFoundOffset && pTable->ulNumEntries) {
		...
		*/
		...
		while ((ulTableIdx > 0) && (pTable->pEntry[ulTableIdx].ulTime > ulSeekTime)) {
			ulTableIdx--;
		}
		/* Report results*/
...
HX_RESULT rm_parseri_search_index_chunk(rm_parser_internal*  pInt, UINT32 ulSeekTime)
{
	...
	if (pInt && pInt->propHdr.index_offset && pInt->pStreamInfo && pInt->ulNumStreams) {
		/* Initialize local variables*/
		...
		while (HX_SUCCEEDED(retVal) && ulIndexOffset && !rm_parseri_is_all_keyframes_found(pInt)) {
...
HX_RESULT rm_parseri_seek(rm_parser_internal*  pInt, UINT32 ulSeekTime)
{
	...
	if (pInt && pInt->ulNumStreams && pInt->pStreamInfo) {
		...
		status = rm_parseri_search_all_seek_tables(pInt, ulSeekTime, &ulFoundTime, &ulDataOffset);
		if (status == HXR_OK) {
			retVal = rm_parseri_find_first_packet_after_seek_time(pInt, ulSeekTime, ulDataOffset);
			...
			*/
			ulDataOffset = pInt->propHdr.data_offset + RM_PARSER_DATA_CHUNK_HEADER_SIZE;
...
HX_RESULT rm_parseri_find_first_packet_after_seek_time(rm_parser_internal*  pInt,
						       UINT32 ulSeekTime, UINT32 ulInitialOffset)
{
	HX_RESULT retVal = HXR_FAIL;
...
HX_RESULT rm_parseri_find_first_keyframe(rm_parser_internal*  pInt)
{
	...
	if (pInt && pInt->pStreamInfo && pInt->ulNumStreams) {
		...
		*/
		...
		for (i = 0; i < pInt->ulNumStreams; i++) {
			if (pInt->pStreamInfo[i].bNeedKeyframe && pInt->pStreamInfo[i].keyFramePacket.bValid) {
				/* Increment the number of valid keyframes*/
				ulNumValid++;
				/* Find the lowest keyframe time and stream*/
				if (!ulLowOffset || pInt->pStreamInfo[i].keyFramePacket.ulFileOffset < ulLowOffset) {
...
void* rm_parseri_av_malloc(rm_parser_internal * pInt, UINT32 ulSize)
{
	void* pRet = HXNULL;
