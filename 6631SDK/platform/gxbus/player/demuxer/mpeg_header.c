...
static int header_process_sequence_extension(MpegHeader*  picture, unsigned char *buffer)
{
...
static int header_process_picture_coding_extension(MpegHeader*  picture, unsigned char *buffer)
{
...
static int read_timeinc(MpegHeader*  picture, unsigned char *buffer, int n)
{
...
static unsigned int read_golomb(unsigned char* buffer, unsigned int *init)
{
	...
	//gxlogd ( "READ_GOLOMB(%u), V=2^%u + %u-1 = %u\n",* init, len, v, v2);
	*init = x;
	return v2;
}

static int h264_parse_vui(MpegHeader*  picture, unsigned char *buf, unsigned int n)
{
	...
	if ((timing = getbits(buf, n++, 1))) {
		picture->timeinc_unit =
		    (getbits(buf, n, 8) << 24) | (getbits(buf, n + 8, 8) << 16) | (getbits(buf, n + 16, 8) << 8) |
		    getbits(buf, n + 24, 8);
		n += 32;

		picture->timeinc_resolution =
		    (getbits(buf, n, 8) << 24) | (getbits(buf, n + 8, 8) << 16) | (getbits(buf, n + 16, 8) << 8) |
		    getbits(buf, n + 24, 8);
		n += 32;
