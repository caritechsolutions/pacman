...
static const char* asf_chunk_type(unsigned char* guid) {
	...
	switch(ASF_LOAD_GUID_PREFIX(guid)){
		...
		default:
			strncpy(tmp, "unknown guid ", sizeof(tmp));
			p = tmp + strlen(tmp);
			for (i = 0; i < 16; i++) {
				if ((1 << i) & ((1<<4) | (1<<6) | (1<<8))) *p++ = '-';
				snprintf(p, sizeof(tmp) - (p - tmp), "%02x", guid[i]);
				p += 2;
			}
			return tmp;
	}
...
int asf_check_header(GxDemuxer *demuxer){
	unsigned char asfhdrguid[16]={0x30,0x26,0xB2,0x75,0x8E,0x66,0xCF,0x11,0xA6,0xD9,0x00,0xAA,0x00,0x62,0xCE,0x6C};
	struct asf_priv* asf = av_calloc(1,sizeof(*asf));
	asf->scrambling_h=asf->scrambling_w=asf->scrambling_b=1;
	GxStream_Read(demuxer->stream,(uint8_t*) &asf->header,sizeof(asf->header)); // header obj
	le2me_ASF_header_t(&asf->header);			// swap to machine endian
	//  for(i=0;i<16;i++) gxlogf(" %02X",temp[i]);gxlogd("\n");
	//  for(i=0;i<16;i++) gxlogf(" %02X",asfhdrguid[i]);gxlogd("\n");
	if(memcmp(asfhdrguid,asf->header.objh.guid,16)){
		gxlogf("ASF_check: not ASF guid!\n");
		av_free(asf);
		return 0; // not ASF guid
	}
	if(asf->header.cno>256){
		gxlogf("ASF_check: invalid subchunks_no %d\n",(int) asf->header.cno);
		av_free(asf);
		return 0; // invalid header???
	}
	demuxer->priv = asf;
	return GX_DEMUXER_TYPE_ASF;
}
	...
	while ((pos = find_asf_guid(buf, asf_ext_stream_header, pos, buf_len)) >= 0) {
		...
		if (this_stream_num == stream_num) {
			...
			for (i=0; i<stnamect; i++) {
				...
				stream_name_len = AV_RL16((char*)buffer);buffer+=2;
			...
			if (is_video) {
				...
				asf->vid_repdata_sizes = av_malloc(payct*sizeof(int));
			} else {
				asf->aud_repdata_count = payct;
				asf->aud_repdata_sizes = av_malloc(payct*sizeof(int));
			}
...
static int asf_init_audio_stream(GxDemuxer *demuxer,struct asf_priv* asf, GxStreamAudioHeader* sh_audio,
	ASF_stream_header_t *streamh, int *ppos, uint8_t** buf, char *hdr, unsigned int hdr_len)
{
	uint8_t *buffer = *buf;
	int pos = *ppos;

	sh_audio->wf=av_calloc(FFMAX(streamh->type_size, sizeof(*sh_audio->wf)), 1);
...
int read_asf_header(GxDemuxer *demuxer,struct asf_priv* asf){
	int hdr_len = asf->header.objh.size - sizeof(asf->header);
	int hdr_skip = 0;
	char *hdr = NULL;
	char guid_buffer[16];
	int pos, start = GxStream_Tell(demuxer->stream);
	...
	if(hdr_len < 0) {
		gxlogf("Header size is too small.\n");
		return 0;
	}
	...
	hdr = av_malloc(hdr_len);
	if (!hdr) {
		gxlogf("%s [%d]: error\n", __FUNCTION__, __LINE__);
		return 0;
	}
	GxStream_Read(demuxer->stream, (uint8_t*)hdr, hdr_len);
	if (hdr_skip)
		GxStream_Skip(demuxer->stream, hdr_skip);
	...
	if ((pos = find_asf_guid(hdr, asf_ext_stream_audio, 0, hdr_len)) >= 0)
	{
		...
		if (sh_pos > 0) {
			...
			gxlogf("read_asf_header found dvr-ms audio stream header pos=%d\n", sh_pos);
			...
			sh_audio=GxStreamHeader_AudioNew(demuxer, streamh->stream_no & 0x7F0, streamh->stream_no & 0x7F);
			//sh_audio->needs_parsing = 1;
			gxlogf("asfheader: stream_no %d\n", streamh->stream_no & 0x7F);
			++audio_streams;
	...
	while ((pos = find_asf_guid(hdr, asf_stream_header_guid, pos, hdr_len)) >= 0)
	{
		...
		le2me_ASF_stream_header_t(streamh);
		gxlogd("stream type: %s\n", asf_chunk_type(streamh->type));
		gxlogf("stream concealment: %s\n", asf_chunk_type(streamh->concealment));
		gxlogf("type: %d bytes,  stream: %d bytes  ID: %d\n", (int)streamh->type_size,
				(int)streamh->stream_size, (int)streamh->stream_no);
		gxlogf("unk1: %lX  unk2: %X\n", (unsigned long)streamh->unk1, (unsigned int)streamh->unk2);
		// type-specific data:
		buffer = (uint8_t*)&hdr[pos];
		...
		switch(ASF_LOAD_GUID_PREFIX(streamh->type)){
			case ASF_GUID_PREFIX_audio_stream:
				{
					GxStreamAudioHeader* sh_audio=GxStreamHeader_AudioNew(demuxer,streamh->stream_no & 0x7F,
							streamh->stream_no & 0x7F);
					gxlogf("asfheader: stream_no %d\n", streamh->stream_no & 0x7F);
					++audio_streams;
					...
					if(demuxer->audio->id==-1){
						demuxer->audio->id=streamh->stream_no & 0x7F;
			...
			case ASF_GUID_PREFIX_video_stream:
				{
					unsigned int len;
					float asp_ratio;
					GxStreamVideoHeader* sh_video=GxStreamHeader_VideoNew(demuxer, streamh->stream_no & 0x7F,
							streamh->stream_no & 0x7F);
					gxlogf("asfheader: stream_no %d\n", streamh->stream_no & 0x7F);
					len=streamh->type_size-(4+4+1+2);
					++video_streams;
					sh_video->bih=av_calloc((len<sizeof(*sh_video->bih))?sizeof(*sh_video->bih):len,1);
					memcpy(sh_video->bih,&buffer[4+4+1+2],len);
					le2me_BITMAPINFOHEADER(sh_video->bih);
					if (sh_video->bih->biSize > len && sh_video->bih->biSize > sizeof(*sh_video->bih))
						sh_video->bih->biSize = len;
					sh_video->format = sh_video->bih->biCompression;
					if (sh_video->bih->biCompression == GX_FOURCC('D', 'V', 'R', ' ')) {
					...
					if(demuxer->video->id==-1){
						demuxer->video->id=streamh->stream_no & 0x7F;
		...
		// GxStream_Read(demuxer->stream,(char*) buffer,streamh.stream_size);
	}
	...
	if (pos >= 0) {
		...
		asf->packet=av_malloc(asf->packetsize); // !!!
	...
	if (pos >= 0) {
		...
		gxlogf("============ ASF Stream group == END ===\n");
	}
	av_free(hdr);
	hdr = NULL;
	start = GxStream_Tell(demuxer->stream); // start of first data chunk
	GxStream_Read(demuxer->stream, (uint8_t*)guid_buffer, 16);
	...
	data_len = GxStream_ReadDword_Le(demuxer->stream);
	demuxer->movi_start = GxStream_Tell(demuxer->stream) + 26;
	demuxer->movi_end = start + data_len - 4;
	gxlogd("Found movie at 0x%X - 0x%X\n", (int)demuxer->movi_start, (int)demuxer->movi_end);
	...
	gxlogf("ASF: %d audio and %d video streams found\n",audio_streams,video_streams);
...
#if 0
	if( 0 ){
		gxlogf("ASF duration: %d\n",(int)fileh.duration);
		gxlogf("ASF start pts: %d\n",(int)fileh.start_timestamp);
		gxlogf("ASF end pts: %d\n",(int)fileh.end_timestamp);
	}
#endif
...
len_err_out:
	gxlogf("%s [%d]: Invalid in ASF header\n", __FUNCTION__, __LINE__);
err_out:
	av_free(hdr);
	av_free(streams);
	return 0;
