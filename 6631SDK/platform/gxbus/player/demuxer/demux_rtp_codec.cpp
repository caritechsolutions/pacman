...
static void
needVideoFrameRate(GxDemuxer* demuxer, MediaSubsession* subsession); // forward
static Boolean
                                               ...
                                               unsigned size)
{
    ...
    if (!size || size > INT_MAX - sizeof(BITMAPINFOHEADER))
        return bih;
    bih = (BITMAPINFOHEADER*)av_realloc(bih, sizeof(BITMAPINFOHEADER) + size);
    if (!bih)
        return original;
...
void rtpCodecInitialize_video(GxDemuxer* demuxer,
			      MediaSubsession* subsession,
			      unsigned& flags) {
  flags = 0;
  // Create a dummy video stream header
  // to make the main MPlayer code happy:
  GxStreamVideoHeader* sh_video = GxStreamHeader_VideoNew(demuxer,0,0);
  BITMAPINFOHEADER* bih
    = (BITMAPINFOHEADER*)av_calloc(1,sizeof(BITMAPINFOHEADER));
  ...
  if (strcmp(subsession->codecName(), "MPV") == 0) {
    ...
    bih->biHeight = qtRTPSource->qtState.height;
      ...
      while (pos+8 < endpos) {
        ...
        if (atomLength == 0 || atomLength > (unsigned)(endpos-pos)) break;
        if (((!memcmp(pos+4, "avcC", 4) && fourcc==GX_FOURCC('a','v','c','1')) ||
             !memcmp(pos+4, "esds", 4) ||
             (!memcmp(pos+4, "SMI ", 4) && fourcc==GX_FOURCC('S','V','Q','3'))) &&
            atomLength > 8) {
          sh_video->bih = bih =
    ...
    gxlogf("Unknown MPlayer format code for MIME type \"video/%s\"\n",
	    subsession->codecName());
  }
}

void rtpCodecInitialize_audio(GxDemuxer* demuxer,
			      MediaSubsession* subsession,
			      unsigned& flags) {
  flags = 0;
  // Create a dummy audio stream header
  // to make the main MPlayer code happy:
  GxStreamAudioHeader* sh_audio = GxStreamHeader_AudioNew(demuxer,0, 0);
  WAVEFORMATEX* wf = (WAVEFORMATEX*)av_calloc(1,sizeof(WAVEFORMATEX));
  sh_audio->wf = wf;
 ...
 // d_audio->id = sh_audio->aid;?????????????????
  ...
  if (strcmp(subsession->codecName(), "MPA") == 0 ||
    ...
    wf->wFormatTag = sh_audio->format = GX_FOURCC('s','a','m','r');
  } else if (strcmp(subsession->codecName(), "AMR-WB") == 0) {
    wf->wFormatTag = sh_audio->format = GX_FOURCC('s','a','w','b');
  } else if (strcmp(subsession->codecName(), "GSM") == 0) {
    wf->wFormatTag = sh_audio->format = GX_FOURCC('a','g','s','m');
    wf->nAvgBytesPerSec = 1650;
    wf->nBlockAlign = 33;
    wf->wBitsPerSample = 16;
    wf->cbSize = 0;
  } else if (strcmp(subsession->codecName(), "QCELP") == 0) {
    wf->wFormatTag = sh_audio->format = GX_FOURCC('Q','c','l','p');
    wf->nAvgBytesPerSec = 1750;
    wf->nBlockAlign = 35;
    wf->wBitsPerSample = 16;
    wf->cbSize = 0;
  } else if (strcmp(subsession->codecName(), "MP4A-LATM") == 0) {
    wf->wFormatTag = sh_audio->format = GX_FOURCC('m','p','4','a');
    ...
    while (pos+8 < endpos) {
      unsigned atomLength = pos[0]<<24 | pos[1]<<16 | pos[2]<<8 | pos[3];
      if (atomLength == 0 || atomLength > (unsigned)(endpos-pos)) break;
      if (!memcmp(pos+4, "wave", 4) && fourcc==GX_FOURCC('Q','D','M','2') &&
          atomLength > 8 &&
          atomLength <= INT_MAX) {
        sh_audio->codecdata = (unsigned char*) av_malloc(atomLength-8);
    ...
    gxlogf("Unknown MPlayer format code for MIME type \"audio/%s\"\n",
	    subsession->codecName());
  }
}

static void needVideoFrameRate(GxDemuxer* demuxer,
			       MediaSubsession* subsession) {
  ...
  GxDemuxStream* d_video = demuxer->video;
  GxStreamVideoHeader* sh_video = (GxStreamVideoHeader*)(d_video->sh);
  ...
  for (unsigned i = 0; i < maxNumFramesToWaitFor; ++i) {
    ...
    if (curPTS != lastPTS && lastPTS != 0.0) {
      ...
      fps = (int)(1/fabs(curPTS-lastPTS) + 0.5); // rounding
        if (fps == lastfps) {
      gxlogf("demux_rtp: Guessed the video frame rate as %d frames-per-second.\n\t(If this is wrong, use the \"-fps <frame-rate>\" option instead.)\n", fps);
      sh_video->fps = fps;
      sh_video->frametime=1.0f/fps;
      return;
        }
  ...
  gxlogf("demux_rtp: Failed to guess the video frame rate\n");
}
