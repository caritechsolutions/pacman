...
static void asf_descrambling(unsigned char **src,unsigned len, struct asf_priv* asf){
	...
	if (len > UINT_MAX - MP_INPUT_BUFFER_PADDING_SIZE)
		return;
	dst = av_malloc(len + MP_INPUT_BUFFER_PADDING_SIZE);
	while(len>=asf->scrambling_h*asf->scrambling_w*asf->scrambling_b+i){
		//    gxlogf("descrambling! (w=%d  b=%d)\n",w,asf_scrambling_b);
...
static void demux_asf_append_to_packet(GxDemuxPacket* dp,unsigned char *data,int len,int offs)
{
	if(dp->len!=offs && offs!=-1) gxlogf("warning! fragment.len=%d BUT next fragment offset=%d  \n",dp->len,offs);
	dp->buffer=av_realloc(dp->buffer,dp->len+len+MP_INPUT_BUFFER_PADDING_SIZE);
	memcpy(dp->buffer+dp->len,data,len);
	memset(dp->buffer+dp->len+len, 0, MP_INPUT_BUFFER_PADDING_SIZE);
	gxlogf("data appended! %d+%d\n",dp->len,len);
	dp->len+=len;
}

static int demux_asf_read_packet(GxDemuxer *demux,unsigned char *data,int len,int id,int seq,uint64_t time,unsigned short dur,int offs,int keyframe){
	struct asf_priv* asf = demux->priv;
	GxDemuxStream *ds=NULL;
	int close_seg=0;

	gxlogf("demux_asf.read_packet: id=%d seq=%d len=%d\n",id,seq,len);
	...
	if(id==demux->audio->id){
		...
		if(!ds->sh){
			...
			gxlogf("Auto-selected ASF audio ID = %d\n",ds->id);
		}
	} else
		if(id==demux->video->id){
			...
			if(!ds->sh){
				...
				gxlogf("Auto-selected ASF video ID = %d\n",ds->id);
			}
		}

	if(ds){
		if(ds->asf_packet){
			GxDemuxPacket* dp=ds->asf_packet;
			...
			if(close_seg){
				...
				GxDemuxStream_AddPacket(ds,ds->asf_packet);
				ds->asf_packet=NULL;
			} else {
				// append data to it!
		...
		{ GxDemuxPacket* dp;
			if(offs>0){
				gxlogf("warning!  broken fragment, %d bytes missing  \n",offs);
				return 0;
			}
			...
			if (asf->asf_is_dvr_ms)
				...
				dp->pts=time*0.001;
			dp->flags=keyframe;
			//      if(ds==demux->video) gxlogf("ASF time: %8d  dur: %5d  \n",time,dur);
			dp->pos=demux->filepos;
			ds->asf_packet=dp;
			ds->asf_seq=seq;
			// we are ready now.
 ...
 *
 */
static void get_payload_extension_data(GxDemuxer *demux, unsigned char** pp, unsigned char id, unsigned int seq, int *keyframe, uint64_t *seg_time){
	struct asf_priv* asf = demux->priv;
...
static int demux_asf_fill_buffer(GxDemuxer *demux, GxDemuxStream *ds){
	struct asf_priv* asf = demux->priv;
	...
	GxStream_Read(demux->stream,asf->packet,asf->packetsize);
...

	{
		...
		if(0){
			int i;
			for(i=0;i<FFMIN(16, asf->packetsize);i++) gxlogf(" %02X",asf->packet[i]);
			gxlogf("\n");
		}
		...
		//if(segtype!=0x5d) gxlogf("Warning! packet[4] != 0x5d  \n");
		...
		if(((flags>>5)&3)!=0){
			...
			gxlogf("Explicit packet size specified: %d  \n",plen);
			if(plen>asf->packetsize) gxlogf("Warning! plen>packetsize! (%d>%d)  \n",plen,asf->packetsize);
		} else {
		...
		gxlogf("%08"PRIu64":  flag=%02X  segs=%d  seq=%u  plen=%u  pad=%u  time=%ld  dur=%d\n",
		...
		for(seg=0;seg<segs;seg++){
			...
			if(p>=p_end) {
				gxlogf("Warning! invalid packet 1, aborting parsing...\n");
				break;
			}
			...
			if(0){
				int i;
				gxlogf("seg %d:",seg);
				for(i=0;i<FFMIN(16, p_end - p);i++) gxlogf(" %02X",p[i]);
				gxlogf("\n");
			}
			...
			switch(rlen){
				...
				default:
					if(rlen>=8){
						...
						gxlogf("unknown segment type (rlen): 0x%02X  \n",rlen);
						time2=0; // unknown
						p+=rlen;
					}
			}
			...
			if(len<0 || (p+len)>p_end){
				gxlogf("ASF_parser: warning! segment len=%d\n",len);
				len = p_end - p;
			}
			gxlogf("  seg #%d: streamno=%d  seq=%d  type=%02X  len=%d\n",seg,streamno,seq,rlen,len);

			switch(rlen){
				case 0x01:
					// GROUPING:
					//gxlogf("ASF_parser: warning! grouping (flag=1) not yet supported!\n",len);
					//gxlogf("  total: %d  \n",len);
					while(len>0){
						int len2=p[0];
						p++;
						//gxlogf("  group part: %d bytes\n",len2);
					...
					if(len!=0){
						gxlogf("ASF_parser: warning! groups total != len\n");
					}
					break;
				default:
					// NO GROUPING:
					//gxlogf("fragment offset: %d  \n",sh->x);
					if (len <= 0) break;
	...
	gxlogf("%08"PRIX64":  UNKNOWN TYPE  %02X %02X %02X %02X %02X...\n",(int64_t)demux->filepos,asf->packet[0],asf->packet[1],asf->packet[2],asf->packet[3],asf->packet[4]);
	return 0;
}

#include "stheader.h"

static void demux_seek_asf(GxDemuxer *demuxer,int64_t rel_seek_secs, int32_t audio_delay,int flags){
	...
	off_t rel_seek_packs=(flags&GX_DEMUXER_SEEK_PERCENT)?	 // FIXME: int may be enough?
	...
	off_t newpos;
	//gxlogf("ASF: packs: %d  duration: %d  \n",(int)fileh.packets,*((int*)&fileh.duration));
	//    gxlogf("ASF_seek: %d secs -> %d packs -> %d bytes  \n",
	//       rel_seek_secs,rel_seek_packs,rel_seek_bytes);
	newpos=((flags&GX_DEMUXER_SEEK_ABSOLUTE)?demuxer->movi_start:demuxer->filepos)+rel_seek_bytes;
	if(newpos<0 || newpos<demuxer->movi_start) newpos=demuxer->movi_start;
	//    gxlogf("\r -- asf: newpos=%d -- \n",newpos);
	GxStream_Seek(demuxer->stream,newpos);

	if (asf->asf_is_dvr_ms) asf->dvr_last_vid_pts = 0.0f;

	if (d_video->id >= 0)
		demux_asf_fill_buffer(demuxer, d_video);
	if(sh_audio){
		demux_asf_fill_buffer(demuxer,d_audio);
	}
#if 0
	if (d_video->id >= 0)
		while(1){
			if(sh_audio && !d_audio->eof){
				...
				if (d_video->pts > a_pts){
					...
					//	      if(!demux_asf_fill_buffer(demuxer,d_audio)) sh_audio=NULL; // skip audio. EOF?
					continue;
				}
			}
			if(d_video->flags&1) break; // found a keyframe!
			if(!demux_asf_fill_buffer(demuxer,d_video)) break; // skip frame.  EOF?
		}
#endif

}

static int demux_asf_control(GxDemuxer *demuxer,int cmd, void *arg){
	struct asf_priv* asf = demuxer->priv;
	/*  GxDemuxStream *d_audio=demuxer->audio;
		GxDemuxStream *d_video=demuxer->video;
		GxStreamAudioHeader *sh_audio=d_audio->sh;
...
static GxDemuxer* demux_open_asf(GxDemuxer* demuxer)
{
	struct asf_priv* asf = demuxer->priv;
...
static void demux_close_asf(GxDemuxer *demuxer) {
	struct asf_priv* asf = demuxer->priv;

	if (!asf) return;

	av_free(asf->aud_repdata_sizes);
	av_free(asf->vid_repdata_sizes);
	av_free(asf->packet);
	av_free(asf);
}

GxDemuxerClass gx_demux_asf = {
	...
	DEF_AUTHOR("demuxer","asf","No description","L.F","No comment"),

	.name        = "Demux ASF",
	.type        = GX_DEMUXER_TYPE_ASF,
	.check_file  = asf_check_header,
	.open        = demux_open_asf,
	.fill_buffer = demux_asf_fill_buffer,
	.close       = demux_close_asf,
	.seek        = demux_seek_asf,
	.control     = demux_asf_control,
};
