		...
		ADPCMChannelStatus *right, int channels, int sample_offset)
{
	...
	for(i=0;i<4;i++) {
		...
		if (filter >= FF_ARRAY_ELEMS(xa_adpcm_table)) {
			gxlogf("unknown XA-ADPCM filter %d", filter);
			filter=0;
		}
		...
		int buf_size, int *coded_samples, int *approx_nb_samples)
{
	...
	*approx_nb_samples = 0;
	...
	case CODEC_ID_ADPCM_IMA_SMJPEG:  header_size = 4 * ch;      break;
	}
	if (header_size > 0)
	...
	case CODEC_ID_ADPCM_THP:
		...
		*coded_samples -= *coded_samples % 14;
		...
		break;
	case CODEC_ID_ADPCM_AFC:
		nb_samples = buf_size / (9 * ch) * 16;
		break;
	case CODEC_ID_ADPCM_XA:
		nb_samples = (buf_size / 128) * 224 / ch;
		break;
	case CODEC_ID_ADPCM_DTK:
...
{
	ADPCMDecodeContext *c = NULL;
	unsigned int min_channels = 1;
	unsigned int max_channels = 2;
	int codec_id = avctx->codec_id;

	avctx->priv_data = c = av_mallocz(sizeof(ADPCMDecodeContext));
	...
	case CODEC_ID_ADPCM_AFC:
	case CODEC_ID_ADPCM_DTK:
		avctx->sample_fmt = SAMPLE_FMT_S16P;
		break;
	case CODEC_ID_ADPCM_IMA_WS:
		avctx->sample_fmt = c->vqa_version == 3 ? SAMPLE_FMT_S16P :
			SAMPLE_FMT_S16;
		break;
	default:
...
GxDemuxPacket* adpcm_decode_frame(GxDemuxer* demuxer, AVCodecContext *avctx, uint8_t* buf, int buf_size)
{
	ADPCMDecodeContext *c = avctx->priv_data;
	...
	if (nb_samples <= 0) {
		gxlogd("invalid number of samples in packet\n");
		goto decode_err;
	}

	/* get output buffer */
	frame_size = avctx->channels*nb_samples*avctx->bits_per_sample/8;
	frame_buf = av_malloc(frame_size);
	if (frame_buf == NULL) goto decode_err;

	samples   = (short *)frame_buf;
	samples_p = (int16_t **)(&frame_buf);
...
update:
	...
	case CODEC_ID_ADPCM_4XM:
		...
		for (i = 0; i < avctx->channels; i++) {
			samples = (int16_t *)(frame_buf+i*nb_samples);
			cs = &c->status[i];
	...
	case CODEC_ID_ADPCM_MS:
		{
			...
			if (block_predictor > 6) {
				gxlogd("ERROR: block_predictor[0] = %d\n", block_predictor);
				goto decode_err;
			}
			...
			if (st) {
				block_predictor = bytestream2_get_byteu(&gb);
				if (block_predictor > 6) {
					gxlogd("ERROR: block_predictor[1] = %d\n", block_predictor);
					goto decode_err;
				}
...
6chan: 0=fl, 1=c,  2=fr, 3=rl,  4=rr, 5=sub */
		const int big_endian = codec_id == CODEC_ID_ADPCM_EA_R3;
		...
		for (channel=0; channel<avctx->channels; channel++) {
			...
			samplesC = (uint16_t*)samples_p[channel];

			if (codec_id == CODEC_ID_ADPCM_EA_R1) {
	...
	case CODEC_ID_ADPCM_SBPRO_2:
		...
		if (codec_id == CODEC_ID_ADPCM_SBPRO_4) {
			for (n = nb_samples >> (1 - st); n > 0; n--) {
				int byte = bytestream2_get_byteu(&gb);
		...
		} else if (codec_id == CODEC_ID_ADPCM_SBPRO_3) {
	...
	if (buf_size && bytestream2_tell(&gb) == 0) {
		av_log(avctx, AV_LOG_ERROR, "Nothing consumed\n");
		goto decode_err;
	}

	dp = GxDemuxPacket_Create(demuxer, NULL, frame_size);
	if (dp)
		GxDemuxPacket_Write(dp, frame_buf, frame_size);
	else
		goto decode_err;
...
void adpcm_decode_uninit(AVCodecContext * avctx)
{
	if (avctx) {
