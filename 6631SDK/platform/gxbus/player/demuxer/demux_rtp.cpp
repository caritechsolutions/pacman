...
static void dump_av_packet_init(void)
{
     packet_cnt = 0;
     remove("dump_live555_packet_gx.a");
     remove("dump_live555_packet_gx.v");		
}

static void dump_av_packet(GxDemuxPacket* dp, int av_flag)
{
     FILE *fp = NULL;

     if(av_flag)//񙮵
		fp = fopen("dump_live555_packet_gx.a", "ab+");
     else
		fp = fopen("dump_live555_packet_gx.v", "ab+");

     if(fp)
     {
        fwrite(dp->buffer, dp->len, 1, fp);
        fclose(fp);
     }
}
...
public:
  ...
  void savePendingBuffer(GxDemuxPacket* dp);
  GxDemuxPacket* getPendingBuffer();

  // For H264 over rtsp using AVParser, the next packet has to be saved
  GxDemuxPacket* nextpacket;

private:
  GxDemuxPacket* pendingDPHead;
  GxDemuxPacket* pendingDPTail;

  FramedSource* fReadSource;
  RTPSource* fRTPSource;
  GxDemuxer* fOurDemuxer;
...
#endif
extern "C" int rtsp_port;
extern "C" int audio_id, video_id, dvdsub_id;
extern "C" GxDemuxer* demux_open_rtp(GxDemuxer* demuxer) {
  ...
  do {
	...
	if (rtspClient == NULL) {
	  gxlogf( "Failed to create RTSP client: %s\n",
		  env->getResultMsg());
	  break;
	}
	...
	sipClient = SIPClient::createNew(*env, desiredAudioType, NULL,
					 verbose, "MPlayer");
	if (sipClient == NULL) {
	  gxlogf("Failed to create SIP client: %s\n",
		  env->getResultMsg());
	  break;
	}
	...
	sdpDescription = openURL_sip(sipClient, url);
      ...
      if (sdpDescription == NULL) {
	gxlogf("Failed to get a SDP description from URL \"%s\": %s\n",
	...
	if (audiofound) {
	  gxlogf( "Additional subsession \"audio/%s\" skipped\n", subsession->codecName());
	  continue;
	}
	desiredReceiveBufferSize = 100000;
      } else if (strcmp(subsession->mediumName(), "video") == 0) {
	if (videofound) {
	  gxlogf( "Additional subsession \"video/%s\" skipped\n", subsession->codecName());
	  continue;
	}
	...
	gxlogf("Failed to initiate \"%s/%s\" RTP subsession: %s\n", subsession->mediumName(), subsession->codecName(), env->getResultMsg());
      } else {
	gxlogf("Initiated \"%s/%s\" RTP subsession on port %d\n", subsession->mediumName(), subsession->codecName(), subsession->clientPortNum());
	...
	receiveBufferSize = increaseReceiveBufferTo(*env, rtpSocketNum, desiredReceiveBufferSize);
	if (verbose > 0) {
	  gxlogf("Increased %s socket receive buffer to %d bytes \n",
  ...
  // then create a new "stream_t" structure to allow the higher-level
  // code to recognize this:
  
#if 0  //???????????????????????????
  if (demux_is_multiplexed_rtp_stream(demuxer)) {
    stream_t* s = new_ds_stream(demuxer->video);
    GxDemuxer* od = demux_open(s, DEMUXER_TYPE_UNKNOWN,
...
extern "C" int demux_is_mpeg_rtp_stream(GxDemuxer* demuxer) {
...
extern "C" int demux_is_multiplexed_rtp_stream(GxDemuxer* demuxer) {
  // Get the RTP state that was stored in the demuxer's 'priv' field:
  RTPState* rtpState = (RTPState*)(demuxer->priv);
...
static GxDemuxPacket* getBuffer(GxDemuxer* demuxer, GxDemuxStream* ds,
				 Boolean mustGetNewData,
				 float& ptsBehind); // forward

extern "C" int demux_rtp_fill_buffer(GxDemuxer* demuxer, GxDemuxStream* ds) {
  ...
  while (1) {
    float ptsBehind;
    //GxDemuxPacket* dp = getBuffer(demuxer, ds, False, ptsBehind); // blocking
    GxDemuxPacket* dp = getBuffer(demuxer, ds, True, ptsBehind); // blocking
    if (dp == NULL)
	return GX_PLAYER_ERROR;

    if (demuxer->stream->eof)
	return GX_PLAYER_ERROR; // source stream has closed down
	...
	gxlogd("\rpacket_cnt=%05d", packet_cnt++);    
	dump_av_packet(dp,0);
    ...
    gxlogf("Discarding %s packet (%fs behind)\n", bufferQueue->tag(), ptsBehind);
#endif
    GxDemuxPacket_Destroy(dp); // give back this packet, and get another one
  }

  return GX_PLAYER_OK;
}

Boolean awaitRTPPacket(GxDemuxer* demuxer, GxDemuxStream* ds,
...
extern "C" void demux_close_rtp(GxDemuxer* demuxer) {
...
static void afterReading(void* clientData, unsigned frameSize,
  ...
  if (frameSize >= MAX_RTP_FRAME_SIZE) {
    gxlogf("Saw an input frame too large (>=%d).  Increase MAX_RTP_FRAME_SIZE in \"demux_rtp.cpp\".\n",
	    MAX_RTP_FRAME_SIZE);
  }
  ReadBufferQueue* bufferQueue = (ReadBufferQueue*)clientData;
  GxDemuxer* demuxer = bufferQueue->ourDemuxer();
  RTPState* rtpState = (RTPState*)(demuxer->priv);

  if (frameSize > 0) demuxer->stream->eof = 0;

  GxDemuxPacket* dp = bufferQueue->dp;

  if (bufferQueue->readSource()->isAMRAudioSource())
    headersize = 1;
  else if (bufferQueue == rtpState->videoBufferQueue &&
      ((GxStreamVideoHeader*)demuxer->video->sh)->format == GX_FOURCC('H','2','6','4')) {
    dp->buffer[0]=0x00;
    dp->buffer[1]=0x00;
    dp->buffer[2]=0x01;
    headersize = 3;
  ...
  dp = GxDemuxPacket_Resize(demuxer, dp, frameSize + headersize);
  if(dp == NULL)
  	return;
  ...
  if (hasBeenSynchronized) {
    if (verbose > 0 && !bufferQueue->prevPacketWasSynchronized) {
      gxlogf("%s stream has been synchronized using RTCP \n",
	      bufferQueue->tag());
    }
    ...
    if (verbose > 0 && bufferQueue->prevPacketWasSynchronized) {
      gxlogf("%s stream is no longer RTCP-synchronized \n",
	      bufferQueue->tag());
    }
...
static GxDemuxPacket* getBuffer(GxDemuxer* demuxer, GxDemuxStream* ds,
  ...
  if (ds == demuxer->video) {
    ...
    if (((GxStreamVideoHeader*)ds->sh)->format == GX_FOURCC('H','2','6','4'))
      headersize = 3;
    ...
    gxlogf("(demux_rtp)getBuffer: internal error: unknown stream\n");
    return NULL;
  }

  if (bufferQueue == NULL || bufferQueue->readSource() == NULL) {
    gxlogf("(demux_rtp)getBuffer failed: no appropriate RTP subsession has been set up\n");
    return NULL;
  }
  ...
  dp = GxDemuxPacket_Create(demuxer, NULL, MAX_RTP_FRAME_SIZE);
  bufferQueue->dp = dp;
  ...
  if (headersize == 1) // amr
    ...
    if (headersize == 3 && h264parserctx) { // h264
      ...
      if (!poutbuf_size) {
        lastpts=dp->pts;
        GxDemuxPacket_Destroy(dp);
        bufferQueue->dp = dp = GxDemuxPacket_Create(demuxer, NULL, MAX_RTP_FRAME_SIZE);
      } else {
        bufferQueue->nextpacket = dp;
        bufferQueue->dp = dp = GxDemuxPacket_Create(demuxer, NULL, poutbuf_size);
...
ReadBufferQueue::ReadBufferQueue(MediaSubsession* subsession,
				 GxDemuxer* demuxer, char const* tag)
  : prevPacketWasSynchronized(False), prevPacketPTS(0.0), otherQueue(NULL),
    ...
    fOurDemuxer(demuxer), fTag(av_strdup(tag)) {
}
...
ReadBufferQueue::~ReadBufferQueue() {
  ...
  GxDemuxPacket* dp = pendingDPHead;
  while (dp != NULL) {
    GxDemuxPacket* dpNext = dp->next;
    dp->next = NULL;
    GxDemuxPacket_Destroy(dp);
    dp = dpNext;
  }
...
void ReadBufferQueue::savePendingBuffer(GxDemuxPacket* dp) {
...
GxDemuxPacket* ReadBufferQueue::getPendingBuffer() {
  GxDemuxPacket* dp = pendingDPHead;
  if (dp != NULL) {
...
extern "C" int demux_rtp_control(GxDemuxer *demuxer, int cmd, void *arg) {
