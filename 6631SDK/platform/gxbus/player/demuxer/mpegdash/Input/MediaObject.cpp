...
MediaObject::MediaObject(ISegment *segment, IRepresentation *rep, PLAYER_INTERRUPT_CBK _interruptcbk) :
	segment                 (segment),
	rep                     (rep),
	mediaObjectInterruptcbk (_interruptcbk)
{
...
void MediaObject::WaitFinished()
{
	...
	while(this->state != COMPLETED && this->state != ABORTED) {
		if (this->mediaObjectInterruptcbk && this->mediaObjectInterruptcbk()) {
...
DownloadState MediaObject::GetDownloadState()
{
	return this->state;
}
