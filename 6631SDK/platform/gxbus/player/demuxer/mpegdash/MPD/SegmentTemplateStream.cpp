...
ISegment*                   SegmentTemplateStream::GetMediaSegment              (size_t segmentNumber)
{
	...
	{
		if (this->segmentStartTimes.size() > segmentNumber)
			return this->segmentTemplate->GetMediaSegmentFromTime(baseUrls, representation->GetId(), representation->GetBandwidth(), this->segmentStartTimes.at(segmentNumber), this->segmentTemplate->GetStartNumber());
