...
//#define AUDIO_AAC2 (short) GX_FOURCC('m','p','4','a')
...
static mpeg_frame_t* init_frames(uint16_t num, size_t size)
{
	...
	tmp = (mpeg_frame_t* ) av_calloc(num, sizeof(mpeg_frame_t));
	if(tmp == NULL)
		return NULL;

	for(i=0; i < num; i++)
	{
		tmp[i].buffer = (uint8_t* ) av_calloc(1, size);
...
static muxer_stream_t* mpegfile_new_stream(muxer_t* muxer,int type){
  ...
  if(muxer->avih.dwStreams>=MUXER_MAX_STREAMS){
    gx_msg(MSGT_MUXER, MSGL_ERR, "Too many streams! increase MUXER_MAX_STREAMS !\n");
    return NULL;
  }
  switch (type) {
    case MUXER_TYPE_VIDEO:
      if (muxer->num_videos >= 16) {
	gx_msg(MSGT_MUXER, MSGL_ERR, "MPEG files can't contain more than 16 video streams!\n");
	return NULL;
      }
      break;
    case MUXER_TYPE_AUDIO:
      if (muxer->num_audios >= 16) {
	gx_msg(MSGT_MUXER, MSGL_ERR, "MPEG files can't contain more than 16 audio streams!\n");
	return NULL;
      }
      break;
    default:
      gx_msg(MSGT_MUXER, MSGL_ERR, "Unknown stream type!\n");
      return NULL;
  }
  s = (muxer_stream_t*) av_calloc(1, sizeof(muxer_stream_t));
  if(!s) return NULL; // no mem!?
  if (!(s->b_buffer = av_malloc(priv->packet_size)))
    goto init_fail;
  s->b_buffer_size = priv->packet_size;
  s->b_buffer_ptr = 0;
  s->b_buffer_len = 0;
  s->priv = (muxer_headers_t*) av_calloc(1, sizeof(muxer_headers_t));
  if(s->priv == NULL)
    goto init_fail;
  spriv = (muxer_headers_t* ) s->priv;
  spriv->pack = av_malloc(priv->packet_size);
  if(! spriv->pack)
    goto init_fail;
  spriv->buffer_track = av_calloc(1, 4096*sizeof(buffer_track_t));
  ...
  if (type == MUXER_TYPE_VIDEO) {
    ...
    if(spriv->framebuf == NULL) {
      gx_msg(MSGT_MUXER, MSGL_FATAL, "Couldn't allocate initial frames structure, abort!\n");
      goto init_fail;
    }
    ...
    gx_msg (MSGT_MUXER, MSGL_DBG2, "Added video stream %d, ckid=%X\n", muxer->num_videos, s->ckid);
    ...
    gx_msg (MSGT_MUXER, MSGL_DBG2, "Added audio stream %d, ckid=%X\n", s->id - muxer->num_videos + 1, s->ckid);
  }
...
static int write_nav_pack(uint8_t* buff)
{
	...
	gx_msg(MSGT_MUXER, MSGL_DBG3, "NAV\n");
	len = 0;
...
static void update_demux_bufsize(muxer_headers_t* spriv, uint64_t dts, int framelen, int type)
{
	...
	{
		buffer_track_t* tmp = av_realloc(spriv->buffer_track, dim);
		if(!tmp)
		{
			gx_msg(MSGT_MUXER, MSGL_ERR, "\r\nERROR, couldn't av_realloc %d bytes for tracking buffer\r\n", dim);
			return;
		}
...
static void update_scr(muxer_t* muxer)
{
	...
	gx_msg(MSGT_MUXER, MSGL_DBG2, "UPDATE SCR TO %"PRIu64" (%.3lf)\n", priv->scr, (double) (priv->scr/27000000.0));
	...
	*/
	...
	if(found)
	{
		...
		if(as != NULL)
		{
			...
			{
				gx_msg(MSGT_MUXER, MSGL_DBG2, "Not enough audio data exit\n");
				return 0;
			}
		}
...
static size_t parse_mpeg12_video(muxer_stream_t* s, muxer_priv_t *priv, muxer_headers_t *spriv, float fps, size_t len)
{
	...
	gx_msg(MSGT_MUXER, MSGL_DBG2,"parse_mpeg12_video, len=%u\n", (uint32_t) len);
	if(s->buffer[0] != 0 || s->buffer[1] != 0 || s->buffer[2] != 1 || len<6)
	{
		gx_msg(MSGT_MUXER, MSGL_ERR,"Unknown video format, possibly non-MPEG1/2 stream, len=%d!\n", len);
		return 0;
	}
	...
	if(err)
		gx_msg(MSGT_MUXER, MSGL_ERR,"Warning: picture too short or broken!\n");
	...
	gx_msg(MSGT_MUXER, MSGL_DBG2,"parse_mpeg12_video, return %u\n", (uint32_t) len);
	return len;
}
...
static uint64_t fix_mp4_frame_duration(muxer_headers_t* vpriv)
{
	...
	for(i = 0; i < 3; i++)
	{
		gx_msg(MSGT_DECVIDEO,MSGL_DBG2, "PTS: %"PRIu64"\n", vpriv->framebuf[i].pts);
	...
	gx_msg(MSGT_DECVIDEO,MSGL_DBG2, "MIN: %"PRIu64", mid: %"PRIu64", max: %"PRIu64", diff: %"PRIu64"\n", mn, md, mx, diff);
	if(diff > 0)
	{
		for(i=0; i<3; i++)
		{
			...
			gx_msg(MSGT_MUXER, MSGL_DBG2, "FIXED_PTS: %.3lf, FIXED_DTS: %.3lf\n",
...
static size_t parse_mpeg4_video(muxer_stream_t* s, muxer_priv_t *priv, muxer_headers_t *vpriv, float fps, size_t len)
{
	...
	gx_msg(MSGT_MUXER, MSGL_DBG2,"parse_mpeg4_video, len=%u\n", (uint32_t) len);
	if(len<6)
	{
		gx_msg(MSGT_MUXER, MSGL_ERR,"Frame too short: %d, exit!\n", len);
		return 0;
	}
	...
	if(!vpriv->frame_duration && vpriv->framebuf_used == 3)
	{
		...
		if(vpriv->frame_duration)
		{
			...
			gx_msg(MSGT_MUXER, MSGL_INFO, "FRAME DURATION: %"PRIu64"   %.3lf\n",
				vpriv->frame_duration, (double) (vpriv->frame_duration/27000000.0));
			vpriv->is_ready = 1;
		}
	}

	gx_msg(MSGT_MUXER, MSGL_DBG2, "LAST_PTS: %.3lf, LAST_DTS: %.3lf\n",
...
static int fill_last_frame(muxer_headers_t* spriv,  uint8_t *ptr, int len)
{
	...
	if(spriv->framebuf[idx].alloc_size < spriv->framebuf[idx].size + len)
	{
		if(spriv->framebuf[idx].size > SIZE_MAX - (size_t)len)
			return 0;
		spriv->framebuf[idx].buffer = (uint8_t*) av_realloc(spriv->framebuf[idx].buffer, spriv->framebuf[idx].size + len);
...
static int add_frame(muxer_headers_t* spriv, uint64_t idur, uint8_t *ptr, int len, uint8_t pt, uint64_t dts, uint64_t pts)
{
	...
	if(idx >= spriv->framebuf_cnt)
	{
		...
		if(spriv->framebuf == NULL)
		{
			gx_msg(MSGT_MUXER, MSGL_FATAL, "Couldn't av_realloc frame buffer(idx), abort\n");
			return -1;
		}
		...
		spriv->framebuf[spriv->framebuf_cnt].buffer = (uint8_t*) av_malloc(len);
		if(spriv->framebuf[spriv->framebuf_cnt].buffer == NULL)
		{
			gx_msg(MSGT_MUXER, MSGL_FATAL, "Couldn't av_realloc frame buffer(frame), abort\n");
			return -1;
		}
	...
	if(spriv->framebuf[idx].alloc_size < spriv->framebuf[idx].size + len)
	{
		if(spriv->framebuf[idx].size > SIZE_MAX - (size_t)len)
		{
			gx_msg(MSGT_MUXER, MSGL_FATAL, "Size overflow, couldn't av_realloc frame buffer(frame), abort\n");
			return -1;
		}
		spriv->framebuf[idx].buffer = av_realloc(spriv->framebuf[idx].buffer, spriv->framebuf[idx].size + len);
...
static int parse_audio(muxer_stream_t* s, int finalize, unsigned int *nf, double *timer, double delay, int drop)
{
	...
	while(1)
	{
		...
		if(frm_idx < 0)
		{
			gx_msg(MSGT_MUXER, MSGL_FATAL, "Couldn't add audio frame buffer(frame), abort\n");
			goto audio_exit;
		}
...
static void mpegfile_write_chunk(muxer_stream_t* s,size_t len,unsigned int flags, double dts_arg, double pts_arg)
{
	...
	{ // try to recognize frame type...
		...
		gx_msg(MSGT_MUXER, MSGL_DBG2,"mpegfile_write_chunk, Video codec=%x, len=%u, mpeg12 returned %u\n", stream_format, (uint32_t) len, (uint32_t) sz);
	}
	else
	{ // MUXER_TYPE_AUDIO
		double fake_timer;
		...
		if(s->b_buffer_size - s->b_buffer_len < len)
		{
			...
			{
				gx_msg(MSGT_MUXER, MSGL_FATAL, "\nFATAL! couldn't av_realloc, integer overflow\n");
				return;
			}
			tmp = av_realloc(s->b_buffer, len  + s->b_buffer_len);
			if(!tmp)
			{
				gx_msg(MSGT_MUXER, MSGL_FATAL, "\nFATAL! couldn't av_realloc %d bytes\n", len  + s->b_buffer_len);
				return;
			}
			s->b_buffer = tmp;

			s->b_buffer_size = len  + s->b_buffer_len;
			gx_msg(MSGT_MUXER, MSGL_DBG2, "REALLOC(%d) bytes to AUDIO backbuffer\n", s->b_buffer_size);
		}
		...
		else
		{
			...
			spriv->vframes += nf;
			if(! spriv->vframes)
				gx_msg(MSGT_MUXER, MSGL_INFO, "AINIT: %.3lf\r\n", (double) spriv->last_pts/27000000.0f);
		}
	}
...
static void mpegfile_write_index(muxer_t* muxer)
{
	...
	gx_msg(MSGT_MUXER, MSGL_INFO, MSGTR_WritingTrailer);
...
static void mpegfile_write_header(muxer_t* muxer)
{
	...
	gx_msg(MSGT_MUXER, MSGL_INFO, MSGTR_WritingHeader);
...
{
	muxer_priv_t* priv;
	priv = (muxer_priv_t* ) av_calloc(1, sizeof(muxer_priv_t));
	if(priv == NULL)
	return 0;
	priv->update_system_header = 1;

	//av_calloc() already zero-ed all flags, so we assign only the ones we need
	...
	if(conf_mux != NULL)
	{
		...
		else
		{
			if(strcasecmp(conf_mux, "mpeg2"))
				gx_msg(MSGT_MUXER, MSGL_ERR, "Unknown format %s, default to mpeg2\n", conf_mux);
			priv->mux = MUX_MPEG2;
			priv->is_genmpeg2 = 1;
			priv->packet_size = 2048;
			priv->muxrate = 1800*  125;	//Constrained parameters
	...
	gx_msg(MSGT_MUXER, MSGL_INFO, "PACKET SIZE: %u bytes, deltascr: %"PRIu64"\n", priv->packet_size, priv->delta_scr);
	...
	if(conf_vaspect > 0)
	{
		...
		if(asp >= 1332 && asp <= 1334)
			...
			gx_msg(MSGT_MUXER, MSGL_ERR, "ERROR: unrecognized aspect %.3f\n", conf_vaspect);
	}

	priv->vframerate = 0;		// no change
	if(conf_telecine && conf_vframerate > 0)
	{
		gx_msg(MSGT_MUXER, MSGL_ERR, "ERROR: options 'telecine' and 'vframerate' are mutually exclusive, vframerate disabled\n");
		conf_vframerate = 0;
	}
	...
	if(conf_telecine_src>0 && conf_telecine_dest>0 && conf_telecine_src < conf_telecine_dest)
	{
		...
		if(((tfps - sfps)>>1) > sfps)
		{
			gx_msg(MSGT_MUXER, MSGL_ERR, "ERROR! Framerate increment must be <= 1.5, telecining disabled\n");
			conf_telecine = 0;
		}
		else
		{
	...
	if(conf_vframerate)
	{
		...
		switch(fps)
		{
			...
			default:
			{
				gx_msg(MSGT_MUXER, MSGL_ERR, "WRONG FPS: %d/1000, ignoring\n", fps);
				if(conf_telecine)
					gx_msg(MSGT_MUXER, MSGL_ERR, "DISABLED TELECINING\n");
				conf_telecine = 0;
			}
		}
	}
	...
	if(priv->vaspect || priv->vframerate || priv->vwidth || priv->vheight || priv->vbitrate || priv->panscan_width || priv->panscan_height)
	{
		...
		gx_msg(MSGT_MUXER, MSGL_INFO, "MPEG MUXER, patching");
		if(priv->vwidth || priv->vheight)
			gx_msg(MSGT_MUXER, MSGL_INFO, " resolution to %dx%d", priv->vwidth, priv->vheight);
		if(priv->panscan_width || priv->panscan_height)
			gx_msg(MSGT_MUXER, MSGL_INFO, " panscan to to %dx%d", priv->panscan_width, priv->panscan_height);
		if(priv->vframerate)
			gx_msg(MSGT_MUXER, MSGL_INFO, " framerate to %s fps", framerates[priv->vframerate]);
		if(priv->vaspect)
			gx_msg(MSGT_MUXER, MSGL_INFO, " aspect ratio to %s", aspect_ratios[priv->vaspect]);
		if(priv->vbitrate)
			gx_msg(MSGT_MUXER, MSGL_INFO, " bitrate to %u", conf_vbitrate);
		gx_msg(MSGT_MUXER, MSGL_INFO, "\n");
	}
	...
	priv->buff = (uint8_t* ) av_malloc(priv->packet_size);
	if((priv->buff == NULL))
	{
