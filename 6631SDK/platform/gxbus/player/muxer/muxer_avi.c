...
static muxer_stream_t* avifile_new_stream(muxer_t* muxer,int type){
    ...
    s=av_malloc(sizeof(muxer_stream_t));
    ...
    s->size=0;
    s->muxer=muxer;
    s->priv=si=av_malloc(sizeof(struct avi_stream_info));
    memset(si,0,sizeof(struct avi_stream_info));
    si->idxsize=256;
    si->idx=av_calloc(si->idxsize, sizeof(struct avi_odmlidx_entry));
    si->riffofssize=16;
    si->riffofs=av_calloc((si->riffofssize+1), sizeof(off_t));
    memset(si->riffofs, 0, sizeof(off_t)*si->riffofssize);

    switch(type){
    case MUXER_TYPE_VIDEO:
      s->ckid=GX_FOURCC(('0'+s->id/10),('0'+(s->id%10)),'d','c');
      s->h.fccType=streamtypeVIDEO;
      if(!muxer->def_v) muxer->def_v=s;
      break;
    case MUXER_TYPE_AUDIO:
      s->ckid=GX_FOURCC(('0'+s->id/10),('0'+(s->id%10)),'w','b');
...
static void avifile_odml_new_riff(muxer_t* muxer)
{
    ...
    /* RIFF/AVIX chunk*/
    riff[0]=le2me_32(GX_FOURCC('R','I','F','F'));
    riff[1]=0;
    riff[2]=le2me_32(GX_FOURCC('A','V','I','X'));
      ...
      case MUXER_TYPE_VIDEO:
{
	  ...
	  if (aspect != 0) {
	      ...
	      write_avi_chunk(muxer->stream,GX_FOURCC('v','p','r','p'),
      ...
      case MUXER_TYPE_AUDIO:
{
	  ...
	  idxhdr[0] = le2me_32(GX_FOURCC('i', 'n', 'd', 'x'));
  ...
  if (isodml) {
      ...
      write_avi_list(muxer->stream,GX_FOURCC('o','d','m','l'),sizeof(dmlh)+8);
      write_avi_chunk(muxer->stream,GX_FOURCC('d','m','l','h'),sizeof(dmlh),dmlh);
  }

// ============= INFO ===============
// always include software info
info[0].id=GX_FOURCC('I','S','F','T'); // Software:
info[0].text="MEncoder " VERSION;
// include any optional strings
i=1;
if(info_name!=NULL){
  info[i].id=GX_FOURCC('I','N','A','M'); // Name:
  info[i++].text=info_name;
}
if(info_artist!=NULL){
  info[i].id=GX_FOURCC('I','A','R','T'); // Artist:
  info[i++].text=info_artist;
}
if(info_genre!=NULL){
  info[i].id=GX_FOURCC('I','G','N','R'); // Genre:
  info[i++].text=info_genre;
}
if(info_subject!=NULL){
  info[i].id=GX_FOURCC('I','S','B','J'); // Subject:
  info[i++].text=info_subject;
}
if(info_copyright!=NULL){
  info[i].id=GX_FOURCC('I','C','O','P'); // Copyright:
  info[i++].text=info_copyright;
}
if(info_sourceform!=NULL){
  info[i].id=GX_FOURCC('I','S','R','F'); // Source Form:
  info[i++].text=info_sourceform;
}
if(info_comment!=NULL){
  info[i].id=GX_FOURCC('I','C','M','T'); // Comment:
...
info[i].id=0;
  ...
  // write infos:
  if (hdrsize!=0){
      write_avi_list(muxer->stream,GX_FOURCC('I','N','F','O'),hdrsize);
      for(i=0;info[i].id!=0;i++) if(info[i].text){
...
static void avifile_odml_write_index(muxer_t* muxer){
  ...
  for (i=0; i<muxer->avih.dwStreams; i++) {
    ...
    */
    ...
    si->superidx = av_calloc(si->superidxsize, sizeof(*si->superidx));
	...
	idxhdr[0] = le2me_32((s->ckid << 16) | GX_FOURCC('i', 'x', 0, 0));
