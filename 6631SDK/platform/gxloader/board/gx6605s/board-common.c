...
{
	int i = 0;
	unsigned char sdas_select = 0;
	unsigned char scls_select = 0;
	...
	for(i = 0; i < array_count; i++){
		if(mulpin_table[i].fun != NOT_CONFIG){
			...
			if (mulpin_table[i].chip_core_num == 126 && mulpin_table[i].fun == 2)
				sdas_select = 1;
			if (mulpin_table[i].chip_core_num == 127 && mulpin_table[i].fun == 2)
				scls_select = 1;
		}
	}
/*	printf("REG_PINMUX_PORTA=0x%x.\n", *(volatile unsigned int*)REG_PINMUX_PORTA);
	...
	if (sdas_select && scls_select)
		REG_SET_CLR_BIT(REG_PINMUX_PORTE, 27, 1);
...
void dac_gain_control(void)
{
	...
	if (otp_data != 0) {
		...
		*(volatile unsigned int*)(VPU_BASE_ADDR + 0x130) = 0x20202020;
