...
{
	unsigned int   freq = 0;
	unsigned char  pll_posdiv2 = 0;
	unsigned char  pll_posdiv1 = 0;
	unsigned char  pll_refdiv = 0;
	unsigned char  pll_fbdiv = 0;
	unsigned int cpu_base = 0;

	cpu_base = *(volatile unsigned int *)(CONFIG_BASE + CONFIG_PLL3_CONFIG);

	pll_fbdiv = cpu_base & 0xFFF;
	pll_posdiv1 = (cpu_base >> 12) & 0xF;
	pll_posdiv2 =  (cpu_base >> 16) & 0xF;
	pll_refdiv = (cpu_base >> 20) & 0x7F;

	freq = (GX_EXT_CLOCK / 1000000) * pll_fbdiv / pll_posdiv1 / pll_posdiv2 / pll_refdiv;
...
void arch_init(void)
{
	...
	extern void mmu_table_setup(void);
	mmu_table_setup();
#endif
