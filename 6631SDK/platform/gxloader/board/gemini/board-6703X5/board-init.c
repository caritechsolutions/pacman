...
int board_init(void)
{
	...
	cmdline_clear();
	if (strncmp((const char *)gx_get_chip_name(), "6702H5", 6) == 0) {
		printf("6702H5");
		cmdline_init(CONFIG_CMDLINE_VALUE);
	}
	else if (strncmp((const char *)gx_get_chip_name(), "6702S5", 6) == 0) {
		printf("6702S5");
		cmdline_init(CONFIG_CMDLINE_VALUE1);
	}
	else if (strncmp((const char *)gx_get_chip_name(), "6701", 4) == 0) {
		printf("6701");
		cmdline_init(CONFIG_CMDLINE_VALUE2);
	}
	else {
		printf("not find chip name : %s", gx_get_chip_name());
		cmdline_init(CONFIG_CMDLINE_VALUE);
	}
