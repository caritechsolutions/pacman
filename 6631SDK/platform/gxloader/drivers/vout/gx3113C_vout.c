...
static volatile Gx3201SvpuReg *gx3201svpu_reg = (void*)SVPU_BASE_ADDR;
...
#if 0
	0x408040, 0x408040, 0x408040, 0x408040,
	0x408040, 0x408040, 0x408040, 0x408040,
	0x408040, 0x408040, 0x408040, 0x408040,
	0x408040, 0x408040, 0x408040, 0x408040,
	0x408040, 0x408040, 0x408040, 0x408040,
...
static unsigned int gx3201_dve_pal_bdghi[] = {
	0x00000000, 0x00000000, 0x0097D47E, 0x001AE677,
...
static unsigned int gx3201_dve_pal_m[] = {
	0x00000001, 0x00000000, 0x009de17e, 0x001A2671,
...
static unsigned int gx3201_dve_pal_n[] = {
	0x00000002, 0x00000000, 0x0097d47e, 0x001AE677,
	0x00781e13, 0x00b0a610, 0x25d05940, 0x1EC39102,
	0x20885218, 0x2010F094, 0x00050000, 0x00000000,
...
static unsigned int gx3201_dve_pal_nc[] = {
	0x00000003, 0x00000000, 0x0097da7e, 0x001AE677,
	0x00781e13, 0x00b0a610, 0x25d05940, 0x1EC39102,
	0x20885218, 0x2010F094, 0x00050000, 0x00000000,
...
static unsigned int gx3201_dve_ntsc_m[] = {
	0x00000004, 0x00000000, 0x0090d47e, 0x0017a65d,//0x001A2671,
	0x00781E13, 0x00B0A610, 0x25D05940, 0x1EC39102,
...
static unsigned int gx3201_dve_ntsc_443[] = {
	0x00000005, 0x00000000, 0x0090c67e, 0x001A2671,
	0x00781E13, 0x00B0A610, 0x25D05940, 0x1EC39102,
	0x20885218, 0x2010F094, 0x0004b800, 0x00000000,
	0x64803c47, 0x00003f37, 0x00000000, 0x0000008e,
	0x00000000, 0x00000000, 0x000000c6, 0x00000000,
			...
			if ((GXAV_VOUT_HDMI_1080I_60HZ == vout_mode) ||
                ...
                case HDMI_YUV_444:
                        if ((GXAV_VOUT_HDMI_480I == vout_mode) ||
                                (GXAV_VOUT_HDMI_576I == vout_mode)) {
                                // TV
                                hdmi_write(0x64, 0x40);
                                hdmi_write(0x65, 0x98);
                                // HDMI内部处理
                                hdmi_write(0x15, 0x00);
                                hdmi_write(0x16, 0x75);
                                hdmi_write(0x17, 0x22);
                                hdmi_write(0xd3, 0xa0);
                                hdmi_write(0xd4, 0x10);
                                hdmi_write(0x3b, 0x00);
                        }
                        else if ((GXAV_VOUT_HDMI_480P == vout_mode) ||
                                        (GXAV_VOUT_HDMI_576P == vout_mode) ||
                                        (GXAV_VOUT_HDMI_720P_50HZ == vout_mode) ||
                                        (GXAV_VOUT_HDMI_720P_60HZ == vout_mode) ||
                                        (GXAV_VOUT_HDMI_1080I_50HZ == vout_mode) ||
                                        (GXAV_VOUT_HDMI_1080I_60HZ == vout_mode) ||
                                        (GXAV_VOUT_HDMI_1080P_50HZ == vout_mode) ||
                                        (GXAV_VOUT_HDMI_1080P_60HZ == vout_mode)) {
                                // TV
                                hdmi_write(0x64, 0x40);
                                ...
                                // HDMI内部处理
                                hdmi_write(0x15, 0x00);
                                hdmi_write(0x16, 0x77);
                                hdmi_write(0x17, 0x22);
                                hdmi_write(0xd3, 0xa1);
                                hdmi_write(0xd4, 0x00);
                                hdmi_write(0x3b, 0x00);
                        }
                        break;
                case HDMI_YUV_422:
                        if ((GXAV_VOUT_HDMI_480I == vout_mode) ||
                                (GXAV_VOUT_HDMI_576I == vout_mode)) {
                                // TV
                                hdmi_write(0x64, 0x20);
                                hdmi_write(0x65, 0x98);
                                // HDMI内部处理
                                hdmi_write(0x15, 0x00);
                                hdmi_write(0x16, 0xb5);
                                hdmi_write(0x17, 0x22);
                                hdmi_write(0xd3, 0xa0);
                                hdmi_write(0xd4, 0x10);
                                hdmi_write(0x3b, 0x00);
...
static void Gx3201_enable_svpu(int mode)
{
	...
	*(volatile unsigned int*)(0xa4900088) = 1;
...
void vpu_svpu_hdmi_init(void)
{
	...
	g_ypbpr_hdmi_mode = G_YPBPR_HDMI_576I;
	...
	switch(cvbs_mode){
		...
		case G_PAL:
			...
			p_vpu_init = gx3201_dve_pal_bdghi;
			vpu_init_cnt = sizeof(gx3201_dve_pal_bdghi) / sizeof(gx3201_dve_pal_bdghi[0]);
		...
		case G_PAL_M:
			...
			p_vpu_init = gx3201_dve_pal_m;
			vpu_init_cnt = sizeof(gx3201_dve_pal_m) / sizeof(gx3201_dve_pal_m[0]);
		...
		case G_PAL_N:
			...
			p_vpu_init = gx3201_dve_pal_n;
			vpu_init_cnt = sizeof(gx3201_dve_pal_n) / sizeof(gx3201_dve_pal_n[0]);
		...
		case G_PAL_NC:
			...
			p_vpu_init = gx3201_dve_pal_nc;
			vpu_init_cnt = sizeof(gx3201_dve_pal_nc) / sizeof(gx3201_dve_pal_nc[0]);
		...
		case G_NTSC_M:
			...
			p_vpu_init = gx3201_dve_ntsc_m;
			vpu_init_cnt = sizeof(gx3201_dve_ntsc_m) / sizeof(gx3201_dve_ntsc_m[0]);
		...
		case G_NTSC_443:
			...
			p_vpu_init = gx3201_dve_ntsc_443;
			vpu_init_cnt = sizeof(gx3201_dve_ntsc_443) / sizeof(gx3201_dve_ntsc_443[0]);
	...
	if(cvbs_mode == G_PAL)
	{
	*(volatile unsigned int *)(0x4804000) = 0x40000000 ;
	*(volatile unsigned int *)(0x4804000) |= 0x1<<25   ; //4   //test mode
	*(volatile unsigned int *)(0xa4800134) = 0 ; //0xb ; //test mode
	
	
	*(volatile unsigned int *)(0xa4804010) =/*	 7 << 20 |	 7 << 10 | 534 ;//*/0x00681a11 ;
	*(volatile unsigned int *)(0xa4804014) =/* 524 << 20 |	49 << 10 | 19  ;//*/0x00f0a215 ;
	*(volatile unsigned int *)(0xa4804018) =/* 622 << 20 | 553 << 10 | 332 ;//*/0x26404547 ;
	*(volatile unsigned int *)(0xa480401c) =/* 566 << 20 | 0x4897a		   ;//*/0x1f64897a ;
	*(volatile unsigned int *)(0xa4804024) = 0x2338706b  ;
	
	*(volatile unsigned int *)(0xa4804028) = 0x0a023cc8  ;
	*(volatile unsigned int *)(0xa4804030) = 0x6a963f3f  ;
	*(volatile unsigned int *)(0xa4804034) = 0x3f61 	 ;

	*(volatile unsigned int *)(0xa4800130) = 0x3f3f3f38  ;

//	*(volatile unsigned int *)(0xa4800134) = 0xe  ;   // dac power down  , bit 0 open
	}
	...
	//video dac config
	if(*(volatile unsigned int*)(SVPU_BASE_ADDR + 0X84) == 0)
		*(volatile unsigned int *)(SVPU_BASE_ADDR + 0X84) = 0x1f;
	*(volatile unsigned int *)(SVPU_BASE_ADDR + 0x80) = 0xffffffff;
...
int gx_layer_zoom(int layer, int source_width, int source_height, int dest_width, int dest_height, int start_x, int start_y)
{
	...
	switch(ypbpr_hdmi_mode) {
		...
		case G_YPBPR_HDMI_1080P_60HZ:
			...
			if((dest_height-1) > (source_height*(1+vpu_mode)/(1+ vdownscale)-1)*4096/(vzoom*(1+vpu_mode)/2))
				//start_y_here = ((dest_height-1) - (((source_height*(1+vpu_mode)/(1+ vdownscale))-1)<<12)/((vzoom*(1+vpu_mode))>>1))>>1;
				start_y_here = 0;
			break;
		default:
			break;
	...
	if(layer == 0){	//spp
		...
		*(volatile u32*)(VPU_BASE_ADDR + 0x54)  = hzoom | vzoom<<16;	//JPG_ZOOM
        buff_len = (((source_width>>2)>>3)<<3);
		
        if((source_width - ((buff_len)<<2)) < 32){
            buff_len += 8;
        }
		/*
			if last data < 32 , then will happen yellow
			eg: (720>>2)>>3)<<3 = 176 . 720 - 176 * 4 = 16 ;
		*/
		*(volatile u32*)(VPU_BASE_ADDR + 0x58) &= ~(0x7ff<<16);	//JPG_PARA
		*(volatile u32*)(VPU_BASE_ADDR + 0x58) |= buff_len <<16 ;
	...
	//enable video dac
	*(volatile unsigned int *)0xa4800134 = 0x0;
