...
struct partition *flash_partition_table = NULL;
struct partition_info *oem_partition = NULL;
struct partition_info *backoem_partition = NULL;

static int g_oem_need_backup = 1;

#define INI_DEFAULT_BLOCK_SIZE 4096
static int ini_block_size = INI_DEFAULT_BLOCK_SIZE;
static handle_t v_ini = 0;
static handle_t ini_mutex = 0;

#define MAX_OEM_FILE_NAME_LEN 100

#define ini_partition_read(p, buf, size, offset) partition_read((p), (offset), (buf), (size))
#define ini_partition_writeraw(p, buf, size, offset) (partition_write((p), (offset), (buf), (size)) ? 0 : 1)
...
static int ini_flash_partition_table_update(struct partition_info* partition)
{
	int32_t i = 0;
	...
	for (i = 0; i < flash_partition_table->count; i++) {
		if (strncasecmp(partition->name, flash_partition_table->tables[i].name, FLASH_PARTITION_NAME_SIZE) == 0) {
			memcpy(&flash_partition_table->tables[i], partition, sizeof(struct partition_info));
...
/* 此函数只能用于存放ascii字符的分区 */
static int32_t ini_write_oem_partition(struct partition_info* partition, char* buf, int32_t size)
{
	int ret = 0;
	char *oem_buf = NULL;

	if (size > (ini_block_size - 5)) {
		printf("oem size is larger than %d - 5\n", ini_block_size);
		ret = -1;
		return ret;
	}

	oem_buf = malloc(ini_block_size);
	...
	ini_partition_read(partition, oem_buf, ini_block_size, 0);
	memcpy(oem_buf, buf, size);
	oem_buf[size] = 0xFF; //Use 0xFF as the terminator
	partition_erase(partition);
	if (ini_partition_writeraw(partition, oem_buf, ini_block_size, 0) != 1) {
		ret = -1;
		printf("write oem error\n");
	}
...
minifs_handle_dev gxopen(char *partition_name,char *file_name)
{
	...
	static struct partition_info *partition_data = NULL;
	minifs_handle_dev fd;
	if (flash_table == NULL)
		flash_table = GxCore_PartitionFlashInit();

	if(partition_data == NULL) {
		partition_data = GxCore_PartitionGetByName((struct partition *)flash_table, partition_name);
		if(partition_data == NULL) {
			printf(" partition data byname err\n");
			goto err;
		}
	}

	if(partition_data->file_system_type != MINIFS){
		printf("this partition fs is not minifs\n");
		goto err;
	}
	...
	if (!oem_device) {
		printf("this partition mount failed!\n");
		goto err;
	}
...
static int ini_open_oem(char* oem_path)
{
...
#endif
	{
		if (flash_partition_table == NULL)
			flash_partition_table = all_partition_update();
		if (flash_partition_table == NULL) {
			printf(" partition table init failed\n");
			return -1;
		}
		oem_partition = all_partition_get(oem_path);
		if (oem_partition == NULL) {
			printf(" oem partition by name err\n");
			return -1;
		}
	}
...
static int ini_oem_size(void)
{
	...
	{
		char* buf = NULL;
		buf = malloc(ini_block_size);
		if (buf == NULL)
			return 0;
		ini_partition_read(oem_partition, buf, ini_block_size, 0);
...
#endif
	{
		ini_partition_read(oem_partition, buf, size, 0);
...
static int ini_open_backoem(char* backoem_path)
{
...
#endif
	{
		...
		backoem_partition = all_partition_get(backoem_path);
		if (backoem_partition == NULL) {
			printf("backoem partition by name err\n");
			return -1;
		}
	}
...
static int ini_backoem_size(void)
{
...
#endif
	{
		...
		if (buf == NULL)
			return 0;
		ini_partition_read(backoem_partition, buf, ini_block_size, 0);
		size = ini_calc_oem_patition_used_size(buf, ini_block_size);
		free(buf);
	}
...
#endif
	{
		ini_partition_read(backoem_partition, buf, size, 0);
		read_size = ini_calc_oem_patition_used_size(buf, size);
	}
...
static int32_t ini_setcrcvalue(uint32_t crc32, uint32_t crc32_back)
{
	...
	if (crc32 != 0) {
		...
		if (g_oem_use_minifs) {
			...
			if (ret < 0) {
				printf("c_ini do not have the section and parameter, append it to the b_ini!\n");
				goto err;
			}
#ifdef NO_OS
			size = minifs_size(crc_file);
			buf = (char *)malloc(2*size);
			if (buf == NULL) {
				goto err;
			}
...
#endif
		{
			...
			partition_erase(oem_partition);
			if (ini_partition_writeraw(oem_partition, buf, ini_block_size, 0) != 1) {
				ret = -1;
				goto err;
			}
		}
	...
	if (crc32_back != 0) {
...
#endif
		{
			...
			partition_erase(backoem_partition);
			if (ini_partition_writeraw(backoem_partition, buf, ini_block_size, 0) != 1) {
				ret = -1;
				goto err;
			}
		}
	...
	if (v_ini != 0)
	{
		if(buf != NULL)
		{
			...
			if (g_oem_need_backup) {
				...
				crc_result = crc32(0, (uint8_t *)buf, file_size);
...
int32_t Ini_SetValue(const char *section, const char *parameter, const char *value)
{
	...
	if(v_ini != 0)
	{
		...
		if (tmp == NULL) {
			printf("I_ini could not be channed!\n");
			ret = -1;
			goto err;
		}
		...
		if (ret < 0) {
			printf("b_ini do not have the section and parameter, append it to the b_ini!\n");
			goto err;
		}
	}
err:
...
int32_t Ini_Sync(void)
{
	...
	if(v_ini != 0) {
		buf = (char *)malloc(ini_block_size);
		if (buf == NULL) {
			goto err;
		}
...
#define MINIFS_OEM_FILE_PATH_PREFIX "/home/gx"
	...
	if (size > 0) {
		...
		crc_result_orig = crc32(0, (uint8_t *)v_buf, file_size);
	...
	if (g_oem_need_backup) {
		...
		if (size > 0) {
			...
			crc_result_back = crc32(0, (uint8_t *)b_buf, backfile_size);
		}
	}

	if (file_size == 0 && backfile_size == 0) {
		printf("oem file is empty\n");
		goto err;
	}
	...
	if (g_oem_need_backup) {
...
#endif
		{
			...
			ini_partition_read(oem_partition, &crc_oem, 4, ini_block_size - 4);
		}
		...
		{
			/* oem和backoem分区的CRC校验放在各自的分区的最后的4个字节*/
			ini_partition_read(backoem_partition, &crc_backoem, 4, ini_block_size - 4);
		}

		printf("crc_oem : 0x%08x\n", crc_oem);
		printf("crc_backoem : 0x%08x\n", crc_backoem);
		printf("crc_result_orig : 0x%08x\n", crc_result_orig);
		printf("crc_result_back : 0x%08x\n", crc_result_back);

		if (file_size == 0 && backfile_size != 0) {
			if (ini_write_oem(b_buf, backfile_size) != 0) {
				printf("check write original err\n");
				goto err;
			}
