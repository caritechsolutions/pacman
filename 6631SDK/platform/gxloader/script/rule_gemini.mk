...
CFLAGS += -DCONFIG_ARCH_CKMMU_GEMINI
CFLAGS += -DCONFIG_ARCH_CKMMU_GEMINI_DEV
endif

ifndef CONFIG_CHIP_PACKAGE
...
extern_param.bin : Makefile
	...
	@xxd -r -p extern_param.txt | od -v -An -t x1 -w4 | awk '{print $$4, $$3, $$2, $$1}' | xxd -r -p >> $@
	@printf %08x 00000000 > extern_param.txt  #current the cpu frequency of all type of gemini chip is same
	@xxd -r -p extern_param.txt | od -v -An -t x1 -w4 | awk '{print $$4, $$3, $$2, $$1}' | xxd -r -p >> $@
...
ifeq ($(ENABLE_BOOT_TOOL), y)
	BOOTLOADER_MAGIC='01670100'
ifeq ($(CONFIG_JZ_TAURUS), y)
	FILE_PREFIX="taurus-$(CHIP_BOARD)"
else
	FILE_PREFIX="gemini-$(CHIP_BOARD)"
endif
	FILE_POSTFIX="boot"
...
ifeq ($(ENABLE_SECURE_VERIFY), y)
	...
	@cd $(SIGN_DIR);./digital_signature.sh 1 $(NORMAL_FILE_NAME) $(SECURE_VERIFY_TYPE) gemini 1 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE);cd $(CUR_PWD)
	@mv $(SIGN_DIR)/output/$(SIGN_LEVEL1_FILE_NAME) $(SIGN_LEVEL1_FILE_NAME)
	@cd $(SIGN_DIR);./digital_signature.sh 1 $(NORMAL_FILE_NAME) $(SECURE_VERIFY_TYPE) gemini 2 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE);cd $(CUR_PWD)
