...
ifeq ($(DDR_TYPE)$(DDR_SIZE), DDR3128)
	DDR_SEC_PAR_INCLUDE =  "ddr2_$(DDR_FREQ)$(DDR_CONFIG)_$(DDR_SIZE)MB.h"
endif
endif
##################### DDR config end #########################################
...
extern_param.bin : Makefile
	...
	@xxd -r -p extern_param.txt | od -v -An -t x1 -w4 | awk '{print $$4, $$3, $$2, $$1}' | xxd -r -p >> $@
	@printf %08x 00000000 > extern_param.txt  #current the cpu frequency of all type of taurus chip is same
	@xxd -r -p extern_param.txt | od -v -An -t x1 -w4 | awk '{print $$4, $$3, $$2, $$1}' | xxd -r -p >> $@
...
ifeq ($(ENABLE_SECURE_VERIFY), y)
	...
	@cd $(SIGN_DIR);./digital_signature.sh 1 $(NORMAL_FILE_NAME) $(SECURE_VERIFY_TYPE) taurus 1 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE);cd $(CUR_PWD)
	@mv $(SIGN_DIR)/output/$(SIGN_LEVEL1_FILE_NAME) $(SIGN_LEVEL1_FILE_NAME)
	@cd $(SIGN_DIR);./digital_signature.sh 1 $(NORMAL_FILE_NAME) $(SECURE_VERIFY_TYPE) taurus 2 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE);cd $(CUR_PWD)
...
ifeq ($(ENABLE_SECURE_VERIFY), y)
	...
	@./$(ENCRYPT_DIR)/data_encrypt.sh 1 $(NORMAL_FILE_NAME) $(NORMAL_ENCRYPT_FILE_NAME) $(DATA_ENCRYPT_TYPE) taurus $(STAGE1_ENCRYPT) $(STAGE2_ENCRYPT) 0 0 0 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE)
...
ifeq ($(ENABLE_SECURE_VERIFY), y)
	...
	@./$(ENCRYPT_DIR)/data_encrypt.sh 1 $(SIGN_LEVEL1_FILE_NAME) $(SIGN_LEVEL1_ENCRYPT_FILE_NAME) $(DATA_ENCRYPT_TYPE) taurus $(STAGE1_ENCRYPT) $(STAGE2_ENCRYPT) 0 0 1 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE)
	...
	@./$(ENCRYPT_DIR)/data_encrypt.sh 1 $(SIGN_LEVEL2_FILE_NAME) $(SIGN_LEVEL2_ENCRYPT_FILE_NAME) $(DATA_ENCRYPT_TYPE) taurus $(STAGE1_ENCRYPT) $(STAGE2_ENCRYPT) 0 0 2 $(ENABLE_EXTERN_PARAM) $(EXTERN_PARAM_FILE_SIZE)
...
LD_FILE=cpu/ck/gemini_taurus/$(LD_FILE_NAME)
