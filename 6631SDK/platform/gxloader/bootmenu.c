...
void command_flash(int argc, const char **argv)
{
	...
	if(argv[1] == NULL){
		...
		printf("flash unalignsingletest <addr> <len> <align>\n");
		printf("flash unaligncomtest [cnt=20]\n");
#endif
		goto gxflash_out;
	}
...
#endif
	else if (strcmp(argv[1], "unalignsingletest") == 0) {
		unsigned int addr, len, align;
		if (argc == 5) {
			addr = atoi(argv[2]);
			len = atoi(argv[3]);
			align = atoi(argv[4]);
			flash_unalign_test_single(addr, len, align);
		} else
			printf("flash unalignsingletest <addr> <len> <align>\n");
	}
	else if (strcmp(argv[1], "unaligncomtest") == 0) {
		unsigned int count = 20;
		if (argc > 2) {
			count = atoi(argv[2]);
		}
		flash_unalign_complete_test(count);
	}
#endif
...
void otp_operation(int argc, const char **argv)
{
	...
	}else if(strcmp("getregionsize", cmd) == 0){
		if (gxflash_otp_get_region_size(&data) == 0)
			printf("otp region size:%d\n", data);
		else
			printf("otp region size fail\n");
	}else{
