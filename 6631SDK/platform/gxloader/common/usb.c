...
/** from linux/ctype.h & lib/ctype.c **/
...
extern int usb_module_clock_disable(void);
 ...
 * @dev : hub usb device
 * @port : port index
 * For Root Hub refer to hcd dirver to get portstatus and
 * portchange info, disposing in ehci_submit_root function.
 * Note : Due to not use status register bit[31],so return
 * int rather than unsigned int. if not modify it
 */
int usb_get_port_stat(struct usb_device *dev, int port)
{
	...
	usb_control_msg(dev, usb_rcvctrlpipe(dev, 0),
			...
			portsts, sizeof(struct usb_hub_status), USB_CNTL_TIMEOUT);

	portstatus = le16_to_cpu(portsts->wPortStatus);
	portchange = le16_to_cpu(portsts->wPortChange);

	return ((portchange << 16) | portstatus);
}
...
#ifndef CONFIG_USB_WORKMODE_SPEED
