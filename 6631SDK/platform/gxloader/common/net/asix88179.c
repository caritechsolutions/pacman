...
struct asix_private st_asix_private;
struct eth_pdata st_eth_pdata;
...
static int asix_init_common(struct ueth_data *dev, struct asix_private *dev_priv)
{
...
static int asix_send_common(struct ueth_data *dev, struct asix_private *dev_priv, void *packet, int length)
{
	int err;
...
static int ax88179_eth_start(struct net_device *net_dev)
{
	struct asix_private *priv = &st_asix_private;
	int ret;

	ret = asix_write_mac(&priv->ueth, net_dev->dev_addr);
	if (ret)
		return ret;
...
static int ax88179_eth_stop(struct net_device *net_dev)
{
	struct asix_private *priv = &st_asix_private;
...
int ax88179_eth_send(struct sk_buff *skb, struct net_device *dev, int async)
{
	struct asix_private *priv = &st_asix_private;
	int ret;

	ret = asix_send_common(&priv->ueth, priv, skb->data, skb->len);
	skb_free(skb);
	return ret;
}

int ax88179_eth_recv(struct net_device *net_dev)
{
	struct asix_private *priv = &st_asix_private;
	struct ueth_data *ueth = &priv->ueth;
	int ret, len = 0;
	...
	*priv->pkt_hdr = le32_to_cpu(*priv->pkt_hdr);
	pkt_len = (*priv->pkt_hdr >> 16) & 0x1fff;
	if (pkt_len > 2) {
		struct sk_buff *skb = NULL;
		/*2字节的0xEE 0xEE头部,需要去掉*/
		skb = skb_alloc(pkt_len-2);
		if (skb != NULL) {
			memcpy(skb->data, priv->pkt_data + 2, pkt_len-2);
			skb->len = pkt_len-2;
			skb_put(skb);
		}
	}
...
static int ax88179_eth_probe(struct usb_device *usb_dev)
{
	struct eth_pdata    *pdata = &st_eth_pdata;
	struct asix_private *priv  = &st_asix_private;
	int ret;


	ret = usb_ether_register(usb_dev, &priv->ueth, AX_RX_URB_SIZE);
	if (ret)
		return ret;
...
int ax88179_eth_init(struct net_device *dev)
{
	return 0;
}

void ax88179_print_status(struct net_device *dev)
{
	return;
}

struct net_device_stats *ax88179_get_stats(struct net_device *dev)
{
	return 0;
}

int ax88179_probe(struct net_device *net_dev)
{
	int ret = 0;
	...
	for (i = 1; i < USB_MAX_DEVICE; i++) {
		usb_dev = usb_get_dev_index(i);
		if (usb_dev == NULL) {
			return 0;
		} else {
			ret = ax88179_eth_probe(usb_dev);
			if (ret) {
				return 0;
			}
			memcpy(net_dev->dev_addr, pdata->enetaddr, 6);
			break;
		}
	}
	net_dev->state          = NETDEV_DOWN;
	net_dev->init           = ax88179_eth_init;
	net_dev->open           = ax88179_eth_start;
	net_dev->close          = ax88179_eth_stop;
	net_dev->send_packet    = ax88179_eth_send;
	net_dev->receive_packet = ax88179_eth_recv;
	net_dev->priv           = &st_asix_private;
	net_dev->print_status   = ax88179_print_status;
	net_dev->get_stats      = ax88179_get_stats;
