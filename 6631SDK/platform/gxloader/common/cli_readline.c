...
char cmd_line_buffer[CONFIG_SYS_CBSIZE + 1];	/* console I/O buffer	*/
...
{
	/*
	 * If cmd_line_buffer isn't 0-length the user will be prompted to modify
	 * it instead of entering it from scratch as desired.
	 */
	cmd_line_buffer[0] = '\0';

	return cmd_line_readline_into_buffer(prompt, cmd_line_buffer, 0);
}


int cmd_line_readline_into_buffer(const char *const prompt, char *buffer,
			     int timeout)
{
	char *p = buffer;
