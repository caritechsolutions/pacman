		...
		const char *vid_header_offset)
{
	...
	sprintf(buffer, "%s,%d", ubidev, pnum);
	...
	if (vid_header_offset)
		sprintf(ubi_mtd_param_buffer, "%d,%s", pnum,
				vid_header_offset);
...
#if defined(CONFIG_ENABLE_UBI_TEST)
extern int mtd_probe_devices(void);
static int do_ubi(int argc, char **argv)
{
	int64_t size = 0;
	ulong addr = 0;
	...
	if (strcmp(argv[1], "detach") == 0) {
		if (argc < 2) {
			printf("ubi detach: invalid parameters\n");
			return;
		}
	...
	if (strncmp(argv[1], "rwtest", 6) == 0) {
		static char read_buffer[512];
		static char write_bufer[512];
		int size = 512;
		int i, ret;
		for (i = 0; i < size; i++) {
			write_bufer[i] = i;
		...
		ret = ubi_volume_write(argv[2], (void *)write_bufer, size);
		if (!ret) {
			printf("%d bytes written to volume %s\n", size, argv[2]);
		} else {
			printf("%d bytes written to volume %s, error\n", size, argv[2]);
			return ret;
		}

		ret = ubi_volume_read(argv[2], (char *)read_buffer, size);
		if (ret) {
			printf("%d bytes read from volume %s, error\n", size, argv[2]);
			return ret;
		}

		for (i = 0; i < size; i++) {
			if (read_buffer[i] != write_bufer[i]) {
				printf("read %s error, read 0x%x, expect 0x%x\n", argv[2],
						read_buffer[i], write_bufer[i]);
			}
		}
		printf("read write ubi volume %s ok\n", argv[2]);
		return 0;
	}
...
static void ubitest(int argc, const char **argv)
{
	do_ubi(argc, (char **)argv);
}
