...
{
	printf(
"stage1 is segemented and stored in each page and multi copy.\n"
"\n"
...
static void process_options(int argc, char * const argv[])
{
	...
	for (;;) {
		int option_index = 0;
		static const char short_options[] = "hi:o:s:l:c:p:";
...
int main(int argc, char * const argv[])
{
	...
	buf_pack = malloc((stage1_size / 2048 + 1) * page_size);
	memset(buf_pack, 0xff, (stage1_size / 2048 + 1) * page_size);
	...
	*(unsigned int *)(buf + stage1_size - crc_len) = crc_res;
	...
	while (readlen < stage1_size) {
		memcpy(&buf_pack[writelen], &buf[readlen], 512);
		readlen += 512;
		writelen += 512;
		subpage_count++;
		/* 每页只放4个512数据 */
	...
	for(i = 0; i < stage1_copy_count; i++) {
		ret = fwrite(buf_pack, 1, writelen, file_w);
		if (ret != writelen) {
