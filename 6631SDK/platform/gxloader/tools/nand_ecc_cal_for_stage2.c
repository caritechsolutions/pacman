...
unsigned int CRC32(unsigned char * pBuffer, unsigned int nstage2_size)
{
	unsigned int nResult = 0xFFFFFFFF;
	while (nstage2_size--) nResult = (nResult << 8) ^ s_nTable[(nResult >> 24) ^ *pBuffer++];
	return nResult;
}
...
{
	printf(
"inser stage2 ecc and multi copy.\n"
"\n"
"-h          --help               Display this help and exit\n"
"-i input    --input=input        input file\n"
"-o output   --output=output      output file\n"
"-c count    --count=count        stage2 copy count\n"
"-s start    --start=start        stage2 start exclude BOOT HEAD\n"
"-l length   --length=length      stage2 size\n"
...
static int         stage2_size;
static int         stage2_start;
static int         stage2_copy_count;
	...
	for (;;) {
		...
		switch (c) {
			...
			case 'o':
				...
				break;
			case 's':
				stage2_start = simple_strtoul(optarg, NULL, 10);
				break;
			case 'l':
				stage2_size = simple_strtoul(optarg, NULL, 10);
				break;
			case 'c':
				stage2_copy_count = simple_strtoul(optarg, NULL, 10);
				break;
			case 'p':
...
int main(int argc, char * const argv[])
{
...
#if 0
	if (stage2_size % 512) {
		printf("stage2 size should be algin 512\n");
		exit(-1);
	}
#endif
	...
	/* write  stage1 */
	if (stage2_start > 0) {
		buf1 = malloc(stage2_start);
		memset(buf1, 0xff, stage2_start);

		ret = fread(buf1, 1, stage2_start, file_r);
		fwrite(buf1, 1, ret, file_w);
		if (ret < stage2_start) {
		...
		fseek( file_r, stage2_start, SEEK_SET);
	}
	...
	if(stage2_size == 0) {
		...
		unsigned int file_len = ftell(file_r);
		stage2_size = file_len - stage2_start;
		fseek( file_r, stage2_start, SEEK_SET);
	}

	stage2_size_pad = roundup(stage2_size, 512);

	buf = malloc(stage2_size_pad);
	memset(buf, 0xff, stage2_size_pad);
	unsigned int buf_ecc_len = (roundup(stage2_size, 1024)) / 1024 * page_size;
	buf_ecc = malloc(buf_ecc_len);
	memset(buf_ecc, 0xff, buf_ecc_len);

	ret = fread(buf, 1, stage2_size, file_r);
	if (ret != stage2_size) {
	...
	if(padflag == 1) {
		block_pad_len = roundup(buf_ecc_len, page_size*64) - buf_ecc_len;
		buf_block_pad = malloc(block_pad_len);
		memset(buf_block_pad, 0x00, block_pad_len);
	}

	printf("writelen %d\n", writelen);
	printf("buf_ecc_len %d\n", buf_ecc_len);
	for(i = 0; i < stage2_copy_count; i++) {
		ret = fwrite(buf_ecc, 1, buf_ecc_len, file_w);
		if (ret != buf_ecc_len) {
	...
	fseek( file_r, stage2_start+stage2_size, SEEK_SET);
