...
{
	printf("./loader_write.elf inputfile outputfile stage1_start stage1_size stage1_copy_count\nor \n");
	printf("./loader_write.elf inputfile file_size\n");
}
...
int main(int argc, char* argv[])
{
	...
	if (argc == 3) {
		stage1_size = simple_strtoul((const char *)argv[2], NULL, 10);
	} else {
		...
		stage1_start = simple_strtoul((const char *)argv[3], NULL, 10);
		stage1_size = simple_strtoul((const char *)argv[4], NULL, 10);
		stage1_copy_count = simple_strtoul((const char *)argv[5], NULL, 10);
	}
	...
	if (argc == 3) {
		...
		*(unsigned int *)(buf + stage1_size - crc_len) = crc_res;
		...
		for(i = 0; i < stage1_copy_count; i++) {
			ret = fwrite(buf, 1, stage1_size, file_w);
			if (ret != stage1_size) {
