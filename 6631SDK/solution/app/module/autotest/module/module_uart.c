...
int uart_init(unsigned int Baudrate)
{
	...
	sg_UartFd = open(UART_DEV_PATH, O_RDWR|O_NONBLOCK);
	if(sg_UartFd < 0)
	{
...
int uart_receive(unsigned char *Buffer, unsigned int byteToRead)
{
	...
	{
		UartPrintf("\nTEST, uart not open or param error\n");
		return -1;
	}
...
int uart_close(void)
{
	...
	sg_UartFd = -1;
#if !USB_TO_SERIAL_SUPPORT
	GxCore_PrintSwitch(GX_PRINT_TO_UART);
#endif
	return 0;
}
