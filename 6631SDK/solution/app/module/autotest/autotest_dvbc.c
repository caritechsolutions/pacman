...
static int dvbc_tp_lock(void* pTpInfo)
{
	...
	params.u.dvbc_param.modulation = p->Qam + DVBC_16QAM - 1;//143549
	params.u.dvbc_param.symbol_rate = p->Sym/1000;
	params.u.dvbc_param.timeout = 1000;
...
static int _parse_tp_para(char *Command, void **ppInOutFrontendInfo, void **ppInOutTpInfo)
{
	...
	if(p == NULL)
	{// DEFAULT TSMODE
		sg_DvbcTpInfo.TpInfo.DemuxPara.TsMode = 1;// paralle
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
	...
	if(p == NULL)
	{// DEFAULT SOURCE
		sg_DvbcTpInfo.TpInfo.DemuxPara.TsSource = 1;// source 0
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
	...
	if(p == NULL)
	{// DEFAULT TS SELECT
		sg_DvbcTpInfo.TpInfo.TunerSelect = 2; // tuner 2
	}
	else
	{// find the keyword
		memset(Buffer, 0, sizeof(Buffer));
