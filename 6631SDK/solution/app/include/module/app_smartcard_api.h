...
typedef GxSciRepeatMode GxSmcRepeat_t;
...
#define O_CDBG  (1<<1)  /* 打开驱动的调试信息 */
    ...
    uint32_t           debug_info;//O_BDBG和O_CDBG现在都是打开驱动调试信息
}GxSmcParams;

/**
 * @brief       智能卡打开函数
 * @param [in]  name                    智能卡设备名,驱动已写死,传不传都一样
 * @param [in]  sc_notify               智能卡状态通知函数指针
 * @return      handle_t 智能卡句柄E_INVALID_HANDLE 执行失败,成功固定返回0x12345678,真正的handle_t由底层自己管理
 */
handle_t GxSmc_Open(const char *name, GxSmcParams *param);
