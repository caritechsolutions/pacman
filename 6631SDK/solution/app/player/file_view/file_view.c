...
bool  check_extend_name(const char* name, const char* extend)
{
	...
	*/
	...
	{
		//app_log_debug( "=_check_extend_name '%s', '%s'= name_len < extend_len\n", name, extend);
		return FALSE;
	}
...
status_t file_view_opendir(const char* path)
{
	...
	{
		app_log_error( "[FILE] view opendir '%s' unexist\n", path);
		return GXCORE_ERROR;
	}
	...
	{
		app_log_error( "[FILE]  view opendir '%s' is not a directory\n", path);
		return GXCORE_ERROR;
	}
	...
	if(NULL == dir)
	{
		app_log_error( "[FILE] view opendir '%s' Error\n", path);
		return GXCORE_ERROR;
	}
...
char* file_view_get_dir_only(char* file)
{
	...
	{
		app_log_debug("cut name, get dir only\n");
		//fine directory
		len = strlen(file);
		for(i = 0; i < len; i++)
		{
...
#define FILE_VIEW_SUFFIX_PIC	"jpg;JPG;jpeg;JPEG;bmp;BMP;png;PNG;"//"jpg;JPG;jpeg;JPEG;bmp;BMP;png:PNG;tiff;TIFF"
#if (FLAC_SUPPORT && OGG_SUPPORT)
#define FILE_VIEW_SUFFIX_MUSIC	"mp3;MP3;aac;AAC;m4a;M4A;ac3;AC3;flac;ogg"
#else
#define FILE_VIEW_SUFFIX_MUSIC  "mp3;MP3;aac;AAC;m4a;M4A;ac3;AC3"
#endif
#define FILE_VIEW_SUFFIX_MOVIE	"mkv;MKV;avi;AVI;ts;TS;mp4;MP4;mpg;MPG;flv;FLV;dat;DAT;vob;VOB;tp;TP;trp;TRP;mov;MOV;dvr;DVR;MPEG;mpeg;m2ts;M2TS"
...
#define WEBAPI_DEFAULT_FILE_LIMIT 20

char *app_webapi_get_file_data(char *path, WebapiMediaType media_type, uint32_t page, uint32_t limit)
{
	int i = 0;
	cJSON *root = NULL;
	cJSON *json1 = NULL;
	...
	else if(media_type == WEBAPI_MEDIA_AUDIO)
	{
		suffixs = FILE_VIEW_SUFFIX_MUSIC;
	}
	else if(media_type == WEBAPI_MEDIA_PICTURE)
	{
		suffixs = FILE_VIEW_SUFFIX_PIC;
	}
	else
	...
	explorer = explorer_opendir(path, suffixs);
	if(explorer)
	{
		root = cJSON_CreateObject();
		cJSON_AddStringToObject(root, WEBAPI_STATUS_STR, WEBAPI_OK_STR);
		json1 = cJSON_CreateObject();
		cJSON_AddItemToObject(root, WEBAPI_FILE_STR, json1);
		cJSON_AddNumberToObject(json1, WEBAPI_TOTAL_STR, explorer->nents);
		if(limit == 0)
		{
			limit = WEBAPI_DEFAULT_FILE_LIMIT;
		}
		if(page>0)
		{
			start_pos = limit*(page-1);
		}
		if((explorer->nents>0)&&(start_pos<explorer->nents))
		{
			prog_num = explorer->nents-start_pos;
			if(prog_num>limit)
			{
				prog_num = limit;
			}
			...
			for(i=start_pos; (i<(start_pos+prog_num)); i++)
			{
				full_name = NULL;
				json3 = cJSON_CreateObject();
				cJSON_AddItemToArray(json2, json3);
				if(explorer->ents[i].ftype == GX_FILE_DIRECTORY)
				{
					type = "directory";
				}
				else
				...
				cJSON_AddStringToObject(json3, WEBAPI_TYPE_STR, type);
				cJSON_AddStringToObject(json3, WEBAPI_NAME_STR, explorer->ents[i].fname);
				full_name = explorer_malloc_path_strcat(explorer->path, explorer->ents[i].fname);
				if(full_name)
				{
					cJSON_AddStringToObject(json3, WEBAPI_PATH_STR, full_name);
					GxCore_Free(full_name);
				}
			}
		}
		out = cJSON_PrintUnformatted(root);
		cJSON_Delete(root);
		explorer_closedir(explorer);
	}
