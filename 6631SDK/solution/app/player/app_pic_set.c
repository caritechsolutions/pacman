...
#define COMBOBOX_SWITCH_SPECIAL_EFFECTS	"pic_set_combo_special_effects"
...
static status_t key_left_right(void)
{
	...
	switch(item_sel)
	{
		...
		case 2:
			GUI_GetProperty(COMBOBOX_SWITCH_SPECIAL_EFFECTS, "select", (void*)&value_sel);
			pmpset_set_int(PMPSET_PIC_SPECIAL_EFFECTS, value_sel);
			break;
	...
	app_log_debug("[PIC] keypress_pic_view_set_ok=item:%d, value:%d\n", item_sel, value_sel);
...
SIGNAL_HANDLER int pic_set_init(const char* widgetname, void *usrdata)
{
	...
	value = pmpset_get_int(PMPSET_PIC_SPECIAL_EFFECTS);
	GUI_SetProperty(COMBOBOX_SWITCH_SPECIAL_EFFECTS, "select", (void*)&value);
