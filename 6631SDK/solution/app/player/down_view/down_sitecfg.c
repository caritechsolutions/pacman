...
channel_view* channel_create(GXxmlNodePtr GXxml_node)
{
	...
	app_log_debug("[DOWN] channel title: %s, link: %s\n", channel->title, channel->link);
...
status_t channel_release(channel_view* channel)
{
	...
	GXxmlFree(channel->title);
	GXxmlFree(channel->link);
	DOWN_FREE(channel);
...
site_view* site_create(GXxmlNodePtr GXxml_node)
{
	...
	app_log_debug("[DOWN] site name: %s, logo: %s, class: %s\n", site->name, site->logo, site->class);
...
status_t site_release(site_view* site_view)
{
	...
	app_log_debug("[DOWN] site release, channel num:%d\n", site_view->channel_num);
	GXxmlFree(site_view->name);
	GXxmlFree(site_view->logo);
	GXxmlFree(site_view->class);
...
status_t down_create(down_view* down, GXxmlNodePtr GXxml_node)
{
	...
	app_log_debug("[DOWN] down create, site num:%d\n", site_num);
...
status_t down_release(down_view* p_down_view)
{
	int32_t i = 0;
	...
	app_log_debug("[DOWN] down release, site num:%d\n", p_down_view->site_num);

	for(i = 0; i < p_down_view->site_num; i++)
	{
		site_release(p_down_view->sites[i]);
	}
	DOWN_FREE(p_down_view->sites);

	memset(p_down_view, 0, sizeof(down_view));
...
status_t sitecfg_parser(const char* class, const char *filename)
{
	...
	if (NULL == doc)
	{
		app_log_error( "[XML] Document '%s' parse error.\n", filename);
		return GXCORE_ERROR;
	}
	...
	if (root == NULL)
	{
		app_log_error( "[XML] Document '%s' empty\n", filename);
		GXxmlFreeDoc(doc);
		return GXCORE_ERROR;
	}
