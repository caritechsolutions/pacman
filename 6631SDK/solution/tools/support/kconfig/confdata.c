...
int conf_write(const char *name)
{
	...
	const char *basename;
	const char *str;
	char dirname[PATH_MAX+1], tmpname[PATH_MAX+128], newname[PATH_MAX+128];
	char *env;
...
int conf_write_autoconf(void)
{
	...
	int i;
	char dir[PATH_MAX+1], buf[PATH_MAX+128];
	char *s;
	...
	snprintf(buf, sizeof(buf), "%s.config.cmd", dir);
	file_write_dep(buf);

	if (conf_split_config())
		return 1;

	snprintf(buf, sizeof(buf), "%s.tmpconfig", dir);
	out = fopen(buf, "w");
	if (!out)
		return 1;

	snprintf(buf, sizeof(buf), "%s.tmpconfig_tristate", dir);
	...
	snprintf(buf, sizeof(buf), "%s.tmpconfig.h", dir);
	out_h = fopen(buf, "w");
	if (!out_h) {
		fclose(out);
		fclose(tristate);
