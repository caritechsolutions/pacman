...
#define IPTV_PRINTF(...)    app_log_min(__VA_ARGS__)
#else
...
#define IPTV_REMOTE_DATABASE    "/tmp/iptv/iptv.xml"
...
#define IPTV_REMOTE_DATABASE    "/mnt/iptv.xml"
...
char* GxCore_Strndup(const char* src, int len)
{
	...
	if(strlen(src) < len)
	{
		IPTV_PRINTF("[GxCore_Strndup] length err!!!\n");
		return NULL;
	}
	...
	if(p == NULL)
	{
		IPTV_PRINTF("[GxCore_Strndup] calloc err!!!\n");
		return NULL;
	}
...
static IptvListClass *_get_list_from_txt_file(char *path)
{
    IptvListClass *list = NULL;
    char *buffer = NULL;
    FILE *fp_handle = NULL;
...
static IptvListClass *_get_list_from_xml_file(char *path)
{
    ...
    if(doc)
    {
        ...
        {
            GXxmlFreeDoc(doc);
            IPTV_PRINTF("\033[1;31m[%s] no valid root node\n\033[0m", __func__);
            return NULL;
        }
    }
    else
    {
        IPTV_PRINTF("\033[1;31m[%s] list parser error\n\033[0m", __func__);
        return NULL;
    }
...
static IptvListClass *_iptv_get_list_from_file(char *path)
{
    IptvListClass *list = NULL;
    char *ext = NULL;
...
IptvListClass* app_iptv_api_get_local_list(void)
{
    ...
    ret_list = _iptv_get_list_from_file(IPTV_LOCAL_DATABASE);
...
IptvListClass* app_iptv_api_get_fav_list(void)
{
    IptvListClass *ret_list = NULL;
    IPTV_PRINTF("iptv db path: %s\n", IPTV_FAV_DATABASE);
    ret_list = _iptv_get_list_from_file(IPTV_FAV_DATABASE);
